function drop_to_html(def, mult, color) {
	...
	html +=
		"<div style='display: inline-block; margin-right: 10px; border: 2px solid " +
		color +
		"; padding: 4px 8px 4px 8px'>";
	var skin = "";
	var prob = parseInt(1 / (def[0] * mult));
	if (G.items[def[1]]) {
...
function visualize() {
	...
	object_sort(G.maps).forEach(function (io) {
		...
		var first = false;
		var mx = false;
		if (!(D.drops.maps[mname] || (map.monsters && map.monsters.length)) || G.maps[mname].ignore) {
			return;
		}
		...
		for (var i = 0; i < (D.drops.maps[mname] || []).length; i++) {
			html += drop_to_html(D.drops.maps[mname][i], 1, "#858B8E");
			mx = true;
		}
		if (mx) {
		...
		for (var i = 0; i < (map.monsters || []).length; i++) {
			var m_def = map.monsters[i];
			var monster = G.monsters[m_def.type];
		...
		for (var i = 0; i < (map.monsters || []).length; i++) {
			...
			if (!monster) {
				alert("no " + JSON.stringify(m_def));
			}
			html +=
				"<div class='gamebutton' style='margin-bottom: 6px; margin-top: 4px; border-color: #6C0001'><div xstyle='border: 2px solid gray; padding: 2px; display: inline-block'>" +
				sprite(monster.skin) +
				"</div>" +
				monster.name +
				" [" +
			...
			if (!drop) {
				html += "<div style='color: orange; font-size: 24px'>No Drop</div>";
			} else {
				html += "<div style='display: inline-block; margin-left: -10px;'></div>";
			}
