...
function is_npc(entity) {
	if (entity && (entity.npc || entity.type == "npc")) {
		return true;
	}
}
...
function is_player(entity) {
	if (entity && entity.type == "character" && !entity.npc) {
		return true;
	}
}
...
function ms_check(element, name, ms) {
	...
	if (element["ms_" + name] && mssince(element["ms_" + name]) < ms) {
		return 0;
	}
	element["ms_" + name] = new Date();
	return 1;
...
function preview_all(element) {
	...
	for (var i = 0; i < 3; i++) {
		for (var j = 0; j < 4; j++) {
			disappearing_clone(element, {
				i: i,
				j: j,
				x: get_x(element) + i * 40,
				y: get_y(element) + j * 40,
				stay: true,
				alpha: 1,
				border: true,
			});
...
function disappearing_clone(element, args) {
	...
	var generated = false;
	if (args.random && element.stype == "full") {
		texture = textures[element.skin][parseInt(Math.random() * 3)][parseInt(Math.random() * 4)];
	}
	if (args.i !== undefined) {
	...
	map.addChild(sprite);
	if (!args.stay) {
		draw_timeout(fade_away(5, sprite), 15);
	}
}
	...
	for (var i = 1; i <= 5; i++) {
		$(".bitem" + i).html(
			item_container(
				...
				{ level: i, name: "goldbooster", expires: future_s(999999999) },
			),
		);
	}
}
...
function show_game_guide() {
	if (gameplay == "hardcore") {
		show_modal($("#hardcoreguide").html(), { styles: "" });
	} // background: #E5E5E5; color: #010805;
	else {
		render_guide();
	}
}
...
function show_credits() {
	show_modal($("#credits").html(), {
		styles:
			"background: #E5E5E5; color: #010805; padding-left: 20px; padding-right: 20px; font-size: 24px; text-align: center",
		url: "/credits",
	});
}
...
function hide_modal(force) {
	...
	if (window.page && (old_url || new_url)) {
...
function show_modal(mhtml, args) {
	...
	if (args.wrap) {
		wrap_styles = "width: " + (args.wwidth || min_width) + "px; border: 5px solid gray; background: black;";
	}
...
function position_modals() {
	$(".imodal").each(function () {
		...
		if (height > iheight) {
			...
			$this.css("margin-bottom", "40px").css("margin-top", "100px");
		}
	});
}
...
function show_json(json, args) {
	...
	if (args.character_ui) {
		if (character && args.name == character.name) {
			name =
				"<span style='font-size:24px'><span style='color: gray'>Showing</span> parent.<span style='color: #39A920'>character</span></span>";
		} else {
			name =
				"<span style='font-size:24px'><span style='color: gray'>Showing</span> parent.entities[<span style='color: " +
				colors.property +
				"'>\"" +
				args.name +
				'"</span>]</span>';
		...
		name =
			"<span style='font-size:24px'><span style='color: gray'>Showing</span> parent.entities[<span style='color: #EF7A4D'>" +
			args.name +
			"</span>]</span>";
		html += "<div style='float: right; text-align: right'>";
		html +=
			"<div class='gamebutton' onclick='open_article(\"data-monster\",\"/docs/code/monster/reference\")'><span style='color:#22F4BE'>Docs:</span> Monster Objects Explained</div>";
		html += "<div></div>";
		html +=
			"<div class='gamebutton mt5' onclick='render_monster_info(\"" +
			args.monster_ui +
			"\")'><span style='color:#EE8E5B'>Guide:</span> " +
		...
		name =
			"<span style='font-size:24px'><span style='color: gray'>Showing</span> character.items[<span style='color: #EF7A4D'>" +
			args.name +
			"</span>]</span>";
		html += "<div style='float: right; text-align: right'>";
		if (character.items[args.name]) {
			html +=
				"<div class='gamebutton' onclick='render_item_info(\"" +
				character.items[args.name].name +
				"\")'><span style='color:#22F4BE'>Ref:</span> Item Information</div>";
		}
		html += "<div></div>";
		html +=
			"<div class='gamebutton mt5' onclick='show_json(character.items,{prefix:\"character.\",name:\"items\"})'><span style='color:#5993F0'>Show:</span> All Inventory</div>";
		html += "</div>";
	} else if (args.name) {
		name =
	...
	if (!is_string(json)) {
		json = safe_stringify(json, "\t");
	}
	// IDEA: XHTML to reference the full object
	show_modal(
		html +
			name +
			"<div style='font-size: 24px; white-space: pre-wrap;' class='yesselect pre shcontainer'>" +
...
function json_to_html(json) {
	...
	return (
		"<div style='font-size: 24px; white-space: pre;' class='yesselect shcontainer'>" + syntax_highlight(json) + "</div>"
	);
}
...
function add_update_notes() {
	update_notes.forEach(function (note) {
		...
		if (note.indexOf("Holiday") != -1) {
			color = "#C82F17";
		}
		if (note.indexOf("Duelland") != -1) {
			color = "#3BB7CB";
		}
		if (note.indexOf("Lunar") != -1) {
			color = "#B02B16";
		}
		if (note.indexOf("Valentine") != -1) {
			color = "#C987B7";
		} // ,color="#85C76B"
		if (note.indexOf("Halloween") != -1) {
			color = "#DE6E37";
		}
		if (note.indexOf("Egg Hunt Event") != -1) {
			color = "#DE5CB8";
		}
		add_log(note, color);
	});
...
var unread_chat = 0;
var no_chat_notification = false;
...
function rebuild_chat() {
	...
	game_chats.forEach(function (log) {
		...
		var rcolor = "#999A4F";
		if (log[0] && log[0] != "^") {
			o_html = "<span style='color:white'>" + log[0] + ":</span> ";
		}
		if (log[2] == colors.server_success || log[2] == "gold") {
			rcolor = "#E0E0E0";
		}
		if (log[2] == colors.server_failure) {
			rcolor = "#626363";
		}
		if (rrpt > 1) {
			rrpt = " <span style='color:" + rcolor + "'>[" + rrpt + "]</span>";
		} else {
			rrpt = "";
		}
		html +=
			"<div class='chatentry' style='color: " +
			(log[2] || "gray") +
			"'>" +
			o_html +
			((log[0] == "^" && log[1]) || html_escape(log[1])) +
			rrpt +
			"</div>";
	});
	$(selector).html(html);
...
function add_chat(owner, message, color, id) {
	if (no_html == "bot") {
		game_chats.push([owner, message, color, id]);
	}
	if (no_html || window.is_comm) {
		return;
	}
	var selector = (!window.character && "#gamelog") || "#chatlog";
	var owner_html = "";
	...
	for (var i = 0; i < game_chats.length; i++) {
		if (
			(id && game_chats[i][3] == id) ||
			(game_chats[i][0] == owner && game_chats[i][1] == message && game_chats[i][3] == id)
		) {
			if (owner && i != game_chats.length - 1) {
				continue;
			}
	...
	var rcolor = "#999A4F";
	if (color == colors.server_success || color == "gold") {
		rcolor = "#E0E0E0";
	}
	if (color == colors.server_failure) {
		rcolor = "#626363";
	}
	...
	if (rpt > 1) {
		rpt = " <span style='color:" + rcolor + "'>[" + rpt + "]</span>";
	} else {
		rpt = "";
	}
	$(selector).append(
		"<div class='chatentry' style='color: " +
			...
			"'>" +
			owner_html +
			((owner == "^" && message) || html_escape(message)) +
			rpt +
			"</div>",
	);
...
function cpm_window(name) {
	...
	var cid = "pm" + name;
	last_say = cid;
	if (!in_arr(cid, cwindows)) {
		open_chat_window("pm", name, 1);
	} else {
		toggle_chat_window("pm", name);
	}
}
...
function add_pmchat(to, owner, message, xserver) {
	...
	var color = "";
	if (!in_arr(cid, cwindows)) {
		open_chat_window("pm", to, owner == character.name);
	}
	if (owner != character.name && in_arr(cid, docked)) {
		$("#chatt" + cid).addClass("newmessage");
	}
	var owner_html = "";
	xserver = (xserver && " <span style='color:#525553'>[X]</span>") || "";
	owner_html = "<span style='color:white'>" + owner + ":</span> ";
	$("#chatd" + cid).append(
		"<div style='color: " + (color || "gray") + "'>" + owner_html + html_escape(message) + xserver + "</div>",
	); //class='chatentry'
...
function add_partychat(owner, message) {
	...
	var color = "";
	if (!in_arr(cid, cwindows)) {
		open_chat_window("party", "", owner == character.name);
	}
	if (owner != character.name && in_arr(cid, docked)) {
		$("#chatt" + cid).addClass("newmessage");
	}
	...
	owner_html = "<span style='color:white'>" + owner + ":</span> ";
	$("#chatd" + cid).append(
		"<div style='color: " + (color || "gray") + "'>" + owner_html + html_escape(message) + "</div>",
	); //class='chatentry'
	$("#chatd" + cid).scrollTop($("#chatd" + cid)[0].scrollHeight);
}
...
function can_use(name) {
	if (!next_skill[name] || new Date() > next_skill[name]) {
		return true;
	}
	return false;
}
...
function get_nearby_hostiles(args) {
	...
	if (!args.range) {
		args.range = (character && character.range) || 12000;
	}
	if (!args.limit) {
		args.limit = 12;
	}
	...
	for (id in entities) {
		...
		if (current.rip || current.invincible || current.npc) {
			continue;
		}
		if (character.team && current.team == character.team) {
			continue;
		}
		if (!character.team && current.party && character.party == current.party) {
			continue;
		}
		if (!character.team && current.guild && character.guild == current.guild) {
			continue;
		}
		if (current.type == "character" && !(is_pvp || G.maps[character.map].pvp)) {
			continue;
		}
		if (in_arr(current.owner, parent.friends)) {
			continue;
		}
...
function get_input(args) {
	...
	var html =
		"<div style='" + ((!args.no_wrap && "border: 5px solid gray; padding: 5px; background: black") || "") + "'>";
	var ilast = 0;
	var focus = null;
	if (is_array(args)) {
		args = { elements: args };
	...
	args.elements.forEach(function (element) {
		if (element.title) {
			html += "<div class='textheader'>" + element.title + "</div>";
		}
		if (element.input) {
			if (!focus) {
				focus = element.input;
...
function show_confirm(text, ok, cancel, onclick) {
	...
	var html = "";
	html +=
		"<div style='width: 400px; border: 5px solid gray; background-color: black; font-size: 24px'><div style='padding: 20px;'>" +
		text +
		"</div></div>";
	...
	html +=
		...
		color +
		"; margin: 6px 6px 6px 0px' onclick='sc_onclick[\"" +
		rid +
		"\"]();'>" +
		ok +
		"</div>";
...
function use_skill(name, target, arg) {
	...
	if (target && is_array(target)) {
		for (var i = 0; i < target.length; i++) {
			...
			if (target[i] && target[i][0] && target[i][0].id) {
				target[i][0] = target[i][0].id;
			} // "cburst"
		}
	}
	if (name == "use_hp" || name == "hp") {
		...
		if (is_array(target)) {
			...
			var targets = [];
			var mp = character.mp - 200;
			var hmp = parseInt(mp / hostiles.length);
		...
		socket.emit("skill", { name: "blink", x: target[0], y: target[1] });
	} else if (name == "dash") {
		var d = character.direction;
		socket.emit("skill", {
			name: "dash",
			x: get_x(character) + [0, -40, 40, 0][d],
			y: get_y(character) + [40, 0, 0, -40][d],
		});
		...
		if (target && is_string(target) && !target[2]) {
			target[2] = character.map;
		} else if (!target || !target[2] || is_string(target)) {
			var trset = false;
			for (var id in G.maps) {
				var map = G.maps[id];
				if (map.ignore || map.instance) {
					continue;
				}
			...
			if (!trset) {
				target = [Math.random() * 100, Math.random() * 100, "main"];
			}
		}
...
function on_skill(key, event) {
	...
	if (skill.type == "item") {
		...
		use_skill(name, xtarget || ctarget);
	}
}
...
function map_keys_and_skills() {
	...
	if (!Object.keys(keymap).length) {
		if (character.ctype == "warrior") {
			keymap = { 1: "use_hp", 2: "use_mp", 3: "cleave", 4: "stomp", 5: "agitate", Q: "taunt", R: "charge" };
		} else if (character.ctype == "mage") {
			keymap = { 1: "use_hp", 2: "use_mp", Q: "light", R: "burst", 6: "cburst", B: "blink", 7: "magiport" };
		} else if (character.ctype == "priest") {
			keymap = { 1: "use_hp", 2: "use_mp", R: "curse", 4: "partyheal", 8: "darkblessing", H: "heal" };
		} else if (character.ctype == "ranger") {
			keymap = { 1: "use_hp", 2: "use_mp", 3: "3shot", 5: "5shot", 6: "4fingers", R: "supershot" };
		} else if (character.ctype == "rogue") {
			keymap = { 1: "use_hp", 2: "use_mp", 3: "quickpunch", 5: "quickstab", R: "invis", Q: "pcoat" };
		} else if (character.ctype == "merchant") {
			keymap = { 1: "use_hp", 2: "use_mp", 3: "mluck" };
		} else if (character.ctype == "paladin") {
			keymap = { 1: "use_hp", 2: "use_mp", 3: "smash", 4: "selfheal", R: "purify", Q: "mshield" };
		}
		keymap["A"] = "attack";
...
function move(x, y, code) {
	var map = map;
	var move = calculate_move(character, parseFloat(x) || 0, parseFloat(y) || 0);
	// alert(move.x+" "+move.y);
...
function arrow_movement_logic() {
	...
	} else if (arrow_up && arrow_right) {
		move(character.real_x + 50, character.real_y - 50);
	} else if (arrow_up) {
		move(character.real_x, character.real_y - 50);
	} else if (arrow_left && arrow_down) {
		move(character.real_x - 50, character.real_y + 50);
	} else if (arrow_left) {
		move(character.real_x - 50, character.real_y);
	} else if (arrow_right && arrow_down) {
		move(character.real_x + 50, character.real_y + 50);
	} else if (arrow_right) {
		move(character.real_x + 50, character.real_y);
	} else if (arrow_down) {
...
function inventory_click(num, event) {
	...
	if (event && (event.which == 2 || event.button == 4)) {
		...
		if (character.items[num]) {
			iname = character.items[num].name + character.items[num].level;
		}
		if (last_invclick && last_invclick == num + iname && $(".inventory-item").html().length) {
			$(".inventory-item").html("");
		} else if (character.items[num]) {
			if (character.items[num].name == "placeholder") {
				return;
			}
			...
			render_item(".inventory-item", {
				id: "citem" + num,
				item: G.items[character.items[num].name],
				name: character.items[num].name,
				actual: character.items[num],
				num: num,
				inventory_ui: num,
			});
...
function sh_click(i) {
	...
	render_item("#topleftcornerdialog", {
		id: "sh" + i,
		item: G.items[secondhands[i].name],
		name: secondhands[i].name,
		actual: secondhands[i],
		secondhand: true,
	});
}
...
function lf_click(i) {
	...
	render_item("#topleftcornerdialog", {
		id: "sh" + i,
		item: G.items[lostandfound[i].name],
		name: lostandfound[i].name,
		actual: lostandfound[i],
		lostandfound: true,
	});
}
...
function slot_click(name) {
	...
	if (target && target.slots && target.slots[name]) {
		...
		render_item("#topleftcornerdialog", {
			id: "item" + name,
			item: G.items[target.slots[name].name],
			name: target.slots[name].name,
			actual: target.slots[name],
			slot: name,
...
function get_entity(id) {
	if (character && (id == character.id || id == character.name)) {
		return character;
	}
	if (entities[id]) {
		return entities[id];
...
function party_click(name) {
	...
	if (!ptarget) {
		add_log(
			name +
				" isn't around. <span class='clickable' onclick='pcs(event); travel_p(\"" +
				name +
				"\")' style='color: #A78059'>Travel</span>",
			"gray",
		);
		return;
	}
...
function show_commander(fvalue) {
	...
	var html =
		"<textarea id='rendererx'></textarea><div class='gamebutton snippetbtn' style='position: absolute; bottom: -68px; right: -5px' onclick='command_snippet()'>COMMAND</div>";
	show_modal(html);
	var value = "";
...
function show_snippet(fvalue) {
	...
	var html =
		"<textarea id='rendererx'></textarea><div class='gamebutton snippetbtn' style='position: absolute; bottom: -68px; right: -5px' onclick='tut(\"x\"); eval_snippet()'>EXECUTE</div>";
	show_modal(html);
	var value = "";
	if (window.codemirror_render3) {
		value = codemirror_render3.getValue();
...
function show_character_snippet(name) {
	...
	name = name.toLowerCase();
	var html =
		"<textarea id='renderer" +
		name +
		"'></textarea><div class='gamebutton' style='position: absolute; bottom: -68px; right: -5px' onclick='eval_character_snippet(\"" +
		name +
		"\")'>EXECUTE</div>";
	show_modal(html);
	var value = "// " + oname + "\n";
...
function character_code_eval(name, snippet) {
	...
	var weval =
		...
		document.getElementById(rid).contentWindow &&
		document.getElementById(rid).contentWindow.eval;
	if (!weval) {
	...
	} else if (document.getElementById(rid).contentWindow.code_run) {
...
function code_eval(snippet) {
	var f = "eval";
	if (snippet.search("output=") != -1 || snippet.search("json_output=") != -1) {
		f = "eval_s";
	}
...
function start_runner(rid, code) {
	...
	if (no_html) {
		$("body").append(
			'<iframe src="/runner" id="' +
				rid +
				'" style="border: none; height: 1px; width: 1px; overflow: hidden; pointer-events: none"></iframe>',
		);
	} else {
		$("#iframelist").append(
...
function code_persistence_logic() {
	...
	try {
		...
		if (data["code_" + real_id + suffix]) {
			code_change = true;
		} // first-save - code is no longer locally saved [30/05/20]
	...
	editor.on("cursorActivity", function () {
		...
		if (text && !in_arr(text, ["0"]) && (window[text] || in_arr(text, G.docs.functions))) {
			if (last_hint === undefined) {
				...
				$("#codelog").prepend(
					"<div id='codehint' class='clickable' onclick='load_documentation($(\".thehint\").html())'></div>",
				);
			}
			last_hint = text;
...
function stop_timer(name, extra) {
	...
	if ((name == "draw" && ms > 10) || name == "remove_sprite") {
		if (log_flags.timers) {
			console.log("timer[" + name + "]" + extra + ": " + mssince(timers[name]));
		}
	}
	timers[name] = new Date();
}
...
function attack_animation_logic(sprite, source) {
	...
	if (in_arr(source, ["partyheal"])) {
		only_up = true;
	}
	if (sprite.fx.aaa || sprite.fx.aaa === 0) {
		return;
	}
	sprite.fx.aaa = sprite.a_direction;
	if (1) {
		if (sprite.fx.aaa == 0 || only_up) {
			safe_y_move(sprite, multiplier * 2);
		} else if (sprite.fx.aaa == 3) {
			safe_y_move(sprite, -multiplier * 2);
		} else if (sprite.fx.aaa == 1) {
			safe_x_move(sprite, -multiplier * 2);
		} else {
			safe_x_move(sprite, multiplier * 2);
		}
		setTimeout(function () {
			if (sprite.fx.aaa == 0 || only_up) {
				safe_y_move(sprite, -multiplier);
			} else if (sprite.fx.aaa == 3) {
				safe_y_move(sprite, multiplier);
			} else if (sprite.fx.aaa == 1) {
				safe_x_move(sprite, multiplier);
			} else {
				safe_x_move(sprite, -multiplier);
			}
		}, 60);
...
function set_direction(sprite, mode) {
	...
	if (mode != "soft" && sprite.moving && sprite.name_tag) {
		start_name_tag(sprite);
	}
	var edge = 70;
	var direction = 0;
	if (mode == "npc") {
	...
	if (abs(sprite.angle) < edge) {
		direction = 2;
	} else if (abs(abs(sprite.angle) - 180) < edge) {
		direction = 1;
	} else if (abs(sprite.angle + 90) < 90) {
		direction = 3;
	}
...
function leave_references(sprite) {
	...
	(function () {
		...
		var yy = sprite.real_y;
		var wwidth = sprite.awidth || 10;
		var hheight = sprite.aheight || 10;
...
function free_children(sprite) {
	...
	for (var i = 0; i < sprite.children.length; i++) {
		...
		if (!sprite.children[i].dead) {
			sprite.children[i].dead = "map";
		}
...
function buy_shells(amount) {
	if (amount * G.multipliers.shells_to_gold > character.gold) {
...
function buy_with_gold(name, quantity) {
	...
	if (mssince(last_npc_right_click) < 100) {
		return rejecting_promise({ reason: "npc_misclickp" });
	}
...
function get_code_function(name) {
	return (
		(code_active &&
			document.getElementById("maincode") &&
			document.getElementById("maincode").contentWindow &&
			document.getElementById("maincode").contentWindow[name]) ||
		function () {}
	);
}
...
function say(message, code) {
	...
	if (message[0] == "/") {
		...
		if (command == "help" || command == "list" || command == "") {
			...
			var code = args.shift();
			if (name) {
				start_character_runner(name, code);
			}
		} else if (command == "codes") {
			if (!is_electron) {
				show_alert("Only works in game clients");
			} else {
				electron_open_codes();
			}
			...
			add_chat(
				"",
				to_pretty_num(parseInt(msince(inception))) + " minutes " + parseInt(ssince(inception) % 60) + " seconds",
				"gray",
			);
			...
			if (!name && target && target.name) {
				socket.emit("duel", { event: "challenge", name: target.name });
			} else if (name) {
				socket.emit("duel", { event: "challenge", name: name });
			} else {
			...
			var target = xtarget || ctarget;
			if (name && name.length) {
				socket.emit("party", { event: "kick", name: name });
			} else if (target && !target.me && !target.npc && target.type == "character") {
				socket.emit("party", { event: "kick", id: target.id });
			} else {
				add_chat("", "Target someone to kick");
			}
			...
			var target = xtarget || ctarget;
			if (name && name.length) {
				socket.emit("party", { event: "request", name: name });
			} else if (target && !target.me && !target.npc && target.type == "character") {
				socket.emit("party", { event: "request", id: target.id });
			} else {
			...
			document.getElementById("maincode") &&
			document.getElementById("maincode").contentWindow &&
			document.getElementById("maincode").contentWindow.handle_command
		) {
			...
			add_chat(
				"Healer",
				"Economy is completely Merchant-to-Merchant, players leave their merchants in the town square to sell or buy items",
				"#58BCA5",
			);
...
function quantity(name, level) {
	var q = 0;
	for (var i = 0; i < character.items.length; i++) {
		if (character.items[i] && character.items[i].name == name && (character.items[i].level || 0) == (level || 0)) {
			q += character.items[i].q || 1;
		}
	}
	return q;
}
...
function auto_craft(name, code) {
	...
	if (!G.craft[name]) {
		issue = "recipe";
	} else if (G.craft[name].gold < character.gold) {
		issue = "gold";
	} else {
		G.craft[name].items.forEach(function (i) {
			if (quantity(i[1], i[2]) < i[0]) {
				issue = "items";
			}
		});
	}
	if (issue) {
		...
		G.craft[name].items.forEach(function (i) {
			for (var j = 0; j < character.items.length; j++) {
				if (
					character.items[j] &&
					character.items[j].name == i[1] &&
					(character.items[j].level || 0) == (i[2] || 0) &&
					(character.items[j].q || 1) >= i[0]
				) {
					items.push([k++, j]);
					break;
				}
...
function upgrade(item, scroll, offering, code, calculate) {
	if (!code && calculate && suppress_calculations) {
		return;
	}
	if (!code && (item == null || (scroll == null && offering == null))) {
		d_text("INVALID", character);
	} else {
		socket.emit("upgrade", {
			item_num: item,
			scroll_num: scroll,
			offering_num: offering,
			clevel: (character.items[item] && character.items[item].level) || 0,
			calculate: calculate,
		});
		last_uping = new Date();
...
function compound(item0, item1, item2, scroll, offering, code, calculate) {
	...
	if (!code && (item0 == null || item1 == null || item2 == null || scroll == null)) {
		d_text("INVALID", character);
	} else {
...
function exchange_animation_logic() {
	...
	if (character.q.exchange.qs == "leather") {
		exccolors = exccolorsl;
	}
	if (character.q.exchange.qs == "lostearring") {
		exccolors = exccolorsg;
	}
	if (character.q.exchange.qs == "seashell") {
		exccolors = exccolorssea;
	}
	if (character.q.exchange.qs == "poof") {
		exccolors = exccolorsgray;
	}
	if (character.q.exchange.name && character.q.exchange.name.startsWith("cosmo")) {
		exccolors = exccolorscosmo;
	}
	if (in_arr(exchange_type, ["mistletoe", "ornament", "candycane"])) {
		exccolors = exccolorsc;
	}
...
var u_valid = false;
var last_uchance = null;
var last_uchance_for = "upgrade_or_compound";
function set_uchance(dchance, html) {
	...
	if (parseInt(dchance * 10000000000)) {
		u_valid = true;
	}
	var chance = min(9999, parseInt(dchance * 10000) || 0);
	var first = "" + parseInt(chance / 100);
	var last = "" + parseInt(chance % 100);
	chance = chance / 100;
	if (first.length == 1) {
		first = "0" + first;
	}
...
function set_uroll(def, rhtml) {
	...
	for (var i = 3; i >= 0; i--) {
		if (def.nums[i] !== undefined) {
			html += "<span style='color:" + (color || "white") + "'>" + def.nums[i] + "</span>";
		} else {
			html +=
				"<span style='color:" + (color || random_one(uroll_colors)) + "'>" + parseInt(Math.random() * 10) + "</span>";
		}
		if (i == 2) {
			html += "<span style='color:" + (color || random_one(uroll_colors)) + "'>.</span>";
		}
	}
	if (rhtml) {
		return html;
...
function reopen() {
	...
	draw_trigger(function () {
		...
		if (rendered_target == "upgrade") {
			originals = render_upgrade_shrine();
		} else if (rendered_target == "compound") {
			originals = render_compound_shrine();
		} else if (rendered_target == "exchange") {
			originals = render_exchange_shrine(exchange_type);
		} else if (rendered_target == "gold") {
			render_gold_npc();
		} else if (rendered_target == "items") {
			render_items_npc();
		} else if (rendered_target == "craftsman") {
			render_craftsman();
		} else if (rendered_target == "dismantler") {
			render_dismantler();
		} else if (rendered_target == "none") {
			render_none_shrine();
		} else if (rendered_target == "locksmith") {
			render_locksmith();
		}
		...
		if (u_retain && u_retain_t && rendered_target == "upgrade" && !character.q.upgrade) {
			...
			for (var i = 0; i < 3; i++) {
				if ((u_retain[i] || u_retain[i] === 0) && character.items[u_retain[i]]) {
					on_rclick($("#citem" + u_retain[i])[0]);
				}
			}
...
function esc_pressed() {
	if (modal_count > 0) {
		...
		$("#rightcornerui").html("");
		topright_npc = false;
	} else if (topleft_npc && topleft_npc != "dice") {
		topleft_npc = false;
	} else if (xtarget) {
		xtarget = null;
	} else if (ctarget && ctarget.type == "character") {
		ctarget = null;
	} else if (inventory) {
		draw_trigger(render_inventory);
	} else if (skillsui) {
		draw_trigger(render_skills);
	} else if (topleft_npc == "dice") {
		topleft_npc = false;
	}
	$(":focus").blur();
}
...
function open_chest(id) {
	var chest = chests[id];
	if (chest) {
		if (chest.openning && ssince(chest.openning) < 5) {
			return resolving_promise({ sucess: false, in_progress: true, reason: "openning" });
		}
		draw_trigger(function () {
			var chest = chests[id];
...
function generate_textures(name, stype) {
	...
	if (stype == "item") {
		...
		textures["item" + name] = [
			null,
			new PIXI.Texture(C[G.imagesets[G.positions[skin][0] || "pack_20"].file], rectangle),
		];
	...
	if (stype == "machine") {
		...
		if (machine.subframes) {
			...
			for (var i = 0; i < machine.subframes.length; i++) {
				var rectangle = new PIXI.Rectangle(
					machine.subframes[i][0],
					machine.subframes[i][1],
					machine.subframes[i][2],
					machine.subframes[i][3],
				);
				textures[name + "sub"][i] = new PIXI.Texture(
					PIXI.utils.BaseTextureCache[G.tilesets[machine.set].file],
					rectangle,
				);
			}
		}
	...
	if (stype == "animation") {
		...
		if (no_graphics) {
			PIXI.utils.BaseTextureCache[a.file] = { width: 20, height: 20 };
		} //#NOGTODO
		var width = PIXI.utils.BaseTextureCache[a.file].width;
		var fwidth = Math.floor(width / a.frames);
...
function new_sprite(skin, stype, n) {
	...
	if (stype == "item") {
		if (!textures["item" + skin]) {
			generate_textures(skin, "item");
		}
		var sprite = new PIXI.Sprite(textures["item" + skin][0]);
	}
	if (stype == "upper") {
		if (!textures["upper" + skin]) {
			generate_textures(skin, "upper");
		}
	...
	if (in_arr(stype, ["emblem", "gravestone"])) {
		if (!textures[skin]) {
			generate_textures(skin, "emblem");
		}
		var sprite = new PIXI.Sprite(textures[skin]);
		sprite.cskin = "";
	}
	if (stype == "machine") {
		if (!textures[skin.type]) {
			generate_textures(skin, "machine");
		}
	...
	if (stype == "v_animation") {
		if (!textures[skin]) {
			generate_textures(skin, "v_animation");
		}
		var sprite = new PIXI.Sprite(textures[skin][0]);
	...
	if (stype == "animatable") {
		if (!textures[skin]) {
			generate_textures(skin, "animatable");
		}
		var sprite = new PIXI.Sprite(textures[skin][0]);
		sprite.cskin = "0" + undefined;
		sprite.i = 0;
	...
	if (stype == "animation") {
		if (!textures[skin]) {
			generate_textures(skin, "animation");
		}
		var sprite;
		if (G.animations[skin] && G.animations[skin].tiling) {
			sprite = new PIXI.extras.TilingSprite(textures[skin][0], textures[skin][0].width, textures[skin][0].height);
		} else {
			sprite = new PIXI.Sprite(textures[skin][0]);
		}
		sprite.cskin = "0" + undefined;
		sprite.i = 0;
		sprite.frame = 0;
	...
	if (stype == "static") {
		...
		if (!texture) {
			...
			var file = G.tilesets[position[0]].file;
...
function recreate_wtextures() {
	...
	for (var last_water_frame = 0; last_water_frame < 4; last_water_frame++) {
		...
		wtextures[last_water_frame] = PIXI.RenderTexture.create(
			wtile_width + 5 * textures[wtile_name][0].width,
			wtile_height + 5 * textures[wtile_name][0].height,
...
function recreate_dtextures() {
	...
	for (var last_water_frame = 0; last_water_frame < 3; last_water_frame++) {
		...
		dtextures[last_water_frame] = PIXI.RenderTexture.create(
...
function random_rotating_rectangle(entity, args) {
	...
	if (args.color == "success") {
		colors = [0x85c76b, 0xadff8b, 0xeefcef];
	} else if (args.color == "purple") {
		colors = [0x9944c8, 0x7c38a3, 0xf4daff];
	}
	var color = random_one(colors);
	var size = random_one([3, 5, 7]);
	var x_step = random_one([1, -1, 2, -2]);
	var y_step = random_one([-0.5, 0, 1, 2, 3]);
	var r_step = random_one([-0.2, 0.2, -0.4, 0.4]);
	e.lineStyle(3, color);
	e.beginFill(color);
...
function random_spark(coord, args) {
	...
	if (args.color == "success") {
		...
		colors = [0x9944c8, 0x7c38a3, 0xf4daff];
	} else if (args.color == "spark") {
		colors = [0xfafce5, 0xfcfcf3, 0xfbc993, 0xc4ffe2];
	} else if (args.color == "blue") {
		colors = [hx("#477FDD"), hx("#86AEDD"), hx("#2F52DD")];
	}
	var color = random_one(colors);
	var size = 0.2;
	var x_step = random_one([0.25, 0.5, 0, -0.25, -0.5]);
	var y_step = random_one([0.25, 0.5, 0.75]);
...
function assassin_smoke(x, y, type) {
	...
	if (use_layers) {
		sprite.parentGroup = player_layer;
	} else {
		sprite.displayGroup = player_layer;
	}
	sprite.x = round(x);
	sprite.y = round(y);
	sprite.real_x = x;
...
function confetti_shower(entity, level) {
	...
	for (var i = 0; i < times; i++) {
		for (var j = 0; j < count; j++) {
			draw_timeout(function () {
				if (entity.real_x === undefined) {
					entity = get_entity(entity);
				}
				if (!entity) {
					return;
				}
				if (entity != character && a_map != current_map) {
					return;
				}
				assassin_smoke(
					entity.real_x + (Math.random() * 80 - 40),
					entity.real_y + (Math.random() * 80 - 40),
					"confetti",
				);
			}, i * interval);
		}
...
function start_emblem(sprite, name, args) {
	...
	if (args.no_dip) {
		esprite.frame_list = [0.57, 0.6, 0.63, 0.66, 0.69, 0.72, 0.75, 0.78, 0.82, 0.86, 0.9, 0.95, 1];
	} else {
		esprite.frame_list = [0.2, 0.33, 0.66, 0.77, 0.88, 0.95, 1];
	}
	for (var i = esprite.frame_list.length - 1; i >= 0; i--) {
		esprite.frame_list.push(esprite.frame_list[i]);
	}
	esprite.frame_list;
...
function start_animation(sprite, name, mode) {
	...
	var width = (sprite.hitArea && sprite.hitArea.width) || sprite.texture.width;
	var height = (sprite.hitArea && sprite.hitArea.height) || sprite.texture.height;
	var def = G.animations[name];
	sprite.animations[name] = asprite;
	if (def.alpha) {
		asprite.alpha = def.alpha;
	} else {
		asprite.alpha = 0.5;
	}
	if (mode == "stun") {
	...
	asprite.aspeed =
		(asprite.aspeed == "fast" && 0.8) || (asprite.aspeed == "mild" && 1.4) || (asprite.aspeed == "slow" && 3) || 2;
...
function map_animation(name, args) {
	...
	var asprite = null;
	if (args.item) {
		asprite = new_sprite(name, "item");
	} else {
		asprite = new_sprite(name, "animation");
	}
	var width = asprite.width;
	var height = asprite.height;
	var def = G.animations[name] || {};
	asprite.atype = "map";
	asprite.continuous = true;
	asprite.speeding = true;
	...
	if (def.directional) {
		asprite.directional = true;
	}
	asprite.speed = args.speed || def.speed || 10;
	asprite.to_fade = (args.fade === true && 0.025) || args.fade;
	if (args.filter) {
		asprite.filters = [args.filter];
	}
	if (args.scale || def.scale) {
		asprite.scale.set(args.scale || def.scale, args.scale || def.scale);
	}
	if (args.target) {
		asprite.target = args.target;
		asprite.going_x = get_x(args.target);
		asprite.going_y = get_y(args.target) - get_height(args.target) / 2;
		if (point_distance(asprite.x, asprite.y, asprite.going_x, asprite.going_y) < 100) {
			asprite.speed *= 0.75;
		}
	}
...
function rotated_texture(base_texture, frame, num) {
	if (!num) {
		return new PIXI.Texture(base_texture, frame);
	}
	//reference: http://pixijs.github.io/examples/index.html?s=demos&f=texture-rotate.js&title=Texture%20Rotate&v=dev
	var D8 = PIXI.GroupD8;
...
function draw_timeouts_logic(mode) {
	...
	if (to_delete) {
		delete_indices(draw_timeouts, to_delete);
	}
}
...
function draw_trigger(f) {
	if (in_draw) {
		...
		draw_timeouts.push([f, new Date(), 2, new Date(), 0]);
	}
}
	...
	for (var i = 0; i < tints.length; i++) {
		...
		if (tint.type == "skill") {
			if (start > tint.end) {
				...
				var to = -mssince(tint.end);
				var height = (2 * 46 * since) / (since + to + 1);
				var ratio = since / (since + to + 1);
				$(tint.selector).css(
					"background-color",
					"rgb(" +
						round(r + (rr - r) * ratio) +
				...
				if (!tint.added) {
					...
					$(tint.selector).append(
						"<div style='position: absolute; " +
							(tint.pos || "bottom") +
							": 0px; left: 0px; right: 0px; height: 1px; background: " +
							tint.color +
							"; z-index: 1' class='thetint'></div>",
					);
				}
				var since = mssince(tint.start);
				var to = -mssince(tint.end);
			...
			$(tint.selector).css(
				"background",
				"-webkit-gradient(linear, left top, right top, from(#f0f), to(#0f0), color-stop(49%,#f0f),color-stop(50%, #0f0)",
			);
		}
	}
	if (to_delete) {
		delete_indices(tints, to_delete);
	}
}

function get_tint(selector) {
	for (var i = 0; i < tints.length; i++) {
		if (tints[i].selector == selector) {
			return tints[i];
		}
	}
	return null;
...
function add_tint(selector, args) {
	...
	if (!args.reset_to) {
		args.reset_to = args.back_to;
	}
	if (!args.start_d) {
		args.start_d = "left bottom";
	}
	if (!args.end_d) {
		args.end_d = "left top";
	}
...
var next_potion = new Date(); // backwards compatibility
...
function attack_timeout_animation(ms) {
	...
	draw_trigger(function () {
		...
		add_tint(".atint", {
			...
			key: "a",
			cur: tint_c.a,
		}); //start_d:"left top",end_d:"left bottom",
	});
}
...
function skill_timeout_singular(name, ms) {
	...
	//var original=$(".ptint").css("background");
	for (var N in keymap) {
		if (keymap[N] && (keymap[N] == name || keymap[N].name == name)) {
			skids.push(N);
		}
	}
	draw_trigger(function () {
		if (name == "attack") {
			attack_timeout_animation(-mssince(next_skill[name]) - DMS);
		}
...
function player_rclick_logic(element) {
	...
	if (element.npc) {
		...
		add = true;
	} // maybe disable when the target is full hp
	else {
		if (!pvp) {
		} else if (pvp) {
			add = true;
...
function regather_filters(entity) {
	var filters = [];
	for (var p in entity) {
		if (p.startsWith("cfilter_")) {
			filters.push(entity[p]);
		}
	}
...
function name_logic(element) {
	...
	if (element.type != "character" && element.type != "npc") {
		return;
	}
	if (
		!options.show_names &&
		!(options.always_hpn || options.always_names) &&
		(mtarget || xtarget || ctarget) != element &&
		character &&
		element.name_tag
	) {
		//show_names
...
function add_name_tag_old(element) {
	...
	if (element.name_tag) {
		if (element.ntag_cache == ntag_cache) {
			return;
		}
		destroy_sprite(element.name_tag, "children");
		element.name_tag = null;
	...
	if (!element.me) {
		...
		multiplier = 4;
	} // Previously 8, "Business" was always rendering bad, so trying 2 [06/08/18]
	...
	} // manually applied to the 4.8.2 pixi.js
...
function add_name_tag(element) {
	...
	var mp_width;
	var bsc = "";
	if (element.type == "character" && (xtarget || ctarget) == element) {
		bar.hp = bar.mp = bar.level = bar.cl = true;
	}
	if (element.type == "character" && mtarget == element) {
		bar.hp = bar.level = bar.cl = true;
	}
	if (!element.max_mp) {
		bar.mp = false;
	}
	if (is_player(element) && !element.afk) {
		bar.online = true;
	}
	...
	if (bar.hp) {
		hp_width = round(((hp_mwidth - 2 - 2) * element.hp) / element.max_hp);
	}
	if (bar.mp) {
		mp_width = round(((mp_mwidth - 2 - 2) * element.mp) / element.max_mp);
	}
	...
	var fp2 = {
		...
		fontSize: 64,
		fill:
			colors[element.ctype != "merchant" && G.classes[element.ctype] && G.classes[element.ctype].main_stat] ||
			"#A29880",
		align: "center",
	};
	var cl;
	...
	bordered_rectangle.drawRect(
		-((bar.cl && cl_width + 1) || 0),
		0,
		width + ((bar.cl && cl_width + 1) || 0) + ((bar.level && level_width + 1) || 0),
		height,
	);
	bordered_rectangle.endFill();
	...
	bordered_rectangle.position = new PIXI.Point(
		-round(width / 2),
		-height - (element.aheight || element.height) - ((bar.mp && 4) || 0),
	);
...
function hp_bar_logic(element) {
	...
	if (element.dead && !element.hp_bar) {
		return;
	}
	if ((!hp_bars || element.me) && !options.always_hpn) {
		return;
	}
	if (
		ctarget == element ||
		...
		(character &&
			character.party &&
			((element.target && in_arr(element.target, party_list)) ||
				(element.focus && in_arr(element.focus, party_list)))) ||
...
function add_hp_bar_old(element) {
	...
	var bsize = 1;
	var bsize2 = round(2 * bsize); // bsize = 1.5 -> awesome [12/08/16]
	...
	if (element.npc) {
		...
		//red
		color = 0x899b92;
	} else if (character && (element.target == character.name || element.focus == character.name) && is_player(element)) {
		color = 0x3fa8d1;
	...
	if (element.hp_bar) {
		if (element.hp_width == hp_width && element.hp_color == color) {
			return;
		} //The Cache! :) [06/08/16]
		destroy_sprite(element.hp_bar, "children");
		element.hp_bar = null;
	...
	var dy = 12;
	var dx = 0;
	if (element.type == "character" && character_names) {
		dy += 8;
	}
	if (element.mscale == 2) {
		(dy += 6), (dx += width / 2);
	...
	hp_bar.position = new PIXI.Point(
		-(width / 2) - dx,
		-dy - (element.aheight || element.height) + ((element.mscale == 2 && -4) || 0),
	);
	if (element.mscale) {
		hp_bar.scale = new PIXI.Point(element.mscale, element.mscale);
	}
...
function add_hp_bar(element) {
	...
	if (element.hp_bar) {
		if (element.hp_width == hp_width && element.hp_color == color && element.hp_level == element.level) {
			return;
		} //The Cache! :) [06/08/16]
		destroy_sprite(element.hp_bar, "children");
		element.hp_bar = null;
	...
	if (level) {
		var lcolor = "#5EC063";
		var diff = calculate_difficulty(element);
		if (diff >= 2) {
			lcolor = "#D52526";
		} else if (diff) {
			lcolor = "#EF9232";
		...
		var level = new PIXI.Text((element.level > 99 && "XX") || element.level, fpl);
		level.scale = new PIXI.Point(0.125, 0.125);
		level.anchor.set(0.5, 0);
		level.x = -4;
		level.y = 0.5;
	...
	if (SCALE) {
		hp_bar.scale = new PIXI.Point(SCALE, SCALE);
	}
	element.hp_bar = hp_bar;
	element.hp_color = color;
	hp_bar.parentGroup = window.hp_layer;
...
function calculate_difficulty(monster) {
	...
	if (hp < 0) {
		return 2;
	} else if ((character && character.hp - hp > character.max_hp * 0.3) || (!character && 6000 - hp > 8000 * 0.3)) {
		return 1;
	} else {
		return 0;
	}
...
function d_line(start, end, args) {
	...
	var party = [
		0xf80c12, 0xee1100, 0xff3311, 0xff4422, 0xff6644, 0xff9933, 0xfeae2d, 0xccbb33, 0xd0c310, 0xaacc22, 0x69d025,
		0x22ccaa, 0x12bdb9, 0x11aabb, 0x4444dd, 0x3311bb, 0x3b0cbd, 0x442299,
	];
	if (
		start.slots &&
		((start.slots.helmet && start.slots.helmet.name == "partyhat") ||
			(start.slots.mainhand && start.slots.mainhand.name == "ornamentstaff")) &&
		args.color != "heal"
	) {
		shuffle(party);
		args.size = 1;
	...
	else if (args.color == "gold") {
		args.color = 0xffd700;
	} else if (args.color == "item") {
		args.color = 0x3ea2b3;
	} else if (args.color == "cx") {
		args.color = hx("#DC8BD4");
	} else if (args.color == "mana") {
...
function step_d_texts(entity, add) {
	...
	for (var i = 0; i < entity.texts.length; i++) {
		...
		var y = round((4 * ms) / text.anim_time) + (add || 0);
		// console.log(y);
		if (2 < y && y < 7) {
			y = 4;
		}
...
function d_text_new(message, entity, args) {
	var x = 0;
	var y = -get_height(entity);
	if (entity.name_tag || entity.hp_bar) {
		y -= 12;
	}
	...
	var fx = null; // "#383537" before text_quality = 2 as default [16/08/16]
	if (color == "hp") {
		color = "green";
	} else if (color == "mp") {
		color = "#317188";
	...
	if (fx) {
		start_filter(text, fx);
	}
	if (text_quality > 1) {
		text.scale = new PIXI.Point((entity.mscale || 1) / text_quality, (entity.mscale || 1) / text_quality);
	}
...
function d_text(message, x, y, args) {
	...
	if (is_object(x)) {
		if (x.type && mode.use_new_d_texts) {
			return d_text_new(message, x, y);
		}
		sprite = x;
		args = y;
		x = get_x(sprite);
	...
	if (text_quality > 1) {
		text.scale = new PIXI.Point(1 / text_quality, 1 / text_quality);
	}
	...
	function disappear_text(step, text) {
		return function () {
			var ms = mssince(text.last_fade);
			var y = round((4 * ms) / text.anim_time);
			// console.log(y);
			if (2 < y && y < 7) {
				y = 4;
			}
			...
			text.last_fade = new Date();
			if (text.alpha > 0.25) {
				draw_timeout(disappear_text(step + 1, text), text.anim_time);
			} else {
				remove_sprite(text);
				try {
					text.destroy({ texture: true, baseTexture: true });
...
function api_call(method, args, r_args) {
	...
	var disable = r_args.disable;
	if (args.ui_loader) {
		r_args.r_id = randomStr(10);
		delete args.ui_loader;
	}
	if (args.callback) {
		r_args.callback = args.callback;
	...
	if (window.is_electron) {
		args.epl = electron_data.platform;
	}
	data = { method: method, arguments: JSON.stringify(args) };
	function on_success(r_args, disable) {
		return function (ct) {
			if (r_args.r_id) {
				hide_loader(r_args.r_id);
			}
			if (r_args.callback) {
				r_args.callback.apply(this, [ct]);
			} else {
				handle_information(ct);
			}
			if (r_args.success) {
				smart_eval(r_args.success);
			}
			if (disable) {
				disable.removeClass("disable");
			}
			if (r_args.pid) {
				resolve_deferred("api_call" + r_args.pid);
			}
		};
	}
	function on_error(r_args, disable) {
		return function (ct) {
			...
			if (r_args.silent || in_arr(method, auto_api_methods)) {
				return;
			}
			...
			if (method != "log_error" && ct.status + "" != "0") {
				setTimeout(
					(function (ct, api_path) {
						return function () {
							api_call("log_error", {
								type: "api_call_error",
								err: ct.status + " " + ct.statusText + " on " + api_path,
								info: ct.getAllResponseHeaders(),
							});
						};
					})(ct, api_path),
					40000,
			...
			if (r_args.pid) {
				reject_deferred("api_call" + r_args.pid);
			}
		};
	}
	if (r_args.r_id) {
		show_loader(r_args.r_id);
	}
	call_args = {
		type: "POST",
		dataType: "json",
		url: window.location.origin + api_path,
		data: data,
		success: on_success(r_args, disable),
		error: on_error(r_args, disable),
	};
	$.ajax(call_args);
	if (r_args.promise) {
...
function light_logic() {
	...
	if (S.schedule && S.schedule.time_offset) {
		h = (24 + S.schedule.time_offset + h) % 24;
	}
	...
	if (!G.maps[current_map].outside && G.maps[current_map].lux) {
		llevel = G.maps[current_map].lux + bonus;
	}
	if (G.maps[current_map].outside && G.maps[current_map].lux) {
		llevel = min(llevel, G.maps[current_map].lux);
	}
	...
	if (G.maps[current_map].outside && !G.maps[current_map].lux) {
		alpha = (levels[h] * (60 - m) + levels[h + 1] * m) / 60;
	}
	// if(llevel) alpha=0.5;

	if (llevel == last_llevel) {
		return;
	}
	...
	if (is_sdk) {
		console.log(lighting.clearColor);
	}
...
function update_servers_and_characters() {
	...
	X.characters.forEach(function (character) {
		if (character.online) {
			$(".characterav" + character.name).html('<span style="color: #F3A05D">[I]</span>');
		} else {
			$(".characterav" + character.name).html('<span style="color: #A4FA64">[A]</span>');
		}
	...
	for (var i = 0; i < infs.length; i++) {
		...
		if (info.type == "code_list") {
			if (info.purpose == "load") {
				...
				if (code_change) {
					html +=
						"<div class='gamebutton block' style='margin-bottom: -4px'><span style='color: #E46A64'>[WARNING]</span> Unsaved Changes</div>";
				}
				html +=
				...
				html +=
					"<div class='gamebutton block' style='margin-bottom: -4px' onclick='load_base_code()'><span style='color: gray'>[Default]</span> Load Base Code</div>";
				if (character) {
					html +=
				...
				html +=
					"<div style='margin-top: 10px; font-size: 24px; line-height: 28px; border: 4px solid gray; background: black; padding: 16px;'>";
				html +=
				...
				if (c_slot) {
					for (var num in info.list) {
						if (parseInt(num) == c_slot) {
							c_name = info.list[num][0];
						}
					}
				}
				...
				html +=
					"<input type='text' style='box-sizing: border-box; width: 15%;; float: left' placeholder='#' autocomplete='nope' id='alcodenumx' name='alcodenumx' class='csharp cinput'/>";
				html +=
					"<input type='text' style='box-sizing: border-box; width: 63%;' placeholder='NAME' autocomplete='nope' id='alcodeinputx' name='alcodeinputx' class='codename cinput' />";
				html +=
					"<div class='gamebutton' style='box-sizing: border-box; width: 20%; padding: 8px; float: right' onclick='save_code_s()'>SAVE</div>";
				html += "</div>";
				// info.list={};
				var ui_list = clone(info.list);
				X.codes = info.list;
				if (!Object.keys(ui_list).length) {
					ui_list = { 1: ["Empty", 0], 2: ["Empty", 0] };
				}
				for (var i = 1; i <= 100; i++) {
					if (!ui_list[i]) {
						ui_list[i] = ["Empty", 0];
				...
				if (character && !ui_list[real_id]) {
					ui_list[real_id] = [character.name, 0];
				}
				ui_list["#"] = ["DELETE", 0];
				html +=
					'<div class="gamebutton block" style="display: block; margin-bottom: -4px" onclick="open_guide(\'8-code-slots-and-files\',\'/docs/guide/code/8-code-slots-and-files\')"><span style="color: #6FD23F">[Documentation]</span> Code Slots and Files</div>';
				...
				show_modal(html, { keep_code: true, wrap: false });
				if (c_slot) {
					$("#alcodenumx").val(c_slot);
				}
				if (c_name) {
					$("#alcodeinputx").val(c_name);
				}
			} else {
				show_json(info.list);
			}
			...
			if (
				window.character &&
				info.code_list[code_slot] &&
				(!X.codes[code_slot] || info.code_list[code_slot][1] > X.codes[code_slot][1])
			) {
			...
			storage_set(
				"reload" + server_region + server_identifier,
				JSON.stringify({ time: reload_time, ip: info.ip, port: "" + info.port }),
			);
			...
			if (info.slot > 0 && info.v) {
				X.codes[info.slot] = [info.name, info.v];
			}
			if (info.save) {
				if (is_electron && electron_is_main()) {
				...
				if (info.reset || new_code_slot != code_slot) {
...
function sfx(type, x, y) {
	try {
		...
		var sound = null;
		if (type == "hit" || type == "monster_hit") {
			sound = sounds.hit_8bit;
		}
		if (type == "explosion") {
			sound = sounds.fx_explosion;
		}
		if (type == "coins") {
			sound = sounds.coin_collect;
		}
		if (type == "hp" || type == "mp") {
			sound = sounds.use_8bit;
		}
		if (type == "chat") {
			sound = sounds.chat;
		}
		...
		if (sound) {
			if (x === undefined) {
				...
				if (mode.directional_sfx) {
					...
					var volume = 1;
					if (!sound.original_volume) {
						sound.original_volume = sound.volume();
					}
					if (distance > 700) {
						volume = 0.05;
...
function performance_trick() {
	...
	sounds.empty = new Howl({
		src: [
			url_factory("/sounds/loops/empty_loop_for_js_performance.ogg"),
			url_factory("/sounds/loops/empty_loop_for_js_performance.wav"),
		],
		volume: 0.5,
		autoplay: true,
		loop: true,
...
function reflect_music() {
	...
	if (current_map == "tavern") {
		the_music = sounds.rpg10;
	}
	if (in_arr(current_map, ["cave", "halloween", "spookytown"]) || current_map.startsWith("level")) {
		the_music = sounds.rpg14;
	}
	if (current_map.startsWith("winter_inn")) {
		the_music = sounds.rpg16;
	}
	if (current_map == "desertland") {
		the_music = sounds.rpg07;
	}
	if (current_map == "winterland" || xmas_tunes) {
		the_music = sounds.christmas;
	}
	if (current_map == "bank") {
		the_music = sounds.casual05;
	}
	if (current_map == "goobrawl") {
		the_music = sounds.casual02;
	}
	if (current_map == "crypt" || current_map == "winter_instance") {
		the_music = sounds.horror02;
	}
...
function sound_on() {
	if (is_electron) {
		Cookies.set("music", "on", { expires: 12 * 365 });
	} else if (character) {
...
function sound_off(just_ui) {
	if (!just_ui) {
		if (is_electron) {
			Cookies.set("music", "off", { expires: 12 * 365 });
		} else if (character) {
...
function sfx_on() {
	if (is_electron) {
		Cookies.set("sfx", "on", { expires: 12 * 365 });
	} else if (character) {
...
function sfx_off() {
	if (is_electron) {
		Cookies.set("sfx", "off", { expires: 12 * 365 });
	} else if (character) {
...
function game_stringify(obj, third) {
	...
	try {
		...
		var result = JSON.stringify(
			obj,
			function (key, val) {
				if (in_arr(key, ignored_properties) || key.indexOf("filter_") != -1 || key[0] == "_") {
					return;
				}
				if (key == "data" && obj[key] && obj[key].x_lines) {
					return;
				}
...
function filename_to_cdata(oname) {
	...
	if (name + "." + slot + ".js" != oname) {
		valid = false;
	}
	name.split("").forEach(function (c) {
		if (!valid_file_chars.includes(c)) {
			valid = false;
		}
	});
	if (!valid) {
...
function get_code_file(name) {
	...
	for (var slot in code_ref) {
		if (slot == name || code_ref[slot].name == filename) {
			...
			if (parseInt(slot) <= 100) {
				folder = ide_root + "/adventureland/codes";
			}
...
async function save_code_file(num, data, v) {
	// only "code"
	var folder = ide_root + "/adventureland/characters";
	var filename = X.codes[num][0] + "." + num + ".js";
	if (parseInt(num) <= 100) {
		folder = ide_root + "/adventureland/codes";
	}
	if (code_ref[num]) {
		await fs.promises
			.copyFile(folder + "/" + code_ref[num].file, ide_root + "/history/" + code_ref[num].slot + ".js")
			.catch();
	}
...
async function process_file(folder, cdata) {
	...
	if (deleted) {
				...
				return function () {
					if (code_ref[num] && code_ref[num].file == fname) {
						...
						show_confirm(
							...
							(function (slot) {
								return function () {
									api_call("save_code", { name: "DELETE", slot: slot, electron: true }, { promise: true });
									hide_modal(true);
								};
							})(num),
						);
		...
		await fs.promises
			...
			.then(async (data) => {
				if (character && cdata.slot == code_slot) {
					handle_information([{ type: "code", code: data.toString() }]);
				}
				if (data) {
					await api_call(
						"save_code",
						{ code: data.toString(), slot: cdata.slot, name: cdata.name, auto: true, electron: true },
						{ promise: true },
					);
				}
			})
		...
		code_ref[cdata.slot] &&
		(code_ref[cdata.slot].m != new_m ||
			cdata.name != X.codes[cdata.slot][0] ||
			X.codes[cdata.slot][1] != code_ref[cdata.slot].v)
	) {
		...
		if (code_ref[cdata.slot] && X.codes[cdata.slot][1] > code_ref[cdata.slot].v && code_ref[cdata.slot].m != new_m) {
			...
			await fs.promises
				.copyFile(folder + "/" + cdata.file, ide_root + "/adventureland/clash/" + cdata.file + ".c" + randomStr(10))
				.catch(() => {});
			file_op[cdata.file] = new Date();
			await fs.promises.unlink(folder + "/" + cdata.file);
			file_op[cdata.file] = new Date();
			...
			.then(async (files) => {
				for (var i = 0; i < files.length; i++) {
					...
					if (cdata && found[cdata.slot]) {
						add_log(
							"You need to either delete " + found[cdata.slot] + " or " + fname + " when the game client is closed!",
							"#A88690",
						);
					} else if (cdata) {
...
function electron_start_watching() {
	...
	watcher1 = fs.watch(ide_root + "/adventureland/characters", function (event, fname) {
		if (file_op[fname] && mssince(file_op[fname]) < 3600) {
			return;
		}
		console.log("watcher1: " + fname);
		var cdata = filename_to_cdata(fname);
		if (cdata) {
			process_file(ide_root + "/adventureland/characters", cdata);
		} else {
			add_log("Invalid filename: " + fname, "#A88690");
		}
	});
	watcher2 = fs.watch(ide_root + "/adventureland/codes", function (event, fname) {
		...
		var cdata = filename_to_cdata(fname);
		if (cdata) {
			process_file(ide_root + "/adventureland/codes", cdata);
		} else {
			add_log("Invalid filename: " + fname, "#A88690");
		}
	});
...
function electron_screenshot(opt, cb) {
	...
	if (!opt.filename) {
		opt.filename = "AL Screenshot " + new Date() + ".png";
	}
	var remote;
	try {
...
function electron_add_webview(url) {
	if (!url) {
		url = "http://thegame.com/character/GG/in/EU/I/";
	}
	$("body").append(
		"<webview src='" +
			url +
			"' style='position: fixed; top: 0px; left: 0px; bottom: 0px; right: 0px' disablewebsecurity nodeintegration></webview>",
	);
}
