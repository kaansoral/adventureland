...
function stripe_pay() {
	...
	if (!window.Stripe) {
		alert(
			"Stripe hasn't loaded. Please refresh the page and email hello@adventure.land if this is persistent. Thank you.",
		);
		return;
	}
	...
	if (in_arr(stripe_state, ["failed", "declined", "success"])) {
		return set_pamount(pamount);
	}
...
function show_payments() {
	...
	var html = "";
	html +=
		"<div style='position: fixed; top: 0px; bottom: 0px; left: 0px; right: 0px; z-index: 9999; background: rgba(0,0,0,0.5); text-align: center' class='paymentsui'>";

	html += $("#paymentshtml").html();

	html +=
		"<div class='gamebutton clickable' onclick='$(\".paymentsui\").remove()' style='position: fixed; z-index: 10000; top: 0px; right: 0px; color: #CFCFCF'>Back &gt;</div>";
...
function show_ppayments() {
	var html = "";
	html +=
		"<div style='position: fixed; top: 0px; bottom: 0px; left: 0px; right: 0px; z-index: 9999; background: rgba(0,0,0,0.5)' class='paymentsui'>";

	html +=
		"<div style='position: fixed; top: " +
		round(($(window).height() - 520) / 2) +
		"px; left: " +
		round(($(window).width() - 750) / 2) +
		"px;'>";
	html +=
		'<iframe src="https://api.paymentwall.com/api/ps/?key=07119679ef07a110740ecfc89da924e6&uid=[USER_ID]&widget=p10_1" width="750" height="520" frameborder="0" ';
...
function show_poffers() {
	...
	html +=
		'<iframe src="https://api.paymentwall.com/api/?key=07119679ef07a110740ecfc89da924e6&uid=[USER_ID]&widget=w6_1" width="800" height="' +
		($(window).height() - 100) +
		'" frameborder="0" ';
	html += 'style="border: 5px solid gray; background: black"></iframe>';
	html += "</div>";
	...
	html +=
		"<div class='gamebutton clickable' onclick='$(\".paymentsui\").remove()' style='position: fixed; z-index: 10000; top: 0px; right: 0px'>Back &gt;</div>";
...
function show_sroffers() {
	var html = "";
	html +=
		"<div style='position: fixed; top: 0px; bottom: 0px; left: 0px; right: 0px; z-index: 9999; background: rgba(0,0,0,0.5); overflow-y: scroll' class='paymentsui'>";

	html += "<div style='margin-top: 40px; margin-left: " + round(($(window).width() - 728) / 2) + "px; z-index: 9000'>";
	html +=
		'<iframe src="https://wall.superrewards.com/super/offers?h=shmimyttqnb.811777903063&uid=' +
		user_id +
		'" frameborder="0" width="728" height="2400" scrolling="no"';
	html += 'style="border: 5px solid gray; background: #FAFAFA"></iframe>';
	html += "</div>";

	html +=
		"<div class='gamebutton clickable' onclick='$(\".paymentsui\").remove()' style='position: fixed; z-index: 10000; bottom: 0px; right: 0px'>BACK &gt;</div>";
