...
function process_game_data() {
	...
	for (var name in G.monsters) {
		if (G.monsters[name].charge) {
			continue;
		}
		if (G.monsters[name].speed >= 60) {
			G.monsters[name].charge = round(G.monsters[name].speed * 1.2);
		} else if (G.monsters[name].speed >= 50) {
			G.monsters[name].charge = round(G.monsters[name].speed * 1.3);
		} else if (G.monsters[name].speed >= 32) {
			G.monsters[name].charge = round(G.monsters[name].speed * 1.4);
		} else if (G.monsters[name].speed >= 20) {
			G.monsters[name].charge = round(G.monsters[name].speed * 1.6);
		} else if (G.monsters[name].speed >= 10) {
			G.monsters[name].charge = round(G.monsters[name].speed * 1.7);
		} else {
			G.monsters[name].charge = round(G.monsters[name].speed * 2);
		}
	...
	for (var c in G.classes) {
		...
		free_cx.forEach(function (cx) {
			if (!G.classes[c].xcx.includes(cx)) {
				G.classes[c].xcx.push(cx);
			}
		});
		G.classes[c].looks.forEach(function (l) {
			if (!G.classes[c].xcx.includes(l[0])) {
				G.classes[c].xcx.push(l[0]);
			}
			for (var slot in l[1]) {
				if (!G.classes[c].xcx.includes(l[1][slot])) {
					G.classes[c].xcx.push(l[1][slot]);
				}
			}
		});
	}
	...
	for (var name in G.maps) {
		...
		(map.npcs || []).forEach(function (npc) {
			...
			if (data.role == "newupgrade") {
				map.upgrade = map.compound = coords;
			} // Refactored the NPC's, but decided to leave these [26/06/18]
			if (data.role == "exchange") {
				map.exchange = coords;
			}
			if (data.quest) {
				G.quests[data.quest] = coords;
			}
		});
	}
...
function prune_cx(cx, skin) {
	if (skin && cx.upper && cx.upper == "skin") {
		delete cx.upper;
	}
	for (var sname in cx) {
		if (
...
function can_stack(a, b, d, args) {
	if (
		...
		if (a.name == "cxjar" && a.data != b.data) {
			return false;
		}
		if (a.name == "emotionjar" && a.data != b.data) {
			return false;
		}
		if (!args || !args.ignore_pvp) {
			if ((a.v && !b.v) || (!a.v && b.v)) {
				return false;
			}
		} // pvp
...
function can_add_items(player, items, args) {
	...
	var overhead = [];
	if (player.esize + (args.space || 0) >= needed || !needed) {
		return true;
	}
	...
	if (player.esize + (args.space || 0) >= needed) {
		return true;
	}
	return false;
}
...
function resolve_deferreds(name, data) {
	while (deferreds[name] && deferreds[name].length) {
...
function resolve_deferred(name, data) {
	...
	if (0) {
		...
		else if (current_deferred.start) {
...
function object_sort(o, algorithm) {
	...
	function gsort(x, y) {
		// console.log(x);
		if (G.items[x[0]].g < G.items[y[0]].g) {
			return -1;
		} else if (G.items[x[0]].g < G.items[y[0]].g && x[0] < y[0]) {
			return -1;
		}
		return 1;
	}
...
function direction_logic(entity, target, mode) {
	...
	if (is_game && !new_attacks && entity.moving) {
		return;
	}
...
function within_xy_range(observer, entity) {
	...
	var x = get_x(entity);
	var y = get_y(entity);
	var o_x = get_x(observer);
	var o_y = get_y(observer);
	if (
...
function distance(_a, _b, in_check) {
	...
	if ("in" in _a && "in" in _b && _a.in != _b.in) {
		return 99999999;
	}
	if ("map" in _a && "map" in _b && _a.map != _b.map) {
		return 99999999;
	}
	...
	// Check overlap
	if (a_x - a_w2 <= b_x + b_w2 && a_x + a_w2 >= b_x - b_w2 && a_y >= b_y - b_h && a_y - a_h <= b_y) {
		return 0;
	}
...
function can_walk(entity) {
	if (entity.s && entity.s.dash) {
		return false;
	}
	if (is_game && entity.me && transporting && ssince(transporting) < 8 && !entity.c.town) {
		return false;
	}
	if (is_code && entity.me && parent.transporting && ssince(parent.transporting) < 8 && !entity.c.town) {
		return false;
	}
...
function is_silenced(entity) {
	if (!entity || is_disabled(entity) || (entity.s && entity.s.silenced)) {
		return true;
	}
}
...
function calculate_item_grade(def, item) {
	...
	if (((item && item.level) || 0) >= (def.grades || [9, 10, 11, 12])[3]) {
		return 4;
	}
	if (((item && item.level) || 0) >= (def.grades || [9, 10, 11, 12])[2]) {
		return 3;
	}
	if (((item && item.level) || 0) >= (def.grades || [9, 10, 11, 12])[1]) {
		return 2;
	}
	if (((item && item.level) || 0) >= (def.grades || [9, 10, 11, 12])[0]) {
		return 1;
	}
	return 0;
}
...
function calculate_item_value(item, m) {
	...
	var value = (def.cash && def.g) || def.g * (m || 0.6);
	var divide = 1; // previously 0.8
	if (def.markup) {
		value /= def.markup;
	}
	if (def.compound && item.level) {
		var grade = 0;
		var grades = def.grades || [11, 12];
		var s_value = 0;
...
function calculate_item_properties(item, args) {
	...
	var cls = "";
	var map = "";
	if (args["class"] && def[args["class"]]) {
		cls = args["class"];
	}
	if (args["map"] && def[args["map"]]) {
		map = args["map"];
	}
	var prop_key =
		def.name +
	...
	if (cls) {
		adopt_extras(def, def[cls]);
	}
	if (map) {
		adopt_extras(def, def[map]);
	}
	...
	if (item.p == "shiny") {
		if (def.attack) {
			prop.attack += 4;
			if (doublehand_types.includes(G.items[item.name].wtype)) {
				prop.attack += 3;
			}
		} else if (def.stat) {
			prop.stat += 2;
	...
	if (def.upgrade || def.compound) {
		...
		for (var i = 1; i <= level; i++) {
			...
			for (p in u_def) {
				if (p == "stat") {
					prop[p] += round(u_def[p] * multiplier);
				} else {
					prop[p] += u_def[p] * multiplier;
				} // for weapons with float improvements [04/08/16]
				if (p == "stat" && i >= 7) {
					prop.stat++;
				}
			}
	...
	if (item.level == 10 && prop.stat && def.tier && def.tier >= 3) {
		prop.stat += 2;
	}
	for (p in def) {
		if (prop[p] === null) {
			prop[p] = def[p];
		} else if (prop[p] != undefined) {
			prop[p] += def[p];
		}
	}
	if (item.p == "legacy" && def.legacy) {
		for (var name in def.legacy) {
			if (def.legacy[name] === null) {
				delete prop[name];
			} else {
				prop[name] = (prop[name] || 0) + def.legacy[name];
			}
		}
	}
	for (p in prop) {
...
function random_one(arr) {
	if (!is_array(arr)) {
		return random_one(Object.values(arr));
	}
	if (!arr.length) {
		return null;
	}
...
function to_pretty_float(num) {
	...
	var num = parseFloat(hnum);
	if (parseFloat(hnum) == parseFloat(num.toFixed(1))) {
		hnum = num.toFixed(1);
	}
	if (parseFloat(hnum) == parseFloat(parseInt(num))) {
		hnum = parseInt(num);
	}
	return hnum;
}
...
function set_xy(entity, x, y) {
	if ("real_x" in entity) {
		(entity.real_x = x), (entity.real_y = y);
	} else {
...
function calculate_vxy(monster, speed_mult) {
	...
	if (1 || is_game) {
		monster.angle = (Math.atan2(monster.going_y - monster.from_y, monster.going_x - monster.from_x) * 180) / Math.PI;
	} // now the .angle is used on .resync [03/08/16]
...
function is_in_front(observer, entity) {
	...
	if (observer.angle !== undefined && Math.abs(observer.angle - angle) <= 45) {
		return true;
	} // drawn at notebook 2, based on those drawings [11/09/16]
	return false;
}

function calculate_movex(map, cur_x, cur_y, target_x, target_y) {
	if (target_x == Infinity) {
		target_x = CINF;
	}
	...
	for (var i = bsearch_start(x_lines, min_x); i < x_lines.length; i++) {
		...
		if (eps_equal(cur_x, target_x) && eps_equal(cur_x, line_x)) {
			...
			if (going_down) {
				...
				(y_intersect = max(line[1], line[2]) + XEPS), (target_y = min(target_y, y_intersect)), (min_y = target_y);
			}
			continue;
		}
	...
	for (var i = bsearch_start(y_lines, min_y); i < y_lines.length; i++) {
		...
		if (eps_equal(cur_y, target_y) && eps_equal(cur_y, line_y)) {
			...
			if (going_right) {
				...
				(x_intersect = max(line[1], line[2]) + XEPS), (target_x = min(target_x, x_intersect)), (min_x = target_x);
			}
			continue;
		}
...
function set_base(entity) {
	...
	if (G.dimensions[type] && G.dimensions[type][3]) {
		...
		if (!entity.base.h) {
			(entity.base.h = 8), (entity.base.v = 7);
		}
	}
}
...
var line_hit_y = null;
var m_dx;
var m_dy; // flags so can_calculate and can_move work in synergy
...
function calculate_move(entity, target_x, target_y) {
	...
	m_calculate = true;
	var map = entity.map;
	var cur_x = get_x(entity);
	var cur_y = get_y(entity);
	var corners = [[0, 0]];
	...
	function check_move(xy) {
		...
		if (line_hit_x !== null) {
			x_moves.push([line_hit_x, line_hit_y]), (line_hit_x = null), (line_hit_y = null);
		}
	}
	...
	if (point_distance(cur_x, cur_y, move.x, move.y) < 10 * EPS) {
...
function can_move(monster, based) {
	...
	if (!based && monster.base) {
		...
		if (1) {
			...
			var px1 = -monster.base.h;
			m_line_x = max; // going left
			if (x1 > x0) {
				(px0 = -monster.base.h), (px1 = monster.base.h), (mcy = m_line_x = min);
			} // going right
			var py0 = monster.base.vn;
			var py1 = -monster.base.v;
			m_line_y = max; // going up
			if (y1 > y0) {
				(py0 = -monster.base.v), (py1 = monster.base.vn), (m_line_y = min);
			} // going down
	...
	for (var i = bsearch_start(GEO.x_lines || [], minx); i < (GEO.x_lines || []).length; i++) {
		...
		if (minx > line[0]) {
			continue;
		} // can be commented out with: while(start<arr.length && arr[start][0]<value) start++;
		if (maxx < line[0]) {
			break;
		} // performance improvement, we moved past our range [16/07/18]
		next = y0 + ((y1 - y0) * (line[0] - x0)) / (x1 - x0 + REPS);
		if (!(line[1] - EPS <= next && next <= line[2] + EPS)) {
			continue;
		} // Fixed EPS [16/07/18]
		//add_log("line clash")
		if (m_line_y) {
	...
	if (
		...
		if (
			can_move({
				...
				y: get_y(entity) + 8.1,
				going_x: get_x(entity),
				going_y: get_y(entity) + 8.1 + EPS / 2,
				base: entity.base,
			})
		) {
...
function stop_logic(monster) {
	...
	if (
		...
		if (monster.s && monster.s.dash) {
			...
			if (is_server) {
				resend(monster, "u+cid");
			}
		}
		if (monster.name_tag) {
			stop_name_tag(monster);
		}
		if (monster.me) {
		...
		if (is_point_inside([rX, rY], polygon)) {
			...
			if (base) {
				base_points(rX, rY, base).forEach(function (xy) {
					if (!is_point_inside([xy[0], xy[1]], polygon)) {
						valid = false;
					}
				});
			}
...
function clone(obj, args) {
	...
	if (!args.seen && args.seen !== []) {
		args.seen = [];
	} // seen modification - manual [24/12/14]
	if (null == obj) {
		return obj;
	}
	if (args.simple_functions && is_function(obj)) {
...
function is_substr(a, b) {
	if (is_array(b)) {
		for (var i = 0; i < b.length; i++) {
			try {
				if (a && a.toLowerCase().indexOf(b[i].toLowerCase()) != -1) {
					return true;
				}
			} catch (e) {}
		}
		...
		try {
			if (a && a.toLowerCase().indexOf(b.toLowerCase()) != -1) {
				return true;
			}
		} catch (e) {}
	}
...
function randomStr(len) {
	...
	var schars = "ABCDEFGHIJKLMNOPQRSTUVWXTZabcdefghiklmnopqrstuvwxyz";
	var str = "";
	for (var i = 0; i < len; i++) {
		if (i == 0) {
...
function log_trace(place, err) {
	...
	if (typeof err === "object") {
		...
		console.log("log_trace: argument is not an object on :" + place);
	}
	console.log("====================\n");
}
