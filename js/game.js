...
var antialias = false;
var mode_nearest = true; // mode_nearest fixes the overflows in general
...
var socket_welcomed = false;
var game_loaded = false;
var friends = [];
var ch_disp_x = 0;
var ch_disp_y = 0; // For shaking the character without modifying coordinates manually [22/06/18]
var head_x = 0;
var head_y = 0; // For head gestures [25/09/18]
var tints = [];
var entities = {};
var future_entities = { players: {}, monsters: {} };
var pull_all_next = false;
var clean_house = false;
...
var ctarget = null;
var ctoggled = null; // character's target
...
var FC = {};
var SS = {};
var SSU = {}; // cache textures here
var M = {};
var GEO = {}; // M - old x_lines/y_lines, - GEO - new logic
/* start map entities */
var total_map_tiles = 0;
var tiles = null;
var dtile = null;
var wtile = null; // cached_map entities
var map_npcs = [];
var map_doors = [];
var map_nights = [];
var map_lights = [];
...
var quirks = {};
var water_tiles = [];
var last_water_frame = -1;
var drawings = [];
var code_buttons = {}; // user drawings
var chests = {};
var party_list = [];
var party = {};
var tracker = {};
...
var actual_code = false;
var CC = {}; // run from game, active from code [30/10/16]
var reload_state = false;
var reload_timer = null;
var first_entities = false;
var blink_pressed = false;
var last_blink_pressed = new Date();
var arrow_up = false;
...
setInterval(function () {
	...
	if (window.last_draw) {
		...
		else if (!(code_run || sound_sfx) && mssince(last_draw) > 15000) {
			draw(0, 1);
		}
	}
...
function log_in(user, character, auth, passphrase) {
	if (!socket) {
		return show_alert("Connect to a server first!");
	}
	real_id = character;
	if (!passphrase) {
		passphrase = storage_get("passphrase") || "";
	}
	if (!game_loaded) {
	...
	if (no_html && parent && parent.character) {
		no_html_value = parent.character.name;
	}
	var data = {
		user: user,
		character: character,
	...
	if (is_electron) {
		...
		if (data.epl == "mas") {
			data.receipt = electron_mas_receipt();
		}
		if (data.epl == "steam") {
			data.ticket = electron_steam_ticket();
		}
	}
...
function disconnect() {
	...
	} else if (window.disconnect_reason) {
			...
			$("body")
				...
				.each(function () {
					if (this.tagName != "CANVAS" && this.id != "bottomrightcorner" && this.id != "bottomleftcorner2") {
						$(this).remove();
					} else if (this.id == "bottomrightcorner" || this.id == "bottomleftcorner2") {
	...
	if (character) {
		$("title").html(title_m + " - " + character.name);
	}
}
...
function position_map() {
	...
	var y = height / 2.0 - map.real_y * scale;
	var change = false;
	x = c_round(x);
	y = c_round(y);
	if (map.x != x) {
...
function ui_logic() {
	if (character && character.ctype == "mage") {
		...
		} else if (!b_pressed && map.cursor == "crosshair") {
			map.cursor = "default";
		}
	}
}
...
var rendered_target = {};
var last_target_cid = null;
var last_xtarget_cid = null;
var dialogs_target = null;
...
function draw_entities() {
	for (entity in entities) {
		...
		if (current.dead || clean_house) {
			...
			if (current.drawn && current.tpd) {
				draw_timeout(fade_away_teleport(1, current), 30, 1);
			} else if (current.drawn) {
				draw_timeout(fade_away(1, current), 30, 1);
			} else {
...
function sync_entity(current, monster) {
	...
	if (current.resync) {
		...
		if (monster.moving) {
			(current.engaged_move = -1), (current.move_num = 0);
	...
	if (current.move_num != current.engaged_move) {
		var speedm = 1;
		var dist = simple_distance({ x: current.real_x, y: current.real_y }, monster);
		if (dist > 120) {
			// previously 40 [07/10/16]
			current.real_x = monster.x;
			current.real_y = monster.y;
			if (log_flags.xy_corrections) {
				console.log("manual x,y correction for: " + (current.name || current.id));
			}
		}
		speedm =
		...
		if (speedm > 1.25 && log_flags.timers && log_flags.xy) {
			console.log(current.id + " speedm: " + speedm);
		}
		current.moving = true;
		current.abs = false;
...
function process_entities() {
	for (var id in future_entities.monsters) {
		...
		if (!current) {
			...
			try {
				...
				if (is_sdk) {
					alert(e + " " + JSON.stringify(monster));
				}
			}
		}
		if (monster.dead) {
		...
		if (ctarget && ctarget.id == current.id) {
			ctarget = current;
		}
		if (xtarget && xtarget.id == current.id) {
			xtarget = current;
		}
	}
	for (var id in future_entities.players) {
		...
		var original_rip = true;
		if (current) {
			original_rip = current.rip;
		}
...
function on_disappear(data) {
	...
	} // moving these outside the `if(entities[data.id])` condition should fix the non-disappearing entities issues [03/02/17]
	...
	if (element.slots) {
		...
		for (var slot in element.slots) {
			if (attire_slots.includes(slot) && element.slots[slot]) {
				...
				if (element.slots[slot].level == 12) {
					element.pzazz += gr * 12;
				} else if (element.slots[slot].level >= 11) {
					element.pzazz += gr * 8;
				} else if (element.slots[slot].level == 10) {
					element.pzazz += gr * 6;
				} else if (element.slots[slot].level == 9) {
					element.pzazz += gr * 2;
				} else if (element.slots[slot].level == 8) {
					element.pzazz += gr / 2;
				}
			}
		}
...
function update_tutorial_ui() {
	...
	if (last_rendered_step > X.tutorial.step) {
		...
		if (last_rendered_step < X.tutorial.step) {
			...
			completion = X.tutorial.progress;
		}
	...
	if (X.tutorial.step > 1) {
		$(".flasht").removeClass("flasht");
	}
	...
	if (X.tutorial.finished || !tutorial_ui) {
		$(".tutorialui").hide();
	} else {
		$(".tutorialui").show();
	}
...
function the_game(demo) {
	...
	if (is_cli && window.ls_emulation) {
		window._localStorage = window.ls_emulation;
	}
	width = $(window).width();
	...
	} // , resolution:window.devicePixelRatio, autoResize:true
	...
	if (PIXI.DisplayList && !no_graphics) {
		...
		monster_layer = new PIXI.DisplayGroup(1, function (sprite) {
			...
			if ("real_y" in sprite) {
				sprite.zOrder = -sprite.real_y + disp + (sprite.y_disp || 0);
			} else {
				sprite.zOrder = -sprite.position.y + disp + (sprite.y_disp || 0);
			}
		});
		...
		var child_sort = function (sprite) {
			var disp = 0;
			if (sprite.parent.stand) {
				disp = -3;
			}
			if ("real_y" in sprite.parent) {
				sprite.zOrder = -sprite.parent.real_y + disp + (sprite.parent.y_disp || 0);
			} else {
				sprite.zOrder = -sprite.parent.position.y + disp + (sprite.parent.y_disp || 0);
			}
		};
	...
	} //,{xhrType:PIXI.loaders.Resource.XHR_RESPONSE_TYPE.DOCUMENT}
...
function demo_entity_logic(sprite) {
	...
	if (sprite.pause && mssince(sprite.pause) < 800) {
		return;
	}
	if (Math.random() < 0.1) {
		sprite.pause = new Date();
	}
	var moves = [
		[1, 0],
		[0, 1],
...
function init_socket(args) {
	...
	} // Cookies.set('windows','1',{expires:12*365});
	else if (is_sdk) {
		if (window.location.origin == "http://127.0.0.1/") {
	...
	if (location.protocol == "https:") {
		...
		window.socket = io(server_addr + ":" + server_port, { transports: ["websocket"], query: query });
	}
	...
	socket.on("welcome", function (data) {
		...
		if (!first_welcome) {
			first_welcome = true;
			if (is_electron && electron_is_main() && user_id) {
				setTimeout(electron_code_sync_logic, 1);
			}
		}
	...
	socket.on("start", function (data) {
		...
		var scode_slot = data.code_slot;
		delete data.code;
		delete data.code_version;
		delete data.code_slot;
		if (scode_slot) {
			console.log("start: " + data.id + " code: " + scode_slot);
		}
		...
		G.classes[character.ctype].xcx.forEach(function (c) {
			if (!character.xcx.includes(c)) {
				character.xcx.push(c);
			}
		});
		if (character.level == 1) {
			if (X && X.tutorial && !X.tutorial.finished && tutorial_ui) {
				open_tutorial();
			} else {
				show_game_guide();
			}
		}
		if (character.ctype == "merchant" || recording_mode || 1) {
	...
	socket.on("game_log", function (data) {
		if ((data.message || data) == "You killed a Goo") {
			tut("killagoo");
		}
		draw_trigger(function () {
			if (is_string(data)) {
	...
	socket.on("light", function (data) {
		draw_trigger(function () {
			...
			if (data.affected) {
				start_animation(character, "light");
			}
			last_light = new Date();
	...
	socket.on("game_response", function (data) {
		...
		try {
			...
			var event = false;
			if (data.cevent) {
				(cevent = data.cevent), delete data.cevent;
			}
			if (cevent === true) {
				cevent = response;
			}
			if (data.event) {
				(event = data.event), delete data.event;
			}
			if (event === true) {
				event = response;
			}
		...
		draw_trigger(function () {
			if (response == "elixir") {
				...
				ui_log("Invalid", "gray");
			} else if (response == "only_in_home") {
				ui_log("You can only do this in your home server!", "gray");
			...
			} else if (response == "party_full") {
				ui_log("The party is full", "gray");
			} else if (response == "already_in_party") {
				ui_log("Already in this party", "gray");
			} else if (response == "player_gone") {
				ui_log(data.name + " is gone", "gray");
			} else if (response == "invitation_expired") {
				ui_log("Invitation expired", "gray");
			} else if (response == "request_expired") {
				ui_log("Request expired", "gray");
			} else if (response == "cant_kick") {
				ui_log("You can't kick someone who's above you.", "gray");
				...
				if (!data.stale) {
					resolve_deferred("compound", { success: false, level: data.level, num: data.num });
				}
				...
				ui_error("ERROR!");
			} else if (response == "got_picked") {
				ui_log("Felt a touch", "#D8866C");
			...
			} else if (response == "inviter_gone") {
				ui_log("Inviter gone", "gray");
			} else if (response == "not_ready") {
				d_text("NOT READY", character);
			} else if (response == "cant_equip") {
				d_text("CAN'T EQUIP", character);
			} else if (response == "cant") {
				d_text("CAN'T", character);
			} else if (response == "muted") {
				d_text("MUTED", character);
			} else if (response == "cant_consume") {
				d_text("CAN'T CONSUME", character);
			} else if (response == "giveaway") {
				d_text("GIVEAWAY?!", character);
			} else if (response == "no_merchants") {
				ui_log("No merchants!", "gray");
			} else if (response == "join_too_late") {
				ui_log("Too late to join", "gray");
			} else if (response == "receiver_unavailable") {
				ui_log("Receiver unavailable", "gray");
				...
				d_text("TOO FAR", (data.id && get_entity(data.id)) || character);
			} else if (response == "invalid_target") {
				d_text("DOESN'T WORK", (data.id && get_entity(data.id)) || character);
				...
				if (data.reason == "level") {
					ui_log("Level gap higher than 10", "gray");
				}
			} else if (response == "no_skill") {
				ui_log("Skill doesn't exist", "gray");
			} else if (response == "target_alive") {
				d_text("LOOKS LIVE?", character);
			} else if (response == "slot_occuppied") {
				ui_log("Slot occuppied", "gray");
			} else if (response == "no_target") {
				d_text((!ctarget && "NO TARGET") || "INVALID TARGET", character);
			} else if (response == "non_friendly_target") {
				d_text("NON FRIENDLY", character);
			} else if (response == "cant_respawn") {
				ui_log("Can't respawn yet.", "gray");
			} else if (response == "chat_slowdown") {
				ui_log("You can't chat this fast.", "gray");
			} else if (response == "not_in_party") {
				ui_log("You are not in a party.", "gray");
				...
				if (Math.random() < 0.5) {
					...
					d_text("GET OUT THE WAY", get_npc("bouncer"), { color: "#F7A9C5" });
				}
			...
			} else if (response == "bank_unavailable") {
				ui_log("Bank unavailable", "gray");
			} else if (response == "bank_withdraw") {
				ui_log("Withdrew " + to_pretty_num(data.gold) + " gold", "gray");
			} else if (response == "bank_store") {
				ui_log("Stored " + to_pretty_num(data.gold) + " gold", "gray");
			} else if (response == "bank_new_pack") {
				if (data.gold) {
					ui_log("Opened an account for " + to_pretty_num(data.gold) + " gold", "gray");
				} else {
					ui_log("Opened an account for " + to_pretty_num(data.shells) + " shells", "gray");
				}
			} else if (response == "locked") {
				ui_log("Locked", "gray");
			} else if (response == "seller_gone") {
				ui_log("Seller gone", "gray");
			} else if (response == "buyer_gone") {
				ui_log("Buyer gone", "gray");
			} else if (response == "item_gone") {
				ui_log("Item gone", "gray");
			} else if (response == "hmm") {
				ui_log("Hmm.", "gray");
			} else if (response == "sneaky") {
				ui_log("Sneaky sneaky.", "gray");
			} else if (response == "need_auth") {
				ui_log("To perform this action your account needs a game client authorization", "gray");
			} else if (response == "giveaway_join") {
				ui_log(data.name + " joined your giveaway!", "gray");
			...
			} else if (response == "tavern_too_late") {
				ui_log("Too late to bet!", "gray");
			} else if (response == "tavern_not_yet") {
				ui_log("Not taking bets yet!", "gray");
			} else if (response == "tavern_too_many_bets") {
				ui_log("You have too many active bets", "gray");
			} else if (response == "tavern_dice_exist") {
				ui_log("You already have a bet", "gray");
			} else if (response == "tavern_gold_not_enough") {
				...
				d_text("WAIT", character);
			} else if (response == "cant_reach") {
				ui_log("Can't reach", "gray");
			} else if (response == "no_item") {
				ui_log("No item provided", "gray");
			} else if (response == "not_enough") {
				ui_log("Not enough", "gray");
			} else if (response == "buyer_gold") {
				ui_log("Not enough gold on buyer", "gray");
			} else if (response == "dont_have_enough") {
				ui_log("Don't have enough", "gray");
			} else if (response == "op_unavailable") {
				add_chat("", "Operation unavailable", "gray");
			} else if (response == "send_no_space") {
				add_chat("", "No space on receiver", "gray");
			} else if (response == "send_no_item") {
				add_chat("", "Nothing to send", "gray");
			} else if (response == "send_no_cx") {
				add_chat("", "Don't have or not enough", "gray");
			} else if (response == "send_diff_owner") {
				add_chat("", "This is not one of ours!", "gray");
			} else if (response == "insufficient_q") {
				ui_log("There aren't that many available", "gray");
			} else if (response == "signed_up") {
				...
				if (data.level) {
					ui_log("Spent " + to_pretty_num(data.cost || 10000) + " gold", "gray");
				} else {
					ui_log("Spent " + to_pretty_num(def.cost) + " gold", "gray");
				}
				...
				no_no_no(2);
			} else if (response == "magiport_failed") {
				ui_log("Magiport failed", "gray"), no_no_no(2);
			} else if (response == "revive_failed") {
	...
	socket.on("game_chat_log", function (data) {
		draw_trigger(function () {
			if (is_string(data)) {
				...
				add_chat("", data.message, data.color);
			} // <- add_chat needs a different color logic
		});
	});
	...
	socket.on("ui", function (data) {
		if (data.event) {
			call_code_function("trigger_event", (data.event === true && data.type) || data.event, data);
		}
		...
		draw_trigger(function () {
			if (in_arr(data.type, ["+$", "-$"])) {
				...
				var player = get_player(data.name);
				if (npc) {
					d_text("+$", npc, { color: "#7E65D3" });
				} //,start_animation(npc,"purple_success");
				if (player) {
					d_text("-$", player, { color: "#7E65D3" });
				}
				call_code_function("trigger_event", "sbuy", { item: data.item, name: data.name });
			} else if (data.type == "+M") {
				var player = get_player(data.name);
				if (player) {
					d_text("+M", player, { color: "#67D385" });
				}
			} else if (data.type == "restore_mp") {
				var player = get_player(data.id);
				if (player) {
					d_text("+" + data.amount + " MPX!", player, { color: "#6585D3", huge: true });
				}
			} else if (data.type == "+$f") {
				...
				var buyer = get_player(data.buyer);
				if (seller) {
					d_text(data.type, seller, { color: colors.white_positive });
				}
				if (buyer) {
					d_text("-$$", buyer, { color: colors.white_negative });
				}
				...
				var receiver = get_player(data.receiver);
				if (sender && receiver) {
					d_line(sender, receiver, { color: "gold" });
				}
			} else if (data.type == "item_sent") {
				var sender = get_player(data.sender);
				var receiver = get_player(data.receiver);
				if (sender && receiver) {
					d_line(sender, receiver, { color: "item" });
				}
			} else if (data.type == "cx_sent") {
				var sender = get_player(data.sender);
				var receiver = get_player(data.receiver);
				if (sender && receiver) {
					d_line(sender, receiver, { color: "cx" });
				}
				...
				if (m) {
					d_text((data.mult == -1 && "-1") || "+1", m, { color: "#9C76D3", size: "huge" });
				}
			} else if (data.type == "disengage") {
				var m = get_entity(data.id);
				if (m) {
					d_text("FFT..", m, { color: "#84A1D1" });
				}
			} else if (data.type == "mheal") {
				var m = get_entity(data.id);
				if (m) {
					d_text(data.heal, m, { color: colors.heal, size: "large" });
				}
			} else if (data.type == "throw") {
				var sender = get_player(data.from);
				var receiver = get_entity(data.to);
				if (sender && receiver) {
					d_line(sender, receiver, { color: "#323232" });
				}
			} else if (data.type == "energize") {
				var sender = get_player(data.from);
				var receiver = get_player(data.to);
				if (sender && receiver) {
					d_line(sender, receiver, { color: "mana" });
				}
				if (receiver) {
				...
				var receiver = get_player(data.to);
				if (sender && receiver) {
					d_line(sender, receiver, { color: "mluck" });
				}
				if (receiver) {
				...
				var receiver = get_player(data.to);
				if (sender && receiver) {
					d_line(sender, receiver, { color: "#D4C392" });
				}
				if (receiver) {
				...
				var receiver = get_player(data.to);
				if (sender && receiver) {
					d_line(sender, receiver, { color: "#9488BF" });
				}
				...
				var receiver = get_player(data.to);
				if (sender && receiver) {
					d_line(sender, receiver, { color: "#6F62AE" });
				}
				if (sender) {
					mojo(sender);
				}
				...
				var sender = get_player(data.name);
				if (sender) {
					d_text("OMG!", sender, { size: "huge", color: "#B9A08C" });
				}
				...
				if (target) {
					d_text("RESIST!", target, { color: "#68B84B" });
				}
			} else if (data.type == "frozen_resist" || data.type == "deepfreezed_resist") {
				var target = get_entity(data.id);
				if (target) {
					d_text("RESIST!", target, { color: "#66C1C8" });
				}
			} else if (data.type == "burned_resist") {
				var target = get_entity(data.id);
				if (target) {
					d_text("RESIST!", target, { color: "#B22F1A" });
				}
			} else if (data.type == "stunned_resist") {
				var target = get_entity(data.id);
				if (target) {
					d_text("RESIST!", target, { color: "crit" });
				}
			} else if (data.type == "huntersmark") {
				var sender = get_player(data.name);
				var target = get_entity(data.id);
				if (sender && target) {
					d_line(sender, target, { color: "#730E0B" });
				}
				if (target) {
					d_text("X", target, { size: "huge", color: "#730E0B" });
				}
				...
				if (attacker) {
					start_emblem(attacker, "rr1", { frames: 10 });
				}
				...
				if (attacker) {
					start_emblem(attacker, "br1", { frames: 5 });
				}
				...
				if (attacker) {
					d_text("BE GONE!", attacker, { size: "huge", color: "#ff5817" });
				}
				...
				if (attacker) {
					points.push({ x: get_x(attacker), y: get_y(attacker) }), flurry(attacker);
				}
	...
	socket.on("server_message", function (data) {
		...
		draw_trigger(function () {
			...
			if (data.type && data.item) {
				call_code_function("trigger_event", data.type, { item: data.item, name: data.name });
			}
		});
	});
	...
	socket.on("chest_opened", function (data) {
		...
		if (data.opener == character.name || data.gone) {
			resolve_deferred("open_chest", data);
		}
		draw_trigger(function () {
			if (chests[data.id]) {
	...
	socket.on("action", function (data) {
		...
		if (target.me) {
			call_code_function("trigger_character_event", "incoming", event_data);
		}
		...
		if (new_attacks) {
			...
			} else if (data.anim == "supershot") {
				d_line(owner, entity, { color: "supershot" });
			} else if (owner && !data.no_lines) {
				d_line(owner, entity);
			}
		}
	});
	socket.on("hit", function (data) {
		...
		if (map_animations[data.pid]) {
			map_animations[data.pid].to_delete = true;
		}
		...
		draw_trigger(function () {
			...
			if (data.crit) {
				...
				if (data.crit > 2.9) {
					disp = "3X";
				} else if (data.crit > 2.74) {
					disp = "2.75X";
				} else if (data.crit > 2.44) {
					disp = "2.5X";
				} else if (data.crit > 2.2) {
					disp = "2.25X";
				}
			...
			if (entity && owner && data.damage !== undefined && !evade) {
				...
				if (data.dreturn) {
					d_text("-" + data.dreturn, owner, { color: color });
				}
				if (data.damage > 0) {
			...
			if (entity && data.heal !== undefined && !evade) {
				var y = 0;
				if (data.source == "partyheal" || data.source == "selfheal") {
					start_animation(entity, "party_heal"), (y = 16);
				}
	...
	socket.on("death", function (data) {
		// unused now
		if (data.place) {
			reject_deferred(data.place, { reason: data.reason || "not_found" });
		}
...
function npc_right_click(event) {
	...
	if (!npc.color && current_map == "main") {
		npc.color = colors.npc_white;
	}
	if (this.role != "shrine" && this.role != "compound") {
		var says = npc.says || "Yes";
		if (is_array(says)) {
			says = says[seed1() % says.length];
		}
		if (says == "rbin") {
			says = random_binary();
		}
	...
	if (npc.interaction) {
		...
		if (is_array(message)) {
			message = message[seed0() % message.length];
		}
		if (message == "rbin") {
			message = random_binaries();
		}
...
function player_click(event) {
	...
	if (is_npc(this) && this.npc == "pvp") {
		player_right_click.apply(this, event);
	} else if (this.npc_onclick) {
		npc_right_click.apply(this, event);
	} else {
		if (topleft_npc && inventory) {
			render_inventory();
		}
...
function player_right_click(event) {
	...
	if (this.npc && this.npc == "pvp") {
		...
		if (!pvp || (character.party && this.party == character.party)) {
			...
			return;
		} // these are important, as event.stopPropagation() shouldn't be called
	}
	if (event) {
		event.stopPropagation();
	}
...
function map_click(event) {
	...
	if (event && event.data && event.data.global) {
		...
		if (manual_centering && character) {
			(dx = x - character.x), (dy = y - character.y);
		}
		dx /= scale;
		dy /= scale;
		// if(log_flags.xy) console.log("dx,dy: "+round(dx)+","+round(dy));
		if (call_code_function("on_map_click", character.real_x + dx, character.real_y + dy)) {
			return;
		}
...
function update_sprite(sprite) {
	if (!sprite || !sprite.stype) {
		return;
	}
	for (name in sprite.animations || {}) {
		update_sprite(sprite.animations[name]);
	}
	for (name in sprite.emblems || {}) {
		update_sprite(sprite.emblems[name]);
	}
	if (sprite.stype == "static") {
		return;
	}

	if (sprite.type == "monster" && sprite.charge_skin) {
		if (sprite.target && sprite.skin != sprite.charge_skin) {
			sprite.skin = sprite.charge_skin;
		}
		if (!sprite.target && sprite.skin != sprite.normal_skin) {
			sprite.skin = sprite.normal_skin;
		}
	}
	...
	if (sprite.type == "character" || sprite.type == "monster") {
		effects_logic(sprite);
	}

	if (is_demo) {
		demo_entity_logic(sprite);
	}
	...
	if (sprite.stype == "full") {
		...
		if (sprite.type == "monster" && G.monsters[sprite.mtype].aa) {
			aa = true;
		}

		if (sprite.npc && !sprite.moving && sprite.allow === true) {
			sprite.direction = 1;
		}
		if (sprite.npc && !sprite.moving && sprite.allow === false) {
			sprite.direction = 0;
		}
		if (sprite.orientation && !sprite.moving && !sprite.target) {
			sprite.direction = sprite.orientation;
		}

		if ((sprite.moving || aa || (sprite.fx && sprite.fx.aaa)) && sprite.walking === null) {
			if (sprite.last_stop && msince(sprite.last_stop) < 320) {
				sprite.walking = sprite.last_walking;
			} else {
				reset_ms_check(sprite, "walk", 350), (sprite.walking = 1);
			}
		...
		var base_ms = 350;
		if (sprite.mtype == "wabbit") {
			(sequence = [0, 1, 2]), (base_ms = 220);
		}
		...
		if (!aa && sprite.s && sprite.s.stunned) {
			i = 1;
		} else if (sprite.walking) {
			i = sequence[sprite.walking % sequence.length];
		...
		if (sprite.rip && !sprite.rtexture) {
			...
			if (sprite.rip !== true) {
				gravestone = sprite.rip;
			}
			if (!textures[gravestone]) {
				generate_textures(gravestone, "gravestone");
			}
	...
	if ((sprite.stype == "animation" || sprite.stype == "item") && sprite.atype == "map") {
		...
		var target_y = get_y(sprite.target) - get_height(sprite.target) / 2;
		if (sprite.m != sprite.target.m) {
			(target_x = sprite.going_x), (target_y = sprite.going_y);
		}
		...
		if (mssince(sprite.last_frame) >= sprite.framefps) {
			(sprite.frame += 1), (sprite.last_frame = new Date());
		}
		if (sprite.to_fade) {
			sprite.alpha -= (((sprite.to_fade !== true && sprite.to_fade) || 0.025) * ms) / 16.6;
		}
		...
		if (ms_check(sprite, "anim" + sprite.skin, steps * 16.5)) {
			sprite.frame += 1;
		} //sprite.updates%steps==1
		if (sprite.frame >= sprite.frames && sprite.continuous) {
			sprite.frame = 0;
	...
	if (sprite.stype == "emblem") {
		...
		if (ms_check(sprite, "emblem" + sprite.skin, 60)) {
			sprite.frames -= 1;
		}
	...
	if (sprite.stype == "emote") {
		...
		if (sprite.atype == "flow") {
			if (ms_check(sprite, "anim", steps * 16.5)) {
				sprite.frame += 1;
			}
			set_texture(sprite, [0, 1, 2, 1][sprite.frame % 4]);
		} else {
			if (ms_check(sprite, "anim", steps * 16.5) && sprite.atype != "once") {
	...
	if (sprite.mtype && !(no_graphics || paused)) {
		if (sprite.mtype == "dice") {
			if (sprite.shuffling) {
				...
				if (!sprite.locked) {
					if (sprite.line) {
						remove_sprite(sprite.line), delete sprite.line;
					}
				...
				if (shake) {
					v_shake_i_minor(sprite);
				} // can't be used for entities
				if (
	...
	if (sprite.real_alpha !== undefined) {
		alpha_logic(sprite);
	}
...
function add_monster(data) {
	...
	var sprite = new_sprite(data.skin || def.skin || data.type, "full");
	sprite.type = "monster";
	...
	sprite.vy = data.vy || 0;
	if (def.slots) {
		sprite.slots = def.slots;
	}
	sprite.level = 1;
	if (sprite.s.young) {
		sprite.real_alpha = 0.4;
	}
	if (def.charge_skin) {
		sprite.normal_skin = sprite.skin;
		sprite.charge_skin = def.charge_skin;
		if (!textures[sprite.charge_skin]) {
			generate_textures(sprite.charge_skin, "full");
		}
	}
	sprite.last_ms = new Date();
	...
	if (def.orientation) {
		sprite.orientation = def.orientation;
	}
...
function update_filters(sprite) {
	...
	if (sprite.pglow) {
		...
		if (sprite.pzazz > 50) {
			steps = [1.7, 1.9, 1.7];
		} else if (sprite.pzazz > 40) {
			steps = [1.5, 1.6, 1.5];
		} else if (sprite.pzazz > 25) {
			steps = [1.4, 1.5, 1.42];
		} else if (sprite.pzazz > 13) {
			steps = [1.3, 1.4, 1.275];
		} else if (sprite.pzazz > 6) {
			steps = [1.2, 1.3, 1.075];
		}

		var filter = sprite.filter_pglow;
		if (filter.b > steps[1]) {
			filter.step = -abs(filter.step);
		}
		if (filter.b < steps[0]) {
			filter.step = abs(filter.step);
		}
		filter.b += filter.step;
		if (sprite.stand || sprite.s.charging || sprite.s.dash) {
			filter.b = steps[2];
		}
	...
	if (sprite.disappearing) {
		if (sprite.real_alpha > 0.5) {
			sprite.real_alpha -= 0.0025;
		}
	}
}
...
function start_filter(sprite, ftype, args) {
	...
	if (ftype == "curse") {
		if (sprite.s && sprite.s.deepfreezed) {
			filter.hue(36);
		} else if (sprite.s && sprite.s.frozen) {
			filter.hue(24);
		} else if (sprite.s && sprite.s.poisoned) {
			filter.hue(-24);
		} else {
			filter.hue(20);
		}
	...
	if (!sprite.filter_list) {
		sprite.filter_list = [filter];
	} else {
		sprite.filter_list.push(filter);
	}
...
function alpha_logic(sprite) {
	if ((!sprite.cx || !sprite.cx.length) && (!sprite.cxc || !Object.keys(sprite.cxc).length)) {
		if (sprite.alpha != sprite.real_alpha) {
			sprite.alpha = sprite.real_alpha;
		}
	} else {
		if (sprite.alpha != 1) {
			sprite.alpha = 1;
		} // sometimes cosmetics logic doesn't execute beforehand, alpha gets set, then gets stuck at 0.5 - this fixes it [28/01/20]
		if (sprite.filter_alphaf) {
			if (sprite.real_alpha == 1) {
				stop_filter(sprite, "alphaf");
			} else if (sprite.filter_alphaf.alpha != sprite.real_alpha) {
				sprite.filter_alphaf.alpha = sprite.real_alpha;
			}
...
function player_effects_logic(sprite) {
	if (no_graphics || !sprite.s) {
		return;
	}
	if (sprite.pzazz >= 4 && !sprite.filter_pglow) {
		start_filter(sprite, "pglow");
	} else if (sprite.pzazz < 4 && sprite.filter_pglow) {
		stop_filter(sprite, "pglow");
	}
...
function effects_logic(sprite) {
	...
	if (sprite.s.filter && !sprite["filter_" + sprite.s.filter.name]) {
		if (sprite.s.filter.name != "scale") {
			start_filter(sprite, sprite.s.filter.name);
		}
	...
	if (sprite.c && sprite.c.pickpocket && !sprite.fx.attack) {
		sprite.fx.attack = [new Date(), 0];
	}
	if (sprite.c && sprite.c.fishing && !sprite.fx.attack) {
		sprite.fx.attack = [new Date(), 0];
	}
	if (sprite.c && sprite.c.mining && !sprite.fx.attack) {
		sprite.fx.attack = [new Date(), 0];
	}
}
...
function cosmetics_logic(sprite) {
	...
	if (T[sprite.skin] == "armor") {
		body_type = "armor";
	} else if (T[sprite.skin] == "body") {
		body_type = "body";
	} else if (T[sprite.skin] == "character") {
		body_type = "character";
	}

	if (sprite.slots && sprite.slots.helmet && sprite.slots.helmet.name.startsWith("ghat")) {
		var found = false;
		for (var slot in sprite.slots) {
			if (sprite.slots[slot] && sprite.slots[slot].giveaway) {
				found = true;
			}
		}
		if (found) {
			sprite.cx.hat = G.items[sprite.slots.helmet.name].hat;
		}
	}

	if (sprite.rip) {
		if (sprite.cx.hat) {
			needed.push(sprite.cx.hat);
		}
	} else {
		needed.push(sprite.skin + "copy");
		if (body_type != "full" && !sprite.cx.head) {
			sprite.cx.head = "makeup117";
		}
		for (var place in sprite.cx) {
			var cid = sprite.cx[place];
			if (!cid || ["stone"].includes(place)) {
				continue;
			}
			if (place == "hair" && cx_prop.no_hair) {
				continue;
			}
			if (body_type == "full" && in_arr(place, ["head", "hair"])) {
				continue;
			}
			if (body_type == "character" && in_arr(place, ["head", "hair"])) {
				continue;
			}
			if (place == "upper" && (body_type == "full" || SSU[cid] != SSU[sprite.skin])) {
				continue;
			}
	...
	needed.forEach(function (cid) {
		...
		if (!sprite.cxc[cid]) {
			if (sprite.rip && T[cid] != "hat" && T[cid] != "a_hat") {
				return;
			}
			if (cid == sprite.skin + "copy") {
				...
				var wcx = G.items[wname].cx || {};
				var item = sprite.slots.mainhand;
				var main = true;
				if (cid[4] == "2") {
					(item = sprite.slots.offhand), (main = false);
				}
				...
				c.x = -5 + x_disp;
				if (cid[4] == "2") {
					(c.x = 5 + x_disp), c.scale.set(-1, 1);
				}

				var bcolor = 0x444444;
				var bsize = 0;
				if (wcx.lightborder) {
					(bcolor = 0x666666), (bsize = 1);
				}
				if (wcx.border) {
					bsize = wcx.border;
				}
				...
				if (item.level + gr >= 10) {
					...
					if (main) {
						sprite.filter_wglow = glow_filter;
					}
				} else if (wcx.scale == 0.5) {
					c.filters = [new PIXI.filters.OutlineFilter(bsize || 2, bcolor)];
				} // ,new PIXI.filters.PixelateFilter(2,2)
				else if (wcx.scale == 0.75) {
					c.filters = [new PIXI.filters.OutlineFilter(bsize || 2, bcolor)];
				} else {
					c.filters = [new PIXI.filters.OutlineFilter(bsize || 2, bcolor)];
				}
		...
		if (c.stype == "copy") {
			...
			if (sprite.j !== undefined) {
				c.texture = textures[sprite.skin][sprite.i][sprite.j];
			} // for _clone
			c.zy = 0;
		} else if (c.stype == "bg") {
			c.zy = -100 * ZEPS;
			...
			if (sprite.j == 0) {
				...
				(c.x = 0 + x_disp), (c.zy = -ZEPS);
			}
			if (sprite.j < 3 && covers) {
				c.zy = -2 * ZEPS;
			}
			if (sprite.j !== undefined) {
				set_texture(c, sprite.j);
			}
			c.moved = false;
			...
			if (sprite.j !== undefined) {
				set_texture(c, sprite.j, sprite.i);
			}
			c.moved = false;
			...
			// }
			if (sprite.j !== undefined) {
				set_texture(c, sprite.j, parseInt(sprite.updates / 4));
			}
			c.moved = false;
			...
			if (sprite.j !== undefined) {
				set_texture(c, sprite.i, sprite.j);
			}
			...
			if (sprite.j == 0) {
				(c.zy = -6 * ZEPS), (c.x = -2.5 + x_disp);
			} else if (sprite.j == 1) {
				(c.zy = -6 * ZEPS), (c.x = 5 + x_disp);
			} else if (sprite.j == 2) {
				(c.zy = -6 * ZEPS), (c.x = -9 + x_disp);
			} else if (sprite.j == 3) {
				(c.zy = 6 * ZEPS), (c.x = -5 + x_disp);
			}
			if (sprite.j !== undefined) {
			...
			c.y_disp = 0;
			if (sprite.i == 0) {
				(c.y = c.y_disp + 1), (c.x = x_disp - 1);
			} else if (sprite.i == 1) {
				(c.y = c.y_disp), (c.x = x_disp);
			} else if (sprite.i == 2) {
				(c.y = c.y_disp + 1), (c.x = x_disp + 1);
			}
			...
			if (sprite.j !== undefined) {
				set_texture(c, (sprite.walking || 0) % 4, sprite.j);
			}
			c.moved = false;
		} else if (cid.startsWith("wea")) {
			var wname = cid.substr(6, 99);
			var wcx = G.items[wname].cx || {};
			var def = G.items[wname];
			var scale = 1;
			var extension = false;
			var blade = false;
			var staff = false;
			...
			var rotation = 0;
			var crossbow = false;
			if (in_arr(def.wtype, ["wblade", "staff"])) {
				staff = true;
			} else if (in_arr(def.wtype, ["rod"])) {
				rod = true;
			} else if (in_arr(def.wtype, ["pickaxe"])) {
				pickaxe = true;
			} else if (in_arr(def.wtype, ["bow"])) {
				bow = true;
			} else if (in_arr(def.wtype, ["crossbow"])) {
				crossbow = true;
			} else {
				blade = true;
			}
			...
			if (sprite.fx && sprite.fx.attack) {
				...
				if (rotation > 0 || sprite.j == 2) {
					rotation += (sprite.fx.attack[1] * Math.PI) / divider;
				} else {
					rotation -= (sprite.fx.attack[1] * Math.PI) / divider;
				}
				// console.log(c.skin+sprite.fx.attack[1]);
				if (textures["item" + c.skin][0] != textures["item" + c.skin][1]) {
					if (sprite.fx.attack[1] > 2 && sprite.fx.attack[1] <= 8) {
						c.texture = textures["item" + c.skin][1];
					} else {
						c.texture = textures["item" + c.skin][0];
					}
				}
	...
	sprite.children.sort(function (a, b) {
		if (a.zy === undefined) {
			a.zy = -CINF;
		} // undefined's break the sort order [27/09/18]
		if (b.zy === undefined) {
			b.zy = -CINF;
		}
		return a.zy - b.zy;
...
function add_npc(npc, position, npc_id) {
	...
	var sprite;
	if (npc.type == "static") {
		sprite = new_sprite(npc.skin, "static");
	} else if (npc.type == "fullstatic") {
		sprite = new_sprite(npc.skin, "full");
	} else {
		sprite = new_sprite(npc.skin, "emote");
	}
	sprite.npc = npc_id;
	...
	if (npc.type == "fullstatic" && position.length == 3) {
		npc.direction = position[2];
	}
	if (npc.role == "citizen") {
		sprite.citizen = true;
	}
...
function add_character(data, me) {
	...
	var stype = "full";
	if (npc && npc.type == "static") {
		stype = "static";
	} else if (npc && npc.type != "fullstatic") {
		stype = "emote";
	}
	...
	if (cscale != 1) {
		sprite.scale = new PIXI.Point(cscale, cscale);
	}
	...
	if (npc) {
		...
		adopt_soft_properties(sprite, npc);
		if (npc.role == "citizen" || npc.movable) {
			sprite.citizen = true;
		}
		if (npc.stand) {
			var stand = new PIXI.Sprite(textures[npc.stand]);
...
function add_door(door) {
	...
	if (is_mobile) {
		sprite.on("mousedown", door_right_click).on("touchstart", door_right_click);
	}
...
function add_quirk(quirk) {
	...
	sprite.interactive = true;
	sprite.buttonMode = true;
	if (quirk[4] != "upgrade" && quirk[4] != "compound") {
		sprite.cursor = "help";
	}
	...
	function quirk_right_click(event) {
		if (quirk[4] == "sign") {
			add_log('Sign reads: "' + quirk[5] + '"', "gray");
		} else if (quirk[4] == "note") {
			add_log('Note reads: "' + quirk[5] + '"', "gray");
		} else if (quirk[4] == "tavern_info") {
			socket.emit("tavern", { event: "info" });
		} else if (quirk[4] == "mainframe") {
			render_mainframe();
		} else if (quirk[4] == "the_lever") {
			the_lever();
		} else if (quirk[4] == "log") {
			add_log(quirk[5], "gray");
		} else if (quirk[4] == "upgrade") {
			render_upgrade_shrine(1);
		} else if (quirk[4] == "compound") {
			render_compound_shrine(1);
		} else if (quirk[4] == "list_pvp") {
			socket.emit("list_pvp");
		} else if (quirk[4] == "invisible_statue") {
...
function create_map() {
	...
	if (window.map) {
		...
		for (var id in chests) {
			var chest = chests[id];
			if (chest.map == window.map.map_name) {
				map.removeChild(chest);
			}
		}
		...
		map.destroy();
		for (var id in map_animations) {
			destroy_sprite(map_animations[id], "children");
		}
		map_entities.forEach(function (e) {
			e.destroy({ children: true });
		});
		if (wtile) {
			wtile.destroy(), (wtile = null);
		}
		if (dtile) {
	...
	for (var i = 0; i < GEO.tiles.length; i++) {
		...
		if (!tile_sprites[current_map][i]) {
			...
			if (element[5]) {
				(frames = element[5]), (frame_width = element[3]);
			}
			...
			if (!cached_map) {
				tile_sprites[current_map][i][sprite_last[current_map][i]] = new_map_tile(tile_textures[current_map][i]);
			} // new - cache one in case it increases performance [03/09/16]
		}
	}
		...
		for (var last_water_frame = 0; last_water_frame < 3; last_water_frame++) {
			...
			for (var i = 0; i < GEO.placements.length; i++) {
				...
				if (tile[3] === undefined || tile[3] === null) {
					...
					if (entity.textures) {
						entity.texture = entity.textures[last_water_frame];
					}
			...
			if (!mode.destroy_tiles) {
				tile_containers[last_water_frame].destroy();
			}
		}
		if (dtile_size) {
	...
	if (GEO.animations) {
		for (var p = 0; p < GEO.animations.length; p++) {
			...
			var w = GEO.tiles[tile[0]][3];
			var h = GEO.tiles[tile[0]][4];
	...
	if (GEO.groups) {
		for (var p = 0; p < GEO.groups.length; p++) {
			...
			for (var i = 0; i < GEO.groups[p].length; i++) {
				...
				if (nunv(tile[4], tile[2]) + def[4] > max_y) {
					max_y = nunv(tile[4], tile[2]) + def[4];
				}
			}
			for (var i = 0; i < GEO.groups[p].length; i++) {
				var tile = GEO.groups[p][i];
			...
			if (GEO.groups[p][0] && GEO.groups[p][0][5]) {
				container.y_disp = -parseFloat(GEO.groups[p][0][5]) || 0;
			}
			container.x = min_x;
			container.y = min_y;
	...
	if (0) {
		...
		for (var i = 0; i < npcs.length; i++) {
			var npc = npcs[i];
			var def = G.npcs[npc.id];
			if (def.type == "full" || def.role == "citizen" || !npc.position) {
				continue;
			}
			if (log_flags.map) {
	...
	for (var i = 0; i < machines.length; i++) {
		...
		if (log_flags.map) {
			console.log("Machine: " + machine.type);
		}
		map.addChild(nsprite);
		map_npcs.push(nsprite);
		map_entities.push(nsprite);
...
function retile_the_map() {
	...
	if (cached_map) {
		...
		if (wtile && (wtile_width < width || wtile_height < height)) {
			recreate_wtextures();
		}
	...
	var x = map.real_x;
	var y = map.real_y;
	var min_x = x - width / scale / 2 - border;
	var max_x = x + width / scale / 2 + border;
	var min_y = y - height / scale / 2 - border;
	var max_y = y + height / scale / 2 + border;
	if (
	...
	if (0) {
		...
		for (var i = 0; i < map_tiles.length; i++) {
			if (map_tiles[i].to_delete) {
				remove_sprite(map_tiles[i]);
			}
		}
	...
	if (GEO["default"] && !mdraw_tiling_sprites) {
		...
		for (var x = min_x; x <= max_x + 10; x += def[3]) {
			for (var y = min_y; y <= max_y + 10; y += def[4]) {
				...
				if (entity.textures) {
					(entity.texture = entity.textures[last_water_frame]), water_tiles.push(entity);
				}
				entity.x = ex * def[3];
				entity.y = ey * def[4];
				if (mdraw_mode != "redraw") {
					(entity.parentGroup = entity.displayGroup = map_layer), (entity.zOrder = 0);
				}
				entity.tid = tid;
				map.addChild(entity);
	...
	for (var i = 0; i < GEO.placements.length; i++) {
		...
		if (is_nun(tile[3])) {
			...
			var w = def[3];
			var h = def[4];
			if (tile[1] > max_x || tile[2] > max_y || tile[1] + w < min_x || tile[2] + h < min_y) {
				continue;
			}
			if (sprite_last[current_map][tile[0]] >= tile_sprites[current_map][tile[0]].length) {
				(tile_sprites[current_map][tile[0]][sprite_last[current_map][tile[0]]] = new_map_tile(
			...
			entity.y = tile[2];
			if (mdraw_mode != "redraw") {
				(entity.parentGroup = entity.displayGroup = map_layer), (entity.zOrder = -(i + 1));
			}
			entity.tid = "p" + i;
			map.addChild(entity);
			map_tiles.push(entity);
			...
			if (
				...
				for (var x = tile[1]; x <= tile[3]; x += w) {
					if (x > max_x || x + w < min_x) {
						continue;
					} // first improvement, a better version is boundary calculations for for loops [03/09/16]
...
function load_game(c) {
	loader.load(function (loader, resources) {
		...
		} // great find - prevents tile gaps and upper shadows / similar imperfections [17/06/16]
		...
		for (name in G.sprites) {
			...
			if (no_graphics) {
				C[s_def.file] = { width: 20, height: 20 };
			} //#NOGTODO: pull these values from SDK + Insert into data.js for usage [03/01/18]
...
function draw(arg1, manual_draw) {
	...
	var mframe_ms = frame_ms;
	if (clean_house) {
		draw_entities(), (mframe_ms = 0);
	}
	process_entities();
	future_entities = { players: {}, monsters: {} };
	...
	if (cframe_ms > ((is_sdk && 200) || 10000)) {
		console.log("cframe_ms is " + cframe_ms);
	}
	while (cframe_ms > 0) {
		if (character && character.moving) {
			moved = true;
			if (character.vx) {
				character.real_x += (character.vx * min(cframe_ms, 50)) / 1000.0;
			}
			if (character.vy) {
				character.real_y += (character.vy * min(cframe_ms, 50)) / 1000.0;
			}
	...
	for (var id in chests) {
		if (chests[id].openning) {
			update_sprite(chests[id]);
		}
	}
	for (var id in map_animations) {
		update_sprite(map_animations[id]);
	}
	...
	if (!manual_draw && (!no_html || no_html == "bot")) {
		...
		if (no_graphics) {
			t = setTimeout(draw, 16);
		} // jsdom patch [18/04/19]
		else {
		...
		try {
			var chars = get_active_characters();
			for (var name in chars) {
				if (chars[name] != "self" && chars[name] != "loading") {
					character_window_eval(name, "draw()");
				}
			}
		} catch (e) {
			console.log(e);
