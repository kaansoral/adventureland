...
for (var p in parent.character) {
	proxy(p);
} // Not all properties are sadly available right away, new properties are captured imperfectly
...
function use_nearest_door() {
	for (var i = 0; i < (G.maps[character.map].doors || []).length; i++) {
		...
		if (simple_distance(character, { map: character.map, x: door[0], y: door[1] }) < 100) {
			...
			return transport(door[4], door[5]);
		} // map and spawn
	}
...
function bank_store(num, pack, pack_num) {
	...
	if (pack_num === undefined) {
		pack_num = -1;
	} // the server interprets -1 as first slot available
	if (!pack) {
		var cp = undefined;
		var cs = undefined;
		for (var cpack in bank_packs) {
			if (pack || bank_packs[cpack][0] != character.map || !character.bank[cpack]) {
				continue;
			}
			for (var i = 0; i < 42; i++) {
		...
		if (!pack && !cp) {
			return rejecting_promise({ reason: "bank_full" });
		}
		if (!pack) {
			pack = cp;
		}
...
function bank_retrieve(pack, pack_num, num) {
	...
	if (!character.bank[pack] || !character.bank[pack][pack_num]) {
		return rejecting_promise({ reason: "no_item" });
	}
	if (num === undefined) {
		num = -1;
...
function game_log(message, color, x) {
	if (game.platform == "electron" && !x) {
		return safe_log(message, color);
	}
	if (!color) {
		color = "#51D2E1";
	}
	if (character.bot) {
		parent.parent.add_log(character.name + ": " + message, color);
	} else {
...
function get_focus() {
	...
	if (parent.xtarget && parent.xtarget.visible) {
		return parent.xtarget;
	}
	return null;
}
...
function get_target() {
	if (parent.ctarget && parent.ctarget.visible) {
...
function change_target_privately(target) {
	parent.ctarget = target;
	if (target) {
		parent.last_id_sent = target.id;
	} // Marks the id as sent, so it doesn't actually get sent
	else {
		parent.last_id_sent = "";
	}
	return parent.send_target_logic();
...
function is_in_range(target, skill) {
	...
	if (G.skills[skill] && G.skills[skill].range) {
		if (distance(character, target) <= G.skills[skill].range) {
			return true;
		}
		return false;
	}
	if (G.skills[skill] && G.skills[skill].range_multiplier) {
		range_multiplier = G.skills[skill].range_multiplier;
	}
	if (G.skills[skill] && G.skills[skill].range_bonus) {
		range_bonus = G.skills[skill].range_bonus;
	}
	if (distance(character, target) <= character.range * range_multiplier + range_bonus) {
		return true;
	}
	return false;
}
...
function is_on_cooldown(skill) {
	...
	if (parent.next_skill[skill] && new Date() < parent.next_skill[skill]) {
		return true;
	}
	return false;
}
...
function can_attack(target) {
	...
	if (!parent.is_disabled(character) && is_in_range(target) && new Date() >= parent.next_skill.attack) {
		return true;
	}
	return false;
}
...
function is_transporting(entity) {
	...
	if (entity.me && parent.transporting) {
		return true;
	}
	return false;
}
...
function heal(target) {
	if (target == character) {
		target = parent.character;
	} // Don't send the proxy object to parent [10/06/19]
	if (!target) {
...
function trade(num, trade_slot, price, quantity) {
	...
	if (!is_string(trade_slot) || !trade_slot.startsWith("trade")) {
		trade_slot = "trade" + trade_slot;
	}
...
function move(x, y) {
	if (!can_walk(character)) {
		return rejecting_promise({ reason: "unable" });
	}
...
function show_json(e) {
	...
	if (character.bot) {
		parent.parent.show_json(parent.game_stringify(e, "\t"));
	} else {
		parent.show_json(parent.game_stringify(e, "\t"));
	}
}
...
function get_nearest_monster(args) {
	...
	if (args && args.type == "monster") {
		game_log("get_nearest_monster: you used monster.type, which is always 'monster', use monster.mtype instead");
	}
	if (args && args.mtype) {
		game_log("get_nearest_monster: you used 'mtype', you should use 'type'");
	}
	...
	for (id in parent.entities) {
		...
		if (args.type && current.mtype != args.type) {
			continue;
		}
...
function get_nearest_hostile(args) {
	...
	if (args.friendship === undefined && character.owner) {
		args.friendship = true;
	}

	for (id in parent.entities) {
		var current = parent.entities[id];
		if (current.type != "character" || !current.visible || current.rip || current.invincible || current.npc) {
			continue;
		}
		...
		if (args.friendship && in_arr(current.owner, parent.friends)) {
			continue;
		}
		if (args.exclude && in_arr(current.name, args.exclude)) {
			continue;
...
function loot(id_or_arg) {
	...
	for (var id in parent.chests) {
		...
		if (id_or_arg === true) {
			last = parent.parent.open_chest(id);
		} else {
			last = parent.open_chest(id);
		}
		looted++;
		if (looted == 2) {
			break;
...
async function send_cm(to, message) {
	...
	if (to_server.length) {
		data = await send_server_cm(to_server, message);
	} // message over the server - has a high call cost / character.cc
...
function add_top_button(id, value, fn) {
	if (!buttons[id]) {
		...
		parent
			.$(".codebuttons")
			.append(
				"<div class='gamebutton codebutton" +
					id +
					"' data-id='" +
					id +
					"' onclick='code_button_click(this)'>BUTTON</div> ",
			);
	}
	if (fn) {
...
function add_bottom_button(id, value, fn) {
	if (!buttons[id]) {
		...
		parent
			.$(".codebbuttons")
			.append(
				"<div class='gamebutton gamebutton-small codebutton" +
					id +
					"' data-id='" +
					id +
					"' onclick='code_button_click(this)'>BUTTON</div> ",
...
function map_key(key, skill, code) {
	...
	if (is_string(skill)) {
		keymap[key] = { name: skill };
	} else {
		keymap[key] = skill;
	}
	if (code) {
		keymap[key].code = code;
	}
	if (keymap[key].keycode) {
		parent.K[keymap[key].keycode] = key;
	}
	set_keymap(keymap);
}
...
function send_local_cm(name, data) {
	if (game.cli) {
		parent.CLI_OUT.push({ type: "cm", to: name, data: [character.name, data, new Date(), ++local_m_num] });
	} else {
		localStorage.setItem(
			"cm_" + name + "_" + randomStr(20),
			JSON.stringify([character.name, data, new Date(), ++local_m_num]),
		);
	}
}
...
function load_code(name_or_slot, onerror) {
	...
	if (code === null) {
		...
		xhrObj.open(
			"GET",
			"/code.js?name=" + encodeURIComponent(name_or_slot) + "&timestamp=" + new Date().getTime(),
			false,
		);
		xhrObj.send("");
...
function require_code(name_or_slot) {
	...
	if (code === null) {
		...
		xhrObj.open(
			"GET",
			"/code.js?name=" + encodeURIComponent(name_or_slot) + "&xrequire=1&timestamp=" + new Date().getTime(),
			false,
		);
		xhrObj.send("");
		code = xhrObj.responseText;
...
function upload_code(slot_number, slot_name, code_string) {
	...
	return parent.api_call(
		"save_code",
		{ code: code_string, slot: slot_number, name: slot_name, auto: true, electron: true },
		{ promise: true },
	);
}
...
function smart_move(destination, on_done) {
	...
	if (smart.moving) {
		smart.on_done(false, "interrupted");
	}
	smart.map = "";
	...
	if (destination.return) {
		...
		smart.on_done = function (done, reason) {
			...
			if (done) {
				resolve_deferreds("smart_move", { success: true });
			} else {
				reject_deferreds("smart_move", { reason: reason });
			}
		};
	} else {
		smart.on_done = function (done, reason) {
...
function stop(action, second) {
	if (!action || action == "move" || action == "smart") {
		if (smart.moving) {
			smart.on_done(second || false, "interrupted");
		}
		smart.moving = false;
		if (action != "smart") {
			return move(character.real_x, character.real_y);
		}
...
function qpush(node) {
	...
	if (smart.prune.map && smart.flags.map && node.map != smart.map) {
		return;
	}
	if (visited[node.map + "-" + node.x + "-" + node.y]) {
		return;
	}
	if (!node.i) {
		node.i = start;
	} // set the index, to aid the plot function
	queue.push(node);
...
function bfs() {
	...
	while (start < queue.length) {
		...
		if (current.map == smart.map) {
			...
			if (c_dist < smart.baby_edge || s_dist < smart.baby_edge || map.small_steps) {
				c_moves = baby_steps;
			}
			if (c_dist < smart.edge) {
				...
				best === null ||
				abs(current.x - smart.x) + abs(current.y - smart.y) <
					abs(queue[best].x - smart.x) + abs(queue[best].y - smart.y)
			) {
				best = start;
			}
			...
			map.doors.forEach(function (door) {
				...
				if ((smart.map != "bank" && door[4] == "bank" && !G.maps[current.map].mount) || door[8] == "complicated") {
					return;
				} // manually patch the bank shortcut
				if (
					is_door_close(current.map, door, current.x, current.y) &&
					can_use_door(current.map, door, current.x, current.y)
				) {
					qlist.push({
						map: door[4],
			...
			map.npcs.forEach(function (npc) {
				if (
					npc.id == "transporter" &&
					simple_distance({ x: npc.position[0], y: npc.position[1] }, { x: current.x, y: current.y }) < 75
				) {
		...
		if (smart.use_town) {
			qpush({ map: current.map, x: map.spawns[0][0], y: map.spawns[0][1], town: true });
		} // "town"

		shuffle(c_moves);
		c_moves.forEach(function (m) {
			var new_x = current.x + m[0];
			var new_y = current.y + m[1];
			...
			if (
				can_move({ map: current.map, x: current.x, y: current.y, going_x: new_x, going_y: new_y, base: character.base })
			) {
		...
		start++;
		if (mssince(timer) > ((!parent.is_hidden() && 40) || 500)) {
			return;
		}
	}
	...
	if (result === null) {
		...
		if (1) {
			...
			if (!last) {
				last = { map: character.map, x: character.real_x, y: character.real_y };
			}
		...
		smart.found = true;
		if (smart.prune.smooth) {
			smooth_path();
		}
		if (optimal) {
			game_log("Path found!", "#C882D1");
		} else {
			game_log("Path found~", "#C882D1");
		}
...
function start_pathfinding() {
	...
	if (game.cli) {
		parent.CLI_OUT.push({
			type: "smart_move",
			G: G,
			start_x: smart.start_x,
			start_y: smart.start_y,
			start_map: character.map,
			x: smart.x,
			y: smart.y,
			map: smart.map,
		});
...
function smart_move_logic() {
	...
	if (!smart.searching && !smart.found) {
		...
		if (current.town) {
			...
			character.map == current.map &&
			((smart.try_exact_spot && !smart.plot.length) || can_move_to(current.x, current.y))
		) {
...
function proxy(name) {
	if (in_arr(name, character.properties)) {
		return;
	}
...
function eval_s(code) {
	...
	eval(code);
	if (output !== undefined) {
		parent.show_modal("<pre>" + output + "</pre>");
	}
	if (json_output !== undefined) {
		parent.show_json(json_output);
	}
}
...
if (parent.is_cli) {
	if (parent.ls_emulation) {
		window._localStorage = parent.ls_emulation;
	}
