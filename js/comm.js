...
function render_characters() {
	...
	X.characters.forEach(function (char) {
		if (char.online) {
			html +=
				"<div class='gamebutton mb5 mr5' onclick='if(bc(this)) return; observe_character(\"" +
				char.name +
				"\");' style='text-align: left; width: 172px'>";
			html +=
				"<span style='float:left; margin-right: 5px; margin-top: -5px; margin-left: -4px; margin-bottom: -7px;'>" +
				sprite(char.skin, { cx: char.cx, rip: char.rip }) +
				"</span>";
	...
	if (!html) {
		html += "<div class='gamebutton mb5'>ALL OFFLINE</div>";
	}
...
function render_servers() {
	...
	X.servers.forEach(function (server) {
		html +=
			"<div class='gamebutton mb5 mr5' onclick='if(bc(this)) return; server_addr=\"" +
			server.addr +
			'"; server_port="' +
			server.port +
			"\"; init_socket();'>";
	...
	if (!html) {
		html += "<div class='gamebutton mb5'>GAME OFFLINE</div>";
	}
