...
function calculate_move(map, cur_x, cur_y, target_x, target_y) {
	// v4, tries to move back, but, if you are moving at a very very high angle, moving 7px back from an x_line, doesn't really move you 7px back in the y-direction, it's more like 0.1px, trial and error ... :)
	...
	if (n - o >= 7.1288 - EPS) {
		return calculate_move_v2(map, cur_x, cur_y, target_x, target_y);
	} // didn't clash with a line, return normal move
	else {
...
function move_further(x0, y0, x1, y1, dt) {
	// Some of the earlier ideas involved trying to move forward and backtracking, imperfect and complex [19/07/18]
	var distance = Math.sqrt((x1 - x0) * (x1 - x0) + (y1 - y0) * (y1 - y0));
	if (!dt || !distance) {
		return { x: x1, y: y1 };
	}
	var a = (y1 - y0) / distance;
	var b = (x1 - x0) / distance;
...
function calculate_movev1(map, cur_x, cur_y, target_x, target_y) {
	// improved, v1
...
function can_move(monster) {
	...
	for (var i = bsearch_start(M.x_lines || [], mx); i < (M.x_lines || []).length; i++) {
		...
		if (line[0] - 0.2 <= x1 && line[0] + 0.2 >= x1 && line[1] - 0.2 <= y1 && line[2] + 0.2 >= y1) {
			return false;
		} // 2 instead of 0.2 causes jailing issues
		if (mx > line[0]) {
			continue;
		} // can be commented out with: while(start<arr.length && arr[start][0]<value) start++;
		if (x0 + 1 < line[0] && x1 + 1 < line[0]) {
			break;
	...
	for (var i = bsearch_start(M.y_lines || [], my); i < (M.y_lines || []).length; i++) {
		...
		if (line[0] - 0.2 <= y1 && line[0] + 0.2 >= y1 && line[1] - 0.2 <= x1 && line[2] + 0.2 >= x1) {
			return false;
		}
		if (my > line[0]) {
			continue;
		}
		if (y0 + 1 < line[0] && y1 + 1 < line[0]) {
			break;
		}
...
function calculate_move_original(M, x0, y0, x1, y1) {
	...
	for (var i = 0; i < (M.x_lines || []).length; i++) {
		var line = M.x_lines[i];
		if (!((x0 <= line[0] && line[0] <= x1) || (x1 <= line[0] && line[0] <= x0))) {
			continue;
		}
		next = y0 + ((y1 - y0) * (line[0] - x0)) / (x1 - x0 + EPS);
		if (!(line[1] <= next && next <= line[2])) {
			continue;
		}
	...
	for (var i = 0; i < (M.y_lines || []).length; i++) {
		var line = M.y_lines[i];
		if (!((y0 <= line[0] && line[0] <= y1) || (y1 <= line[0] && line[0] <= y0))) {
			continue;
		}
		next = x0 + ((x1 - x0) * (line[0] - y0)) / (y1 - y0 + EPS);
		if (!(line[1] <= next && next <= line[2])) {
	...
	for (
		var i = 0;
		i < (M.x_lines || []).length;
		i++ // extremely dirty-fix, the worst part is, don't even know why it works :( [03/10/16]
	) {
		var line = M.x_lines[i];
		if (!((x0 <= line[0] && line[0] <= x1) || (x1 <= line[0] && line[0] <= x0))) {
			continue;
...
function can_move_original(monster) {
	// used at server, .going_x + .going_y is pre-set [14/08/16]
...
function can_move_170718(monster) {
	...
	var y_disp = -3;
	if (x1 < x0) {
		(going_left = true), (x_disp = 3);
	}
	if (y1 < y0) {
		(going_up = true), (y_disp = 7);
	}
	for (var i = bsearch_start2(M.x_lines || [], mx, 3); i < (M.x_lines || []).length; i++) {
		var line = M.x_lines[i];
		if (line[0] - 3 <= x1 && line[0] + 3 >= x1 && line[1] - 7 <= y1 && line[2] + 3 >= y1) {
			return false;
		}
		if (mx > line[0]) {
			continue;
	...
	for (var i = bsearch_start(M.y_lines || [], my); i < (M.y_lines || []).length; i++) {
		var line = M.y_lines[i];
		if (line[0] - 3 <= y1 && line[0] + 3 >= y1 && line[1] - 7 <= x1 && line[2] + 3 >= x1) {
			return false;
		}
		if (my > line[0]) {
			continue;
...
function can_movex(map, cur_x, cur_y, target_x, target_y) {
	// draivin's can_move - didn't use for a long while - don't remember whether it had issues
	if (target_x == Infinity) {
		target_x = CINF;
	}
	if (target_y == Infinity) {
