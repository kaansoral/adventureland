...
function open_chat_window(type, id, open) {
	...
	var name = id;
	var cid = type + id;
	var zindex = 70 + cwindows.length - docked.length;
	var onkeypress = 'last_say="' + cid + '"; if(event.keyCode==13) private_say("' + id + '",$(this).rfval())';
	if (type == "party") {
		(name = "Party"), (onkeypress = 'last_say="' + cid + '"; if(event.keyCode==13) party_say($(this).rfval())');
	}
	var html =
		"<div style='position:fixed; bottom: 0px; left: 0px; background: black; border: 5px solid gray; z-index: " +
		zindex +
		"' id='chatw" +
		cid +
		"' onclick='last_say=\"" +
	...
	html +=
		"<div style=''><input type='text' class='chatinput' id='chati" +
		cid +
		"' onkeypress='" +
		onkeypress +
		"' autocomplete='nope'/></div>";
	html += "</div>";
	$("body").append(html);
	docked.push(cid);
	cwindows.push(cid);
...
function prop_remains(remains) {
	if (remains <= 1 / 60.0) {
		return bold_prop_line("Seconds", to_pretty_float(remains * 3600), "gray");
	} else if (remains < 1) {
		return bold_prop_line("Minutes", to_pretty_float(remains * 60), "gray");
	} else {
		return bold_prop_line("Hours", to_pretty_float(remains), "gray");
	}
}
...
function render_party_old(list) {
	var html =
		"<div style='background-color: black; border: 5px solid gray; padding: 6px; font-size: 24px; display: inline-block' class='enableclicks'>";
	if (list) {
		html += "<div class='slimbutton block'>PARTY</div>";
		list.forEach(function (name) {
			html +=
				"<div class='slimbutton block mt5' style='border-color:#703987' onclick='party_click(\"" +
				name +
				"\")'>" +
				name +
				"</div>";
...
function render_party() {
	...
	for (var name in party) {
		...
		if (member.rip) {
			html += "<div style='color:gray; margin-top: 1px'>RIP</div>";
		} else {
			html += "<div style='margin-top: 1px'>" + name.substr(0, 3).toUpperCase() + "</div>";
		}
		html += "</div>";
	}
	$("#newparty").html(html);
...
function duel_click(name) {
	...
	if (sm_target) {
		call_code_function_f("smart_move", sm_target);
	}
}
...
function render_member(member, space) {
	var html = "";
	if (space) {
		html += "<div style='margin-left: 4px; display: inline-block'></div>";
	}
	html +=
		"<div class='gamebutton' style='padding: 6px 8px 6px 8px; font-size: 0px; line-height: 0px; text-align: center' onclick='pcs(event); duel_click(\"" +
		member.name +
		"\")'>";
	...
	if (member.rip || !member.active) {
		...
		html +=
			"<div style='margin-top: 1px; font-size: 24px; line-height: 18px'>" +
			member.name.substr(0, 3).toUpperCase() +
			"</div>";
		...
		html +=
			...
			"%; background: " +
			colors.hp +
			"; height: 4px; margin-top: 2px; display: inline-block'></div>";
	}
	html += "</div>";
	return html;
}
...
function render_map() {
	...
	if (events.duel) {
		if (I.seconds) {
			$(".dueltime").html("0:" + ("00" + I.seconds).slice(-2));
		} else {
			$(".dueltime").remove();
		}
		...
		html +=
			"<div></div><div class='gamebutton gamebutton-small' style='margin-top: 3px; margin-bottom: 3px'>vs</div><div></div>";
		m_first = true;
		I.B.forEach(function (member) {
			html += render_member(member, !m_first);
			m_first = false;
...
function wabbit_click() {
	if (!S.wabbit.live) {
		...
		add_log(
			"Engage Wabbit? <span class='clickable' onclick='pcs(event); call_code_function_f(\"smart_move\",{x:S.wabbit.x,y:S.wabbit.y,map:S.wabbit.map});' style='color: #A78059'>Go</span>",
			"gray",
		);
	}
}
...
function render_server() {
	...
	if (quirks.crypt) {
		html +=
			" <div class='gamebutton' style='padding: 6px 8px 6px 8px; font-size: 24px; line-height: 18px' onclick='pcs(event); open_guide(\"dungeon-crypt\",\"/docs/ref/dungeon-crypt\")'>";
		html +=
			"<div style='margin-top: -1px; margin-left: -3px; margin-right: -3px'>" +
			item_container({ skin: G.items.cryptkey.skin, bcolor: "black" }) +
			"</div>";
	...
	if (quirks.darkmage) {
		html +=
			" <div class='gamebutton' style='padding: 6px 8px 6px 8px; font-size: 24px; line-height: 18px' onclick='pcs(event); open_guide(\"dungeon-darkmage\",\"/docs/ref/dungeon-darkmage\")'>";
		html +=
			"<div style='margin-top: -1px; margin-left: -3px; margin-right: -3px'>" +
			item_container({ skin: G.items.frozenkey.skin, bcolor: "black" }) +
			"</div>";
		html += "<div style='color:#CFD1D1; margin-top: 1px'>INFO</div>";
		html += "</div>";
		content = true;
	...
	if (quirks.fishing) {
		html +=
			" <div class='gamebutton' style='padding: 6px 8px 6px 8px; font-size: 24px; line-height: 18px' onclick='pcs(event); open_guide(\"skill-fishing\",\"/docs/ref/skill-fishing\")'>";
		html +=
			"<div style='margin-top: -1px; margin-left: -3px; margin-right: -3px'>" +
			item_container({ skin: G.items.rod.skin, bcolor: "black" }) +
			"</div>";
		html += "<div style='color:#CFD1D1; margin-top: 1px'>INFO</div>";
		html += "</div>";
		content = true;
	...
	if (quirks.mining) {
		html +=
			" <div class='gamebutton' style='padding: 6px 8px 6px 8px; font-size: 24px; line-height: 18px' onclick='pcs(event); open_guide(\"skill-mining\",\"/docs/ref/skill-mining\")'>";
		html +=
			"<div style='margin-top: -1px; margin-left: -3px; margin-right: -3px'>" +
			item_container({ skin: G.items.pickaxe.skin, bcolor: "black" }) +
			"</div>";
		html += "<div style='color:#CFD1D1; margin-top: 1px'>INFO</div>";
		html += "</div>";
		content = true;
	...
	["crabxx", "goobrawl", "abtesting", "franky", "icegolem"].forEach(function (type) {
		if (S[type]) {
			...
			var s = type;
			if (type == "goobrawl") {
				(lcolor = "#FF5D34"), (s = "rgoo");
			}
			if (type == "abtesting") {
				(lcolor = "#E10029"), (s = "thehelmet");
			} else if (G.monsters[type] && G.monsters[type].announce) {
				lcolor = G.monsters[type].announce;
			}
			html +=
				" <div class='gamebutton' style='padding: 6px 8px 6px 8px; font-size: 24px; line-height: 18px' onclick='pcs(event); open_guide(\"event-" +
				type +
				'","/docs/ref/event-' +
				type +
				"\")'>";
	...
	["wabbit", "mrpumpkin", "mrgreen", "snowman", "dragold", "grinch", "pinkgoo", "slenderman", "tiger"].forEach(
		function (type) {
			if (S[type]) {
				var scolor = "#ECECEC";
				var lcolor = "#ECECEC";
				var lphrase = "LIVE";
				if (type == "snowman") {
					(lcolor = colors.xmasgreen), (scolor = colors.xmas);
				}
				if (type == "grinch") {
					(scolor = colors.xmasgreen), (lcolor = colors.xmas), (lphrase = "BEWARE");
				}
				html +=
					" <div class='gamebutton' style='padding: 6px 8px 6px 8px; font-size: 24px; line-height: 18px' onclick='pcs(event); emonster_click(\"" +
					type +
					"\")'>";
				...
				if (!S[type].live) {
					html +=
						...
						scolor +
						"; margin-top: 1px'>" +
						parseInt(round(-msince(new Date(S[type].spawn)))) +
						"M</div>";
	...
	if (S.halloween) {
		html +=
			" <div class='gamebutton' style='padding: 6px 8px 6px 8px; font-size: 24px; line-height: 18px' onclick='pcs(event); open_guide(\"event-halloween\",\"/docs/ref/halloween\")'>";
		html +=
			"<div style='margin-top: -1px; margin-left: -3px; margin-right: -3px'>" +
			item_container({ skin: "candy0", bcolor: "black" }) +
			"</div>";
		html += "<div style='color:#CFD1D1; margin-top: 1px'>INFO</div>";
		html += "</div>";
		content = true;
	...
	if (S.holidayseason) {
		html +=
			" <div class='gamebutton' style='padding: 6px 8px 6px 8px; font-size: 24px; line-height: 18px' onclick='pcs(event); open_guide(\"event-holidayseason\",\"/docs/ref/holidayseason\")'>";
		html +=
			"<div style='margin-top: -1px; margin-left: -3px; margin-right: -3px'>" +
			item_container({ skin: "candycane", bcolor: "black" }) +
			"</div>";
		html += "<div style='color:#CFD1D1; margin-top: 1px'>INFO</div>";
		html += "</div>";
		content = true;
	...
	if (S.lunarnewyear) {
		html +=
			" <div class='gamebutton' style='padding: 6px 8px 6px 8px; font-size: 24px; line-height: 18px' onclick='pcs(event); open_guide(\"event-lunarnewyear\",\"/docs/ref/lunarnewyear\")'>";
		html +=
			"<div style='margin-top: -1px; margin-left: -3px; margin-right: -3px'>" +
			item_container({ skin: "greenenvelope", bcolor: "black" }) +
			"</div>";
		html += "<div style='color:#CFD1D1; margin-top: 1px'>INFO</div>";
		html += "</div>";
		content = true;
	...
	if (S.egghunt) {
		html +=
			" <div class='gamebutton' style='padding: 6px 8px 6px 8px; font-size: 24px; line-height: 18px' onclick='pcs(event); open_guide(\"event-egghunt\",\"/docs/ref/egghunt\")'>";
		html +=
			"<div style='margin-top: -1px; margin-left: -3px; margin-right: -3px'>" +
			item_container({ skin: "basketofeggs", bcolor: "black" }) +
			"</div>";
		html += "<div style='color:#CFD1D1; margin-top: 1px'>INFO</div>";
		html += "</div>";
		content = true;
	...
	if (S.valentines) {
		html +=
			" <div class='gamebutton' style='padding: 6px 8px 6px 8px; font-size: 24px; line-height: 18px' onclick='pcs(event); open_guide(\"event-valentines\",\"/docs/ref/valentines\")'>";
		html +=
			"<div style='margin-top: -1px; margin-left: -3px; margin-right: -3px'>" +
			item_container({ skin: "cupid", bcolor: "black" }) +
			"</div>";
		html += "<div style='color:#CFD1D1; margin-top: 1px'>INFO</div>";
		html += "</div>";
		content = true;
...
function render_character_sheet() {
	var html =
		"<div style='background-color: black; border: 5px solid gray; padding: 20px; font-size: 24px; display: inline-block; vertical-align: top; text-align: left' class='disableclicks'>";
	...
	html +=
		"<div><span style='color:gray'>XP:</span> " +
		to_pretty_num(character.xp) +
		" / " +
		to_pretty_num(character.max_xp) +
		"</div>";
	var divider = 1;
	var disclaimer = "";
	if (pvp && !(!is_pvp && G.maps[character.map].safe_pvp)) {
		(divider = 10), (disclaimer = "<span style='color:#605B85'>(PVP)</span>");
	}
	var lost_xp = floor(min(max((character.max_xp * 0.01) / divider, (character.xp * 0.02) / divider), character.xp));
	if (character.ctype != "merchant") {
		html += "<div><span style='color:gray'>Max XP Loss:</span> " + to_pretty_num(lost_xp) + " " + disclaimer + "</div>";
	}
	if (character.party && party && party[character.name]) {
		html +=
			...
			"% <span style='color:gray'>(Your Share)</span></div>";
	}
	if (character.ctype == "merchant") {
		html += "<div><span style='color:gray'>Tax:</span> " + character.tax * 100 + "%</div>";
	}
	...
	html +=
		...
		"%)</span></div>";
	html +=
		"<div><span style='color:gray'>Armor:</span> " +
		character.armor +
		" <span style='color:gray'>(" +
	...
	if (character.lifesteal) {
		html += "<div><span style='color:gray'>Lifesteal:</span> " + to_pretty_float(character.lifesteal) + "%</div>";
	}
	if (character.manasteal) {
		html += "<div><span style='color:gray'>Manasteal:</span> " + to_pretty_float(character.manasteal) + "%</div>";
	}
	if (character.dreturn) {
		html += "<div><span style='color:gray'>Damage Return:</span> " + to_pretty_float(character.dreturn) + "%</div>";
	}
	if (character.reflection) {
		html += "<div><span style='color:gray'>Reflection:</span> " + to_pretty_float(character.reflection) + "%</div>";
	}
	if (character.evasion) {
		html += "<div><span style='color:gray'>Evasion:</span> " + to_pretty_float(character.evasion) + "%</div>";
	}
	if (character.miss) {
		html += "<div><span style='color:gray'>Miss:</span> " + to_pretty_float(character.miss) + "%</div>";
	}
	if (character.crit) {
		html += "<div><span style='color:gray'>Crit:</span> " + to_pretty_float(character.crit) + "%</div>";
	}
	if (character.critdamage) {
		html +=
			"<div><span style='color:gray'>Critical Damage:</span> " +
			to_pretty_float(200 + character.critdamage) +
			"%</div>";
	}
	if (character.apiercing) {
		html += "<div><span style='color:gray'>Armor Piercing:</span> " + character.apiercing + "</div>";
	}
	if (character.rpiercing) {
		html += "<div><span style='color:gray'>Resistance Piercing:</span> " + character.rpiercing + "</div>";
	}
	if (character.goldm != 1) {
		if (character.party && party && party[character.name] && party[character.name].gold) {
			...
			html += "<div><span style='color:gray'>Gold:</span> " + round(character.goldm * 100) + "%</div>";
		}
	}
	if (character.xpm != 1) {
	...
	if (character.luckm != 1) {
		if (character.party && party && party[character.name] && party[character.name].luck) {
			...
			html += "<div><span style='color:gray'>Luck:</span> " + round(character.luckm * 100) + "%</div>";
		}
	}
	html += "</div>";
...
function render_conditions(player) {
	...
	for (var condition in player.s) {
		...
		var actual = player.s[condition];
		if (!actual.skin && (!prop || (!prop.ui && (!actual.s || actual.s < 20)))) {
			continue;
		}
		if (player.type == "monster" && condition == "poisonous") {
			continue;
		}
	...
	html += "</div>";
	if (current) {
		if ($(".rconditions").length) {
			$(".rconditions").replaceWith(html);
		} else {
			$(".renderedinfo").append(html);
		}
		if (rids.length) {
			for (var i = 0; i < rids.length; i++) {
				$(".loadercplc" + rids[i][0]).css("opacity", 0.5);
				add_tint(".loadercplc" + rids[i][0], {
					ms: rids[i][2],
					start: future_ms(rids[i][2] - rids[i][1]),
					type: "progress",
				});
			}
		}
...
function render_mimickers() {
	...
	$(".tomimick").each(function () {
		var $this = $(this);
		var key = $this.offset().top + "|" + $this.offset().left + "|" + $this.outerWidth() + "|" + $this.outerHeight();
		// if(!isElementInViewport(this)) return; // #TODO: auto hide with overlay check
		if (window.inside != "game" || window.modal_count || !$this.is(":visible")) {
			return;
		}
		if (mimickers[key]) {
			return (new_mimickers[key] = mimickers[key]);
		}
		mimickers[key] = new_mimickers[key] = $(
			"<div class='mimicker' style='top: " +
				$this.offset().top +
				"px; left: " +
				$this.offset().left +
				"px; width: " +
			...
			.click(function (event) {
				...
				for (var i = 0; i < elements.length; i++) {
					if (
						!elements[i]["class"] != "mimicker" &&
						(elements[i].onclick || elements[i].classList.contains("clickable"))
					) {
						elements[i].click();
						break;
					}
...
function render_npc(npc) {
	var html =
		"<div style='background-color: black; border: 5px solid gray; padding: 20px; font-size: 24px; display: inline-block; vertical-align: top;' class='renderedinfo'>";
...
function render_monster(monster) {
	...
	var styles = (def.explanation && "max-width: 200px") || "";
	var name = def.name;
	var html =
		"<div style='background-color: black; border: 5px solid gray; padding: 20px; font-size: 24px; display: inline-block; vertical-align: top; " +
		styles +
		"' class='renderedinfo'>";
	if (monster.dead) {
		(name += " X"), (monster.hp = 0);
	...
	if (monster.level > 1) {
		name += " Lv." + monster.level;
	}
	var hp = monster.hp;
	var max_hp = monster.max_hp;
	var xp = monster.xp;
	if (max_hp >= 1000000) {
		(hp = to_pretty_num(hp)), (max_hp = to_pretty_num(max_hp));
	}
	if (xp >= 1000000) {
		xp = to_pretty_num(xp);
	}
	...
	if (monster.attack) {
		html += info_line({
			name: "ATT",
			color: "#316EE6",
			value: smart_num(monster.attack, 10000),
			stunned: monster.s.stunned,
			poisoned: monster.s.poisoned,
		});
	}
	if (def.avoidance) {
		html += info_line({ name: "AVOIDANCE", color: "gray", value: def.avoidance + "%" });
	}
	if (def.evasion) {
		html += info_line({ name: "EVASION", color: "gray", value: def.evasion + "%" });
	}
	if (def.reflection) {
		html += info_line({ name: "REFLECT.", color: "gray", value: def.reflection + "%" });
	}
	if (def.dreturn) {
		html += info_line({ name: "D.RETURN", color: "gray", value: def.dreturn + "%" });
	}
	if (monster.armor) {
		html += info_line({ name: "ARMOR", color: "gray", value: monster.armor });
	}
	if (monster.resistance) {
		html += info_line({ name: "RESIST.", color: "gray", value: monster.resistance });
	}
	if (def.rpiercing) {
		html += info_line({ name: "PIERCE.", color: "gray", value: def.rpiercing });
	}
	if (def.apiercing) {
		html += info_line({ name: "PIERCE.", color: "gray", value: def.apiercing });
	}
	if (def.explosion) {
		html += info_line({ name: "EXPL.", color: "gray", value: def.explosion });
	}
	if (monster.lifesteal) {
		html += info_line({ name: "LIFESTEAL", color: colors.lifesteal, value: monster.lifesteal + "%" });
	}
	if (monster["1hp"]) {
		html += info_line({ line: "1HP HITS", color: "#AEAEAE" });
	}
	if (monster.cooperative) {
		html += info_line({ line: "COOPERATIVE", color: "#AEAEAE" });
	}
	if (def.immune) {
		html += info_line({ line: "IMMUNE", color: "#AEAEAE" });
	}
	if (def.peaceful) {
		html += info_line({ line: "PEACEFUL", color: "#54B25F" });
	}
	if (def.supporter) {
		html += info_line({ line: "SUPPORTER", color: "#CA5931" });
	}
	if (def.spawns) {
	...
	if (def.spawns) {
		def.spawns.forEach(function (s) {
			html += info_line({
				name: "SPAWNS",
				color: "#237B2A",
				value: G.monsters[s[1]].name.toUpperCase(),
				onclick: "render_monster_info('" + s[1] + "')",
			});
		});
	}
	if (monster.target) {
		html += info_line({ name: "TRG", color: "orange", value: monster.target });
	}
	if (monster.pet) {
	...
	if (character) {
		...
		if (diff >= 2) {
			html += info_line({ name: "DIFF.", color: "gray", value: "Hard", vcolor: "#ED4047" });
		} else if (diff) {
			html += info_line({ name: "DIFF.", color: "gray", value: "Challenging", vcolor: "#EF9232" });
		} else {
			html += info_line({ name: "DIFF.", color: "gray", value: "Easy", vcolor: "#8BF54D" });
		}
	}
	if (def.poisonous) {
...
function render_character(player) {
	var html =
		"<div style='background-color: black; border: 5px solid gray; padding: 20px; font-size: 24px; display: inline-block; vertical-align: top;' class='renderedinfo' data-id='" +
		player.id +
		"'>";
	var cccx = $(".cccx").length;
	var ihtml = "";
	var bhtml = "";
	var xhtml = "";
	var already = false;
	if ($(".renderedinfo").length && $(".renderedinfo").data("id") == player.id) {
		already = true;
	}
	html += info_line({
	...
	if (player.heal) {
		ihtml += info_line({ name: "HEAL", color: "#CB83AC", value: round(player.heal) });
	}
	ihtml += info_line({ name: "ATT", color: "green", value: round(player.attack), cursed: player.s.cursed });
	ihtml += info_line({
		name: "ATTSPD",
		color: "gray",
		value: round(player.frequency * 100),
		poisoned: player.s.poisoned,
	});
	...
	if (player.code) {
		ihtml += info_line({ name: "CODE", color: "gold", value: "Active" });
	}
	if (player.party) {
		ihtml += info_line({ name: "PARTY", color: "#FF4C73", value: player.party });
	}
	html += ihtml;
	html += "</div>";
	...
	if (
		...
		!player.me &&
		character.party &&
		player.party == character.party &&
		party_list.indexOf(character.name) < party_list.indexOf(player.name)
	) {
		bhtml += button_line({
			name: "KICK",
			onclick: "socket.emit('party',{event:'kick',name:'" + player.name + "'}); push_deferred('party')",
			color: "#875045",
		});
	}
	...
	if (player.stand) {
		bhtml += button_line({
			name: "TOGGLE",
			onclick:
				"$('.cmerchant').toggle(); if(ctoggled==(xtarget||ctarget).name) ctoggled=null; else ctoggled=(xtarget||ctarget).name;",
			color: "#A99A5B",
	...
	if (character && !player.me && character.slots.gloves && character.slots.gloves.name == "poker") {
		bhtml += button_line({
			name: "POKE!",
			onclick: "socket.emit('poke',{name:'" + player.name + "'})",
			color: "#DF962B",
		});
	}
	html += bhtml;
...
function info_line(info) {
	...
	if (info.onclick) {
		info.value =
			"<span class='clickable tomimick inline-block' onclick=\"" +
			info.onclick +
			'" ontouchstart="' +
			info.onclick +
			'">' +
	...
	if (info.afk && info.afk == "bot") {
		addition = " <span class='gray'>[BOT]</span>";
	} else if (info.afk && info.afk == "code") {
		addition = " <span class='gray'>[CODE]</span>";
	} else if (info.afk) {
		addition = " <span class='gray'>[AFK]</span>";
	}
	if (info.cursed) {
		addition = " <span style='color: #7D4DAA'>[C]</span>";
	}
	if (info.poisoned) {
		addition = " <span style='color: #45993F'>[P]</span>";
	}
	if (info.stunned) {
		addition = " <span style='color: #FF9601'>[STUN]</span>";
	}
	if (info.line) {
		if (info.onclick) {
			info.line =
		...
		html +=
			"<span class='cbold' style='color: " + color + "'>" + info.name + "</span>: " + info.value + addition + "<br />";
	}
	return html;
}
...
function render_slots(player, args) {
	...
	function render_slot(slot, shade, op) {
		...
		var chtml = "";
		var cached = cache_slots && cache_slots[slot];
		var cid = "slot" + slot; // empty border color
		if (!window.mode || mode.empty_borders_darker) {
			(ecolor = "#222424"), (ecolor = "#292929");
		} //,ecolor="black";
		if (!op) {
			op = 0.4;
		}
		if (player.slots[slot]) {
			...
			var item = G.items[current.name];
			if (!item) {
				item = G.items.placeholder_m;
			}
			...
			if (
				...
				(player.skin.startsWith("mm_") ||
					player.skin.startsWith("mf_") ||
					player.skin.startsWith("tm_") ||
					player.skin.startsWith("tf_"))
			) {
				skin = item.skin_a;
			}
	...
	var my_slots =
		((player.me && player.slots.trade1 !== undefined) ||
			player.slots.trade1 ||
			player.slots.trade2 ||
			player.slots.trade3 ||
			player.slots.trade4) &&
	...
	var already = false;
	var sid = player.stand + "|" + my_slots;
	if (!args.pure && $(".slots").length && $(".slots").data("id") == player.id && sid == cache_sid) {
		already = true;
	} else {
		cache_slots = {};
	}
	...
	if (player.stand) {
		...
		var found = false;
		for (var t = 30; t >= 25; t--) {
			if ("trade" + t in player.slots) {
				(row = 5), (col = 6), (found = true);
			}
		}
		for (var t = 24; t >= 17; t--) {
			if (!found && "trade" + t in player.slots) {
				(row = 4), (col = 6), (found = true);
			}
		}
	...
	if (player.stand) {
		html += "<div class='cmerchant hidden'>";
	}
	html += "<div>";
	...
	if (!args.pure && !already) {
		if ($(".slots").length) {
			$(".slots").replaceWith(html);
		} else {
			$("#topleftcornerui").append(html);
		}
	}
...
function render_gold_npc() {
	...
	if (options.bank_max) {
		html +=
			"<div class='gamebutton clickable mr5' onclick='$(\".npcgold\").html(max(character.bank.gold,character.gold))'>MAX</div>";
	}
	html +=
		"<div class='gamebutton clickable mr5' onclick='deposit()'>DEPOSIT</div><div class='gamebutton clickable' onclick='withdraw()'>WITHDRAW</div></div>";
	html += "</div>";
	$("#topleftcornerui").html(html);
	cfocus(".npcgold");
}
...
function render_items_npc(pack) {
	...
	for (var i = 0; i < Math.ceil(max(character.isize, items.length) / 7); i++) {
		...
		for (var j = 0; j < 7; j++) {
			...
			if (current) {
				...
				html += item_container(
					{ skin: skin, def: item, id: "str" + id, draggable: true, strnum: last - 1, snum: last - 1 },
					current,
				);
	...
	html +=
		"</div><div id='storage-item' class='rendercontainer' style='display: inline-block; vertical-align: top; margin-left: 5px'></div>";
	$("#topleftcornerui").html(html);
	...
	if (!inventory) {
		render_inventory(), (inventory_opened_for = topleft_npc);
	}
}
...
function update_inventory() {
	...
	for (var i = 0; i < Math.ceil(max(character.isize, character.items.length) / 7); i++) {
		for (var j = 0; j < 7; j++) {
			...
			var cc_id = "c" + id;
			var html = "";
			var cached = cache_i[last];
			if (last < character.items.length) {
				current = character.items[last];
			}
	...
	["upgrade", "compound", "exchange"].forEach(function (e) {
		if (character.q[e] && rids[character.q[e].num]) {
			...
			add_tint(".loaderqplc" + rids[character.q[e].num], {
				ms: character.q[e].ms,
				start: future_ms(character.q[e].ms - character.q[e].len),
				type: "progress",
			});
		}
	});
...
function render_inventory(reset) {
	...
	if (c_enabled) {
		if (is_comm) {
			...
			html +=
				...
				"</span></div>";
			html +=
				"<div style='border-bottom: 5px solid gray; margin-bottom: 2px; margin-left: -5px; margin-right: -5px'></div>";
			right_style = "";
		} else if (is_electron) {
			html +=
				"<div style='padding: 4px; display: inline-block' class='clickable' onclick='pcs(event); show_shells_info()'>"; // '
			html +=
				"<span class='cbold' style='color: " +
				colors.cash +
				"'>SHELLS</span>: <span class='cashnum'>" +
	...
	html +=
		...
		right_style +
		"'><span class='cbold' style='color: gold'>GOLD</span>: <span class='goldnum'>" +
	...
	cache_i = character.items.slice();
	if (is_comm) {
		return show_modal(html, { wrap: false });
	}
	inventory = true;
	if (!reset) {
...
function render_craftsman() {
	...
	var html =
		"<div style='background-color: black; border: 5px solid gray; padding: 20px; font-size: 24px; display: inline-block; vertical-align: top; text-align: center'>";
...
function render_dismantler() {
	...
	/*html+="</div>";
		...
		html+="</div>";*/
	html +=
		"<div style='margin-top: 12px'><div class='gamebutton clickable' onclick='dismantle()'>" + button + "</div></div>";
	html += "</div>";
	$("#topleftcornerui").html(html);
	if (!inventory) {
		render_inventory(), (inventory_opened_for = topleft_npc);
...
function render_locksmith(mode) {
	...
	var button = "LOCK";
	var f = "lock_item";
	var shade = "shade_seal";
	if (mode == "unlock") {
		(button = "UNLOCK"), (f = "unlock_item"), (shade = "shade_unlock");
	}
	if (mode == "seal") {
		(button = "SEAL"), (f = "seal_item"), (shade = "shade_lock");
	}
	reset_inventory(1);
	...
	/*html+="</div>";
		...
		html+="</div>";*/
	html +=
		"<div style='margin-top: 12px'><div class='gamebutton clickable' onclick='" + f + "()'>" + button + "</div></div>";
	html += "</div>";
	$("#topleftcornerui").html(html);
	if (!inventory) {
		render_inventory(), (inventory_opened_for = topleft_npc);
...
function render_recipes(type) {
	...
	var items = [];
	var html =
		"<div style='background-color: black; border: 5px solid gray; padding: 2px; font-size: 24px; display: inline-block'>";
	if (type == "dismantle") {
		...
		for (var j = 0; j < 5; j++) {
			if (i == 3 && j == 0 && r_page[type] != 0) {
				html += item_container(
					{
						skin: "left",
						onclick: "r_page['" + type + "']=" + (r_page[type] - 1) + "; render_recipes('" + type + "');",
					},
					{ q: r_page[type], left: true },
				);
				...
				var id = "item" + randomStr(10);
				var item = G.items[current];
				html += item_container(
					{
						skin: item.skin_a || item.skin,
						def: item,
						id: id,
						draggable: false,
						onclick: "render_recipe(this,'" + type + "','" + current + "')",
					},
					{ name: current },
				);
			} else {
...
function render_recipes_old(quest) {
	...
	object_sort(G.craft).forEach(function (io) {
		...
		var name = io[0];
		html += item_container(
			{ skin: G.items[name].skin, onclick: "render_recipe(null,'craft','" + name + "')" },
			{ name: name },
		);
		i += 1;
		if (!(i % 6)) {
	...
	object_sort(G.dismantle).forEach(function (io) {
		...
		var name = io[0];
		html += item_container(
			{ skin: G.items[name].skin, onclick: "render_recipe(null,'dismantle','" + name + "')" },
			{ name: name },
		);
		i += 1;
		if (!(i % 6)) {
...
function render_exchange_shrine(type) {
	...
	exchange_type = type;
	if (type == "leather") {
		(shade = "leather"), (button = "GIVE");
	}
	if (type == "lostearring") {
		(shade = "lostearring"), (button = "PROVIDE");
	}
	if (type == "mistletoe") {
		(shade = "mistletoe"), (button = "GIVE IT");
	}
	if (type == "candycane") {
		(shade = "candycane"), (button = "FEED");
	}
	if (type == "ornament") {
		(shade = "ornament"), (button = "GIVE");
	}
	if (type == "seashell") {
		(shade = "seashell"), (button = "GIVE");
	}
	if (type == "gemfragment") {
		(shade = "gemfragment"), (button = "PROVIDE");
	}
	if (type == "cx") {
		(shade = "cosmo0"), (button = "SHAZAM");
	}
	e_item = null;
	var html =
		"<div style='background-color: black; border: 5px solid gray; padding: 20px; font-size: 24px; display: inline-block; vertical-align: top; text-align: center'>";
	...
	if (character.q.exchange) {
		...
		var q = undefined;
		if (character.q.exchange.q > 1) {
			q = character.q.exchange.q;
		}
		html += item_container(
			{ cid: "eitem", draggable: false, droppable: false, skin: G.items[name].skin },
			{ name: name, level: level, q: q },
		);
	} else {
		html += item_container({ shade: shade, cid: "eitem", s_op: 0.5, draggable: false, droppable: true });
	}
	//html+="</div>";
	html += "</div>";
	html += "</div>";
	...
	html += "</div>";
	html +=
		"<div id='exc-ui' class='rendercontainer' style='display: inline-block; vertical-align: top; margin-left: 5px'>" +
		"</div>";
	$("#topleftcornerui").html(html);
	if (!inventory) {
		render_inventory(), (inventory_opened_for = topleft_npc);
...
function render_shells_buyer() {
	...
	rendered_target = topleft_npc;
	var html =
		"<div style='background-color: black; border: 5px solid gray; padding: 20px; font-size: 24px; display: inline-block; vertical-align: top; text-align: left'>";
	var prefix = "";
	html +=
		"<div><span style='color: #5DAC40'>10</span> Shells = <span style='color: gold'>1,500,000</span> <span style='color: #71AF83' class='clickable' onclick='buy_shells(10)'>BUY</span></div>";
	html +=
		"<div><span style='color: #5DAC40'>100</span> Shells = <span style='color: gold'>15,000,000</span> <span style='color: #71AF83' class='clickable' onclick='buy_shells(100)'>BUY</span></div>";
	html +=
		"<div><span style='color: #5DAC40'>500</span> Shells = <span style='color: gold'>75,000,000</span> <span style='color: #71AF83' class='clickable' onclick='buy_shells(500)'>BUY</span></div>";
	html +=
	...
	if (!is_electron) {
		prefix =
			"<a href='https://adventure.land/shells' class='cancela' target='_blank'><span class='clickable' onclick='rendered_target=null;' style='color: #359ECF'>Buy With $</span></a> | ";
	}
	html +=
		"<div>" + prefix + "<span class='clickable' onclick='topleft_npc=false;' style='color: #555556'>Nope</span></div>";
	html += "</div>";
	$("#topleftcornerui").html(html);
	if (!inventory) {
		render_inventory(), (inventory_opened_for = topleft_npc);
...
function render_upgrade_shrine(explicit) {
	...
	var already = topleft_npc == "upgrade";
	topleft_npc = "upgrade";
	rendered_target = topleft_npc;
	...
	if (
		character.q.upgrade &&
		character.items[character.q.upgrade.num] &&
		character.items[character.q.upgrade.num].name == "placeholder"
	) {
		...
		if (def.offering) {
			core += item_container(
				{ skin: G.items[def.offering].skin, loader: !def.scroll && "theuitem" + rid },
				{ name: def.offering },
			);
		} else {
			core += item_container({ shade: "shade_offering", s_op: 0.36 });
		}
		if (def.scroll) {
			core += item_container({ skin: G.items[def.scroll].skin, loader: "theuitem" + rid }, { name: def.scroll });
		} else {
	...
	html += "</div>";
	html +=
		"<div class='gamebutton clickable' onclick='draw_trigger(function(){ render_upgrade_shrine(1); reset_inventory(); });'>RESET</div>";
	...
	if (character.q.upgrade) {
		...
		add_tint(".loadertheuitem" + rid, {
			ms: character.q.upgrade.ms,
			start: future_ms(character.q.upgrade.ms - character.q.upgrade.len),
			type: "progress",
			upgrade: true,
		});
	}
...
function render_compound_shrine(explicit) {
	...
	var already = topleft_npc == "compound";
	...
	if (
		...
		character.items[character.q.compound.num] &&
		character.items[character.q.compound.num].name == "placeholder"
	) {
		...
		core += "<div>";
		if (def.offering) {
			core += item_container({ skin: G.items[def.offering].skin }, { name: def.offering });
		} else {
			core += item_container({ shade: "shade_offering", s_op: 0.36 });
		}
		core += item_container(
			{ skin: G.items[def.scroll].skin, pui: def, loader: "theuitem" + rid },
			{ name: def.scroll },
		);
		core += "</div>";
	} else {
	...
	html += "</div>";
	html +=
		"<div class='gamebutton clickable' onclick='draw_trigger(function(){ render_compound_shrine(1); reset_inventory(); });'>RESET</div>";
	html +=
		"<div class='gamebutton clickable ml5' onclick=' compound(c_items[0],c_items[1],c_items[2],c_scroll,c_offering);'>COMBINE</div>";
	html += "</div>";
	if (already) {
		$("#core").html(core);
	} else {
	...
	if (character.q.compound) {
		$(".loadertheuitem" + rid).css("opacity", 0.8);
		add_tint(".loadertheuitem" + rid, {
			ms: character.q.compound.ms,
			start: future_ms(character.q.compound.ms - character.q.compound.len),
			type: "progress",
			compound: true,
		});
	}
...
function render_dice() {
	...
	rendered_target = topleft_npc;
	var html =
		"<div style='background-color: black; border: 5px solid gray; padding: 20px; font-size: 32px; display: inline-block; vertical-align: top'>";
	html += "<div class='mb5' align='center'>";
	html +=
		"<div><span class='gray clickable' onclick='$(\".dicenum\").cfocus()'>NUMBER:</span> <div class='inline-block dicenum' contenteditable=true onblur='on_dice_change()'>" +
		num +
		"</div></div>";
	html += "</div>";
	html += "<div class='mb5' align='center'>";
	...
	html += "<div class='mb5' align='center'>";
	html +=
		"<div class='gamebutton clickable diceup' onclick='dice_bet.dir=1; on_dice_change()' style='width: 64px;'>UP</div>";
	html +=
		"<div class='gamebutton clickable ml5 dicedown' onclick='dice_bet.dir=2; on_dice_change()' style='width: 64px'>DOWN</div>";
	html += "</div>";
	html += "<div class='mb5' align='center'>";
	html +=
		"<div class='gamebutton clickable diceb' onclick='on_dice_bet()' style='width: 200px;'>BET <span class='gray dicexx'>FOR 2X</span></div>";
	html += "</div>";
	html += "</div>";
	$("#topleftcornerui").html(html);
	if (!inventory) {
...
function render_donate() {
	...
	html += "<div class='mb5' align='center'>";
	html +=
		"<div class='gamebutton clickable diceb' onclick='donate()' style='width: 160px; margin-top: 20px'>DONATE</div>";
	html += "</div>";
	html += "</div>";
	$("#topleftcornerui").html(html);
	if (!inventory) {
	...
	for (var i = 0; i < 4; i++) {
		...
		for (var j = 0; j < 5; j++) {
			if (last < npc.items.length && npc.items[last++] && (c_enabled || !G.items[npc.items[last - 1]].cash)) {
				...
				if (premium) {
					collection.push({ id: id, item: item, name: current, value: item.g, cash: item.cash });
				} else if (item.cash) {
					collection.push({ id: id, item: item, name: current, value: item.g * G.inflation });
				} else {
					collection.push({ id: id, item: item, name: current, value: item.g });
				}
			} else {
				html += item_container({ size: 40, draggable: false, droppable: true });
			}
		...
		for (var j = 0; j < 5; j++) {
			if (i == 3 && j == 0 && t_page[token] != 0) {
				html += item_container(
					{
						skin: "left",
						onclick: "t_page['" + token + "']=" + (t_page[token] - 1) + "; render_token_exchange('" + token + "');",
					},
					{ q: t_page[token], left: true },
				);
			} else if (i == 3 && j == 4 && last < items.length - 1) {
				html += item_container(
					{
				...
				collection.push({
					...
					token: token,
					key: items[last - 1],
					actual: { name: current, level: 0, q: 1, data: data },
				});
			} else {
				html += item_container({ size: 40, draggable: false, droppable: true });
			}
...
function render_drop(def, mult, color) {
	...
	if (def[1] == "cx") {
		...
		html +=
			"<span class='clickable' onclick='pcs(event); render_item_info(\"" +
			def[1] +
			'",0,"' +
			((actual && actual.data) || "") +
			"\")'>" +
			item_container({ skin: skin }, actual) +
			"</span>";
	}
	if (def[0] * mult >= 1) {
		html +=
		...
		1 / (def[0] * mult) >= 1.1 &&
		1 / (def[0] * mult) < 10 &&
		parseInt(1 / (def[0] * mult)) * 10 != parseInt(10 / (def[0] * mult))
	) {
		html +=
		...
		html +=
			...
			((1 / (def[0] * mult) >= 2 && to_pretty_num(round(1 / (def[0] * mult)))) ||
				to_pretty_float(1 / (def[0] * mult))) +
			"</div>";
	}
	html += "</div>";
	return html;
...
function render_equip_info(name) {
	...
	var html = "";
	html +=
		"<div style='background-color: black; border: 5px solid gray; font-size: 24px; display: inline-block; padding: 20px; line-height: 24px; max-width: 360px;' class='buyitem'>";
	html +=
		"<div style='padding: 4px; margin: 4px; text-align: center; color: #CDCAB7'>" +
		(weapon_types[def.wtype] || offhand_types[def.type] || def.wtype || def.type).toTitleCase() +
		"</div>";
	["ranger", "rogue", "warrior", "mage", "priest", "paladin", "merchant"].forEach(function (ctype) {
		var color = "#DDDDDD";
		if (window.character && character.ctype == ctype) {
			color = "#36813A";
		...
		if (G.classes[ctype].mainhand[def.wtype || def.type]) {
			...
			if (!s) {
				html += "<div style='color: #788783'>No Modifier</div>";
			} else {
				html += s;
			}
			html += "</div>";
...
function render_item_help(container, name, level, pure) {
	...
	var names = [name];
	html +=
		"<div style='background-color: black; border: 5px solid gray; font-size: 24px; display: inline-block; padding: 20px; line-height: 24px; max-width: 240px;' class='buyitem'>";
	for (var depth = 0; depth < 3; depth++) {
		for (var dname in G.drops) {
			if (in_arr(dname, names) || dname == "glitch" || dname == "lglitch") {
				continue;
			}
	...
	for (var mname in G.drops.maps) {
		...
		if (mname != "global" && (!G.maps[mname] || G.maps[mname].ignore)) {
			continue;
		}
		for (var i = 0; i < table.length; i++) {
			if (table[i][1] == name || (table[i][1] == "open" && in_arr(table[i][2], names))) {
	...
	for (var iname in G.craft) {
		...
		G.craft[iname].items.forEach(function (ii) {
			if (!done && ii[1] == name) {
				if (G.craft[iname].quest == "mcollector") {
					collecting.push(iname);
				} else {
					crafting.push(iname);
				}
				done = true;
	...
	if (tokens.length) {
		...
		tokens.forEach(function (token) {
			...
			html += item_container(
				{ skin: G.items[token].skin, onclick: "stpr(event); render_item_popup('" + token + "')" },
				{ name: token },
			);
			html += "</div>";
		});
	...
	if (G.items[name].e) {
		...
		var phrase = "Exchange From";
		var id = "exchange";
		for (var nname in G.npcs) {
			if (G.items[name].quest && G.npcs[nname].quest == G.items[name].quest) {
				(npc = G.npcs[nname]), (phrase = "Bring To"), (id = nname);
			}
		}
		...
		html += item_container(
			{
				...
				onclick:
					"stpr(event); render_exchange_info('" +
					(name + (((G.items[name].upgrade || G.items[name].compound) && (level || "0")) || "")) +
					"')",
			},
	...
	if (G.items[name].upgrade || G.items[name].compound) {
		...
		var phrase = "Upgrade At";
		var id = "newupgrade";
		if (G.items[name].compound) {
			phrase = "Combine 3 At";
		}
		html += "<div style='color:#DDDDDD'>" + phrase + ":</div>";
		html +=
	...
	if (crafting.length) {
		...
		crafting.forEach(function (i) {
			html += item_container(
				{ skin: G.items[i].skin, onclick: "stpr(event); render_recipe(null,'','" + i + "')" },
				{ name: i },
			);
		});
	}
	if (collecting.length) {
		...
		collecting.forEach(function (i) {
			html += item_container(
				{ skin: G.items[i].skin, onclick: "stpr(event); render_recipe(null,'mcollector','" + i + "')" },
				{ name: i },
			);
		});
	}
	if (G.craft[name]) {
		...
		if (G.craft[name].quest == "mcollector") {
			(phrase = "Obtainable From"), (npc = G.npcs.mcollector), (id = "mcollector"), (rphrase = "Materials");
		}
		if (G.craft[name].quest == "witch") {
			(phrase = "Concoctiable At"), (npc = G.npcs.witch), (id = "witch"), (rphrase = "Materials");
		}
		html += "<div style='color:#DDDDDD'>" + phrase + ":</div>";
		html +=
			"<div style='display:inline-block; text-align: center' class='clickable' onclick='smart_smart_move(\"npc\",\"" +
			...
			"</div></div>";
		html +=
			"<span class='clickable' onclick='stpr(event); show_recipe(\"" +
			name +
			"\")'>" +
			bold_prop_line("Show", rphrase, "#6F75DC") +
			"</span>";
	}
	if (G.dismantle[name]) {
	...
	if (monsters.length) {
		...
		monsters.forEach(function (mo) {
			var m = mo[0];
			html +=
				"<div style='display:inline-block; text-align: center' class='clickable hspace5' onclick='smart_smart_move(\"monster\",\"" +
				m +
				"\")'>";
			html +=
			...
			html += "</div>";
			if (mo[1]) {
				html += "<div>" + to_pretty_fraction(mo[1]) + "</div>";
			}
	...
	if (maps.length) {
		...
		maps.forEach(function (map) {
			if (map == "global") {
				html +=
					"<div style='color:#DD3177' onclick='stpr(event); render_all_monsters()' class='clickable'>Everywhere</div>";
			} else {
				html +=
					"<div style='color:#47A642' onclick='render_travel(\"" +
					map +
					"\")' class='clickable'>" +
					G.maps[map].name +
					"</div>";
	...
	if (items.length) {
		...
		items.forEach(function (i) {
			html += item_container(
				{ skin: G.items[i[0]].skin, onclick: "stpr(event); render_item_popup('" + i[0] + "'," + i[1] + ")" },
				{ name: i[0], level: i[1] },
			);
		});
	}
...
function render_item_info(name, level, data) {
	...
	if (name == "empty") {
		...
		html += render_item("html", {
			item: {
				name: "Shells",
				explanation:
					"Premium currency, can be used to buy cosmetics, extra bank storage, or for account operations like transferring a character.",
			},
			prop: {},
		});
	} else if (name == "gold") {
		...
		for (var i = 0; i <= 15; i++) {
			html +=
				...
				render_item("html", { item: G.items[name], actual: { level: i, name: name }, guide: true }) +
				"</div>";
			if (calculate_item_grade(G.items[name], { level: i }) == 4) {
				break;
			}
		}
	...
	html +=
		"<div></div><div style='display: inline-block; margin: 5px'>" +
		render_item_help(null, name, level, true) +
		"</div>";
	html += "</div>";
...
function render_monster_info(name) {
	...
	var mowner = "";
	html +=
		"<div class='clickable' onclick='smart_smart_move(\"monster\",\"" +
		name +
		"\")'>" +
		sprite(name, { full: true, scale: 3 }) +
		"</div>";
	var RF = (Object.keys(tracker).length && tracker) || (G && G.drops) || { monsters: {}, maps: {} };
	var MR = (Object.keys(tracker).length && tracker && tracker.drops) || RF.monsters;
	if (tracker && tracker.monsters) {
		count = tracker.monsters[name] || 0;
		diff = tracker.monsters_diff[name] || 0;
		if (tracker.max.monsters[name]) {
			(mcount = tracker.max.monsters[name][0]), (mowner = tracker.max.monsters[name][1]);
		}
	}
	html += render_item("html", {
		pure: true,
		item: G.monsters[name],
		prop: G.monsters[name],
		monster: name,
		count: count,
		mcount: mcount,
		score: count + diff,
	...
	object_sort(G.maps).forEach(function (io) {
		var map = io[1];
		var mname = io[0];
		if (map.ignore || !((RF.maps && RF.maps[mname]) || []).length) {
			return;
		}
	...
	if (
		(RF.maps && RF.maps.global_static && RF.maps.global_static.length) ||
		(RF.maps && RF.maps.global && RF.maps.global.length)
	) {
		var mult = 1;
		if (G.monsters[name]["1hp"]) {
			mult = 1000;
		}
...
function render_tracker() {
	...
	object_sort(G.monsters, "hpsort").forEach(function (e) {
		if (((e[1].stationary || e[1].cute) && !e[1].achievements) || e[1].unlist) {
			return;
		}
	...
	object_sort(G.items).forEach(function (e) {
		if (e[1].e && !e[1].ignore) {
			...
			list.forEach(function (d) {
				...
				if (G.drops[d[1]]) {
					html +=
						" class='clickable' onclick='pcs(event); render_exchange_info(\"" +
						d[1] +
						'",' +
						(tracker.exchanges[d[1]] || 0) +
						")'>";
				} else {
					html += ">";
				}
...
function render_computer($element, type = "computer", slot = 0) {
	...
	html +=
		"<div onclick='socket.emit(\"trade_history\",{})' class='clickable' style='color: #E4E4E4'><span style='color: #BA61A4'>&gt;</span> TRADE HISTORY</div>";
	html +=
		"<div onclick='toggle_merchant(\"" +
		slot +
		"\")' class='clickable' style='color: #E4E4E4'><span style='color: #BA61A4'>&gt;</span> TOGGLE STAND</div>";
	if (type == "supercomputer") {
		html +=
			"<div onclick=\"socket.emit('tracker')\" class='clickable' style='color: #E4E4E4'><span style='color: #BA61A4'>&gt;</span> TRACKER</div>";
	}

	html +=
		"<div onclick='render_upgrade_shrine()' class='clickable' style='color: #E4E4E4'><span style='color: #BA61A4'>&gt;</span> UPGRADE</div>"; // style='color: #C3C3C3' style='color: #D6D6D6'
	html +=
		"<div onclick='render_compound_shrine()' class='clickable' style='color: #E4E4E4'><span style='color: #BA61A4'>&gt;</span> COMPOUND</div>";
	html +=
		"<div onclick='render_exchange_shrine()' class='clickable' style='color: #E4E4E4'><span style='color: #BA61A4'>&gt;</span> EXCHANGE</div>";
	html +=
		"<div onclick='render_interaction({auto:true,dialog:\"locksmith\",skin:\"asoldier\"});' class='clickable' style='color: #E4E4E4'><span style='color: #BA61A4'>&gt;</span> LOCKSMITH</div>";
	html +=
		"<div onclick='render_interaction(\"crafting\");' class='clickable' style='color: #E4E4E4'><span style='color: #BA61A4'>&gt;</span> CRAFTING</div>";
	html +=
		"<div onclick='render_merchant(G.npcs.pots)' class='clickable' style='color: #E4E4E4'><span style='color: #BA61A4'>&gt;</span> POTIONS</div>";
	html +=
		"<div onclick='render_merchant(G.npcs.scrolls)' class='clickable' style='color: #E4E4E4'><span style='color: #BA61A4'>&gt;</span> SCROLLS</div>";
	html +=
		"<div onclick='render_merchant(G.npcs.basics)' class='clickable' style='color: #E4E4E4'><span style='color: #BA61A4'>&gt;</span> BASICS</div>";

	html +=
		"<div onclick='render_merchant(G.npcs.premium,false)' class='clickable' style='color: #E4E4E4'><span style='color: #BA61A4'>&gt;</span> PREMIUM</div>";
...
function render_skill(selector, skill_name, args) {
	...
	if (!skill) {
		...
		html +=
			"<div style='color: #4EB7DE; display: inline-block; border-bottom: 2px dashed gray; margin-bottom: 3px' class='cbold'>" +
			skill.name +
			"</div>";
		if (skill.explanation) {
			html += "<div style='color: #C3C3C3'>" + skill.explanation + "</div>";
			if (skill.mp) {
				html += bold_prop_line("MP", skill.mp, colors.mp);
			}
			if (skill.duration) {
				html += bold_prop_line("Duration", skill.duration / 1000.0 + " seconds", "gray");
			}
			if (skill.cooldown && skill.cooldown / 1000.0) {
				html += bold_prop_line("Cooldown", skill.cooldown / 1000.0 + " seconds", "gray");
			}
			if (skill.reuse_cooldown && skill.reuse_cooldown / 1000.0) {
				html += bold_prop_line("R.Use Cooldown", skill.reuse_cooldown / 1000.0 + " seconds", "gray");
			}
			if (skill.share) {
				html += bold_prop_line(
					"Cooldown",
					to_pretty_float(skill.cooldown_multiplier || 1) + "X of " + G.skills[skill.share].name,
					"gray",
				);
			}
			if (skill.range) {
				html += bold_prop_line("Range", skill.range, "gray");
			}
			if (skill.use_range) {
				html += bold_prop_line("Range", "Character Range", "gray");
			}
			if (skill.range_multiplier && skill.range_bonus) {
				html += bold_prop_line(
					"Range",
					to_pretty_float(skill.range_multiplier || 1) + "X + " + skill.range_bonus,
					"gray",
				);
			} else if (skill.range_multiplier) {
				html += bold_prop_line("Range", to_pretty_float(skill.range_multiplier || 1) + "X of Character Range", "gray");
			}
			if (skill.level) {
				html += bold_prop_line("Level Requirement", skill.level, "gray");
			}
			if (skill.max) {
				html += bold_prop_line("Max", skill.max, "gray");
			}
			if (skill.type == "passive") {
				html += "<div><span style='color: #696C68;'>Passive</span></div>";
			}
			if (skill.damage_type) {
				if (skill.damage_type == "pure") {
					html += bold_prop_line("Damage Type", "Pure", "#AA9B55");
				} else if (skill.damage_type == "magical") {
					html += bold_prop_line("Damage Type", "Magical", "#8998AA");
				} else if (skill.damage_type == "physical") {
					html += bold_prop_line("Damage Type", "Physical", "#93AB98");
				}
			}
			if (skill.condition && G.conditions[skill.condition]) {
				html += info_line({
					...
					onclick:
						"dialogs_target=xtarget||ctarget; show_json(G.conditions." +
						skill.condition +
						",{name:'G.conditions." +
						skill.condition +
						"'})",
			...
			if (skill.consume) {
				html +=
					...
					item_container({ skin: G.items[skill.consume].skin, def: G.items[skill.consume] });
				+"</div>";
			}
			html +=
...
function render_secondhands(type) {
	...
	for (var i = 0; i < 4; i++) {
		html += "<div>";
		for (var j = 0; j < 5; j++) {
			if (!type && i == 3 && j == 0 && s_page != 0) {
				html += item_container(
					{ skin: "left", onclick: "s_page=" + (s_page - 1) + "; render_secondhands();" },
					{ q: s_page, left: true },
				);
			} else if (!type && i == 3 && j == 4 && last < items.length - 1) {
				html += item_container(
					{ skin: "right", onclick: "s_page=" + (s_page + 1) + "; render_secondhands();" },
					{ q: s_page + 2 },
				);
			} else if (type && i == 3 && j == 0 && l_page != 0) {
				html += item_container(
					{ skin: "left", onclick: "l_page=" + (l_page - 1) + "; render_secondhands('lostandfound');" },
					{ q: l_page, left: true },
				);
			} else if (type && i == 3 && j == 4 && last < items.length - 1) {
				html += item_container(
					{ skin: "right", onclick: "l_page=" + (l_page + 1) + "; render_secondhands('lostandfound');" },
					{ q: l_page + 2 },
				);
			} else if (last < items.length && items[last++]) {
				var current = items[last - 1];
...
function old_render_stepv1(args) {
	...
	args.main =
		"Welcome to the first step of the tutorial. In this step, we are going to move! Now move your character near the green goo's by clicking on the map and walking below the town!";
	args.code = "Using the CODE feature. You can use the `move` function, or, the more costly `smart_move` function.";
	var html = "";
	if (args.title) {
		html +=
			"<div style='border: 5px solid #65A7E6; background-color: #E6E6E6; color: #333333; margin: 3px; padding: 5px; font-size: 24px; display: inline-block'>" +
			args.title +
			"</div>";
	}
	if (args.main) {
		html +=
			"<div style='border: 5px solid gray; background-color: #E6E6E6; color: #333333; margin: 3px; padding: 5px; font-size: 24px;'>" +
			args.main +
			"</div>";
	}
	if (args.code) {
		html +=
			"<div style='border: 5px solid #E4738A; background-color: #E6E6E6; color: #333333; margin: 3px; padding: 5px; font-size: 24px;'>" +
			args.code +
			"</div>";
	}
	show_modal(html);
...
function load_documentation(name) {
	if (in_arr(name, G.docs.documented)) {
		api_call("load_article", { name: name, func: true });
	} else if (name == "character") {
		show_json(game_stringify(character, "\t"));
	} else if (in_arr(name, G.docs.functions)) {
		render_function_reference(name);
	} else {
		$(".codesearch").val(name);
		csearch_logic("ui");
	}
...
function render_tutorial(article, step, url) {
	...
	var cphrase = "CONTINUE";
	if (step == G.docs.tutorial.length - 1) {
		cphrase = "COMPLETE";
	}
...
function render_function_reference(n, f, c) {
	// hide_modal();
	if (n == "character") {
		return show_json(game_stringify(character, "\t"));
	}
	if (!c) {
	...
	if (render_function_html) {
		render_learn_article(render_function_html + "<textarea class='codemirror" + rid + "'></textarea>", {
			url: "/docs/code/functions/" + n,
		});
...
function render_functions_directory() {
	...
	G.docs.functions.forEach(function (n) {
		if (in_arr(n, G.docs.documented)) {
			html +=
				"<div style='display: block; margin-bottom: 4px; font-size: 32px' class='clickable' onclick='api_call(\"load_article\",{name:\"" +
				n +
				"\",func:true});'>" +
				n +
				"</div>";
			...
			html +=
				"<div style='display: block; margin-bottom: 4px; font-size: 32px' class='clickable' onclick='render_function_reference(\"" +
				n +
				"\")'>" +
				n +
				"</div>";
...
function render_all_recipes() {
	var html =
		"<div style='border: 5px solid gray; background-color: black; padding: 10px; width: 734px; font-size: 32px'>";
	...
	object_sort(G.craft, "gold_value").forEach(function (r) {
		if (xprev) {
			html += "<div style='border-top: 4px solid gray; margin-left: -12px; margin-right: -12px'></div>";
		}
		var prev = false;
		var name = r[0];
		var recipe = r[1];
		...
		recipe.items.forEach(function (i) {
			...
			html += item_container(
				{ skin: G.items[i[1]].skin, onclick: "render_item_info('" + i[1] + "')" },
				{ name: i[1], q: i[0], level: i[2] },
			);
			prev = true;
		});
		if (recipe.cost) {
			html += " <span style='color: gray'>+</span> <span style='color: gold'>" + to_pretty_num(recipe.cost) + "</span>";
		}
		html += "</div>";
		xprev = true;
	});
...
function render_cx_info(name) {
	...
	html +=
		...
		name +
		"<br /><span class='gray'>Type:</span> " +
		T[name] +
		" <br /><span class='gray'>Slot:</span> " +
...
function render_all_cosmetics() {
	...
	var visited = {};
	var html = "<div style='border: 5px solid gray; background-color: black; padding: 10px; width: 456px'>";
	...
	types.forEach(function (type) {
		...
		type[2].forEach(function (cid) {
			html +=
				"<span class='clickable' onclick='render_cx_info(\"" +
				cid +
				"\")'>" +
				cx_sprite(cid, { mright: 4, labels: true }) +
				"</span>";
		});
		html += "</div>";
	});
...
function render_all_items() {
	...
	var visited = {};
	var html = "<div style='border: 5px solid gray; background-color: black; padding: 10px; width: 434px'>";
	...
	types.forEach(function (type) {
		...
		type[2].forEach(function (item) {
			html += item_container(
				{ skin: G.items[item[0]].skin, onclick: "render_item_info('" + item[0] + "')" },
				{ name: item[0] },
			);
		});
		html += "</div>";
...
function render_all_monsters() {
	...
	object_sort(G.monsters, "hpsort").forEach(function (e) {
		if (((e[1].stationary || e[1].cute) && !e[1].achievements) || e[1].hide) {
			return;
		}
		html +=
			"<div style='background-color:#575983; border: 2px solid #9F9FB0; position: relative; display: inline-block; margin: 2px; /*" +
...
function render_all_events() {
	function event_html(e, key) {
		var ehtml = "";
		ehtml +=
			" <div class='gamebutton mb5' style='padding: 6px 8px 6px 8px; font-size: 24px; line-height: 18px' onclick='pcs(event); open_guide(\"event-" +
			key +
			'","/docs/ref/event-' +
			key +
			"\")'>";
...
function render_guide(path, title, color) {
	...
	// hide_modal();
	var html =
		"<div style='/*background-color: black; border: 5px solid gray; padding: 4px; */ width: 360px; text-align: center'>";
	var index = 0;
	if (ref) {
		html += "<div style='margin-bottom: 4px; height: 56px'>";
		html +=
			"<div class='gamebutton' style='background-color: #E5E5E5; color: #010805; float: left; width: 145px' onclick='render_all_items()'><span style='color: #328355'>[I]</span> All Items</div>";
		html +=
	...
	if (title) {
		html +=
			"<div class='gamebutton' style='display: block; margin-bottom: 4px; background-color: #E5E5E5; color: #010805;'><span style='color: " +
			color +
			"'>[" +
			title[0] +
			"]</span> " +
	...
	if (more) {
		html +=
			"<div class='gamebutton' style='display: block; margin-bottom: 4px; color: #85C76B'>Guide is a Work in Progress!</div>";
	}
	html += "</div>";
...
function render_code_articles(path, title, color) {
	...
	// hide_modal();
	var html =
		"<div style='/*background-color: black; border: 5px solid gray; padding: 4px; */min-width: 320px; text-align: center'>";
	var index = 0;
	if (title) {
		html +=
			"<div class='gamebutton' style='display: block; margin-bottom: 4px; background-color: #E5E5E5; color: #010805;'><span style='color: " +
	...
	if (more) {
		html +=
			"<div class='gamebutton' style='display: block; margin-bottom: 4px; color: #85C76B'>More Articles Coming Soon!</div>";
	}
	html += "</div>";
...
function render_useful_links() {
	...
	html +=
		"<a class='gamebutton eexternal' style='display: block; margin-bottom: 4px; border-color: #4B95B2' target='_blank' href='https://jsconsole.com'>JSConsole.com</a>";
	html +=
		'<div class="mt4 blockbutton" style="text-align: left; margin-bottom: 4px">A very practical website to play with Javascript in a Console.</div>';
	html +=
	...
	html +=
		"<a class='gamebutton eexternal' style='display: block; margin-bottom: 4px; border-color: #4B95B2' target='_blank' href='https://github.com/kaansoral/adventureland'>Adventure Land's Github</a>";
	html +=
		'<div class="mt4 blockbutton" style="text-align: left; margin-bottom: 4px">#TODO: Create a gallery of player\'s Github repos</div>';
	html +=
		"<a class='gamebutton eexternal' style='display: block; margin-bottom: 4px; border-color: #4B95B2' target='_blank' href='https://discord.gg/X3QyCJd'>#code_beginner on Discord</a>";
...
function render_data_reference(path, id) {
	...
	if (is_object(data) && path.length < 2 && !in_arr(id, ["levels"])) {
		...
		object_sort(data).forEach(function (nd) {
			...
			html +=
				"<div style='display: block; margin-bottom: 4px; font-size: 32px' class='clickable' onclick='render_data_reference(" +
				JSON.stringify(path) +
				',"' +
				n +
				"\")'>" +
...
var codesearch_value = undefined;
function csearch_logic(place) {
	var value = "";
	var one = false;
	if (place == "ui") {
		value = $(".codesearch").val();
		if (value == codesearch_value) {
			return;
		}
		codesearch_value = value;
	} else {
		value = $(".csearchi").val();
		if (value == csearch_value) {
			return;
		}
		csearch_value = value;
	}
	if (value.length || place == "ui") {
		...
		G.docs.references.forEach(function (ref) {
			if (ref[2].search(value) !== -1 || (place == "ui" && !value)) {
				...
				if (!place) {
					...
					html +=
						"<div class='clickable' onclick='open_guide(\"" +
						ref[0] +
						'","/docs/guide/' +
						ref[0] +
						"\")'><span style='color:#69BE86'>[R]</span> " +
						ref[1] +
						"</div>";
				}
			}
			...
			if (article[2].search(value) !== -1 || (place == "ui" && !value)) {
				...
				if (!place) {
					...
					html +=
						...
						'","/docs/guide/' +
						article[0] +
						"\")'><span style='color:#E78E4E'>[A]</span> " +
						article[1] +
						"</div>";
				}
			}
		...
		G.docs.javascript.forEach(function (ref) {
			if (ref[1].search(value) !== -1 || (place == "ui" && !value)) {
				...
				if (!place) {
					...
					html +=
						"<div><a class='cancela eexternal' href='" +
						ref[2] +
						"' target='_blank'><span style='color:#A6B7C9'>[J]</span> " +
						ref[0] +
						"</a></div>";
				}
			}
			...
			if (name.search(value) !== -1 || value == "[G]" || (place == "ui" && !value)) {
				...
				if (!place) {
					...
					html +=
						...
						name +
						"\")'><span style='color:#8468BB'>[G]</span> G." +
						name +
						"</div>";
				}
			}
		...
		G.docs.functions.forEach(function (n) {
			if (n.search(value) == -1 && value != "[F]" && !(place == "ui" && !value)) {
				return;
			}
			if (in_arr(n, G.docs.documented)) {
				if (!place) {
					...
					html +=
						"<div class='clickable' onclick='api_call(\"load_article\",{name:\"" +
						n +
						"\",func:true});'><span style='color:#d6d135'>[F]</span> " +
						n +
						"</div>";
				}
			} else {
				if (!place) {
					...
					html +=
						"<div class='clickable' onclick='render_function_reference(\"" +
						n +
						"\")'><span style='color:#d6d135'>[F]</span> " +
						n +
						"</div>";
				}
			}
		...
		if (!one && !place) {
			html +=
				"<div class='gamebutton' style='display: block; margin-bottom: 4px'><span style='color:#575455'>[NONE FOUND]</span></div>";
		} else if (!one) {
...
function render_code_docs() {
	...
	html +=
		'<div class="mt4 blockbutton" style="text-align: left">Note: CODE Documentation is a work in progress. You can use Discord/#feedback for ideas/requests.</div>';
	html += "</div>";
	html += "</div>";
...
function render_wishlist(num, page) {
	...
	html +=
		"<div style='color: #f1c054; border-bottom: 2px dashed #C7CACA; margin-bottom: 3px; margin-left: 3px; margin-right: 3px' class='cbold'>Wishlist</div>";
	var items = [];
	var last = 0;
	for (var name in G.items) {
		if (!G.items[name].ignore) {
			items.push([name, G.items[name], G.items[name].g || 0]);
		}
	}
	...
	for (var i = 0; i < 4; i++) {
		...
		for (var j = 0; j < 5; j++) {
			if (i == 3 && j == 0 && page != 0) {
				html += item_container(
					{ skin: "left", onclick: "render_wishlist(" + num + "," + (page - 1) + ");" },
					{ q: page, left: true },
				);
			} else if (i == 3 && j == 4 && last < items.length - 1) {
				html += item_container(
				...
				var item = items[last - 1][1];
				var name = items[last - 1][0];
				html += item_container(
					{
						skin: item.skin,
...
function render_item(selector, args) {
	...
	var prop =
		args.prop ||
		calculate_item_properties(actual || {}, {
			def: item,
			class: window.character && character.ctype,
			map: window.character && character.map,
		});
	var grade = calculate_item_grade(item, actual || {});
	var html = "";
	if (!args.pure) {
		html +=
			"<div style='background-color: black; border: 5px solid gray; font-size: 24px; display: inline-block; padding: 20px; line-height: 24px; max-width: 240px; " +
	...
	if (!item) {
		...
		if (item.type == "tarot" && item.minor) {
			html +=
				"<img style='display: inline-block; margin: -8px 2px -6px -8px;' src='/images/cards/tarot/minor_arcana/tarot__" +
				item.minor +
				".png' />";
		} else if (item.type == "tarot") {
			html +=
				"<img style='display: inline-block; margin: -8px 2px -6px -8px;' src='/images/cards/tarot/major_arcana/tarot__" +
				item.major +
				".png' />";
		}
		...
		if (item.grade == "mid") {
			color = "blue";
		} // lol - must be old [13/04/20]
		if (actual && actual.p && G.titles[actual.p]) {
			item_name = G.titles[actual.p].title + " " + item_name;
		} else if (actual && actual.p) {
			item_name = actual.p.toTitleCase() + " " + item_name;
		}
		if (prop.level) {
			if (item.upgrade && prop.level == 12) {
				item_name += " +Z";
			} else if (item.upgrade && prop.level == 11) {
				item_name += " +Y";
			} else if (item.upgrade && prop.level == 10) {
				item_name += " +X";
			} else if (item.compound && prop.level == 7) {
				item_name += " +R";
			} else if (item.compound && prop.level == 6) {
				item_name += " +S";
			} else if (item.compound && prop.level == 5) {
				item_name += " +V";
			} else {
				item_name += " +" + prop.level;
			}
		}
		if (args.thumbnail) {
		...
		if (item.card) {
			...
			html +=
				"<div style='color: " +
				color +
				"; display: inline-block; border-bottom: 2px dashed gray; margin-bottom: 3px' class='cbold'>" +
				item_name +
				"</div><div></div>";
			html +=
				"<div style='color: " +
				color +
				"; display: inline-block; border-bottom: 2px dashed gray; margin-bottom: 3px; color: #AB7951' class='cbold'>" +
				item.card +
				"</div>";
			html += "</div>";
		} else if (!args.pure) {
		...
		if (prop.miss && item.type == "elixir") {
			html += bold_prop_line("Alcohol", prop.miss + "%", "#7CAAF6");
		}
		(item.gives || []).forEach(function (prop) {
			if (prop[0] == "hp" && prop[1] < 0) {
				html += bold_prop_line("HP", to_pretty_num(prop[1]), colors.hp);
			} else if (prop[0] == "hp") {
				html += bold_prop_line("HP", "+" + to_pretty_num(prop[1]), colors.hp);
			}
			if (prop[0] == "mp") {
				html += bold_prop_line("MP", "+" + to_pretty_num(prop[1]), colors.mp);
			}
		});
		if (item.debuff) {
			html += bold_prop_line("Effect", "Debuff", "#343792");
		}
		if (args.monster) {
			...
			html +=
				...
				bold_prop_line(
					"Show",
					"G.monsters.<span style='color:" + colors.property + ";'>" + args.monster + "</span>",
					colors.inspect,
				) +
				"</span>";
			html +=
		...
		if (prop.gold) {
			html += bold_prop_line("Gold", ((prop.gold > 0 && "+") || "") + prop.gold + "%", "gold");
		}
		if (prop.luck) {
			html += bold_prop_line("Luck", ((prop.luck > 0 && "+") || "") + prop.luck + "%", "#5DE376");
		}
		if (prop.xp) {
			html += bold_prop_line(
				"XP",
				((!args.monster && prop.xp > 0 && "+") || "") + prop.xp + ((!args.monster && "%") || ""),
				"#1E73DE",
			);
		}
		if (prop.lifesteal) {
			html += bold_prop_line("Lifesteal", to_pretty_float(prop.lifesteal) + "%", colors.lifesteal);
		}
		if (prop.manasteal) {
			html += bold_prop_line("Manasteal", to_pretty_float(prop.manasteal) + "%", colors.manasteal);
		}
		if (item.goldsteal) {
			html += bold_prop_line("Goldsteal", "Dynamic", "gold");
		}
		if (prop.evasion) {
			html += bold_prop_line("Evasion", to_pretty_float(prop.evasion) + "%", "#7AC0F5");
		}
		if (prop.avoidance) {
			html += bold_prop_line("Avoidance", to_pretty_float(prop.avoidance) + "%", "#7AC0F5");
		}
		if (prop.miss && item.type != "elixir") {
			html += bold_prop_line("Miss", prop.miss + "%", "#F36C6E");
		}
		if (prop.reflection) {
			html += bold_prop_line("Reflection", to_pretty_float(prop.reflection) + "%", "#B484E5");
		}
		if (prop.dreturn) {
			html += bold_prop_line("D.Return", to_pretty_float(prop.dreturn) + "%", "#E94959");
		}
		if (prop.crit) {
			html += bold_prop_line("Crit", to_pretty_float(prop.crit) + "%", "#E52967");
		}
		if (prop.critdamage) {
			html += bold_prop_line("Crit Damage", "+" + to_pretty_float(prop.critdamage) + "%", "#A8214E");
		}
		if (prop.attack) {
			html += bold_prop_line("Damage", prop.attack, colors.attack);
		}
		if (item.damage_type) {
			if (item.damage_type == "pure") {
				html += bold_prop_line("Type", "Pure", "#AA9B55");
			} else if (item.damage_type == "magical") {
				html += bold_prop_line("Type", "Magical", "#8998AA");
			} else if (item.damage_type == "physical") {
				html += bold_prop_line("Type", "Physical", "#93AB98");
			}
		}
		if (prop.range) {
			html += bold_prop_line("Range", ((!args.monster && "+") || "") + prop.range, colors.range);
		}
		if (prop.hp) {
			html += bold_prop_line("HP", prop.hp, colors.hp);
		}
		if (prop.str) {
			html += bold_prop_line("Strength", prop.str, colors.str);
		}
		if (prop["int"]) {
			html += bold_prop_line("Intelligence", prop["int"], colors["int"]);
		}
		if (prop.dex) {
			html += bold_prop_line("Dexterity", prop.dex, colors.dex);
		}
		if (prop.vit) {
			html += bold_prop_line("Vitality", prop.vit, colors.hp);
		}
		if (prop["for"]) {
			html += bold_prop_line("Fortitude", prop["for"], colors["for"]);
		}
		if (prop.mp) {
			html += bold_prop_line("MP", prop.mp, colors.mp);
		}
		if (prop.mp_cost > 0) {
			html += bold_prop_line("Attack MP Cost", "+" + prop.mp_cost, colors.mp);
		} else if (prop.mp_cost) {
			html += bold_prop_line("Attack MP Cost", prop.mp_cost, colors.mp);
		}
		if (prop.mp_reduction > 0) {
			html += bold_prop_line("Skill MP Reduction", "%" + prop.mp_reduction, colors.mp);
		} else if (prop.mp_reduction) {
			html += bold_prop_line("Skill MP Increase", "%" + -prop.mp_reduction, colors.mp);
		}
		if (prop.stat) {
			html += bold_prop_line("Stat", prop.stat);
		}
		if (prop.armor) {
			html += bold_prop_line("Armor", prop.armor, colors.armor);
		}
		if (prop.apiercing) {
			html += bold_prop_line("A.Piercing", prop.apiercing, colors.armor);
		}
		if (prop.rpiercing) {
			html += bold_prop_line("R.Piercing", prop.rpiercing, colors.resistance);
		}
		if (prop.resistance) {
			html += bold_prop_line("Resistance", prop.resistance, colors.resistance);
		}
		if (prop.pnresistance) {
			html += bold_prop_line("Poison Res.", prop.pnresistance, "#68B84B");
		}
		if (prop.firesistance) {
			html += bold_prop_line("Fire Res.", prop.firesistance, "#B42B22");
		}
		if (prop.fzresistance) {
			html += bold_prop_line("Freeze Res.", prop.fzresistance, "#69B1B6");
		}
		if (prop.phresistance) {
			html += bold_prop_line("Impact Res.", prop.phresistance, "#69B1B6");
		}
		if (prop.stresistance) {
			html += bold_prop_line("Status Res.", prop.stresistance, "#9FA7B6");
		}
		if (item.wspeed) {
			html += bold_prop_line("Speed", item.wspeed.toTitleCase(), "gray");
		}
		if (prop.speed) {
			html += bold_prop_line(
				(item.wtype && "Run Speed") || "Speed",
				((!args.monster && prop.speed > 0 && "+") || "") + prop.speed,
				colors.speed,
			);
		}
		if (prop.frequency || args.monster) {
			html += bold_prop_line("A.Speed", (prop.frequency || 1) * ((args.monster && 100) || 1), "#3BE681");
		}
		if (prop.output) {
			html += bold_prop_line("Damage Output", ((prop.output > 0 && "+") || "") + prop.output + "%", "#D93319");
		}
		if (prop.incdmgamp) {
			html += bold_prop_line("Incoming Damage", prop.incdmgamp + "%", "#D93319");
		}
		if (prop.stun) {
			html += bold_prop_line("Stun", prop.stun + "%", "#784224");
		}
		if (prop.explosion) {
			html += bold_prop_line("Explosion", prop.explosion + "%", "#782D33");
		}
		if (prop.blast) {
			html += bold_prop_line("Blast", prop.blast + "%", "#685079");
		}
		if (prop.breaks && prop.breaks > 0) {
			html += bold_prop_line("Breaks", to_pretty_float(prop.breaks) + "%", "#782D33");
		}
		if (prop.charisma) {
			html += bold_prop_line("Charisma", prop.charisma, "#4DB174");
		}
		if (prop.awesomeness) {
			html += bold_prop_line("Awesomeness", prop.awesomeness, "#FFDE2F");
		}
		if (prop.bling) {
			html += bold_prop_line("Bling", prop.bling, "#A4E6FF");
		}
		if (prop.cuteness) {
			html += bold_prop_line("Cuteness", prop.cuteness, "#FD82F0");
		}
		if (prop.intensity) {
			html += bold_prop_line("Intensity", prop.intensity, "#786D6A");
		}
		if (prop.courage) {
			html += bold_prop_line("Courage", prop.courage, "#9E1813");
		}
		if (prop.mcourage) {
			html += bold_prop_line("M.Courage", prop.mcourage, "#4628A0");
		}
		if (prop.pcourage) {
			html += bold_prop_line("P.Courage", prop.pcourage, "#D19D32");
		}
		if (grade == 1 && item.type != "booster") {
			html += bold_prop_line("Grade", "High", "#696354");
		}
		if (grade == 2 && item.type != "booster") {
			html += bold_prop_line("Grade", "Rare", "#6668AC");
		}
		if (grade == 3 && item.type != "booster") {
			html += bold_prop_line("Grade", "Legendary", "#39A868");
		}
		if (grade == 4 && item.type != "booster") {
			html += bold_prop_line("Grade", "Exalted", "#2875F9");
		} // gold: "#E5A818" purple: #8B3EE6" dark-pink: #e84664
		if (prop.poisonous) {
			html += "<div style='color: " + colors.poison + "'>Poisonous</div>";
		}
		if (prop.cooperative) {
			html += "<div style='color: #aeaeae'>Cooperative</div>";
		}
		if (prop.peaceful) {
			html += "<div style='color: #54B25F'>Peaceful</div>";
		}
		if (prop.supporter) {
			html += "<div style='color: #CA5931'>Supporter</div>";
		}
		if (prop.abilities) {
		...
		if (args.count) {
			html += bold_prop_line("Kills", to_pretty_num(args.count), "#7D0C15");
		}
		if (args.score) {
			html += bold_prop_line("Score", to_pretty_num(args.score), "#C38737");
		}
		if (args.mcount) {
			html += bold_prop_line(
				"Max Score",
				to_pretty_num(args.mcount) + " <span class='gray'>[" + args.mowner + "]</span>",
				"#DCC343",
			);
		}
		if (args.monster && G.base_gold) {
			for (mname in G.base_gold[args.monster]) {
				if (!G.maps[mname] || G.maps[mname].ignore) {
					continue;
				}
				html += bold_prop_line(
					"Base Gold",
					G.base_gold[args.monster][mname] + " <span class='gray'>(" + G.maps[mname].name + ")</span>",
					"gold",
				);
			}
		}
		...
		if (item.achievements) {
			...
			item.achievements.forEach(function (a) {
				var acolor = "white";
				if (max(args.score, args.mcount) >= a[0]) {
					acolor = "#2EA436";
				}
				var an = a[2];
				if (an == "frequency") {
			...
			if (args.count < 100 && 0) {
				...
				html +=
					"<div><span style='color:#DAE2DF'>100 kills are needed to discover the monster specific droprates.</span></div>";
			}
		}
		if (item.ability) {
			if (item.ability == "bash") {
				...
				html +=
					"<div style='color: #C3C3C3'>" +
					"Burns the opponent with a " +
					prop.attr0 +
					"% chance. Deals damage over time.</div>";
				...
				html +=
					...
					prop.attr0 +
					"% chance. Gain 240 Attack Speed for 10 seconds!</div>";
		...
		if (item.aura) {
			if (G.conditions[item.aura]) {
				...
				if (prop.attr0) {
					html += bold_prop_line("Amount", "%" + prop.attr0);
				}
			}
		}
		...
		if (item.multiplier && item.multiplier != 1) {
			html += bold_prop_line("Multiplier", item.multiplier, "gray");
		}
		if (prop.set) {
			html +=
				"<div><span style='color: #f1c054;'>Set</span>: <span class='clickable' onclick='stpr(event); render_set(\"" +
				prop.set +
				"\")'>" +
		...
		if (actual && actual.l) {
			if (actual.l == "s") {
				html += "<div class='ilsu'>Sealed</div>";
			} else if (actual.l == "u") {
				html += "<div class='iluu'>Unsealing</div>";
			} else {
				html += "<div style='color: #404141'>Locked</div>";
			}
		}

		if (actual && actual.acl) {
			html +=
				"<div style='color: #ADA68E'>Account Bound <span class='clickable' style='color: #C49F8D' onclick='show_alert(\"Unbind the item? [Soon]\")'>[X]</span></div>";
		}
		...
		if (!(args && args.prop)) {
			...
			if (item.e) {
				phrase = "Exchangeable";
			} else if (item.upgrade && (!actual || actual.level < 10)) {
				phrase = "Upgradeable";
			...
			if (item.type == "weapon" || offhand_types[item.type]) {
				...
				if (0 && parseInt(item.tier) < item.tier) {
					t +=
						...
						parseInt(item.tier) +
						"+ " +
						(weapon_types[item.wtype] || offhand_types[item.wtype] || (item.wtype || item.type).toTitleCase());
				} else {
					t +=
						"T" +
		...
		if (args.trade && actual && character.slots.helmet && character.slots.helmet.name.startsWith("ghat")) {
			...
			if ((actual.q || 1) > 1) {
				html +=
					"<div><span class='gray clickable' onclick='$(\".tradenum\").cfocus()'>Q:</span> <div class='inline-block tradenum' contenteditable=true>" +
					actual.q +
					"</div></div>";
			}
			html +=
				"<div><span class='clickable' style='color:#35AD4B' onclick='$(\".sellmins\").focus()'>MINUTES:</span> <div class='inline-block sellmins editable' contenteditable=true>20</div></div>";
			html +=
				"<div><span class='clickable' style='color:#EF5EA8' onclick='giveaway(\"" +
				args.slot +
				'","' +
				args.num +
				'",$(".tradenum").shtml(),$(".sellmins").shtml())\'>GIVEAWAY!</span></div>'; // style='color:#A99A5B'
			html += "</div>";
			...
			html +=
				...
				'","' +
				args.num +
				'",$(".sellprice").shtml(),$(".tradenum").shtml())\'>PUT UP FOR SALE</span></div>'; // style='color:#A99A5B'
			html += "</div>";
		}
		if (actual && actual.name == "cxjar") {
			...
			if (!actual.data) {
				...
				html +=
					"<div class='clickable' onclick='render_cx_info(\"" +
					actual.data +
					"\")'>" +
					cx_sprite(actual.data) +
					"</div>";
			}
		}
		if (actual && actual.name == "emotionjar") {
			if (!G.emotions[actual.data]) {
				...
				html +=
					...
					bold_prop_line(
						"Includes",
						"G.emotions.<span style='color:" + colors.property + ";'>" + actual.data + "</span>",
						colors.inspect,
					) +
					"</span></div>";
			}
		...
		if (in_arr(args.slot, trade_slots) && actual && actual.price && args.from_player && !actual.b && !actual.giveaway) {
			...
			if ((actual.q || 1) > 1) {
				html +=
					"<div><span class='gray clickable' onclick='$(\".tradenum\").cfocus()'>Q:</span> <div class='inline-block tradenum' contenteditable=true>1</div></div>";
			}
			html +=
				"<div style='color: gold'>" +
				to_pretty_num(actual.price) +
				" GOLD" +
				(((actual.q || 1) > 1 && " <span style='color: white'>[EACH]</span>") || "") +
				"</div>";
			html +=
		...
		if (in_arr(args.slot, trade_slots) && actual && args.from_player && actual.giveaway) {
			...
			html +=
				"<div><span class='clickable' style='color:#35AD4B' onclick='$(\".sellmins\").focus()'>MINUTES:</span> " +
				actual.giveaway +
				"</div>";
			html +=
		...
		if (in_arr(args.slot, trade_slots) && actual && actual.price && args.from_player && actual.b) {
			...
			html +=
				...
				to_pretty_num(actual.price) +
				" GOLD" +
				((q && " <span style='color: white'>[EACH]</span>") || "") +
				"</div>";
			html +=
		...
		if (args.secondhand) {
			...
			html +=
				"<div style='color: gold'>" +
				to_pretty_num(calculate_item_value(actual) * mult * (actual.q || 1)) +
				" GOLD</div>";
		...
		if (args.lostandfound) {
			trade_item = true;
			html +=
				"<div style='color: gold'>" + to_pretty_num(calculate_item_value(actual) * 4 * (actual.q || 1)) + " GOLD</div>";
			html +=
				"<div><span class='clickable' onclick='lostandfound_buy(\"" + (actual.rid || "") + "\")'>BUY</span></div>";
		}
		if (value) {
			var f = "buy_with_gold";
			if (item.days) {
			...
			if (cash && character && item.cash >= character.cash) {
				if (is_electron) {
					html +=
						"<div style='border-top: solid 2px gray; margin-bottom: 2px; margin-top: 3px; margin-left: -1px; margin-right: -1px'></div>";
					html +=
						"<div style='color: #C3C3C3'>You can find SHELLS from gems, monsters. In future, from achievements.</div>";
				} else {
					html +=
						"<div style='border-top: solid 2px gray; margin-bottom: 2px; margin-top: 3px; margin-left: -1px; margin-right: -1px'></div>";
					html +=
						"<div style='color: #C3C3C3'>You can find SHELLS from gems, monsters. In future, from achievements. For the time being, to receive SHELLS and support our game:</div>";
					html +=
						"<a href='https://adventure.land/shells' class='cancela' target='_blank'><span class='clickable' style='color: #EB8D3F'>BUY or EARN SHELLS</span></a> "; // onclick='shells_click(); $(this).parent().remove()'
				...
				if (item.s) {
					...
					html +=
						"<span class='gray clickable' onclick='$(\".buynum\").cfocus()'>Q:</span> <div class='inline-block buynum' contenteditable=true>" +
						q +
						"</div> <span class='gray'>|</span> ";
					html +=
						"<span class='clickable' onclick='" + f + '("' + name + '",parseInt($(".buynum").html()))\'>BUY</span> ';
					html += "</div>";
				} else {
		...
		if (args.token && args.key && args.key != args.token) {
			...
			var phrase = "TOKENS";
			var text = "#D3D5E0";
			if (args.token == "funtoken") {
				color = "#AA6AB3";
			} else if (args.token == "pvptoken") {
				text = "#CCAE08";
			} else if (args.token == "monstertoken") {
				text = "#6C531B";
			} else if (args.token == "friendtoken") {
				text = "#AA6AB3";
			}
		...
		if (args.sell && actual) {
			...
			if (item.s && actual.q) {
				...
				html += "<div style='margin-top: 5px'>";
				html +=
					"<span class='gray clickable' onclick='$(\".sellnum\").cfocus()'>Q:</span> <div class='inline-block sellnum' contenteditable=true>" +
					q +
					"</div> <span class='gray'>|</span> ";
				html +=
					...
					actual.name +
					'") sell(inum,parseInt($(".sellnum").html()))\'>SELL</span> ';
				html += "</div>";
			} else {
				html +=
		...
		if (in_arr(name, booster_items)) {
			...
			if (!args.sell) {
				html +=
					"<div class='clickable' onclick=\"btc(event); show_modal($('#boosterguide').html())\" style=\"color: #D86E89\">HOW TO USE</div>";
			}
		}
		if (!value && !args.sell && actual && !trade_item && !args.trade && !args.npc) {
			...
			if (item.type == "stand") {
				html +=
					"<div class='clickable' onclick='socket.emit(\"trade_history\",{}); $(this).parent().remove()' style=\"color: #44484F\">TRADE HISTORY</div>";
			}
			if (0 && item.type == "computer" && (actual.charges === undefined || actual.charges) && gameplay == "normal") {
				html +=
					'<div class=\'clickable\' onclick=\'socket.emit("unlock",{name:"code",num:"' +
					args.num +
					'"});\' style="color: #BA61A4">UNLOCK</div>';
			}
			if (item.type == "computer") {
				html +=
					"<div class='clickable' onclick='render_computer($(this).parent())' style=\"color: #32A3B0\">NETWORK</div>";
			}
			if (item.type == "stand" && !character.stand) {
				html +=
					"<div class='clickable' onclick='open_merchant(\"" +
					args.num +
					'"); $(this).parent().remove()\' style="color: #8E5E2C">OPEN</div>';
			}
			if (item.type == "stand" && character.stand) {
				html +=
					"<div class='clickable' onclick='close_merchant(); $(this).parent().remove()' style=\"color: #8E5E2C\">CLOSE</div>";
			}
			if (item.type == "elixir" && !args.from_player) {
				...
				html +=
					...
					args.num +
					'"}); push_deferred("equip"); $(this).parent().remove()\' style="color: #D86E89">' +
					phrase +
					"</div>";
			}
			if ((item.type == "licence" || item.type == "spawner") && !args.from_player) {
				html +=
					"<div class='clickable' onclick='socket.emit(\"equip\",{num:\"" +
					args.num +
					'"}); push_deferred("equip"); $(this).parent().remove()\' style="color: #574F58">USE</div>';
			}
			if (in_arr(actual.name, ["stoneofxp", "stoneofgold", "stoneofluck"])) {
				html +=
					"<div class='clickable' onclick='socket.emit(\"convert\",{num:\"" +
					args.num +
					'"});\' style="color: ' +
					colors.cash +
					'">CONVERT TO SHELLS</div>';
			}
			if (in_arr(actual.name, booster_items)) {
				if (actual.expires) {
					...
					html +=
						"<div class='clickable' onclick='activate(\"" +
						args.num +
						'","activate"); $(this).parent().remove()\' style="color: #438EE2">ACTIVATE</div>';
				}
			}
		}
		if (args.craft) {
			...
			var action = "CRAFT";
			var ecolor = "#419FBE";
			if (G.craft[name].quest) {
				(phrase = "Collect"), (action = "EXCHANGE"), (ecolor = "#4DC353");
			}
			...
			G.craft[name].items.forEach(function (item) {
				...
				html += item_container(
					{ skin: G.items[item[1]].skin, onclick: "render_item_by_name('" + item[1] + "')" },
					{ name: item[1], q: q, level: item[2] },
				);
				i += 1;
				if (!(i % 4)) {
					html += "<div></div>";
			...
			if (G.craft[name].cost) {
				html += bold_prop_line("Cost", to_pretty_num(G.craft[name].cost), "gold");
			}
			html +=
				"<div class='clickable' onclick='auto_craft(\"" +
				name +
				'")\' style="color: ' +
				ecolor +
				'">' +
		...
		if (args.dismantle) {
			...
			html +=
				"<div style='color: " +
				color +
				"; display: inline-block; border-bottom: 2px dashed gray; margin-bottom: 3px' class='cbold'>Dismantles-to</div>";
			html += "<div></div>";
			...
			if (G.dismantle[name].cost) {
				html += bold_prop_line("Cost", to_pretty_num(G.dismantle[name].cost), "gold");
			}
		}
		if (args.condition && args.condition.sn) {
			html += bold_prop_line("Server", args.condition.sn, "#BED4DE");
		}
		if (args.condition && args.condition.f) {
			html += bold_prop_line("From", args.condition.f, "#BED4DE");
		}
		if (args.condition && args.condition.c) {
			html += bold_prop_line("Count", args.condition.c + " left", "#891C13");
		}
...
function render_wishlist_item(name, num) {
	...
	var html = "";
	html +=
		"<div style='background-color: black; border: 5px solid gray; font-size: 24px; display: inline-block; padding: 20px; line-height: 24px; max-width: 240px; min-width:200px;' class='buyitem'>";
	html +=
		"<div style='margin-left:-2px; display:inline-block; vertical-align:middle'>" +
		item_container({ skin: def.skin, def: def }) +
		"</div>";
	html += "<div style='display:inline-block; vertical-align:top; margin-left: 4px'>";
	html +=
		"<div style='color: #f1c054; border-bottom: 2px dashed #C7CACA; margin-bottom: 3px; margin-left: 3px; margin-right: 3px; display: inline-block' class='cbold'>Wishlist</div>";
	html += "<div></div>";
	html +=
		"<div style='color: #E4E4E4; border-bottom: 2px dashed gray; margin-bottom: 3px; display: inline-block' class='cbold'>" +
		def.name +
		"</div>";
	html += "</div>";

	html +=
		"<div><span class='gray clickable' onclick='$(\".wnumq\").cfocus()'>Q:</span> <div class='inline-block wnumq' contenteditable=true>1</div></div>";
	html +=
...
function render_set(name) {
	var set = G.sets[name];
	var selector = last_selector;
	var html =
		"<div style='background-color: black; border: 5px solid gray; font-size: 24px; display: inline-block; padding: 20px; line-height: 24px; max-width: 280px;' class='buyitem'>";
	html +=
		"<div style='color: #f1c054; border-bottom: 2px dashed #C7CACA; margin-bottom: 3px' class='cbold'>" +
		set.name +
		"</div>";
	...
	[1, 2, 3, 4, 5, 6, 7, 8].forEach(function (num) {
		var rep = num;
		if (num != set.items.length) {
			rep = num + "+";
		}
		if (set[num] && Object.keys(set[num]).length) {
			html +=
				"<div><span style='color:#8A8D8F'>[" +
				rep +
				" Equipped]</span> " +
				render_item("html", { pure: true, item: set[num], prop: set[num] }) +
				"</div>";
		}
	});
	if (set.explanation) {
...
function render_condition(selector, name) {
	...
	if (target && target.s[name] && target.s[name].ms) {
...
function render_item_selector(selector, args) {
	...
	var items = [];
	var row = 0;
	var html = "<div style='border: 5px solid gray; height: 400px; overflow: scroll; background: black'>";
	for (var id in G.items) {
		if (!G.items[id].ignore) {
			items.push(G.items[id]);
		}
	}
	items.sort(function (a, b) {
		return b.g - a.g;
...
function on_rclick(current) {
	var $current = $(current);
	var inum = $current.data("inum");
	var snum = $current.data("snum");
	var sname = $current.data("sname");
	var on = $current.data("onrclick");
	if (on) {
		smart_eval(on);
		...
		if (topleft_npc == "items") {
			...
			var def = null;
			if (current) {
				def = G.items[current.name];
			}
			if (!def || character.q.exchange) {
				return;
			}
			if (def.quest && exchange_type != def.quest) {
				return;
			}
			if (def.e) {
				if (e_item !== null) {
					...
					item_container(
						{
							...
							cid: "uweapon",
							pui: true,
							skin: G.items[character.items[u_item].name].skin,
						},
						character.items[u_item],
					),
				);
				if (u_scroll !== null || u_offering !== null) {
			...
			if (def.compound && c_last < 3) {
				...
				if (c_last == 3 && c_scroll !== null) {
					compound(c_items[0], c_items[1], c_items[2], c_scroll, c_offering, null, true);
				}
			}
			if (def.type == "cscroll") {
				if (c_scroll !== null) {
					...
					item_container(
						{
							...
							cid: "cscroll",
							pui: true,
							skin: G.items[character.items[c_scroll].name].skin,
						},
						character.items[c_scroll],
					),
				);
				if (c_last == 3 && c_scroll !== null) {
					compound(c_items[0], c_items[1], c_items[2], c_scroll, c_offering, null, true);
			...
			if (0 && character && character.items[inum] && G.items[character.items[inum].name].type == "elixir") {
				return;
			}
...
function on_drop(event) {
	...
	while (target && target.parent() && target.attr("ondrop") == undefined) {
		target = target.parent();
	}
	var cnum = target.data("cnum");
	var slot = target.data("slot");
	var strnum = target.data("strnum");
	var trigrc = target.data("trigrc");
	var skid = target.data("skid"); // containers
	var inum = element.data("inum");
	var sname = element.data("sname");
	var snum = element.data("snum");
	var skname = element.data("skname"); // items + skills
	...
	if (inum !== undefined && skid !== undefined) {
		...
		if (skname == "eval") {
			keymap[skid] = { name: "eval", code: "add_log('Empty eval','gray')" };
		} else if (skname == "snippet") {
			keymap[skid] = { name: "snippet", code: "game_log('Empty snippet','gray')" };
			...
			while (change) {
				...
				for (var id in keymap) {
					if (keymap[id] && keymap[id].name && keymap[id].name == "throw" && keymap[id].num == num) {
						num++, (change = true);
					}
				}
			}
...
function item_container(item, actual) {
	...
	if (actual && actual.name) {
		def = G.items[actual.name] || G.items.placeholder_m;
	}
	if (actual && def) {
		if ((def.upgrade && actual.level > 8) || (def.compound && actual.level > 4)) {
			bcolor = xbcolor;
		}
		if (
			bcolor == "gray" &&
			(def.special || calculate_item_grade(actual) == 2 || calculate_item_value(actual) > 5000000)
		) {
			bcolor = xbcolor;
			pompous = true;
		}
	}
	if (def && actual && def.type == "booster" && actual.level) {
		bcolor = xbcolor;
	}
	...
	if (item.onclick) {
		if (item.draggable) {
			...
			if (item.onmousedown) {
				container_prop += ' onmousedown="' + item.onmousedown + '"';
			} // to handle middle clicks
		} else {
			container_prop += ' onmousedown="' + item.onclick + '" ontouchstart="' + item.onclick + '" class="clickable" ';
		}
	}

	// cls="rotate12";
	if (item.cnum != undefined) {
		cnum = "data-cnum='" + item.cnum + "' ";
	}
	if (item.trigrc != undefined) {
		cnum = "data-trigrc='1'";
	} // on_drop, just trigger on_rclick
	if (item.strnum != undefined) {
		cnum = "data-strnum='" + item.strnum + "' ";
	} // render_items_npc - slot
	if (item.slot != undefined) {
		cnum = "data-slot='" + item.slot + "' ";
	}
	if (item.skid != undefined) {
		cnum = "data-skid='" + item.skid + "' ";
	}
	if (item.cid) {
		container_prop += " id='" + item.cid + "' ";
	}

	if (!item.skin && item.loader) {
		xstyles = "overflow: hidden;";
	}
	...
	if (item.skin) {
		if (!G.positions[item.skin]) {
			item.skin = "placeholder";
		}
		var pack = G.imagesets[G.positions[item.skin][0] || "pack_20"];
		var x = G.positions[item.skin][1];
		var y = G.positions[item.skin][2];
		var scale = size / pack.size;
		if (actual && actual.level && actual.level > 7) {
			classes += " glow" + min(item.level, 10);
		}
		if (item.num != undefined) {
			rclick = "class='rclick" + classes + "' data-inum='" + item.num + "'";
		}
		if (item.snum != undefined) {
			rclick = "class='rclick" + classes + "' data-snum='" + item.snum + "'";
		} // render_items_npc - item
		if (item.sname != undefined) {
			rclick = "class='rclick" + classes + "' data-sname='" + item.sname + "'";
		}
		if (item.skname != undefined) {
			rclick = "class='rclick" + classes + "' data-skname='" + item.skname + "'";
		}
		if (item.on_rclick) {
			rclick = "class='rclick" + classes + "' data-onrclick=\"" + item.on_rclick + '"';
		}
		html +=
			"<div " +
			rclick +
			" style='background: black; position: absolute; bottom: -2px; left: -2px; border: 2px solid " +
			bcolor +
			";";
		html +=
			"padding:" +
		...
		if (actual && actual.name == "monsterbox") {
			...
			var yy = G.positions["egg2"][2];
			html +=
				"<div style='overflow: hidden; height: " +
				size / 2 +
				"px; width: " +
				size / 2 +
				"px; z-index: 1; position: absolute; top: 3px; left: 13px'>";
			html +=
				"<img style='width: " +
		...
		if (actual) {
			...
			if (actual.c) {
				...
				if (actual.b) {
					html += "<div class='iqui gray'>" + actual.q + "</div>";
				} else if (def && def.debuff) {
					html += "<div class='iqui iqdbf'>" + actual.q + "</div>";
				} else if (def && def.gives && def.gives[0] && def.gives[0][0] == "hp") {
					html += "<div class='iqui iqhp'>" + actual.q + "</div>";
				} else if (def && def.gives && def.gives[0] && def.gives[0][0] == "mp") {
					html += "<div class='iqui iqmp'>" + actual.q + "</div>";
				} else {
					html += "<div class='iqui'>" + actual.q + "</div>";
				}
			}
			if (actual.level) {
				...
				if (pompous && def.compound && clevel == 3) {
					clevel = 4;
				}
				if (pompous && def.upgrade && clevel == 7) {
					clevel = 8;
				}
				html +=
					"<div class='iuui " +
			...
			if (actual.s) {
				html += "<div class='iqui'>" + actual.s + "</div>";
			}
		}
		if ((item.slot && in_arr(item.slot, trade_slots)) || item.trade_for_ui) {
			if (actual && actual.giveaway) {
				html += "<div class='truui igu' style='border-color: " + bcolor + ";'>@</div>";
			} else if (actual && actual.b) {
				html += "<div class='truui ibu' style='border-color: " + bcolor + ";'>?</div>";
			} else {
				html += "<div class='truui itu' style='border-color: " + bcolor + ";'>$</div>";
			} //€
		} else if (actual && actual.l && !item.slot) {
			if (actual.l == "s") {
				html += "<div class='truui ilsu' style='border-color: " + bcolor + ";'>S</div>";
			} else if (actual.l == "u") {
				html += "<div class='truui iluu' style='border-color: " + bcolor + ";'>U</div>";
			} else {
				html += "<div class='truui ixu' style='border-color: " + bcolor + ";'>X</div>";
			}
		}
		if (actual && actual.v) {
			html += "<div class='trruui ivu' style='border-color: " + bcolor + "; line-height: 7px'><br />^</div>";
		} else if (actual && actual.m) {
			html += "<div class='trruui imu' style='border-color: " + bcolor + ";'>M</div>";
		}
		if (item.loader) {
			html +=
				"<div class='loader" +
				item.loader +
				"' style='position: absolute; bottom: 0px; right: 0px; width: 3px; height: 0px; background-color: yellow'></div>";
		}
		if (item.skid) {
			// Skill ID
			html +=
				...
				item.skid +
				"' style='position: absolute; bottom: 0px; right: 0px; width: 4px; height: 0px; background-color: yellow'></div>";
			html += "<div class='truui' style='border-color: gray; color: white'>" + item.skid + "</div>";
	...
	if (!item.skin && item.loader) {
		html +=
			"<div class='loader" +
			item.loader +
			"' style='position: absolute; bottom: 0px; left: 0px; width: 52px; height: 0px; background-color: yellow;'></div>";
	}

	if (!item.skin && item.level) {
		var level = item.level;
		var clevel = level;
		var def = G.items[item.iname];
		var prefix = "u";
		if (def && def.compound) {
			prefix = "c";
		}
...
function render_skillbar(empty) {
	...
	// $("#topmid").html("");
	var html =
		"<div style='background-color: black; border: 5px solid gray; padding: 2px; display: inline-block' class='enableclicks'>";
	var i = 0;
	skillbar.forEach(function (id) {
		var current = keymap[id];
		var skin = current;
		if (current) {
			if (current && current.skin) {
				skin = current.skin;
			} else if (current.type == "item" && G.items[current.name]) {
				skin = G.items[current.name].skin;
			} else if (G.skills[current.name || current]) {
				skin = G.skills[current.name || current].skin;
			}
			html += item_container(
				{ skid: id, skin: skin || "", draggable: false, droppable: true, onclick: "on_skill('" + id + "')" },
				current,
			);
		} else {
			html += item_container({ skid: id, draggable: false, droppable: true });
		}
		if (!(skillbar.length >= 8 && !(skillbar.length % 2) && !(i % 2))) {
			html += "<div></div>";
		}
		i++;
	});
...
function skill_click(slot) {
	if (skillsui && keymap[slot]) {
		render_skill("#skills-item", keymap[slot].name || keymap[slot], keymap[slot]);
	}
	if (G.skills[slot]) {
		render_skill("#skills-item", slot);
	}
}
...
function render_skills() {
	...
	var right_style = "text-align: right";
	var html =
		"<div id='skills-item' class='rendercontainer' style='display: inline-block; vertical-align: top; margin-right: 5px'></div>";
	html +=
		"<div style='background-color: black; border: 5px solid gray; padding: 2px; font-size: 24px; display: inline-block'>";
	html +=
	...
	var km2 = ["Q", "W", "E", "R", "X", "T", "B"];
	if (skills_page == "II") {
		(km1 = ["8", "9", "0", "G", "H", "J", "K"]), (km2 = ["SHIFT", "Z", "V", "M", "P", "D", "BACK"]);
	}
	if (skills_page == "U") {
		(km1 = ["ESC", "A", "C", "F", "I", "TAB", "ENTER"]), (km2 = ["UP", "LEFT", "DOWN", "RIGHT", ",", "S", "U"]);
	}
	html += "<div>";
	km1.forEach(function (N) {
		...
		if (current && current.skin) {
			skin = current.skin;
		} else if (current && current.type == "item" && G.items[current.name]) {
			skin = G.items[current.name].skin;
		} else if (current && G.skills[current.name || current]) {
			skin = G.skills[current.name || current].skin;
		}
	...
	object_sort(G.skills).forEach(function (io) {
		...
		if (skill.slot) {
			...
			skill.slot.forEach(function (p) {
				if (character.slots[p[0]] && character.slots[p[0]].name == p[1]) {
					found = true;
				}
			});
			if (!found) {
		...
		if (
			skill.type == "skill" &&
			(!skill["class"] || in_arr(character.ctype, skill["class"]) || character.role == "gm")
		) {
			s.push({ name: name });
		}
		if (
	...
	for (var i = 0; i < 10; i++) {
		...
		for (var j = 0; j < 7; j++) {
			if (slast < s.length) {
				html += item_container(
					{
						skin: G.skills[s[slast].name].skin,
						onclick: "skill_click('" + s[slast].name + "')",
						skname: s[slast].name,
					},
					s[slast],
				);
	...
	for (var i = 0; i < 10; i++) {
		...
		for (var j = 0; j < 7; j++) {
			if (alast < a.length) {
				html += item_container(
					{
						skin: G.skills[a[alast].name].skin,
						onclick: "skill_click('" + a[alast].name + "')",
						skname: a[alast].name,
					},
					a[alast],
				);
	...
	render_skillbar(1);
	$("body").append(
		"<div id='theskills' style='position: fixed; z-index: 310; bottom: 0px; right: 0px' class='disableclicks bpclicks'></div>",
	);
	$(".skillsui").show();
	$("#theskills").html(html);
	restart_skill_tints();
...
function render_all_skills_and_conditions() {
	...
	var html = "";
	html +=
		"<div style='background-color: black; border: 5px solid gray; padding: 14px; font-size: 24px; display: inline-block; max-width: 640px'>";
	// html+="<div style='padding: 10px; color: #CC863B; text-align: center'>Work in Progress</div>";
	["ranger", "rogue", "warrior", "mage", "priest", "paladin", "merchant"].forEach(function (ctype) {
...
function render_teleporter() {
	...
	for (var id in G.maps) {
		if (!G.maps[id].ignore && !G.maps[id].instance) {
			html +=
				"<div class='gamebutton' style='margin-left: 5px; margin-bottom: 5px' onclick='socket.emit(\"transport\",{to:\"" +
				id +
...
function render_travel(the_map) {
	...
	var places = false;
	if (!the_map) {
		(the_map = character["map"]), (places = true);
	}
	(G.maps[the_map].npcs || []).forEach(function (def) {
		...
		if (!in_arr(npc.role, ["citizen", "guard", "pvp_announcer"])) {
			if (!one) {
				one = true;
				html +=
					"<div class='gamebutton' onclick='stpr(event);' style='cursor:inherit !important'>NPCs in " +
					G.maps[the_map].name +
					"</div><div></div>";
			}
	...
	object_sort(G.maps, "random").forEach(function (e) {
		...
		cshuffle(map.monsters || []).forEach(function (pack) {
			if ((name != the_map && !pack.boundaries) || parsed[pack.type]) {
				return;
			}
			if (pack.boundaries) {
				cshuffle(pack.boundaries).forEach(function (b) {
					if (b[0] != the_map || parsed[pack.type]) {
						return;
					}
	...
	if (Object.keys(packs).length) {
		html +=
			"<div></div><div class='gamebutton' onclick='stpr(event);' style='cursor:inherit !important'>Monsters in " +
			G.maps[the_map].name +
			"</div><div></div>";
		html += "<div style='margin: 8px'>";
		object_sort(packs, "hpsort").forEach(function (e) {
			if ((G.monsters[e[0]].cute || G.monsters[e[0]].stationary) && !G.monsters[e[0]].achievements) {
				return;
			}
	...
	if (places) {
		html +=
			"<div></div><div class='gamebutton' onclick='stpr(event);' style='cursor:inherit !important'>Places</div><div></div>";
		object_sort(G.maps).forEach(function (io) {
			var id = io[0];
			if (
				!G.maps[id].ignore &&
				...
				html +=
					"<div class='gamebutton' style='margin: 4px' onclick='hide_modal(); code_travel(\"" +
					id +
					"\");'>" +
					G.maps[id].name +
					"</div>";
	...
	if (!$(".cxmodalteleporter").length) {
		show_modal(html, { wrap: false });
	} //true,styles:"background-color:#ABACB6",wwidth:420});
}
...
function render_gtravel() {
	...
	object_sort(G.maps).forEach(function (io) {
		...
		if (!G.maps[id].ignore && !G.maps[id].instance) {
			html +=
				"<div class='gamebutton' style='margin-left: 5px; margin-bottom: 5px' onclick='hide_modal(); " +
				f +
				'("' +
				id +
				"\");'>" +
...
function render_gmonsters(t) {
	...
	object_sort(G.monsters).forEach(function (io) {
		var id = io[0];
		html +=
			"<div class='gamebutton' style='margin-left: 5px; margin-bottom: 5px' onclick='hide_modal(); socket.emit(\"gm\",{action:\"mjump\",monster:\"" +
			id +
			"\"});'>" +
			G.monsters[id].name +
			"</div>";
...
function render_spawns(id) {
	...
	G.maps[id].spawns.forEach(function (io) {
		html +=
			"<div class='gamebutton' style='margin-left: 5px; margin-bottom: 5px' onclick='direct_travel(\"" +
			id +
			'","' +
			i +
			"\"); hide_modal()'>" +
...
function render_interaction(type, sub_type, args) {
	...
	var left = 0;
	var top = 0;
	var file = "/images/tiles/characters/npc1.png";
	var img_type = "normal";
	var pass = false;
	var html =
		"<div style='background-color: #E5E5E5; color: #010805; border: 5px solid gray; padding: 6px 12px 6px 12px; font-size: 30px; display: inline-block; max-width: 420px'>";
	...
	if (type.auto) {
		...
		in_arr(type, ["unlock_items2", "unlock_items3", "unlock_items4", "unlock_items5", "unlock_items6", "unlock_items7"])
	) {
		left = 3;
		top = 1;
		file = "/images/tiles/characters/npc4.png";
		if (type == "unlock_items2") {
			(top = 1), (left = 0);
		}
		if (type == "unlock_items3") {
			(top = 1), (left = 0);
		}
		if (type == "unlock_items4") {
			(top = 1), (left = 2);
		}
		if (type == "unlock_items5") {
			(top = 1), (left = 2);
		}
		if (type == "unlock_items6") {
			(top = 0), (left = 1);
		}
		if (type == "unlock_items7") {
			(top = 0), (left = 1);
		}
	...
	if (type.auto) {
		...
		if (type.button) {
			(interaction_onclick = type.onclick),
				(html +=
					"<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='interaction_onclick()'>" +
					type.button +
					"</div></span>");
		}
		if (type.button2) {
			(interaction_onclick2 = type.onclick2),
				(html +=
					"<span style='float: right; margin-top: 5px; margin-right: 5px'><div class='slimbutton' onclick='interaction_onclick2()'>" +
					type.button2 +
					"</div></span>");
		}
		...
		html +=
			"<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='render_exchange_shrine(\"seashell\")'>I HAVE 20!</div></span>";
	} else if (type == "buyshells") {
		html +=
			"Yo dawg, I can hook you up with some shells If you want. I get these directly from Wizard in bulk so they are legit.";
		html +=
			"<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='render_shells_buyer()'>HMM, SURE...</div></span>";
	} else if (type == "noshells") {
		html +=
			"Ugh, maybe go farm more gold. It's not like they grow on trees. Just kill some puny monsters, you'll have plenty!";
		...
		html +=
			"<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='render_teleporter()'>TELEPORT</div></span>";
	} else if (type == "seashell_success") {
		if (Math.random() < 0.001) {
			html += "Awww. Ty. Ty. Ty. Xoxo.";
		} else {
			html += "How kind of you! Please accept this small gift in return.";
		}
		d_text("+1", get_npc("fisherman"), { color: "#DFE9D9" });
	} else if (type == "subscribe") {
		html += "It's that time of the day! Are you in?!";
		html +=
			"<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='socket.emit(\"signup\")'>SIGN ME UP!</div></span>";
	} else if (type == "tavern") {
		html +=
		...
		html +=
			"I can craft or dismantle items for you. Price differs from item to item. Check out my recipes if you are interested!";
		html +=
		...
		html +=
			"<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='render_exchange_shrine(\"candycane\")'>I HAVE ONE!</div></span>";
	} else if (type == "standmerchant") {
		html += "Anyone can become a merchant and start trading. You only need a merchant stand to display your items on!";
		html +=
			"<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='render_merchant(get_npc(\"standmerchant\"))'>LET ME BUY ONE!</div></span>";
		...
		html +=
			"Ewww. Ewww. Ewww. These wretched things ate my earrings. Kill them, kill them all. Bring my earrings back!";
		html +=
			"<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='render_exchange_shrine(\"lostearring\")'>AS YOU WISH</div></span>";
		...
		html +=
			"You know, It gets boring in here sometimes ... I'm looking for some excitement. Uhm, Do you have a Mistletoe?";
		html +=
			"<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='render_exchange_shrine(\"mistletoe\")'>OH MY, I DO!</div></span>";
		...
		html +=
			"Hmm. We should decorate these trees. I need some Ornaments tho. If you happen to collect " +
			G.items.ornament.e +
			" of them, let me know!";
		html +=
			"<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='render_exchange_shrine(\"ornament\")'>YOU GOT IT!</div></span>";
		...
		html +=
			"<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='render_exchange_shrine(\"gemfragment\")'>I GOT " +
			G.items.gemfragment.e +
			"!</div></span>";
		...
		html +=
			"<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='render_exchange_shrine(\"leather\")'>I HAVE " +
			G.items.leather.e +
			"!</div></span>";
		...
		html +=
			"<span style='float: right; margin-top: 5px'><div class='slimbutton' onclick='pcs(); socket.emit(\"leave\"); push_deferred(\"leave\")'>LEAVE</div></span>";
	} else if (type == "blocker" || type == "guard") {
		var roll = Math.random();
		if (roll < 0.5) {
			html += "Hmm. hmm. hmm. Can't let you pass. Check again later tho!";
		} else {
			html += "There's some work going on inside. Maybe check back later!";
		}
	} else if (type == "lottery") {
		html += "Hi Dear! The lottery tickets for this week haven't arrived yet. Apologies :)";
	} else if (type.startsWith("unlock_")) {
		var pack = args.pack;
		var gold = bank_packs[pack][1];
		var shells = bank_packs[pack][2];
		if (is_electron) {
			html +=
				"Hello! You don't seem to have an account open with me. Would you like to open one? It costs " +
				to_pretty_num(gold) +
				" Gold. We hold onto your items forever.";
			html +=
...
function load_nearby(fallback) {
	...
	if (someone) {
		...
		l.forEach(function (player) {
			...
			if (!player.afk) {
				afk = "<span style='color: #34bf15'>ACTIVE</span>";
			} else if (player.afk == "code") {
				afk = "<span style='color: gray'>CODE</span>";
			} else if (player.afk == "bot") {
				afk = "<span style='color: gray'>BOT</span>";
			}
			//if(!player.party) actions+=" <span style='color: #2799DD'>PM</span>";
			if (player.owner && in_arr(player.owner, friends)) {
				actions += " <span style='color: #EC82C4'>FRIENDS!</span>";
			} else {
				actions +=
					" <span style='color: #2799DD' class='clickable' onclick='socket.emit(\"friend\",{event:\"request\",name:\"" +
					player.name +
					'"}); push_deferred("friend")\'>+FRIEND</span>';
			}
			if (!actions) {
				actions = "None";
			}
...
function load_friends(info) {
	if (info) {
		...
		} else if (!info.chars.length) {
			html += "<div style='margin-top: 8px'>No one online.</div>";
		} else {
			html += "<table style='margin: 5px; text-align: center'>";
			html +=
...
function load_server_list(info) {
	if (info) {
		...
		html += render_com_buttons();
		if (!info.length) {
			html += "<div style='margin-top: 8px'>No one discoverable.</div>";
		} else {
			html += "<table style='margin: 5px; text-align: center'>";
			html +=
			...
			if (is_pvp) {
				html += "<th style='width: 120px'>Kills</th>";
			}
			html += "</tr>";
			...
			info.forEach(function (player) {
				...
				if (!player.party && player.name != character.name && player.name != "Hidden") {
					party =
						...
						player.name +
						'"}); push_deferred("party")\'>Invite</span>';
				} else if (player.name == "Hidden") {
					party = "<span style='color: #999999'>None</span>";
				} else if (!player.party) {
					party = "<span style='color: #999999'>You</span>";
				} else {
					party =
				...
				if (player.name != character.name && player.name != "Hidden") {
					party +=
						" <span style='color: #A255BA' class='clickable' onclick='hide_modal(); cpm_window(\"" +
						player.name +
						"\");'>PM</span>";
				}
				if (name == "Hidden") {
					name = "<span style='color:gray'>Hidden</span>";
				}
				html +=
					"<tr><td>" +
					name +
					...
					"</td>";
				if (is_pvp) {
					html += "<td>" + to_pretty_num(player.kills) + "</td>";
				}
				html += "</tr>";
			});
			html += "</table>";
...
function load_merchants(info) {
	if (info) {
		if (friends_inside != "servers") {
			return;
		}
		var html = "";
		if (!info.chars.length) {
			html = "<div style='margin-top: 8px'>No merchants with a stand online.</div>";
		} else {
...
function load_servers_list(info) {
	...
	html +=
		"<tr style='color: gray; text-decoration: underline'><th style='width: 120px'>Name</th><th style='width: 40px'>#</th><th style='width: 60px'>Action</th>";
	html += "</tr>";
	html +=
	...
	X.servers.forEach(function (server) {
		...
		if (!(server_region == server.region && server.name == server_identifier)) {
			action =
				...
				"/" +
				server.name +
				"/' class='cancela' style='color: #4C9BC8'>Switch</span>";
		}
		html +=
			"<tr><td>" +
			server_regions[server.region] +
			" " +
			server.name +
			"</td><td><span style='color:" +
			colors.server_success +
			"'>" +
			server.players +
			"</span></td><td>" +
			action +
...
function load_character_list() {
	...
	X.characters.forEach(function (player) {
		...
		var online = false;
		if (player.online) {
			(afk = "<span style='color: #34bf15'>ONLINE</span>"), (link = "<span class='gray'>Deployed</span>");
		} else {
			(afk = "<span style='color: gray'>OFFLINE</span>"),
				(link =
					...
					"/" +
					server_identifier +
					"/' target='_blank' class='cancela' style='color: #4C9BC8'>Deploy</span>");
		}
		if (player.name != character.name && player.name != "Hidden") {
			party +=
				" <span style='color: #A255BA' class='clickable' onclick='hide_modal(); cpm_window(\"" +
...
function render_mail(id) {
	...
	var html = "<div style='font-size: 24px'>";
	html +=
		"<div class='clickable' style='color: #DB090A; float: right' onclick='show_delete_mail(\"" +
		id +
		"\");'>Delete</div>";
	...
	if (mail.item) {
		...
		if (!mail.taken) {
			take =
				" <span class='clickable takeitem' style='color: #6DAD47' onclick='parent.socket.emit(\"mail_take_item\",{id:\"" +
				id +
				"\"})'> TAKE </span>";
		}
		html +=
			"<div class='mailsubject'><span style='color: gray'>Item:</span> " +
...
function load_mail(info) {
	var html = "";
	html +=
		"<div class='gamebutton gamebutton-small' onclick='pcs(); show_mail_modal()'>Send Mail <span style='color: gray'>Cost:</span> <span style='color: gold'>48,000</span></div>";
	if (info) {
		if (friends_inside != "mail") {
			return;
		}
		if (info.cursored) {
			html += "<div style='margin-top: 8px; margin-bottom: 8px'>(Previous emails not shown)</div>";
		}
		if (!info.mail.length && info.cursored) {
			html += "<div style='margin-top: 8px'>End of mail.</div>";
		} else if (!info.mail.length) {
			html += "<div style='margin-top: 8px'>No mail yet.</div>";
		} else {
			html += "<div style='text-align: left; margin: 20px'>";
			window.mail = {};
			...
			html += "</div>";
			if (!(info.cursor && info.more)) {
				html += "<div style='margin-top: 8px'>End of mail.</div>";
			} else {
				html +=
					"<div style='margin-top: 8px' class='clickable' onclick='api_call(\"pull_mail\",{cursor:\"" +
					info.cursor +
					"\"});'>Load More</div>";
			}
		}
...
function load_chat(info, type) {
	...
	html +=
		...
		"' style='color: #CDD584' onclick='pcs(); load_chat(null,\"global\")'>Global</div> ";
	html +=
		"<div class='gamebutton gamebutton-small " +
		((type == "party" && "gamebutton-active") || "") +
		"' style='color: #3B8ED2' onclick='pcs(); load_chat(null,\"party\")'>Party</div> ";
	html +=
		"<div class='gamebutton gamebutton-small " +
		((type == "private" && "gamebutton-active") || "") +
		"' style='color: #D0598B' onclick='pcs(); load_chat(null,\"private\")'>Private</div> ";
	html +=
		"<div class='gamebutton gamebutton-small " +
		((type == "all" && "gamebutton-active") || "") +
		"' style='color: #717171' onclick='pcs(); load_chat(null)'>All Incoming</div> ";
	X.servers.forEach(function (server) {
		html +=
			"<div class='gamebutton gamebutton-small " +
			((type == server.region + server.name && "gamebutton-active") || "") +
	...
	if (info) {
		if (friends_inside != "chat") {
			return;
		}
		if (info.cursored) {
			html += "<div style='margin-top: 8px; margin-bottom: 8px'>(Previous messages not shown)</div>";
		}
		if (!info.messages.length && info.cursored) {
			html += "<div style='margin-top: 8px'>End of chat.</div>";
		} else if (!info.messages.length) {
			html += "<div style='margin-top: 8px'>No messages yet.</div>";
		} else {
			html += "<div style='text-align: left; margin: 20px'>";
			window.messages = {};
			info.messages.forEach(function (message) {
				...
				if (message.type == "private") {
					(color = "#CD7879"), (server = " <span style='color: #505259'>[" + message.to[0] + "]</span>");
				}
				if (message.type == "party") {
					color = "#5B8DB0";
				}
				if (message.type == "ambient" || type == "global") {
					server = " <span style='color: #505259'>[" + message.server + "]</span>";
				}
				html +=
					"<div title='" +
					message.date +
					"'>" +
					html_escape(message.fro) +
					":" +
					" <span style='color: " +
					color +
					"'>" +
			...
			html += "</div>";
			if (!(info.cursor && info.more)) {
				html += "<div style='margin-top: 8px'>End of messages.</div>";
			} else {
				html +=
					"<div style='margin-top: 8px' class='clickable' onclick='api_call(\"pull_messages\",{cursor:\"" +
					info.cursor +
					"\"});'>Load More</div>";
			}
		}
...
function load_pvp_list(list) {
	...
	if (!pwn) {
		html += "<div>Noone pwned Anyone</div>";
	}
	html += "</div>";
...
function load_coming_soon(num) {
	...
	if (num == 1) {
		...
		$(".fguild").addClass("active2"), (message = "Coming Soon!");
	} else if (num == 3) {
		$(".fleaders").addClass("active2"),
			(message = "Planned, along with achievements, character statistics, weekly, monthly leaderboards");
	} else if (num == 4) {
		$(".fmail").addClass("active2"), (message = "Coming Soon!");
	}
...
function render_com() {
	...
	html +=
		" <div class='gamebutton fcharacters' onclick='load_character_list();'>Characters [<span class='ccount'>" +
		c_count +
		"</span>/4]</div>";
	html += " <div class='gamebutton fchat' onclick='load_chat();'>Chat</div>";
	html +=
		" <div class='gamebutton fmail' onclick='load_mail();'>Mail [<span class='mcount'>" +
		((window.X && X.unread) || 0) +
		"</span>]</div>";
	...
	html +=
		"<div class='friendslist mt5' style='height: 400px; border: 5px solid gray; font-size: 24px; overflow: scroll; padding: 6px'></div>";
	html +=
		"<div style='font-size: 16px; margin-top: 5px; color: gray; text-align: center'>NOTE: The Communicator is an evolving protoype</div>";
...
function precompute_image_positions() {
	...
	for (var name in G.sprites) {
		...
		var col_num = 3;
		var s_type = "full";
		if (in_arr(s_def.type, ["animation"])) {
			(row_num = 1), (s_type = s_def.type);
		}
		if (in_arr(s_def.type, ["tail"])) {
			(col_num = 4), (s_type = s_def.type);
		}
		if (in_arr(s_def.type, ["v_animation", "head", "hair", "hat", "s_wings", "face", "makeup", "beard"])) {
			(col_num = 1), (s_type = s_def.type);
		}
		if (in_arr(s_def.type, ["a_makeup", "a_hat"])) {
			(col_num = 3), (s_type = s_def.type);
		}
		if (in_arr(s_def.type, ["wings", "body", "armor", "skin", "character"])) {
			s_type = s_def.type;
		}
		if (in_arr(s_def.type, ["emblem", "gravestone"])) {
			(row_num = 1), (col_num = 1), (s_type = s_def.type);
		}
		var matrix = s_def.matrix;
		var width =
			(G.images[s_def.file.split("?")[0]] && G.images[s_def.file.split("?")[0]].width) ||
			s_def.width ||
			(window.C && C[s_def.file] && C[s_def.file].width) ||
			312;
		var height =
			(G.images[s_def.file.split("?")[0]] && G.images[s_def.file.split("?")[0]].height) ||
			s_def.height ||
			(window.C && C[s_def.file] && C[s_def.file].height) ||
			288;
		...
		for (var i = 0; i < matrix.length; i++) {
			for (var j = 0; j < matrix[i].length; j++) {
				...
				IID[name] = [
					width,
					height,
					(j * width) / s_def.columns,
					(i * height) / s_def.rows,
					width / (s_def.columns * col_num),
					height / (s_def.rows * row_num),
					col_num,
					s_def.file,
					s_type,
				];
				...
				if (G.cosmetics.prop[name] && G.cosmetics.prop[name].includes("slender")) {
					SSU[name] += "slender";
				}
	...
	if (0) {
		for (var name in IID) {
			for (var j = 0; j < IID[name].length - 1; j++) {
				IID[name][j] *= 1.5;
			}
		}
	}
...
function sprite_image(name, args) {
	try {
		...
		if (G.dimensions[name]) {
			(width = G.dimensions[name][0]), (height = G.dimensions[name][1]);
		}
		if (args.cwidth) {
			l_disp = (args.cwidth - width * scale) / 2;
		}
...
function sprite(name, args) {
	try {
		...
		if (is_string(args.cx)) {
			args.cx = args.cx.split(",");
		}
		if (!args.cx) {
			args.cx = {};
		}
		...
		if (G.monsters[name] && G.monsters[name].size) {
			args.scale -= 1 - G.monsters[name].size;
		}
		precompute_image_positions();
		if (!IID[name] && G.items[name]) {
			return item_container({ skin: name, bcolor: "black" });
		}
		if (G.monsters[name] && G.monsters[name].skin) {
			name = G.monsters[name].skin;
		}
		...
		if (args.full) {
			if (G.dimensions[name]) {
				(args.width = (G.dimensions[name][0] + 4) * args.scale),
					(args.height = (G.dimensions[name][1] + 5) * args.scale);
			} else {
				(args.width = IID[name][4] * args.scale), (args.height = IID[name][5] * args.scale);
			}
		}
		if (G.dimensions[name] && G.dimensions[name][3]) {
			args.rx_disp = -G.dimensions[name][3] * args.scale;
		}
		var html =
			"<div style='height: " +
			args.height +
		...
		var opacity = 1;
		var j = args.j || 0;
		if (body_type != "full" && !args.cx.head) {
			args.cx.head = "makeup117";
		}
		if (body_type != "character" && !args.cx.head) {
			args.cx.head = "makeup117";
		}
		...
		for (var place in args.cx) {
			...
			if (!cid || !cid.length || !IID[cid]) {
				continue;
			}
			if (place == "head") {
				if (body_type != "full" && body_type != "character") {
					cx.head = cid;
				}
			} else if (place == "hair") {
				if (body_type != "full" && body_type != "character" && !cx_prop.no_hair) {
					cx.hair = cid;
				}
			} else if (place == "upper") {
				if (body_type != "full" && T[cid] == "armor" && SSU[cid] == SSU[name]) {
					cx.upper = cid;
				}
			} else {
				cx[place] = cid;
		...
		if (cx.head && hidden_head) {
			html += sprite_image(cx.head, { p: head_y, cwidth: args.width, scale: args.scale, opacity: opacity, j: j });
		}
		if (cx.back && j != 3) {
			html += sprite_image(cx.back, { cwidth: args.width, scale: args.scale, opacity: opacity, j: j });
		}
		if (skin) {
			html += sprite_image(skin, {
				cwidth: args.width,
				scale: args.scale,
				opacity: opacity,
				j: j,
		...
		if (!(IID[name][4] % 2)) {
			html += sprite_image(name, {
				cwidth: args.width,
				scale: args.scale,
				opacity: opacity,
				j: j,
				x_disp: args.rx_disp + -0.5,
			});
		} // old 26px width frame
		else {
			html += sprite_image(name, {
				cwidth: args.width,
				scale: args.scale,
		...
		if (cx.upper) {
			html += sprite_image(cx.upper, { cwidth: args.width, scale: args.scale, opacity: opacity, j: j, rheight: 8 });
		}
		if (cx.head && !hidden_head) {
			html += sprite_image(cx.head, { p: head_y, cwidth: args.width, scale: args.scale, opacity: opacity, j: j });
		}
		if (j != 3 && cx.makeup) {
			html += sprite_image(cx.makeup, {
				p: head_y + G.cosmetics.default_makeup_position,
				cwidth: args.width,
				scale: args.scale,
				opacity: opacity,
				j: j,
		...
		if (cx.hair) {
			html += sprite_image(cx.hair, { p: hair_y, cwidth: args.width, scale: args.scale, opacity: opacity, j: j });
		}
		if (j != 3 && cx.face) {
			html += sprite_image(cx.face, {
				p: head_y + G.cosmetics.default_face_position,
				cwidth: args.width,
				scale: args.scale,
				opacity: opacity,
				j: j,
		...
		if (j != 3 && cx.chin) {
			html += sprite_image(cx.chin, {
				p: head_y + G.cosmetics.default_beard_position,
				cwidth: args.width,
				scale: args.scale,
				opacity: opacity,
				j: j,
		...
		if (cx.tail) {
			html += sprite_image(cx.tail, { p: 0, cwidth: args.width, scale: args.scale, opacity: opacity, j: j });
		}
		if (cx.hat) {
			html += sprite_image(cx.hat, { p: hat_y, cwidth: args.width, scale: args.scale, opacity: opacity, j: j });
		}
		if (cx.back && j == 3) {
			html += sprite_image(cx.back, { cwidth: args.width, scale: args.scale, opacity: opacity, j: j });
		}
		if (rip) {
			html += sprite_image(rip, { cwidth: args.width, scale: args.scale, j: j });
		}
		html += "</div>";
		return html;
	} catch (e) {
...
function cx_sprite(name, args) {
	...
	function render_cosmetic(slot, rargs) {
		...
		if (!rargs.color && (cx[slot] || slot == "skin")) {
			rargs.color = "#17B8E3";
		} else if (!rargs.color) {
			rargs.color = "#5DBA50";
		}
		...
		if (args.labels) {
			...
			var icx = false;
			for (var dname in G.drops) {
				if (!(G.items[dname] && G.items[dname].e)) {
					continue;
				}
			...
			for (var mname in G.monsters) {
				if (!(G.drops && G.drops.monsters && G.drops.monsters[mname])) {
					continue;
				}
			...
			for (var cname in G.classes) {
				if ((G.classes[cname].xcx || []).includes(name)) {
					ccx = true;
				}
			}
			for (var iname in G.items) {
				if ((G.items[iname].xcx || []).includes(name)) {
					icx = true;
				}
			}
			if (drop) {
				labels += "<span style='color:#3696CE'>E</span>";
			}
			if (mdrop) {
				labels += "<span style='color:#896CF7'>M</span>";
			}
			if (ccx) {
				labels += "<span style='color:#61C14A'>C</span>";
			}
			if (icx) {
				labels += "<span style='color:#BDB094'>I</span>";
			}
			if (!labels) {
				labels += "<span style='color:gray'>X</span>";
			}
		}
		html +=
			...
			"px; margin-right: " +
			(args.mright || 0) +
			"px; vertical-align: middle; margin-bottom: 4px; vertical-align: bottom'>";
		html +=
		...
		html +=
			...
			sprite(rargs.skin, {
				cx: rargs.cx,
				scale: rargs.scale,
				height: rargs.height * rargs.scale + 20 * rargs.scale,
				j: rargs.j,
				width: width,
				rip: rargs.rip,
			}) +
	...
	if (["hair", "face", "head", "makeup", "a_makeup"].includes(T[name])) {
		...
		render_cosmetic(T[name], {
			skin: (T[name] == "head" && "nothing") || "mabw",
			top: 50 + ((T[name] == "head" && -6) || 0),
			scale: 3,
			cx: cx,
			height: 16,
			aheight: 4,
		...
		var cx = { head: "bwhead" };
		cx[cxtype_to_slot[T[name]]] = name;
		render_cosmetic((T[name] == "character" && "char") || T[name], {
			skin: name,
			top: -7,
			scale: 2,
			cx: cx,
...
function cx_move(x, y) {
	...
	if (!last_cx_d[0] && !last_cx_d[1]) {
		log += " default position";
	}
	if (last_cx_d[0] < 0) {
		log += " [move left " + -last_cx_d[0] + "px]";
	}
	if (last_cx_d[0] > 0) {
		log += " [move right " + last_cx_d[0] + "px]";
	}
	if (last_cx_d[1] < 0) {
		log += " [move up " + -last_cx_d[1] + "px]";
	}
	if (last_cx_d[1] > 0) {
		log += " [move down " + last_cx_d[1] + "px]";
	}
	add_log(log, "#BD6BB6");
}
...
function insert_cx_tuners() {
	var html =
		...
		"px; top: " +
		$(window).height() / 2 +
		"px; width: 0px; height: 0px; position: fixed; z-index: 100; overflow: visible'>";
	html +=
		"<div style='position: absolute; top: -130px; left: -35px; text-align: center; width: 50px;' class='gamebutton gamebutton-small' onclick='cx_move(0,-1)'>UP</div>";
	html +=
		"<div style='position: absolute; top: 20px; left: -35px; text-align: center; width: 50px;' class='gamebutton gamebutton-small'  onclick='cx_move(0,1)'>DOWN</div>";
	html +=
		"<div style='position: absolute; top: -20px; left: -110px; text-align: center; width: 50px;' class='gamebutton gamebutton-small' onclick='cx_move(-1,0)'>LEFT</div>";
	html +=
		"<div style='position: absolute; top: -20px; left: 40px; text-align: center; width: 50px;' class='gamebutton gamebutton-small' onclick='cx_move(1,0)'>RIGHT</div>";
	html += "</div>";
	$("body").append(html);
}
...
function render_cgallery(skin, cx, slot) {
	...
	var old = null;
	var j = 0;
	var cx_count = all_cx(character, 1);
	var cx_map = map_cx(character);
	var me = false;
	if ((xtarget || target).me) {
		me = true;
	}
	var bg = "#504254";
	function render_cosmetic(reset) {
		...
		if (
			(!me && cx_map[s[0]] && cx_count[cx_map[s[0]]] > 0) ||
			(me && cx_count[s[0]] !== undefined) ||
			(me && character.role === "cx") ||
			reset
		) {
			found = true;
		} else {
		...
		if (mode.cosmetics) {
			html += "<div style='font-size: 16px; text-align: center'>" + s[0] + "</div>";
		}
		html +=
			"<div style='background-color: " +
	...
	if (slot == "back") {
		types.push("tail");
	} // synced with server.js/'cx'
	if (slot == "face") {
		types.push("makeup"), types.push("a_makeup");
	}

	if (slot == "tail" || slot == "back") {
		j = 3;
	}
	object_sort(T).forEach(function (x) {
		s = x;
		if (in_arr(s[1], types)) {
			if (T[skin] == "full" && slot == "head") {
				return;
			}
			if (slot == "upper" && (T[skin] == "full" || SSU[s[0]] != SSU[skin] || skin == s[0] || T[s[0]] != "armor")) {
				return;
			}
			if (cxtype_to_slot[s[1]] == "skin") {
				skin = s[0];
			} else if (slot == "upper") {
				cx.upper = s[0];
			} else {
	...
	if (a.length || b.length) {
		...
		show_modal(
			"<div style='background-color: " +
				bg +
				"; border: 2px solid gray; vertical-align: top; overflow: hidden; text-align: center; padding: 12px; font-size: 32px'>No Alternatives</div>",
			{ wrap: false, styles: "max-width:400px" },
		);
	}
}
...
function render_cosmetics(player, args) {
	...
	if (args.toggle && $(".cccx").html()) {
		return $("#topleftcornerdialog").html("");
	}
	function render_cosmetic(slot, rargs) {
		if (!rargs) {
			rargs = {};
		}
		if (!rargs.color && (player.cx[slot] || slot == "skin")) {
			rargs.color = "#17B8E3";
		} else if (!rargs.color) {
			rargs.color = "#5DBA50";
		}
		...
		html +=
			...
			height +
			"px; text-align: center'";
		if (player.me || player.owner == character.owner) {
			html += " onclick='render_cgallery(last_skin,last_cx,\"" + slot + "\")' class='clickable'>";
		} else {
			html += ">";
		}
		...
		html +=
			...
			sprite(player.skin, {
				cx: rargs.cx,
				scale: rargs.scale,
				height: player.aheight * rargs.scale + 20 * rargs.scale,
				j: rargs.j,
				width: width,
				rip: rargs.rip,
			}) +
	...
	var html =
		"<div style='background-color: black; border: 5px solid gray; padding: 20px; font-size: 24px; display: inline-block; vertical-align: top;' class='cccx'>";
	var color = "";
	html += "<div style='display: inline-block; vertical-align: top'>";
	html += "<div style='margin-top: -5px'>";
	if (IID[player.skin][8] == "full") {
		color = "#BB2242";
	}
	render_cosmetic("head", { text: "skin", top: -9, scale: 3, color: color, cx: { head: player.cx.head } });
	render_cosmetic("hair", {
		top: 6,
		scale: 3,
		mleft: 8,
		color: color,
		cx: { head: player.cx.head, hair: player.cx.hair || "bwhair" },
	});
	color = "";
	render_cosmetic("hat", {
		top: 12,
		...
		mleft: 8,
		color: color,
		cx: { head: player.cx.head, hair: player.cx.hair, hat: player.cx.hat || "bwhat" },
	});
	html += "</div>";
	html += "<div>";
	render_cosmetic("face", {
		top: -9,
		scale: 3,
		cx: { head: player.cx.head, hair: player.cx.hair, face: player.cx.face || "bwglasses" },
	});
	...
	html += "</div>";
	html += "<div>";
	render_cosmetic("chin", {
		top: -20,
		scale: 3,
		cx: { head: player.cx.head, hair: player.cx.hair, chin: player.cx.chin || "beard112" },
	});
...
function load_class_info(name, look) {
	...
	html +=
		...
		"</div>";
	html +=
		"<div><span style='color: white'>Class:</span> <span style='color: " +
		colors.male +
		"'>" +
		name.toTitleCase() +
		"</span></div>";
	html +=
		"<div><span style='color: white'>Primary Attribute:</span> <span style='color: " +
		colors[G.classes[name].main_stat] +
		"'>" +
		G.classes[name].main_stat.toTitleCase() +
		"</span></div>";
	if (G.classes[name].side_stat) {
		html +=
			"<div><span style='color: white'>Secondary Attribute:</span> <span style='color: " +
			colors[G.classes[name].side_stat] +
			"'>" +
			G.classes[name].side_stat.toTitleCase() +
			"</span></div>";
	}
	html +=
		"<div><span style='color: white'>Description:</span> <span style='color: gray'>" +
		G.classes[name].description +
		"</span></div>";
