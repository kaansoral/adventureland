...
function auto_reload(value) {
	...
	if (value === false) {
		parent.auto_reload = "off";
	} else if (value == "auto") {
		parent.auto_reload = "auto";
	} // code or merchant stand
	else {
...
function can_use(name) {
	if (G.skills[name] && G.skills[name].class && !in_arr(character.ctype, G.skills[name].class)) {
		return false;
	} // checks the class
...
function use(name, target) {
	// a multi-purpose use function, works for skills too
	if (isNaN(name)) {
		// if name is not an integer, use the skill
		if (!target) {
			target = get_target();
		}
