var is_game=0,is_server=1,is_code=0,is_pvp=false;
var server={
	started:false,
	live:false,
	last_update:false,
...
var io=require('socket.io')(app,{pingInterval:4000,pingTimeout:12000,cors:{
    origin:false,
    methods:"GET,HEAD,PUT,PATCH,POST,DELETE",
    credentials:true,
  }}); // default is 25000 to 60000
var fs=require('fs'),url=require('url');
var {Worker,SHARE_ENV}=require('worker_threads'),workers=[],wlast=0;
...
var chests={};
var projectiles={};
var name_to_id=new Map(),id_to_id={};
var invitations={},challenges={},magiportations={},requests={},parties={},frequests={},trequests={};
var total_moves=1,total_players=0,unique_players=0;
var observer_map="winterland",observer_x=0,observer_y=0;
var merchant_map="main",merchant_y=-50,merchant_x=0,total_merchants=0;
var csold=[],cfound=[]; // cache of S.sold + S.found
...
var TX=[],TXX={}; // all inter account transactions [07/02/22]
var B={
...
var dailies=["crabxx","goobrawl","abtesting"]; shuffle(dailies);
var nightlies=["icegolem","franky"]; shuffle(nightlies);
if(events.holidayseason) events.snowman=60;
if(events.valentines) events.pinkgoo=60;
TIMEO={
	"EU":1,
	"US":-5,
	"ASIA":7,
...
var luckm=1,xpm=1,goldm=1,buym=1;
...
else if(server_name=="DUNGEON")
{
	...
	for(var name in events)
		if(is_number(events[name])) events[name]=events[name]!==0&&9999999999||0;
		else events[name]=false;
}
...
function init_game()
{
	appengine_call("create_server",{keyword:variables.keyword,port:port,region:region,name:server_name,pvp:is_pvp||'',gameplay:gameplay},function(result){
		try{
			if(result.exists) return [console.log("Server Exists!"),process.exit()];
			// console.log(result);
			server_id=result.id;
			...
			D=result.dynamics;
			if(S) init_server_data(S);
			if(G) sprocess_game_data();
			try{
				app.listen(port);
				init_io();
			...
			if(gameplay=="normal" || gameplay=="hardcore" || gameplay=="test")
			{
				...
				create_instance("winter_inn"); create_instance("winter_inn_rooms");
				create_instance("winter_cave"); create_instance("winter_cove");
				...
				create_instance("d_e"); create_instance("d_g"); create_instance("d_b1"); create_instance("d_a1"); create_instance("d_a2");
				server_bfs("crypt");
...
{
	appengine_call("reload_server",{keyword:variables.keyword,port:port,region:region,pvp:is_pvp||'',gameplay:gameplay},function(result){
		try{
			G=result.game;
			D=result.dynamics;
			sprocess_game_data();
			prop_cache={};
			if(to_broadcast) broadcast("reloaded",{change:change||""});
		}
		catch(e)
		{
...
function decode_http_data(data)
{
	...
	var result=decodeURIComponent(data).replace_all("u'","'").replace_all("'",'"').replace_all("+"," ").replace_all("%2B","+");
	return result;
}

function parse_http_json(data)
{
	var result=decode_http_data(data),json={};
	try{
...
function http_handler(request,response)
{
	...
	request.on('error', function(err) {
		...
		try{
			...
			var url_parts = url.parse(request.url,true),args=url_parts.query,output="";
			//console.log(body);
...
function player_to_server(player,place)
{
	var char={};
	for(prop in player)
		if(!in_arr(prop,["auth","last_sync","socket","character","last_upush","push","last","last_u","width","height","u"]))
			char[prop]=player[prop];
	if(place=="sync" && !Object.keys(player.q).length && player.type!="merchant") delete char.p; // ~20KB - too much [19/11/18]
	return char;
}
...
{
	var data={};
	["hp","max_hp","mp","max_mp","xp","attack","heal","frequency","speed","range","armor","resistance","level","party",
		"rip","npc","allow","code","afk","target","focus","role","s","c","q","b","age","pdps",
		"id","x","y","moving","going_x","going_y","abs","move_num","angle","cid","guild","team"].forEach(function(p){
			// removed "vx","vy"
		if(player[p]!==undefined) data[p]=player[p];
	});
	["stand"].forEach(function(p){
		if(player.p && player.p[p]!==undefined) data[p]=player.p[p];
	});
	if(player.afk=="code") data.controller=player.controller;
	if(player.rip && (player.tcx&&player.tcx.gravestone||player.cx.gravestone)) player.rip=(player.tcx&&player.tcx.gravestone||player.cx.gravestone);
	data.skin=player.tskin||player.skin;
	data.cx=player.tcx||player.cx;
	data.slots=player.cslots;
	if(player.tp) data.tp=true;
	...
	if(player.is_npc)
	{
		...
		if(player.direction!==undefined) data.direction=player.direction,data.npc=player.npc;
	}

	if(!stranger)
	{
		["int","str","dex","vit","for","mp_cost","mp_reduction","max_xp","goldm","xpm","luckm","map","in","isize","esize","gold","cash","targets","m","evasion","miss","reflection","lifesteal","manasteal","rpiercing","apiercing","crit","critdamage","dreturn","tax","xrange","pnresistance","firesistance","fzresistance","phresistance","stresistance","incdmgamp","stun","blast","explosion","courage","mcourage","pcourage","fear"].forEach(function(p){ //"vision",
			data[p]=player[p];
		});
		...
		if(player.user!==undefined)
		{
			...
			if(player.user) data.user.gold=player.user.gold;
		}
		if(player.socket) data.cc=get_call_cost(player);
	}
	return data;
}
...
{
	var data={},def=G.monsters[monster.type];
	["speed","hp","mp","max_mp","attack","xp","frequency","armor","resistance","1hp","skin","cooperative","drops"].forEach(function(p){ //same array as game.js adopt_soft_properties
		if(p in monster && monster[p]!=def[p]) data[p]=monster[p];
	});
	if(monster.max_hp!=def.hp) data.max_hp=monster.max_hp;
	["id","x","y","moving","going_x","going_y","abs","move_num","angle","type","cid","target","focus","s"].forEach(function(p){ // removed "vx","vy" from both datasets [16/04/18]
		if(monster[p]!==undefined) data[p]=monster[p];
	});
	if(monster.level>1) data.level=monster.level;
	if(monster.pet) data.pet=true,data.owner=monster.owner,data.name=monster.name;
	if(monster.trap) data.trap=true,data.owner=monster.owner;
...
{
	var summary={skin:player.tskin||player.skin,level:player.level,type:player.type,x:player.x,y:player.y,in:player.in,map:player.map,name:player.name,hp:player.hp,max_hp:player.max_hp};
	if(player.rip) summary.rip=(player.tcx&&player.tcx.gravestone||player.cx.gravestone)||true;
...
{
	player.state={"map":player.map,"in":player.in,"hp":player.hp,"mp":player.mp,"s":player.s,"x":player.x,"y":player.y,"restored":false};
}
...
function restore_state(player,dc)
{
	if(player.state && !player.state.restored)
	{
		...
		player.rip=false; if(player.state.rip) player.rip=true;
		if(dc)
		{
...
function party_to_client(oname)
{
	...
	list.forEach(function(name){
		var player=players[name_to_id.get(name)],dps_multiplier=1; if(!player) return;
		if(player.type=="merchant") return;
		if(player.type=="priest") dps_multiplier=1.36;
		length+=1;
		if(player.level<60) newbies+=1;
	...
	list.forEach(function(name){
		...
		party[name]={skin:player.tskin||player.skin,level:player.level,type:player.type,x:player.x,y:player.y,in:player.in,map:player.map,share:player.share,pdps:player.pdps,l:length,xp:player.party_xp,luck:player.party_luck,gold:player.party_gold};
		if(player.rip) party[name].rip=(player.tcx&&player.tcx.gravestone||player.cx.gravestone)||true;
		if(Object.keys(player.tcx||player.cx).length) party[name].cx=(player.tcx||player.cx);
	});
	// git test
	return party;
}
...
function apply_stats(player,prop,args)
{
	for(var stat in prop)
	{
		...
		else if(stat=="frequency") player.frequency+=prop[stat]/100.0;
...
function calculate_player_stats(player)
{
	...
	if(level_up)
	{
		if(player.level>=80) realm_broadcast("server_message",{message:player.name+" is now level "+player.level,color:"#968CFA"});
		else if(player.level>=70) broadcast("server_message",{message:player.name+" is now level "+player.level+"!",color:"#968CFA"});
	...
	var class_def=G.classes[player.type],item_attack=0,the_date=new Date();
	if(!class_def) class_def=G.classes.merchant; // when npc's get resend't
	player.range=class_def.range;
	player.max_hp=class_def.hp;
	player.max_mp=class_def.mp;
	["stealth"].forEach(function(prop){player[prop]=false;});
	["a_mp_cost","a_attack","stones","lifesteal","manasteal","incdmgamp","mp_reduction","stun","blast","explosion","cuteness","bling"].forEach(function(prop){player[prop]=0;});
	["speed","attack","frequency","mp_cost","armor","resistance","apiercing","rpiercing","a","aura","evasion","miss","reflection","crit","critdamage","dreturn","computer","xxp","xluck","xgold","output","courage","mcourage","pcourage","pnresistance","firesistance","fzresistance","phresistance","stresistance"].forEach(function(prop){
		player[prop]=class_def[prop]||0;
	});
	if(!player.a) player.a={}; //abilities
	if(!player.aura) player.aura={};
	if(player.paura) for(var id in player.paura) player.aura[id]=player.paura[id];
	...
	while(player.items.length>42 && !player.items[player.items.length-1]) player.items.splice(player.items.length-1);
	while(player.citems.length>42 && !player.citems[player.citems.length-1]) player.citems.splice(player.citems.length-1);
	player.isize=42; player.sets={};
	player.esize=player.isize-player.items.length;
	player.xpm=player.goldm=player.luckm=1;
	for(var i=0;i<player.items.length;i++)
	{
		var current=player.items[i];
		if(!current) player.esize++;
		else if(current.name=="supercomputer") player.tracker=player.computer=player.supercomputer=true;
		else if(current.name=="computer") player.computer=true;
		else if(current.name=="tracker") player.tracker=true;
		else if(current.expires) // &&!current.ex = ex=elixier
		{
			if(the_date>current.expires)
			{
				player.items[i]=null; player.citems[i]=null;
			}
	...
	if(player.tracker)
	{
		for(var name in G.monsters)
		{
			...
			if(!mx || !G.monsters[name].achievements) continue;
			G.monsters[name].achievements.forEach(function(def){
				if(mx<def[0]) return;
				if(def[1]=="stat") player.monster_stats[def[2]]=(player.monster_stats[def[2]]||0)+def[3];
			});
		}
	}
	character_slots.forEach(function(slot)
	{
		var current=player.slots[slot]; if(!current) return;
		var def=G.items[current.name]; if(!def) { console.log("#X Undefined item: "+current.name+" ("+slot+")"); return; }
		var prop=calculate_item_properties(current,{class:player.type,map:player.map});
		if(prop.class && !prop.class.includes(player.type)) return;

		apply_stats(player,prop,{no_range:(slot=="offhand"&&def.type=="weapon")});
		if(prop.attack)
		{
			if(slot=="offhand") item_attack+=prop.attack*0.7;
			else item_attack+=prop.attack;
		}
		if(def.ability)
			player.a[def.ability]={attr0:(player.a[def.ability]&&player.a[def.ability].attr0||0)+prop.attr0,attr1:(player.a[def.ability]&&player.a[def.ability].attr1||0)+prop.attr1};
		if(def.aura)
		...
		if(slot=="offhand")
		{
			apply_stats(player,class_def.offhand[def.wtype]||class_def.offhand[def.type]||{},{no_range:!player.slots.mainhand});
		}
		if(prop.set) player.sets[def.set]=(player.sets[def.set]||0)+1;
	});
	...
	if(player.slots.mainhand && player.slots.offhand && G.items[player.slots.mainhand.name].wtype=="stars" && G.items[player.slots.offhand.name].wtype!="stars") item_attack/=3.0;
	if(player.slots.cape && player.slots.cape.name=="stealthcape") player.stealth=true;
	item_attack=max(item_attack,5)
	if(player.type=="paladin") player.attack+=item_attack*(player.str/20.0+player.int/40.0);
	else player.attack+=item_attack*(player[class_def.main_stat]/20.0);
	player.attack+=player.a_attack;
	if(player.type=="priest") player.attack*=1.6;
	if(player.type=="warrior") player.courage+=round(player.str/30);
	if(player.type=="priest") player.mcourage+=round(player.int/30);
	if(player.type=="paladin") player.pcourage+=round(player.str/30+player.int/30);
	...
	if(player.last_attack_ms && player.attack_ms!=player.last_attack_ms)
	{
		...
		player.socket.emit("skill_timeout",{name:"attack",ms:player.attack_ms-mssince(player.last.attack),reason:"attack_ms"});
	}
	...
	if(player.damage_type=="physical") player.mp_cost+=player.apiercing/15.0;
	else player.mp_cost+=player.rpiercing/15.0;
	player.mp_cost=max(1,player.mp_cost);
	if(!player.hp && !player.rip) player.hp=player.max_hp,player.mp=player.max_mp; // used for level-ups
	if((player.gold||0)<=0) player.gold=0;
	player.heal=0;
	...
	player.hp=max(0,min(player.hp,player.max_hp)); player.mp=max(0,min(player.mp,player.max_mp));

	if(player.party && parties[player.party])
	{
		if(player.party_xp) player.xxp+=player.party_xp;
		if(player.party_luck) player.xluck+=player.party_luck;
		if(player.party_gold) player.xgold+=player.party_gold;
	}

	if(goldm!=1) player.xgold+=(goldm-1)*100.0,player.xluck+=(luckm-1)*100.0,player.xxp+=(xpm-1)*100.0;
	player.luckm=1+(player.xluck/100.0); player.xpm=1+(player.xxp/100.0); player.goldm=1+(player.xgold/100.0);
	["luckm","xpm","goldm"].forEach(function(p){player[p]=max(0.01,player[p])});

	if(player.tskin=="konami") player.range=120,player.frequency+=0.25,player.goldm*=0.25,player.luckm+=0.25;

	if(player.s.invis) player.speed=max(player.speed*0.6,player.speed-25),player.attack=round(player.attack*1.25);
	if(player.s.invincible) player.attack=round(player.attack*0.45);
	if(player.s.dash) player.speed=500;
	calculate_common_stats(player)

	player.tax=(player.level>80)&&0.010||(player.level>80)&&0.012||(player.level>70)&&0.020||(player.level>60)&&0.025||(player.level>50)&&0.03||(player.level>20)&&0.04||0.05;
	...
	if(excess>2) player.attack=round(player.attack*0.2);
	else if(excess>1) player.attack=round(player.attack*0.4);
	else if(excess) player.attack=round(player.attack*0.6);
	player.fear=excess;

	if(player.map=="winterland") player.speed*=0.95;
	...
	if(!player.gold && player.gold!==0) player.gold=0,server_log("#X - GOLD BUG calculate",1);
...
{
	if(entity.s.poisoned) entity.frequency*=0.9;
	if(entity.s.frozen) entity.frequency*=0.3,entity.speed-=40;
	if(entity.speed<1) entity.speed=1;
	if(entity.s.tangled) entity.speed=min(entity.speed,24);
}
...
function calculate_monster_stats(monster)
{
	...
	monster.output=100;
	if(monster.target || monster.focus) monster.speed=G.monsters[monster.type].charge;
	...
	if(monster.focus && instances[monster.in].monsters[monster.focus]) monster.speed=min(monster.speed,instances[monster.in].monsters[monster.focus].speed+4);
	if(monster.level>1)
	{
		var att_mult=0.125,freq_mult=0.034,speed_mult=0.24; // speed originally 0.34
		var mlevel=min(monster.level,12);
		if(monster.map_def.grow) att_mult=0.05,freq_mult=0.008,speed_mult=0.16;
	...
	if(E.schedule.night) monster.speed=ceil(monster.speed*0.7);
...
{
	if(Object.keys(monster.s).length) calculate_monster_stats(monster);
}
...
function can_equip_item(player,item,slot)
{
	...
	if(slot=="offhand" && item.type!="weapon") slot=item.type; //Easiest solution to the slot:"offhand" challenge [15/11/16]
	if(item.type=="tool") slot="mainhand";
	if(item.type=="test" && slot!="test") return slot;
	if(!in_arr(item.type,["helmet","pants","chest","weapon","amulet","earring","shoes","gloves","ring","shield","belt","source","orb","quiver","cape","misc_offhand","tool"])) return "no";
	if(slot!=item.type && in_arr(item.type,["shield","source","quiver","misc_offhand"]) && slot!="offhand") return "no";
	if(in_arr(slot,["weapon","mainhand","offhand","tool"]))
	{
		if(slot=="weapon" && player.slots.mainhand && class_def.mainhand[G.items[player.slots.mainhand.name].wtype] && !player.slots.offhand && class_def.offhand[item.wtype])
		{
			slot="offhand";
		}
		else if(slot=="offhand" && (!player.slots.mainhand || player.slots.mainhand && class_def.mainhand[G.items[player.slots.mainhand.name].wtype]) && class_def.offhand[item.wtype])
		{
			slot="offhand";
		}
		else if(slot=="weapon" || slot=="mainhand")
		{
			...
			else if(!class_def.mainhand[item.wtype]) return "no";
			slot="mainhand";
		}
		else {return "no";}
	}
	...
	{
		if(class_def.offhand[item.type] && !(player.slots.mainhand && class_def.doublehand[G.items[player.slots.mainhand.name].wtype])) slot="offhand";
		else {return "no";}
	}
	else if(item.type=="quiver")
	{
		if(class_def.offhand.quiver && !(player.slots.mainhand && class_def.doublehand[G.items[player.slots.mainhand.name].wtype])) slot="offhand";
		else {return "no";}
	}
	else if(item.type=="source")
	{
		if(class_def.offhand.source && !(player.slots.mainhand && class_def.doublehand[G.items[player.slots.mainhand.name].wtype])) slot="offhand";
		else {return "no";}
	}
	else if(item.type=="ring")
	{
		if(slot=="ring1" || slot=="ring2");
		else if(!player.slots["ring1"]) slot="ring1";
		else slot="ring2";
	}
	else if(item.type=="earring")
	{
		if(slot=="earring1" || slot=="earring2");
		else if(!player.slots["earring1"]) slot="earring1";
		else slot="earring2";
	}
...
function consume(player,num,quantity)
{
	...
	if(quantity>available) exception=not_enough_items;
	if(available==quantity) player.items[num]=null,player.esize++;
	else player.items[num].q-=quantity;
...
function create_new_item(name,quantity)
{
	...
	if(G.items[name].s) new_item.q=quantity||1;
	if(G.items[name].upgrade || G.items[name].compound) new_item.level=0;
	return new_item;
}

function aadd_item(player,new_item) // admin [24/06/23]
{
...
function add_item(player,new_item,args)
{
	...
	var done=false,num=0,a=false;
	if(!args) args={};
	if(!new_item.name) new_item=create_new_item(new_item,args.q||1);
	if(args.p) new_item.p=args.p;
	if((args.m||args.r) && (G.items[new_item.name].upgrade && Math.random()<1.0/500 || G.items[new_item.name].compound && Math.random()<1.0/20000))
			new_item.p="shiny",a=true;
	if(G.items[new_item.name].s)
	{
		for(var i=0;i<player.items.length;i++)
		{
			...
			{
				if(args.v && is_in_pvp(player,1) || new_item.v) item.v=new Date();
				if(new_item.m) item.m=new_item.m;
				...
				done=true; num=i; break;
			}
		}
	}
	if(!new_item.oo) new_item.oo=player.name;
	if(!done)
	{
		for(var i=0;i<player.items.length;i++)
		{
			...
			if(!item)
			{
				if(args.v && is_in_pvp(player,1)) new_item.v=new Date();
				player.items[i]=new_item; player.citems[i]=cache_item(player.items[i]);
	...
	if(args.m && player.s.mluck && get_player(player.s.mluck.f) && Math.random()<=0.02 && can_add_item(get_player(player.s.mluck.f),new_item.name))
	{
		var mr=get_player(player.s.mluck.f),item=create_new_item(new_item.name); item.m=player.name;
		if(new_item.data) item.data=new_item.data; // "cxjar"
	...
	{
		var event=G.items[new_item.name].event,e_type="server_found",e_phrase=" found ";
		if(!args.found) e_type="server_received",e_phrase=" received ";
		if(args.phrase) e_type="server_received",e_phrase=" "+args.phrase.toLowerCase()+" ";
		broadcast("server_message",{"message":player.name+e_phrase+item_to_phrase(new_item),"color":"#85C76B","type":e_type,item:cache_item(new_item),name:player.name,event:event});
...
function list_to_pseudo_items(list,type,add)
{
	...
	if(!type || type=="multi")
	{
		...
		list.forEach(function(el){
			if(G.items[el[1]].s)
			{
				if(ex[el[1]]) ex[el[1]].q+=max(1,el[0]);
				else
				{
			...
			else items.push({name:el[1],q:max(1,el[0])});
		});
	}
	else if(type=="chest")
	{
		list.forEach(function(name){
			...
			else items.push(create_new_item(name));
		});
...
{
	var done=false;
	if(!new_item.name) new_item=create_new_item(new_item);
	if(new_item.q)
	{
		for(var i=0;i<player.user[slot].length;i++)
		{
			...
			if(can_stack(item,new_item))
			{
				item.q=(item.q||1)+(new_item.q||1); player.cuser[slot][i]=cache_item(player.user[slot][i]);
				done=true; break;
			}
		}
	}
	if(!done)
	{
		for(var i=0;i<player.user[slot].length;i++)
		{
			...
			if(!item)
			{
				player.user[slot][i]=new_item; player.cuser[slot][i]=cache_item(player.user[slot][i]);
				done=true; break;
			}
		}
	}
...
function drop_item_logic(drop,def,pvp)
{
	...
	else if(def[1]=="cxjar" || def[1]=="emotionjar")
	{
		...
		if(!added) drop.items.push(item);
	}
	else if(def[1]=="open") chest_exchange(drop,def[2]);
	else
	{
...
{
	if(!args) args={};
	var drop_id=randomStr(30),chest=args.chest||"chest1";
	if(!is_array(items)) items=[items];
	drop=chests[drop_id]={items:[],cash:0};
	for(var i=0;i<items.length;i++) if(is_string(items[i]))	drop_item_logic(drop,[1,items[i]],is_in_pvp(player,1));
...
function drop_something(player,monster,share)
{
	...
	else player.socket.emit("drop",{"x":drop.x,"y":drop.y,"items":drop.items.length,"chest":chest,"id":drop_id,"map":drop.map,owners:[player.owner]});
}

function drop_something_hardcore(player,target)
{
	var drop_id=randomStr(30),drop;
	...
	for(var name in target.slots)
		...
		{
			var prob=0.1;
			if(name=="mainhand" || name=="offhand") prob=0.05;
			if(Math.random()<prob) drop.pvp_items.push(target.slots[name]),target.slots[name]=target.cslots[name]=null;
		}

	for(var i=0;i<42;i++)
		if(target.items[i])
		{
			if(Math.random()<0.20) drop.pvp_items.push(target.items[i]),target.items[i]=target.citems[i]=null;
	...
	else player.socket.emit("drop",{"x":drop.x,"y":drop.y,"items":drop.pvp_items.length,"chest":"chest8","id":drop_id,"map":target.map,owners:[player.owner]});
}
...
function drop_something_pvp(player,target)
{
	...
	for(var name in target.slots)
		...
		{
			var prob=0.5;
			if(name=="mainhand" || name=="offhand") prob=0.3;
			if(Math.random()<prob) drop.pvp_items.push(target.slots[name]),target.slots[name]=target.cslots[name]=null;
		}
	...
	for(var i=0;i<42;i++)
		if(target.items[i] && target.items[i].v)
		{
			...
			if(Math.random()<prob) drop.pvp_items.push(target.items[i]),target.items[i]=target.citems[i]=null;
			// else if(Math.random()<0.05) target.items[i]=target.citems[i]=null;
		}
	...
	{
		player.socket.emit("drop",{"x":drop.x,"y":drop.y,"items":drop.pvp_items.length,"chest":"chest8","id":drop_id,"map":target.map,owners:[player.owner]});
	}
}
...
{
	var target=monster;
	if(!player.s.monsterhunt || player.s.monsterhunt.sn!=region+" "+server_name) return;
	if(player.s.monsterhunt.id==monster.type && player.s.monsterhunt.c) player.s.monsterhunt.c--;
	if(target.level==1 && player.s.monsterhunt.id==target.type && player.s.monsterhunt.dl)
	{
		...
		get_monsters(player.s.monsterhunt.id).forEach(function(m){
			if(!player.s.monsterhunt.dl && m.level>1) // && !m.target - it was possible to keep a high level monsters aggroed and abuse the system [21/07/23]
			{
...
function calculate_monster_score(player,monster,share)
{
	...
	for(var id in players)
	{
		...
		else if(current.owner==player.owner && current.party==player.party && simple_distance(current,player)<600 && current.type!="merchant") score+=0.3/divider;
		else if(current.owner==player.owner && current.party && current.party==player.party && current.type!="merchant") score+=0.3/divider; // originally 0.25
	}
	if(simple_distance(player,monster)>600 && share<0.01)
	{
		score-=0.3/divider;
	}
	if(player.type=="merchant" && player.party) score/=2;
	if(score<0) score=0;
	if(gameplay=="hardcore") score*=10000;
	return score;
}
...
function issue_monster_awards(monster)
{
	...
	for(var name in monster.points)
	{
		...
		if(current && share>0.0025) //  && current.map==monster.map
		{
			...
			else drop_something(current,monster,share);
			...
			if(current.type=="merchant") continue;
			current.xp+=round(monster.xp*share*current.xpm);
			if(current.t) current.t.xp+=round(monster.xp*share*current.xpm);
...
function issue_monster_award(monster)
{
	...
	if(!player.party)
	{
		if(monster.rbuff && G.conditions[monster.rbuff]) player.s[monster.rbuff]={ms:G.conditions[monster.rbuff].duration};
		if(monster.cbuff) for(var i=0;i<monster.cbuff.length;i++) if(player.level<=monster.cbuff[i][0] && G.conditions[monster.cbuff[i][1]]) { player.s[monster.cbuff[i][1]]={ms:G.conditions[monster.cbuff[i][1]].duration}; break; }
		...
		if(player.t) player.t.xp+=monster.xp*player.xpm*monster.mult;
		player.cid++; player.u=true;
	...
	else
	{
		...
		// xp=round(xp);
		parties[player.party].forEach(function(name){
			var current=players[name_to_id.get(name)],cxp=round(xp*current.xpm*current.share);
			if(monster.rbuff && G.conditions[monster.rbuff]) current.s[monster.rbuff]={ms:G.conditions[monster.rbuff].duration};
			var score=calculate_monster_score(current,monster);
			current.p.stats.monsters[monster.type]=(current.p.stats.monsters[monster.type]||0)+1;
			current.p.stats.monsters_diff[monster.type]=(current.p.stats.monsters_diff[monster.type]||0)+(score-1);
			monster_hunt_logic(current,monster);
			if(current.type=="merchant") return;
			current.xp+=cxp*monster.mult;
			if(current.t) current.t.xp+=cxp*monster.mult;
			current.cid++; current.u=true;
			calculate_player_stats(current);
			if(current!=player) current.socket.emit("player",player_to_client(current));
			// current.socket.emit("game_log",{message:xp+" XP",color:"#416F3A"});
			disappearing_text(current.socket,current,"+"+cxp,{map:current.map,color:"#753D8C","size":"large",nv:1}); //party:player.party, - better send it to individuals and multiply on client with a setting [19/08/16]
		});
	}
}
...
{
	if(target.dead) return; // [04/02/23]
	var no_decrease=false;
	if(attacker)
	{
		if(!attacker.party) attacker.socket.emit("game_log","You "+killed_message(target.type)); // tut("killagoo") on game.js
		else party_emit(attacker.party,"game_log",attacker.name+" "+killed_message(target.type));
		if(B.free_last_hits && target.target && target.target!=attacker.name) stop_pursuit(target,{force:true,cause:"kill_monster call"});
		if(!target.target) target_player(target,attacker,1),no_decrease=true;
	}
...
function pwn_routine(victor,target)
{
	...
	rip(target);
	instance_emit(target.in,"server_message",{message:victor.name+" pwned "+target.name,color:victor.team&&colors[victor.team]||"gray"});
	if(target.map=="arena") xy_emit(npcs.pvp,"chat_log",{owner:npcs.pvp.name,message:victor.name+" pwned "+target.name,id:"pvp"});
}
...
function issue_player_award(attacker,target)
{
	...
	{
		if(target.duel) duel_defeat(target);
		return;
	}
	...
	var lost_gold=100,gain_gold=100;
	if(target.level>=10) lost_gold=1000;
	if(target.level>=20) lost_gold=5000;
	if(target.level>=30) lost_gold=12500;
	if(target.level>=40) lost_gold=25000;
	if(target.level>=50) lost_gold=50000;
	if(target.level>=55) lost_gold=75000;
	if(target.level>=60) lost_gold=125000;
	if(target.level>=65) lost_gold=250000;
	if(target.level>=70) lost_gold=500000;
	if(target.level>=75) lost_gold=1000000;
	...
	if(target.type=="merchant") lost_xp=0;
	if(gameplay!="hardcore" && gameplay!="test" && is_same(attacker,target,1)) lost_gold=gain_gold=0,lost_xp=0;

	if(!is_same(attacker,target,1)) attacker.kills++;

	if(mode.log_pvp) appengine_log("pvp",attacker.name+" pwned "+target.name+" For "+lost_gold+" Gold "+lost_xp+" XP");
	pwns[(pend++)%200]=[attacker.name,target.name];


	target.socket.emit("game_log",{message:"Slain by "+attacker.name,color:"#F12F02"});
	var lost_shells=0,psize=1;
	if(lost_xp)
	...
	target.socket.emit("disappearing_text",{"message":"-"+lost_xp,"x":target.x,"y":target.y-30,"args":{color:colors.party_xp}});

	if(gameplay=="hardcore") drop_something_hardcore(attacker,target);
	if(is_in_pvp(target)) drop_something_pvp(attacker,target);

	if(!attacker.party)
	{
		if(attacker.type=="merchant") lost_xp=0;
		...
		attacker.socket.emit("disappearing_text",{"message":"+"+gain_gold,"x":target.x,"y":target.y-40,"args":{color:"+gold","size":"large"}});
		attacker.socket.emit("game_log","Gained "+to_pretty_num(round(lost_xp*0.95))+" experience");
		attacker.socket.emit("disappearing_text",{"message":"+"+lost_xp,"x":target.x,"y":target.y-30,"args":{color:colors.party_xp}});
		if(lost_shells)
		{
	...
	else
	{
		...
		lost_xp=floor(lost_xp*0.92/parties[attacker.party].length),gain_gold=floor(gain_gold/parties[attacker.party].length);
		if(lost_shells)
		{
			add_shells(attacker,lost_shells,"xptome");
		}
...
{
	var attack=attacker.attack,mp_cost=0;
	var info={apiercing:0,damage_type:attacker.damage_type,heal:false,lines:true,positive:false,first_attack:0,procs:false,conditions:[]}; // server projectile

	if(!G.skills[atype].hostile) info.positive=true;

	// TARGET CHANGE
	if(attacker.is_player) attacker.target=target.id;

	// FAILURE SCENARIOS
	if(attacker.type=="merchant" && !G.skills[atype].merchant_use && !(atype=="attack" && attacker.slots.mainhand && G.items[attacker.slots.mainhand.name].wtype=="dartgun"))
	{
	...
	if(mode.pvp_level_gap && attacker.is_player && target.is_player && !info.positive && abs(attacker.level-target.level)>10)
	{
	...
	var dist=distance(attacker,target),range=0;
	var def={hid:attacker.id,source:atype,projectile:null};

	// PROJECTILE LOGIC
	if(attacker.is_monster) def.projectile=G.monsters[attacker.type].projectile||"stone";
	if(attacker.is_player && G.classes[attacker.type].projectile) def.projectile=G.classes[attacker.type].projectile;
	if(attacker.projectile) def.projectile=attacker.projectile;
	if(attacker.slots && attacker.slots.mainhand && G.items[attacker.slots.mainhand.name].projectile) def.projectile=G.items[attacker.slots.mainhand.name].projectile;
	if(attacker.tskin=="konami") def.projectile="stone_k";
	if((atype!="attack" || !def.projectile || atype=="heal") && G.skills[atype].projectile) def.projectile=G.skills[atype].projectile;

	// DAMAGE TYPE LOGIC
	if(attacker.is_monster && G.monsters[attacker.type].damage_type) info.damage_type=G.monsters[attacker.type].damage_type;
	if(attacker.is_player && G.classes[attacker.type].damage_type) info.damage_type=G.classes[attacker.type].damage_type;
	if(attacker.is_player && attacker.slots.mainhand && G.items[attacker.slots.mainhand.name].damage_type) info.damage_type=G.items[attacker.slots.mainhand.name].damage_type;
	if(atype!="attack" && G.skills[atype].damage_type) info.damage_type=G.skills[atype].damage_type;

	// PROCS
	if(!attacker.is_player || G.skills[atype].procs) info.procs=true;

	// HEAL / POSITIVE
	if(G.skills[atype].heal || attacker.is_player && attacker.slots.mainhand && attacker.slots.mainhand.name=="cupid")
		info.heal=true,info.positive=true;
	...
	// COMMON RANGE CHECKS	
	if(G.skills[atype].use_range) range=attacker.range;
	if(G.skills[atype].range) range=G.skills[atype].range;

	// DAMAGE
	if(G.skills[atype].damage) attack=G.skills[atype].damage;

	// SKILL MP
	if(G.skills[atype].mp) mp_cost=G.skills[atype].mp;
	...
	if(attacker.is_player && range && !attacker.is_npc)
	{
		...
		if(dist>range) attacker.xrange+=range-dist;
	}

	if(info.procs && attacker.s.poisonous) info.conditions.push("poisoned");
	...
	else if(attacker.is_player && (atype=="attack" || atype=="3shot" || atype=="5shot" || atype=="cleave" || atype=="shadowstrike"))
	{
		var mp_mult=1,att_mult=1;
		if(atype=="3shot") mp_mult=0,att_mult=0.7;
		if(atype=="5shot") mp_mult=0,att_mult=0.5;
		if(atype=="cleave") mp_mult=0.02,info.lines=false,att_mult=0.1+Math.random()*0.8,def.aoe=true,attacker.first=true;
		if(atype=="shadowstrike") mp_mult=0,info.lines=false,att_mult=(0.2+Math.random()*1.2)*((Math.random()<0.05)&&12||1),def.aoe=true;
	...
	{
		if(attacker.level>=80) attack=800;
		else if(attacker.level>=72) attack=720;
		else if(attacker.level>=60) attack=600;
		else attack=400;
	}
	...
	{
		if(target.is_monster && target.target && get_player(target.target) && is_same(attacker,get_player(target.target),1))
		{
	...
	{
		var mp_cutoff=attacker.next_mp,mp=attacker.next_mp;
		if(atype=="cburst") mp=mp_cutoff=attacker.next_mp;
	...
	{
		if(attacker.in!=target.in || distance(attacker,target,true)>attacker.range*G.skills.mentalburst.range_multiplier+G.skills.mentalburst.range_bonus)
		{
			attacker.socket.emit("game_response",{response:"too_far",id:target.id,dist:dist});
			return {failed:true,reason:"too_far",place:atype,id:target.id,dist:dist}
		}
	...
	{	
		...
		if(attacker.s.poisonous) info.conditions.push("poisoned");
	...
	if(!info.positive && (attacker.is_player && G.maps[attacker.map].safe || !mode.friendly_fire && (!attacker.team || attacker.team!=target.team) && (attacker.party && attacker.party==target.party || attacker.guild && attacker.guild==target.guild)) || attacker.map=="duelland" && (!attacker.duel || !target.duel))
	{
	...
	if(mp_cost && attacker.first && !attacker.is_npc) consume_mp(attacker,mp_cost,atype!="attack"&&atype!="heal"&&target);
	...
	if(attacker.is_player)
	{
		...
		if(attacker.p && attacker.p.stand) attacker.p.stand=false;
	...
	if(info.procs)
		["crit","critdamage","explosion","blast","lifesteal","manasteal"].forEach(function(p){if(attacker[p]) info[p]=attacker[p];});
	["apiercing","rpiercing","miss"].forEach(function(p){if(attacker[p]) info[p]=attacker[p];});
	if(info.procs && attacker.a.freeze && Math.random()<attacker.a.freeze.attr0*(G.maps[attacker.map].freeze_multiplier||1)/100.0)
		info.conditions.push("frozen");
	if(info.procs && attacker.a.burn && info.procs && Math.random()<attacker.a.burn.attr0*(G.maps[attacker.map].burn_multiplier||1)/100.0)
		info.conditions.push("burned");
	...
	var action={attacker:attacker.id,target:target.id,"type":atype,source:atype,"x":target.x,"y":target.y,"eta":400,"m":target.m,pid:pid};
	...
	if(info.heal) action.heal=attack;
	else if(attack) action.damage=attack;
	if(info.positive) action.positive=true;
	if(info.conditions.length) action.conditions=info.conditions;
	info.action=action;
...
{
	var defense="armor",pierce="apiercing";
	var combo=1,combo_m=1;
	var atype=info.atype;
	var evade=false,first=true;
	var attack=info.attack,o_attack,i_attack=info.attack,def=info.def;
	if(G.monsters[attacker.type] && G.monsters[attacker.type].good) info.heal=info.positive=true;
	var change=false;
	var targets=[[target,"normal"]],otarget=target;
	var events=[];
	info.action.map=attacker.map;
	info.action.in=attacker.in;

	if(info.damage_type=="pure" || target===attacker) defense="none_existent",pierce="non_existent",info.apiercing=0;
	else if(info.damage_type=="magical") defense="resistance",pierce="rpiercing",info.apiercing=0;
	else info.damage_type="physical";

	if(target.is_player && attack>0 && !info.heal)
	{
		add_pdps(target,attacker,attack*B.dps_tank_mult); // "tank"
		if(attacker.cooperative) add_coop_points(attacker,target,attack*B.dps_tank_mult);
	}
	...
	if(target.reflection && defense=="resistance" && Math.random()*100<target.reflection && !info.heal && attacker!=target)
	{
		var pid=randomStr(6),eta=0,opid=info.action.pid;
		// info.attack=ceil(attack*damage_multiplier(attacker.resistance||0))||1;
		info.attack=ceil(attack*(0.90+(attack&&(Math.random()*0.2)||0))); // A pure reflection was requested [29/03/22]
		if(!info.action.instant) eta=1000*distance(target,attacker,true)/G.projectiles[info.action.projectile].speed;
		...
		info.action.x=attacker.x; info.action.y=attacker.y;
		info.action.reflect=info.attack;
		info.action.m=attacker.m;

		if(attacker.is_player && target.is_monster || info.reflections) info.reflections=(info.reflections||0)+1;
		if(info.reflections>=4 && attacker.is_player) item_achievement_increment(attacker,attacker.slots.chest,"reflector");
		if(info.reflections>=4 && target.is_player) item_achievement_increment(target,target.slots.chest,"reflector");

		xy_emit(target,"hit",{pid:def.pid,hid:attacker.id,id:target.id,damage:0,reflect:info.attack},attacker.id);
		return xy_emit(target,"action",info.action,attacker.id);
	}

	if(attacker==target && !target.dead); // reflect after dead fix [04/02/23]
	else if(target.evasion && defense=="armor" && Math.random()*100<target.evasion)
		return xy_emit(info.action,"hit",{pid:def.pid,hid:attacker.id,id:target.id,damage:0,evade:true},attacker.id);
	else if(target.dc || target.dead || attacker.miss && Math.random()*100<attacker.miss || target.avoidance && Math.random()*100<target.avoidance)
		return xy_emit(info.action,"hit",{pid:def.pid,hid:attacker.id,id:target.id,damage:0,miss:true},attacker.id);
	else if(target.m!=info.action.m || point_distance(target.x,target.y,info.action.x,info.action.y)>72*(info.heal&&1.5||1))
		return xy_emit(info.action,"hit",{pid:def.pid,hid:attacker.id,id:target.id,damage:0,avoid:true,x:info.action.x,y:info.action.y,map:info.attacker.map,in:info.attacker.in},attacker.id);

	if(info.positive || !info.procs) combo=0;
	if(combo && target.targets>3)
	{
		if(!target.last_combo || ssince(target.last_combo)>5) target.combo=1;
		combo+=target.combo;
		target.last_combo=new Date();
		target.combo+=1;
		def.mobbing=target.combo;
	}
	if(combo && target.is_player && instances[target.in].pmap[target.last_hash] && Object.keys(instances[target.in].pmap[target.last_hash]).length>1)
	{
		...
		for(var id in instances[target.in].pmap[target.last_hash])
		{
			...
			if(target.id==ntarget.id || is_invinc(ntarget) || !is_same(target,ntarget,2) && !is_in_pvp(target) || attacker.id==ntarget.id) continue; //is_same(attacker,ntarget,1)
		...
		if(combo_check)
		{
			...
			{
				var ntarget=instances[target.in].pmap[target.last_hash][id];
				if(is_invinc(ntarget) || attacker.id==ntarget.id || !is_same(target,ntarget,2)) continue; //is_same(attacker,ntarget,1)
				targets.push([ntarget,"stack"]);
				if(!ntarget.last_combo || ssince(ntarget.last_combo)>5) ntarget.combo=1;
				combo+=ntarget.combo;
			...
			{
				def.stacked=[];
				targets.forEach(function(t){ def.stacked.push(t[0].id); })
			}
		}
	}
	
	if(combo>10) combo_m=combo/4.0;
	else if(combo>1) combo_m=[1,1.6,1.62,1.64,1.70,1.72,1.75,1.8,1.9,2,2,2,2][combo];
	combo_m=min(combo_m,max(300/attack,1.2)); // previously 2.4

	if(!info.positive && (info.explosion && info.damage_type=="physical" || info.blast && info.damage_type=="magical"))
	{	
		var intensity=info.blast,defense="resistance";
		if(info.damage_type=="physical") intensity=info.explosion,defense="armor";
		...
		if(attacker.is_player)
			for(var id in instances[target.in].monsters)
			{
				...
				{
					xy_emit(info.action,"hit",{pid:def.pid,hid:attacker.id,id:target.id,damage:0,miss:true},attacker.id);
					continue;
				}
				if(target.id!=otarget.id && distance(target,otarget)<radius)
					targets.push([target,"splash",damage_multiplier(target[defense]||0)*intensity/100.0]);
	...
	targets.forEach(function(target_def){
		delete def.splash; delete def.dreturn; delete def.kill; delete def.unintentional;

		var target=target_def[0];
		if(target_def[1]!="normal") def.unintentional=true;
		if(target_def[1]=="splash") def.splash=true;
		...
		if(info.heal)
		{
			if(first)
			{
				o_attack=attack=-ceil(B.heal_multiplier*attack*(0.90+Math.random()*0.2)*damage_multiplier(((target[defense]||0)-(attacker[pierce]||0))/2.0));
				if(target.s.poisoned) attack=round(attack*0.25);
			}
			else
				attack=o_attack;
			def.heal=-attack;
		...
		else
		{
			...
			if(first)
			{
				...
				if(def.purify)
				{
					...
					{
						if(G.conditions[name] && (G.conditions[name].buff || G.conditions[name].debuff) && !G.conditions[name].persistent || target.s[name].citizens)
						{
							delete target.s[name];
							attack+=400; info.first_attack+=400;
							target.cid++; target.u=true;
						}
					}
				}
				var dmg_mult=1;
				if(attacker.is_player && target["for"]) dmg_mult=damage_multiplier(target["for"]*5);
				i_attack=attack=ceil(combo_m*attack*(0.90+(attack&&(Math.random()*0.2)||0)));
				attack=ceil(attack*dmg_mult*damage_multiplier((target[defense]||0)-(attacker[pierce]||0)-info.apiercing))||0;

				if(target.incdmgamp) attack=round(attack*(100+target.incdmgamp)/100.0);

				if(target["1hp"]) attack=def.crit&&2||1;
				if(attack>0 && target.s.invincible) attack=0;
				if(attacker.tskin=="konami" && target.type!=attacker.p.target_lock) attack=0;
				...
				if(info.conditions.includes("burned") && !target.immune && target.hp>attack)
					add_condition(target,"burned",{divider:attacker.a && attacker.a.burn && attacker.a.burn.unlimited && 1.5,
						fid:attacker.id,f:attacker.name||G.monsters[attacker.type].name,attack:attack})

				if(info.conditions.includes("woven") && !target.immune)
					add_condition(target,"woven");

				if(info.conditions.includes("stunned") && target.hp>attack && add_condition(target,"stunned",{duration:2000}))
					disappearing_text(target.socket,target,"STUN!",{xy:1,size:"huge",color:"stun",nv:1}); //target.is_player&&"huge"||undefined
				...
				info.conditions.forEach(function(c){
					if(["frozen","burned","woven","stunned"].includes(c)) return;
				...
				if(attacker.s.invis) // && target.is_player
				{
					def.sneak=true;
					disappearing_text(target.socket,target,"SNEAK!",{xy:1,size:"huge",color:"sneak",nv:1});
				}
				if(info.damage_type=="pure") attack=ceil(info.first_attack);
				if(target["1hp"]) attack=def.crit&&2||1;
			}
			else
			{
				attack=o_attack;
				if(target_def[1]=="splash") attack=ceil(attack*target_def[2]);
				if(target["1hp"]) def.damage=o_attack=attack=1;
			}
			if(attack>=1)
			{
				target.last.attacked=new Date();
				if(target.is_monster && attacker.is_player) target.points[attacker.name]=(target.points[attacker.name]||0)+1;
			}
			...
			if(info.lifesteal)
			{
				...
				attacker.hp=min(attacker.max_hp,attacker.hp+hp); change=true;
				if(hp) def.lifesteal=hp;
			}
			if(info.manasteal)
			{
				...
				{
					attacker.mp=min(attacker.max_mp||0,attacker.mp+mp); change=true;
				}
				if(mp) def.manasteal=mp;
			}
		...
		if(target.s.mshield && target.mp>200 && attack>0)
		{
			...
			var max_mp=target.mp-200,damage_per_mp=1.5;
			if(target.level>99) damage_per_mp=3;
			else if(target.level>89) damage_per_mp=2.4;
			else if(target.level>79) damage_per_mp=2;
			else if(target.level>69) damage_per_mp=1.75;
		...
		{
			def.kill=true;
			if(G.skills[atype].kill_buff) add_condition(attacker,G.skills[atype].kill_buff);
		}

		if(target.dreturn && i_attack>0 && first && attacker.range<75 && info.damage_type=="physical" && !attacker["1hp"]) // dreturn happens at every hit
		{
			def.dreturn=ceil(i_attack*target.dreturn/100.0);
			if(attacker.is_monster) attacker.u=true,attacker.cid++;
		...
		if(attacker.is_player)
		{
			...
			if(m && m.is_monster && m.cooperative) add_coop_points(m,attacker,mnet);
		}
		
		if(mode.instant_monster_attacks || attacker.is_player)
			xy_emit(target,"hit",def,attacker.id); // always sends the event to attacker.id
		else
			events.push(["hit",def]);

		if(attacker.is_monster) //monster attacks player
		{
			...
			if(target.hp<=0 && !target.rip)
			{
				...
				{
					if(target.s.block && get_player(target.s.block.f)) pwn_routine(get_player(target.s.block.f),target);
					else defeated_by_a_monster(attacker,target);
				}
			}
			target.c={};
		}
		else if(target.is_monster) //player attacks monster
		{
			achievement_logic_monster_damage(attacker,target,net);
			target.u=true; target.cid++; ccms(target);
			if(target.hp<=0)
			{
				if(atype=="mentalburst") attacker.mp+=net;
			...
			else
			{
				...
				if(target.drop_on_hit) drop_something(attacker,target,1);
				if(!attacker.is_npc && !target.target && !evade && atype!="shadowstrike" && !G.monsters[target.type].passive) target_player(target,attacker);
			}
		}
		else if(target.is_player) //player attacks player
		{
			if(!info.positive && !target.s.invincible)
			{
				...
				if(!is_same(target,attacker,1)) target.s.block.f=attacker.name;
				target.c={};
			}

			if(target.hp<=0 && !target.rip)
			{
				if(atype=="mentalburst") attacker.mp+=net,change=true;
				if(target.a && target.a.secondchance && Math.random()*100<target.a.secondchance.attr0)
				{
					target.hp=target.max_hp;
					disappearing_text(target.socket,target,"SECOND CHANCE!",{xy:1,size:"huge",color:"green",nv:1});
				...
				{
					var victor=attacker;
					if(target.s.block && get_player(target.s.block.f)) victor=get_player(target.s.block.f);
	...
	if(attacker.hp<=0 && !attacker.dead && !attacker.rip) // dreturn
	{
		if(attacker.is_monster) kill_monster(attacker.target&&get_player(attacker.target)||target,attacker); // monster to player
		else if(target.is_monster) defeated_by_a_monster(target,attacker),change=true; // player to monster
		else pwn_routine(target,attacker); // player to player
	}

	if((change || attacker.to_resend) && attacker.is_player)
		resend(attacker,"u+cid");
	if(change && attacker.is_monster && !attacker.dead)
		attacker.u=true,attacker.cid++,ccms(attacker);
}

function target_player(monster,player,no_increase)
{
	// if(is_sdk) console.log("target_player: "+player.name+" "+(!no_increase));
	if(!no_increase && (monster.s.charmed || monster.peaceful)) return;
	if(monster.dead || monster.pet || monster.trap) return;
	monster.target=player.name;
	if(!no_increase) increase_targets(player,monster);
	delete monster.s.sleeping;
	monster.last.attacked=new Date();
	monster.last_level=future_s(Math.random()*100-50);
	monster.ex=monster.x,monster.ey=monster.y;
...
function duel_defeat(player)
{
	...
	info.A.forEach(function(p){ if(p.name==player.name) p.active=false; });
	info.B.forEach(function(p){ if(p.name==player.name) p.active=false; });
	delete player.duel; delete player.team;
}

function resend(player,events)
{
	if(player.halt || player.is_npc) return;
	if(!player.gold && player.gold!==0) player.gold=0,server_log("#X - GOLD BUG resend",1);
	...
	if(in_arr("cid",events)) player.cid++;
	...
	{
		if(current_socket!=player.socket) add_call_cost(call_modifier*4);
		data.reopen=true;
		player.socket.emit("player",data);
		delete player.to_reopen;
	}
	else player.socket.emit("player",data);
	delete player.to_resend;
}
...
function transport_observer_to(observer,to_in,map,x,y)
{
	...
	observer.socket.emit("new_map",{name:instance.map,in:to_in,x:observer.x,y:observer.y,direction:0,effect:0,info:instance.info,m:0,entities:send_all_xy(observer,{raw:true})});
}
...
function transport_player_to(player,name,point,effect)
{
	...
	if(!instances[name]) name="main";
	var instance=instances[name];
	var new_map=G.maps[instance.map],direction=0,scatter=0;
	var data={id:player.id,reason:"transport"};
	if(!is_invis(player) && !player.stealth && G.maps[name]) data.to=name,data.s=point;
	...
	if(!instances[player.in].mount && player.user && !player.mounting && !player.unmounting) player.unmounting=new Date(),sync_loop(); // patches the exit by jail loophole [20/08/18]
	if(Object.keys(player.bets).length && name!="tavern")
	{
		for(var bid in player.bets) player.gold+=player.bets[bid].gold;
		player.bets={};
		resend(player,"reopen+nc");
	}
	...
	{
		if(!new_map.spawns[point||0]) point=0;player.x=new_map.spawns[point||0][0];
	...
	if(0 && !effect && mssince(player.last.transport)>6000)
	{
		...
		for(var i in player.last)
		{
			if(G.skills[i] && player.last[i]>future_ms(-3000))
				player.last[i]=future_ms(3200),EV+="skill_timeout('"+i+"',"+(3200+(G.skills[i].cooldown||0))+"); ";
		}
	}
	if(effect) player.s.penalty_cd={ms:min(((player.s.penalty_cd&&player.s.penalty_cd.ms)||0)+812,120000)};
	else player.s.penalty_cd={ms:min(((player.s.penalty_cd&&player.s.penalty_cd.ms)||0)+3200,120000)};
	player.socket.emit("new_map",{name:instance.map,in:name,x:player.x,y:player.y,direction:direction,effect:effect||0,info:instance.info,m:player.m,entities:send_all_xy(player,{raw:true}),eval:EV});
...
function add_shells(player,amount,reason,announce,override)
{
	...
	var phrase="Received";
	if(reason=="xptome") phrase="Earned";
	player.cash+=amount;
	player.socket.emit("game_log",{message:phrase+" "+to_pretty_num(amount)+" SHELLS",color:"green"});
	disappearing_text(player.socket,player,"+"+to_pretty_num(amount),{color:colors.cash,xy:1,s:"cash",size:"huge"});
	appengine_call("bill_user",{auth:player.auth,amount:-parseInt(amount),reason:reason+"_drop",name:player.name,override:override},function(result){
		if(result.failed || !result.done) return;
		player.cash=result.cash;
		resend(player,"reopen");
	});
	if(announce) broadcast("server_message",{"message":player.name+" found "+to_pretty_num(amount)+" shells","color":"#85C76B","type":"server_found",shells:amount,name:player.name});
}
...
io.on('connection', function (socket) {
	...
	if(!is_socket_allowed(socket)) disconnect_old_sockets(socket);
	...
	socket.on=function(method,f){
		...
		var g=function(data){
			ls_method=method;
			if(mode.log_all) console.log("'"+method+"': "+JSON.stringify(data));
			try{
				var climit=limits.calls,name="_observer";
				if(data===undefined) data={}; // data normalisation [28/08/18]
				...
				if(method=="cm")
				{
					var add=1,len=data.message.length,mult=1;
					if(len>100) add=2;
					else if(len>1000) add=3;
					else if(len>10000) add=10;
					else if(len>50000) add=20;
					// console.log("add: "+add+"len: "+len);
					if(data.to.length>1) mult=0.8;
					add_call_cost(add*data.to.length*mult,undefined,"cm_data");
				}
				else
				{
					if(CC[method]) add_call_cost(CC[method]||0);
				}
				...
				try{
					...
					if(get_call_cost()>climit)
					{
						...
						socket.emit("limitdcreport",{calls:socket.calls,climit:climit,total:socket.total_calls,method:method.substr(0,200)});
						socket.emit("disconnect_reason","limitdc");
						socket.disconnect();
					}
					else
	...
	var data={region:region,name:server_name,pvp:is_pvp,gameplay:gameplay,info:instances[socket.first_map]&&instances[socket.first_map].info||{}};
	...
	if(socket.request && socket.request._query && socket.request._query.secret)
	{
		for(var id in players)
		{
			...
			if(player.secret==socket.request._query.secret)
			{
				...
				data.character=player_to_client(player); data.character.id=data.character.name=player.name;
				socket.first_map=player.map;
				socket.first_in=player.in;
				socket.first_x=player.x;
				socket.first_y=player.y;
				if(socket.request._query.desktop) socket.desktop=true,socket.first_y+=120;
				else socket.desktop=false;
			}
		}
	}
	data.x=socket.first_x; data.y=socket.first_y; data.map=socket.first_map; data.in=socket.first_in;
	broadcast_e(true);
	data.S=E;
	...
	socket.on('loaded',function(data){
		var observer=observers[socket.id]={
			socket:socket,
			x:socket.first_x,y:socket.first_y,
		...
		if(socket.player) observer.player=socket.player;
	...
	socket.on('say',function(data){
		...
		if(data.party)
		{
			if(!player.party) return fail_response("not_in_a_party");
		...
		else if(data.name)
		{
			...
			{
				if(target.name==player.name) return fail_response("invalid");
				...
				appengine_call("log_chat",{to:[target.owner,target.name],type:"private",message:message,fro:player.name,author:player.owner});
			}
		}
		else
		{
			if(1)
			{
				...
				appengine_call("log_chat",{to:Object.entries(owners),type:"ambient",message:message,fro:player.name,author:player.owner});
			}
			else
	...
	socket.on('target',function(data){
		...
		if(!target) target=instances[player.in].players[data.id];
		if(!target) target=instances[player.in].players[NPC_prefix+data.id];
		if(!target) player.target=null;
		var focus=instances[player.in].monsters[data.xid];
		if(!focus) focus=instances[player.in].players[data.xid];
		if(!focus) focus=instances[player.in].players[NPC_prefix+data.xid];
		if(!focus) player.focus=null;
		...
		if(focus && player.map=="cgallery" && focus.npc)
		{
			...
			if(!player.tcx) player.tcx={};
			if(focus.ctype=="body")
				player.tskin=focus.skin;
			else if(player.cx.length>5) return fail_response("invalid");
	...
	socket.on('ureward',function(data){
		if(!player || player.user || !data.name || !G.docs.rewards[data.name]) return;
		if(!player.verified || !player.auth_id) return socket.emit("game_response","reward_notverified");
		if(!player.user.rewards) player.user.rewards=[];
		if(player.user.rewards.includes(data.name)) return socket.emit("game_response","reward_already");
	...
	socket.on('creward',function(data){
		if(!player || !data.name || !G.classes[player.type].rewards[data.name]) return;
		if(!player.verified || !player.auth_id) return socket.emit("game_response","reward_notverified");
		if(!player.p.rewards) player.p.rewards=[];
		if(player.p.rewards.includes(data.name)) return socket.emit("game_response","reward_already");
	...
	socket.on('cx',function(data){
		...
		{
			if(data.slot=="back") delete cx.tail; // synced with render_cgallery
			if(data.slot=="face") delete cx.makeup;
			delete cx[data.slot];
		}
		else
		{
			if(!T[data.name] || !cxl[data.name] && player.role!="cx") return fail_response("cx_not_found");
			if((T[data.name]=="body" || T[data.name]=="armor" || T[data.name]=="character") && data.slot=="skin") player.skin=data.name;
			else if((T[data.name]=="body" || T[data.name]=="armor") && data.slot=="upper") cx.upper=data.name;
	...
	socket.on('gm',function(data){
		...
		else if(action=="jump")
		{
			...
			else return socket.emit("game_log","No safe spot near: "+data.id);
		}
		else if(action=="mjump")
		{
			target=get_monsters(data.monster)[0];
			if(!target) return socket.emit("game_log","Monster not found: "+data.monster);
			var spot=safe_xy_nearby(target.map,target.x-8,target.y-6);
			if(spot)
			{
				pulled=player;
			...
			else return socket.emit("game_log","No safe spot near: "+data.monster);
		}
	...
	socket.on('monsterhunt',function(data){
		...
		if(player.s.monsterhunt && player.s.monsterhunt.c) return fail_response("monsterhunt_already");
		...
		if(player.type=="merchant") return socket.emit("game_response","monsterhunt_merchant");
		var mmax=-1,name="goo",count=100,times=0,the_hp=0;
		for(var id in instances)
		{
			...
			for(var mid in instances[id].monsters)
			{
				...
				if(monster.level>mmax && !in_arr(monster.type,hunted) && !monster.target) // added the target condition [21/07/23]
				{
					name=monster.type;
					mmax=monster.level;
	...
	socket.on('tracker',function(data){
		...
		if(D.drops.maps.global) data.global=D.drops.maps.global,register_table(data.global);
		if(D.drops.maps.global_static) data.global_static=D.drops.maps.global_static,register_table(data.global_static);
		...
		for(var name in G.items)
			if(G.items[name].e && (player.computer || 1 || player.p.stats.exchanges && player.p.stats.exchanges[name] && player.p.stats.exchanges[name]>=100))
			{
	...
	socket.on('property',function(data){
		...
		else
		{
			...
			{
				if(player.afk!==undefined) player.afk=false;
			}
	...
	socket.on('test',function(data){
		if(is_sdk) console.log(data.test);
	...
	socket.on('mail',function(data){
		...
		{
			if(player.items[0].l) return fail_response("item_locked");
			if(player.items[0].b || player.items[0].v) return fail_response("item_blocked");
			if(player.gold<312000) return fail_response("gold_not_enough");
			player.gold-=312000;
	...
	socket.on('transport',function(data){
		...
		(G.maps[player.map].doors||[]).forEach(function(door){
			if(!can_reach && door[4]==data.to && s==(door[5]||0) && simple_distance({map:player.map,x:G.maps[player.map].spawns[door[6]][0],y:G.maps[player.map].spawns[door[6]][1]},player)<B.door_dist)
				can_reach="door",the_door=door;
		});
		if(player.map=="woffice" && gameplay=="hardcore" || player.role=="gm") can_reach="transport"; 
		if(!can_reach && G.maps[player.map].ref.transporter && simple_distance(G.maps[player.map].ref.transporter,player)<B.transporter_dist && G.npcs.transporter.places[data.to]===s) can_reach="transport";
		if(can_reach=="transport" && player.s.dampened) return fail_response("transport_cant_dampened");
		if(!can_reach) return fail_response("transport_cant_reach");
		if(the_door && the_door[7]=="protected")
		{
			var protected=false;
			for(var x in instances[player.in].monsters||{})
			{
				if(instances[player.in].monsters[x].map_def.gatekeeper) protected=true;
			}
			if(protected) return fail_response("transport_cant_protection");
		}
		if(the_door && the_door[7]=="ulocked" && G.maps[data.to].mount && player.user && !player.user.unlocked[data.to])
			return fail_response("transport_cant_locked");

		if(instances[data.to].mount && !player.user)
		{
			if(player.mounting || player.unmounting) return fail_response("bank_opi");
		...
		else if(!instances[data.to].mount && player.user) // previously handled at transport_player_to
		{
			if(player.mounting || player.unmounting) return fail_response("bank_opi");
		...
		else if(0 && instances[data.to].mount && player.user && data.to!=player.map) // an easy prevention for the bank re-entry nuisance, bank can be re-entered physically but not digitally [03/11/16]
		{
	...
	socket.on('enter',function(data){
		...
		else if(data.place=="duelland")
		{
			...
			else return fail_response("cant_enter");
		}
		else if(data.place=="crypt" || data.place=="winter_instance")
		{
			var f="cave",ref=G.maps.cave.spawns[2],item="cryptkey";
			if(data.place=="winter_instance") f="winterland",ref=G.maps.winterland.spawns[5],item="frozenkey";
		...
		{
			var point=null;
			if(player.map=="cgallery") point=[player.x,player.y];
			...
			for(var s in G.sprites)
			{
				...
				if(current.skip || current.rskip) continue;
				if(!in_arr(current.type,["body","head","hair","s_wings","hat"])) continue;
				for(var i=0;i<matrix.length;i++)
					for(var j=0;j<matrix[i].length;j++)
					{
						if(!matrix[i][j]) continue;
						if(current.type=="body") bodies.push(matrix[i][j]);
						if(current.type=="head") heads.push(matrix[i][j]);
						if(current.type=="hair") hairs.push(matrix[i][j]);
						if(current.type=="hat") hats.push(matrix[i][j]);
						if(current.type=="s_wings") wings.push(matrix[i][j]);
			...
			{
				var npc=create_npc({name:bodies[n],level:1,speed:12,hp:1000,skin:bodies[n],cx:{}},{position:[xs[n%xs.length],-parseInt(n/xs.length)*40],id:bodies[n]},instance);
				npc.ctype="body";
				npc.cx={head:heads[n]};
			...
			{
				var npc=create_npc({name:heads[n],level:1,speed:12,hp:1000,skin:bodies[0],cx:{}},{position:[xs[n%xs.length]+240,-parseInt(n/xs.length)*40],id:heads[n]},instance);
				npc.ctype="head";
				npc.cx={head:heads[n]};
				npc.id="head: "+heads[n];
				instance.players[NPC_prefix+npc.id]=npc;
			...
			{
				var npc=create_npc({name:hairs[n],level:1,speed:12,hp:1000,skin:bodies[1],cx:{}},{position:[xs[n%xs.length]+480,-parseInt(n/xs.length)*40],id:hairs[n]},instance);
				npc.ctype="hair";
			...
			{
				var npc=create_npc({name:hats[n],level:1,speed:12,hp:1000,skin:bodies[2],cx:{}},{position:[xs[n%xs.length]+720,-parseInt(n/xs.length)*40],id:hats[n]},instance);
				npc.ctype="hat";
	...
	socket.on('respawn',function(data){
		...
		player.rip=false;
		if(gameplay=="hardcore") reset_player(player,1);

		if(data && data.safe) // gameplay=="hardcore" && 
		{
		...
		if(player.party) send_party_update(player.party);
	...
	socket.on('random_look',function(data){
		...
		var head="head";
		if(Math.random()<0.1) head=random_one(heads);
		player.tskin=random_one(bodies);
		player.tcx=[head,random_one(hairs)];
		if(Math.random()<0.08) player.tcx.push(random_one(wings));
		if(Math.random()<0.4) player.tcx.push(random_one(hats));
		resend(player,"u+cid");
	});
	socket.on('unlock',function(data){
		...
		if(data.name=="code")
		{
			...
			if(!item || item.name!="computer" || item.charges===0 || item.charges<0 || player.unlocking_code) return;
			if(!item.charges) item.charges=2;
			item.charges--; player.citems[data.num||0]=player.items[data.num||0]; // explicitly linked to the actual item
			player.unlocking_code=true;
			appengine_call("user_operation",{auth:player.auth,operation:"code_unlock",suffix:"/code_unlock/"+player.owner,retries:4},function(result){
				server_log("user_operation_code: "+JSON.stringify(result),1);
				if(result.failed)
				{
					if(result.reason=="already")
					{
						socket.emit("game_log","Your CODE slots are already unlocked. You can lend your Ancient Computer to a friend in need, that's why there are 2 charges :]");
						item.charges++;
					}
					else socket.emit("game_log","Unlock Failed. Email hello@adventure.land with a screenshot.");
					return;
				}
	...
	socket.on('dismantle',function(data){
		...
		if(item && item.level && G.items[item.name].compound)
		{
			...
			if(player.gold<cost) return fail_response("gold_not_enough");
			if(player.esize<2) return fail_response("inv_size");
			if(G.items[item.name].type=="booster") return fail_response("dismantle_cant");
			player.gold-=cost;
			consume_one(player,data.num);
			add_item(player,{name:item.name,level:item.level-1,grace:parseInt((item.grace||0)/3)},{announce:false});
			add_item(player,{name:item.name,level:item.level-1,grace:parseInt((item.grace||0)/3)},{announce:false});
			add_item(player,{name:item.name,level:item.level-1,grace:parseInt((item.grace||0)/3)},{announce:false});
			resend(player,"reopen+nc+inv");
			return success_response("dismantle",{name:item.name,level:item.level,cost:cost,cevent:true});
		}
		if(!item || !G.dismantle[item.name]) return fail_response("dismantle_cant");
		if(item.l) return fail_response("item_locked");
		if(item.b) return fail_response("item_blocked");
		if(player.gold<G.dismantle[item.name].cost) return fail_response("gold_not_enough");
		if(!can_add_items(player,list_to_pseudo_items(G.dismantle[item.name].items),{space:((item.q||1)==1&&1||0)}))
			return fail_response("inv_size");
		player.gold-=G.dismantle[item.name].cost;
		consume_one(player,data.num);
		G.dismantle[item.name].items.forEach(function(e){
			if(e[0]<1 && Math.random()>e[0]) return;
	...
	socket.on('craft',function(data){
		...
		data.items.forEach(function(x){
			...
			else if(player.items[x[1]].l || player.items[x[1]].b) check=false,locked=true;
			else
			{
				var name=player.items[x[1]].name;
				if(player.items[x[1]].level) name+="+"+player.items[x[1]].level;
				items.push(name);
				...
				if(player.items[x[1]].p && !G.titles[player.items[x[1]].p].misc) p[player.items[x[1]].p]=player.items[x[1]].p;
			}
		});
		if(locked) return fail_response("item_locked")
		if(!check) return fail_response("no_item");
		//if(items.length<2) return socket.emit("game_response","craft_atleast2");
		items.sort();
		var key=items.join(",");
		// console.log(key);
		if(!D.craftmap[key]) return fail_response("craft_cant");
		var name=D.craftmap[key],enough=true;
		if(!player.computer && !G.craft[name].quest && simple_distance(get_npc_coords('craftsman'),player)>B.sell_dist) return fail_response("distance");
		if(!player.computer && G.craft[name].quest && simple_distance(get_npc_coords(G.craft[name].quest),player)>B.sell_dist) return fail_response("distance");
		if(player.gold<G.craft[name].cost) return fail_response("gold_not_enough");
		G.craft[name].items.forEach(function(i){
			if(quantity[i[1]]<i[0]) enough=false;
			if(quantity[i[1]]==i[0]) space=true;
		});
		if(!space && !can_add_item(player,create_new_item(name))) return fail_response("inventory_full");
		if(!enough) return fail_response("craft_cant_quantity");
	...
	socket.on('exchange',function(data){
		...
		if(!def || !def.e || item.q!=data.q || !D.drops[item.name+suffix]) return fail_response("invalid");
		if(item.l) return fail_response("item_locked");
		if(!player.computer && !def.quest && simple_distance(G.maps.main.exchange,player)>B.sell_dist) return fail_response("distance");
		if(!player.computer && def.quest && simple_distance(G.quests[def.quest],player)>B.sell_dist) return fail_response("distance");
		if(player.esize<=0 && !((item.q||1)==1)) return fail_response("inventory_full");
		if(def.e>1 && item.q<def.e) return fail_response("exchange_notenough");
		...
		var num=add_item(player,"placeholder"),ms=3000+parseInt(Math.random()*3000);
		if(gameplay=="hardcore") ms=400;
		player.q.exchange={ms:ms,len:ms,name:item.name,id:item.name+suffix,q:def.e,num:num};
		if(suffix) player.q.exchange.s=suffix;
		if(item.v) player.q.exchange.v=item.v;
		if(def.quest) player.q.exchange.qs=def.quest;
		resend(player,"reopen+nc+inv");
	...
	socket.on('exchange_buy',function(data){
		...
		if(!G.tokens[item.name][data.name]) return fail_response("invalid");
		if(item.l) return fail_response("item_locked");
		npc.name=player.name;
		if(item.q!=data.q) return fail_response("safety_check");
		if(!player.computer && simple_distance(npc,player)>B.sell_dist) return fail_response("distance");
		if(item.q<G.tokens[item.name][data.name]) return fail_response("exchange_notenough");

		if(G.tokens[item.name][data.name]<1)
		{
			var q=parseInt(1/G.tokens[item.name][data.name]);
			if(item.q!=1 && !can_add_item(player,{name:data.name,q:q})) return fail_response("inventory_full");
			consume(player,data.num,1);
			num=add_item(player,data.name,{q:q,announce:false,r:true});
		}
		else
		{
			var name=data.name,idata=undefined;
			if(name.search("-")!=-1) { idata=name.split("-")[1]; name=name.split("-")[0]; }
			var new_item=create_new_item(name);
			if(idata) new_item.data=idata;
			if(item.q!=G.tokens[item.name][data.name] && !can_add_item(player,new_item)) return fail_response("inventory_full");
	...
	socket.on('locksmith',function(data){
		...
		if(in_arr(def.type,["uscroll","cscroll","pscroll","offering","tome"])) return fail_response("locksmith_cant");
		if(data.operation=="unlock")
		{
			if(!item.l) { resend(player,"reopen+nc"); return fail_response("locksmith_aunlocked","locksmith","already_unlocked"); }
			if(item.l=="s")
			{
				if(player.gold<250000) return fail_response("gold_not_enough");
				player.gold-=250000;
		...
		{
			if(item.l) { resend(player,"reopen+nc"); return fail_response("locksmith_alocked","locksmith","already_locked"); }
			if(player.gold<250000) return fail_response("gold_not_enough");
			player.gold-=250000;
			item.l="l";
	...
	socket.on('compound',function(data){
		try{
			...
			if(grade==4) return socket.emit("game_response",{response:"max_level",level:item.level,place:"compound",failed:true});
			if(!(item0.name == item1.name && item1.name==item2.name && (item0.level||0)==(item1.level||0) && (item1.level||0)==(item2.level||0) )) return socket.emit("game_response","compound_mismatch"); 
			if(!def.compound) return socket.emit("game_response","compound_cant");
			if(scroll_def.type!="cscroll" || grade>scroll_def.grade) return socket.emit("game_response","compound_incompatible_scroll");
			if(data.items[0]==data.items[1] || data.items[1]==data.items[2] || data.items[0]==data.items[2]) return socket.emit("game_response",{response:"misc_fail",place:"compound",failed:true});
			if(item0.l || item1.l || item2.l) return socket.emit("game_response",{response:"item_locked",place:"compound",failed:true});

			if(!data.calculate) consume_one(player,data.scroll_num);

			var new_level=(item0.level||0)+1,probability=1,oprobability=1,result=0,proc=0,grace=0,igrade=def.igrade,high=false,grace_bonus=0;
			if(item0.level>=3) igrade=calculate_item_grade(def,{name:item0.name,level:item0.level-2});

			delete player.p.c_item; delete player.p.c_itemx; delete player.p.c_roll;
			...
			if(scroll_def.grade > grade)
			{
				...
				if(!data.calculate) grace_bonus+=0.4;
			...
			{
				var increase=0.5;
				if(!data.calculate) consume_one(player,data.offering_num);
				grace=0.027*((item0.grace||0)+(item1.grace||0)+(item2.grace||0)+0.5+player.p.ograce);

				if(offering_def.grade>grade+1)
					probability=probability*1.64+grace*2,high=true,increase=3;
				else if(offering_def.grade>grade)
					probability=probability*1.48+grace,high=true,increase=1;
				else if(offering_def.grade==grade)
					probability=probability*1.36+min(30*0.027,grace);
				else if(offering_def.grade==grade-1)
					probability=probability*1.15+min(25*0.019,grace)/max(item0.level-2,1),increase=0.2;
				else
					probability=probability*1.08+min(15*0.015,grace)/max(item0.level-1,1),increase=0.1;

				if(!data.calculate) item0.grace=(item0.grace||0)+(item1.grace||0)+(item2.grace||0),grace_bonus+=increase;
			...
			else
			{
				...
				if(!data.calculate) item0.grace=max(max((item0.grace||0),(item1.grace||0)),(item2.grace||0));
			}

			if(!data.calculate) item0.grace=item0.grace/6.4+grace_bonus;

			if(def.type=="booster") probability=0.9999999999999,proc=offering&&0.12;
			else probability=min(probability,min(oprobability*(3+(high&&high*0.6||0)),oprobability+0.2+(high&&high*0.05||0)));

			if(gameplay=="test") result=0;

			server_log(result+" < "+probability+" grace: "+grace);

			if(data.calculate) return success_response("compound_chance",{calculate:true,chance:probability,item:cache_item(item0),scroll:scroll.name,offering:offering&&offering.name||undefined,grace:(item0.grace||0)+(item1.grace||0)+(item2.grace||0)});

			player.p.c_roll=result;
			var len=10000;
			if(gameplay=="hardcore") len=1200;
			if(player.s.massproduction) { len/=2; delete player.s.massproduction; ex="+u+cid"; }
			if(player.s.massproductionpp) { len/=10; delete player.s.massproductionpp; ex="+u+cid"; }
			player.q.compound={ms:len,len:len,num:data.items[0],nums:[]};
			player.items[data.items[0]]={"name":"placeholder",p:{chance:probability,name:item0.name,level:item0.level,scroll:scroll.name,offering:offering&&offering.name,nums:[]}};
			player.items[data.items[1]]=null;
			player.items[data.items[2]]=null;
				
			if(result<=probability)
			{
				if(offering) player.p.ograce=0;
				else player.p.ograce*=1-new_level*0.02;
				item0.level=new_level;
				if((item0.p||item1.p||item2.p)!="legacy") item0.p=(item0.p||item1.p||item2.p);
				else { delete item0.p; }
				player.p.c_item=item0;
				if(item0.oo!=player.name) item0.o=player.name;
				player.esize+=2;
				if(parseInt(result*10000)==parseInt(probability*10000)) // && grade>=1
						add_achievement(player,"lucky"),add_item_property(item0,"lucky");
				if(item1.v||item2.v) item0.v=item1.v||item2.v;
				if(item0.name=="ctristone" && item0.level==1 && Math.random()<0.012) item0.name="cdarktristone";
				if(def.type=="booster")
				{
					var activate=false; item0.extra=0;
					...
					[item0,item1,item2].forEach(function(i){
						if(i.expires) seconds-=ssince(i.expires),activate=true;
						else seconds+=def.days*24*60*60;
					});
					if(activate) item0.expires=future_s(seconds/3);
				}
			}
			else
			{
				if(offering) player.p.ograce+=0.4;
				player.esize+=2;
	...
	socket.on('upgrade',function(data){
		try{
			...
			if(!player.computer && simple_distance(G.maps.main.upgrade,player)>B.sell_dist) return socket.emit("game_response",{response:"distance",place:"upgrade",failed:true});
			if(!item) return socket.emit("game_response","upgrade_no_item");
			if(offering && !(G.items[offering.name] && G.items[offering.name].type=="offering" || G.items[offering.name] && G.items[offering.name].offering!==undefined && !(item.level>0))) return socket.emit("game_response","upgrade_invalid_offering");
			if(!scroll && !offering) return socket.emit("game_response","upgrade_no_scroll");
			if((item.level||0)!=data.clevel) return socket.emit("game_response","upgrade_mismatch");
			var item_def=G.items[item.name],scroll_def=scroll&&G.items[scroll.name],offering_def=offering&&G.items[offering.name],grade=calculate_item_grade(item_def,item);
			if(!item_def.upgrade) return socket.emit("game_response","upgrade_cant");
			if(scroll && (!in_arr(scroll_def.type,["uscroll","pscroll"]) || (scroll_def.type=="uscroll" && !item_def.upgrade) || (scroll_def.type=="pscroll" && !item_def.stat) || grade>scroll_def.grade))
			{
				if(grade==4 && scroll_def.type=="uscroll") return socket.emit("game_response",{response:"max_level",level:item.level,place:"upgrade",failed:true});
				return socket.emit("game_response","upgrade_incompatible_scroll");
			}

			var new_level=(item.level||0)+1,probability=1,oprobability=1,grace=0,high=false;
			var ograde=calculate_item_grade(item_def,{name:item.name,level:0}),tmult=1;
			if(ograde==1) tmult=1.5; else if(ograde==2) tmult=2;

			delete player.p.u_item; delete player.p.u_type; delete player.p.u_itemx; delete player.p.u_roll; delete player.p.u_fail; delete player.p.u_level;
			...
			if(!scroll)
			{
				...
				{
					var chance=0.16,ms=2000;
					if(G.items[offering.name].offering>ograde || G.items[offering.name].offering==2 && calculate_item_value(item)<=20000000) chance=0.32;
					chance*=[2.8,1.6,1][ograde];
					if(G.items[offering.name].offering<ograde) return socket.emit("game_response","upgrade_invalid_offering");
					if(data.calculate) return success_response("upgrade_chance",{calculate:true,chance:chance,offering:offering.name,item:cache_item(item),grace:item.grace||0});
					var result=Math.random();
						
					consume_one(player,data.offering_num);
					player.p.u_type="normal";
					player.p.u_roll=result;
					if(player.s.massproduction) { ms/=2; delete player.s.massproduction; ex="+u+cid"; }
					if(player.s.massproductionpp) { ms/=10; delete player.s.massproductionpp; ex="+u+cid"; }
					player.q.upgrade={ms:ms,len:ms,num:data.item_num,silent:true};
					player.items[data.item_num]={"name":"placeholder",p:{chance:chance,name:item.name,level:item.level,scroll:null,offering:offering.name,nums:[]}};
				...
				{
					var ms=1000;
					if(data.calculate) return success_response("upgrade_chance",{calculate:true,chance:1,offering:offering.name,item:cache_item(item),grace:item.grace||0});
					consume_one(player,data.offering_num);
					...
					player.items[data.item_num]={"name":"placeholder",p:{chance:1,name:item.name,level:item.level,scroll:null,offering:offering.name,nums:[]}};
				}
			}
			else if(scroll_def.type=="uscroll")
			{
				...
				grace=max(0,min(new_level+1, (item.grace||0) + min(3,player.p.ugrace[new_level]/4.5) +item_def.igrace) + min(6,S.ugrace[new_level]/3.0) + player.p.ograce/3.2);
				server_log("Grace num: "+grace+"\nItem: "+(item.grace||0)+"\nPlayer: "+min(3,player.p.ugrace[new_level]/4.5)+"\nDef: "+item_def.igrace+"\nOgrace:"+(player.p.ograce/3.2)+"\nS.ugrace: "+min(6,S.ugrace[new_level]/3.0));
				...
				if(offering)
				{
					...
					if(offering_def.grade>grade+1)
						probability=probability*1.7+grace*4,high=true,increase=3;
					else if(offering_def.grade>grade)
						probability=probability*1.5+grace*1.2,high=true,increase=1;
					else if(offering_def.grade==grade)
						probability=probability*1.4+grace;
					else if(offering_def.grade==grade-1)
						probability=probability*1.15+grace/3.2,increase=0.2;
					else
						probability=probability*1.08+grace/4,increase=0.1;

					if(!data.calculate) item.grace=(item.grace||0)+increase; // previously +1 [16/07/18]
				}
				else
				{
				...
				if(!data.calculate && Math.random()<0.025) // Bonus grace
					item.grace=(item.grace||0)+1;

				if(data.item_num==player.p.item_num && Math.random()<0.6) // Added [29/10/17]
				{
					server_log("16 cheat");
					result=max(Math.random()/10000.0,result*0.975-0.012);
				}

				if(high) probability=min(probability,min(oprobability+0.36,oprobability*3));
				else probability=min(probability,min(oprobability+0.24,oprobability*2));
				server_log(result+" < "+probability+" grace: "+grace);

				if(data.calculate) return success_response("upgrade_chance",{calculate:true,chance:probability,offering:offering&&offering.name||undefined,item:cache_item(item),grace:item.grace||0,scroll:scroll.name});
				...
				if(player.s.massproductionpp) { ms/=10; delete player.s.massproductionpp; ex="+u+cid"; }
				player.q.upgrade={ms:ms,len:ms,num:data.item_num};
				if(gameplay=="hardcore") player.q.upgrade.ms=player.q.upgrade.len=500;
				player.items[data.item_num]={"name":"placeholder",p:{chance:probability,name:item.name,level:item.level,scroll:scroll.name,offering:offering&&offering.name,nums:[]}};
				...
				if(result<=probability)
				{
					...
					if(offering) player.p.ograce*=0.25;
					else player.p.ograce*=1-new_level*0.005;
					item.level=new_level;
					if(item.oo!=player.name) item.o=player.name;
					player.p.u_item=item;
					if(parseInt(result*10000)==parseInt(probability*10000) && grade>=1)
					{
				...
				{
					player.p.ugrace[new_level-1]+=1; S.ugrace[new_level-1]+=1
					player.p.ugrace[new_level]+=1; S.ugrace[new_level]+=1;
					if(new_level>=8 && new_level<=15)
					{
						player.p.ugrace[new_level-1]+=1; S.ugrace[new_level-1]+=1;
						player.p.ugrace[new_level-2]+=2+(offering&&1||0); S.ugrace[new_level-2]+=2;
						player.p.ugrace[new_level-3]+=2+(offering&&2||0); S.ugrace[new_level-3]+=3+(offering&&1||0);
					}
					if(offering) player.p.ograce+=0.6; // previously 1 [16/07/18]
					if(scroll_def.grade!=3.6) player.p.u_itemx=item;
					else player.p.u_item=item,player.p.u_fail=true;
					if(parseInt(result*10000)==parseInt(probability*10000))
					{
						if(player.esize && grade>=1) add_item(player,"essenceofgreed");
			...
			else if(scroll_def.type=="pscroll")
			{
				...
				if(scroll.q<needed[grade]) return socket.emit("game_response",{response:"upgrade_scroll_q",q:needed[grade],h:scroll.q});
				if(!data.calculate) consume(player,data.scroll_num,needed[grade]);

				if(data.calculate) return success_response("upgrade_chance",{calculate:true,chance:0.99999,scroll:scroll.name,item:cache_item(item),grace:item.grace||0});
				...
				if(player.p.u_roll<=0.99999 || offering) player.p.u_item=item;
				else player.p.u_itemx=item;
				var ms=2000*tmult*tmult;
				if(player.s.massproduction) { ms/=2; delete player.s.massproduction; ex="+u+cid"; }
				if(player.s.massproductionpp) { ms/=10; delete player.s.massproductionpp; ex="+u+cid"; }
				player.q.upgrade={ms:ms,len:ms,num:data.item_num};
				player.items[data.item_num]={"name":"placeholder",p:{chance:0.99999,name:item.name,level:item.level,scroll:scroll.name,offering:offering&&offering.name,nums:[]}};
			}
	...
	socket.on('equip',function(data){
		...
		var def=G.items[item.name],to_update="reopen+u+cid",resolve={num:data.num},resolve_type="data";
		def.iname=item.name; //just for orb name checks [08/10/16]
		if(!def) return fail_response("no_item");
		...
		{
			if(item.acl || item.v) return fail_response("item_locked");
			var slot=data.slot,price=round(min(99999999999,max(parseInt(data.price)||1,1))),minutes=1;
			data.q=max(1,parseInt(data.q)||1);
			if((item.q||1)<data.q) return fail_response("not_enough");
			if(data.giveaway) minutes=max(5,min(600,min(parseInt(data.minutes)||5,max(60,round( calculate_item_value(item)*data.q/40000 )))));
			if(!price || data.giveaway) price=1;
			if(player.slots[slot]) return fail_response("slot_occuppied");
			if(def.s)
			{
				...
				if(player.items[data.num].data) player.slots[slot].data=player.items[data.num].data;
				if(data.giveaway) player.slots[slot].giveaway=minutes,player.slots[slot].list=[],player.giveaway=true;
				player.cslots[slot]=cache_item(player.slots[slot],true);
				consume(player,data.num,data.q);
				if(data.giveaway) socket.emit("game_log","Listed "+data.q+" "+item_name(player.slots[slot])+" to giveaway!");
				else socket.emit("game_log","Listed "+data.q+" "+item_name(player.slots[slot])+" at "+to_pretty_num(price)+" gold each");
			}
			else
			{
				...
				if(data.giveaway) player.slots[slot].giveaway=minutes,player.slots[slot].list=[],player.giveaway=true;
				player.cslots[slot]=cache_item(player.slots[slot],true);
				player.items[data.num]=null; player.citems[data.num]=null;
				if(data.giveaway) socket.emit("game_log","Listed "+item_name(player.slots[slot])+" to giveaway!");
				else socket.emit("game_log","Listed "+item_name(player.slots[slot])+" at "+to_pretty_num(price)+" gold");
			}
			resolve.slot=slot;
		}
		...
		{
			if(item.l) return fail_response("item_locked");
			if(player.slots.elixir && G.items[player.slots.elixir.name].withdrawal) add_condition(player,G.items[player.slots.elixir.name].withdrawal);
			...
			if(G.items[item.name] && G.items[item.name].withdrawal && player.s.withdrawal) delete player.s.withdrawal;
		...
		{
			if(!item.data || item.l) return fail_response("item_locked");
		...
		else if(item.name=="emotionjar")
		{
			...
			socket.emit("game_response",{response:"emotion_new",emx:player.p.emx,name:item.data,from:"emotionjar"});
			consume_one(player,data.num);
		}
		...
		else if(def.gives)
		{
			...
			if(item.l) return fail_response("item_locked");
			var timeout=2000,timeout_ui=null,xp=false;
			consume_one(player,data.num);
			(def.gives||[]).forEach(function(p){
				var amount=p[1];
				if(player.s.poisoned) amount=round(amount/2);
				player[p[0]]=(player[p[0]]||0)+amount;
				player[p[0]]=max(1,player[p[0]]);
				if(p[0]=="hp")
				{
					if(amount>0) disappearing_text(socket,player,"+"+amount,{color:"hp",xy:1,s:"hp"});
					else disappearing_text(socket,player,amount,{color:"hp",xy:1,s:"hp"});
				}
				if(p[0]=="mp") disappearing_text(socket,player,"+"+amount,{color:"mp",xy:1,s:"mp"});
				if(p[0]=="xp") disappearing_text(socket,player,"+1M",{color:"1mxp",xy:1,s:"xp",size:"large"}),xp=true,timeout_ui=120,timeout=0.1;
			});
			if(def.debuff)
			{
			...
			player.hp=min(player.hp,player.max_hp); player.mp=min(player.mp,player.max_mp);
			if(def.cooldown) timeout=def.cooldown;
			player.last.potion=future_ms(timeout);
			if(!xp) to_update="u+cid+reopen+nc";
			socket.emit("eval",{code:"pot_timeout("+(timeout_ui||timeout)+")"});
		}
		else if(!data.consume)
		{
			// console.log(data);
			var slot=data.slot||def.type,existing;
			...
			player.slots[slot]=player.items[data.num]; player.cslots[slot]=cache_item(player.slots[slot]);
			if(existing && existing.b) existing=null;
			player.items[data.num]=existing,player.citems[data.num]=cache_item(existing);
		...
		if(to_update) resend(player,to_update);
	...
	socket.on('unequip',function(data){
		...
		if(in_arr(data.slot,trade_slots) && item.giveaway!==undefined) return fail_response("giveaway");
		// an oversight allowed .giveaway items to be equipped, so now they can be unequipped from regular slots [04/11/21]
		if(player.esize<=0 && !item.b) return fail_response("no_space");
		player.slots[data.slot]=null; player.cslots[data.slot]=null;
		player.c={};
		if(!item.b) add_item(player,item,{announce:false});
	...
	socket.on('split',function(data){
		...
		if(!player.esize) return fail_response("cant_space");
		if(item.q==quantity) return success_response();
		if(item.name=="placeholder") return fail_response("item_placeholder");
		if(item.l) return fail_response("item_locked");
		if(item.b) return fail_response("item_blocked");
	...
	socket.on('sell',function(data){
		...
		(G.maps[player.map].merchants||[]).forEach(function(m){
			if(simple_distance(player,m)<B.sell_dist) can_reach=m;
		});
		if(player.computer && !can_reach) can_reach=G.maps.main.merchants[0];
		if(!can_reach) return fail_response("distance");
		can_reach.name=player.name;
		var value=calculate_item_value(item);
		consume(player,data.num,quantity);
		player.gold+=value*quantity;
		var new_item=cache_item(item); if(item.q) new_item.q=quantity;
		xy_emit(can_reach,"ui",{type:"-$",id:can_reach.id,name:player.name,item:new_item,num:num,event:"sell"});
		resend(player,"reopen+nc+inv");
	...
	socket.on('buy_shells',function(data){
		...
		appengine_call("bill_user",{auth:player.auth,amount:-shells,reason:"buy_shells",name:player.name},function(result){
			server_log("buy_with_cash: "+JSON.stringify(result));
			if(result.failed || !result.done) { socket.emit("game_log","Purchase failed"); return; }
			player.cash=result.cash;
	...
	socket.on('buy_with_cash',function(data){
		...
		if(def.p2w) return fail_response("invalid");
		if(!def.s) quantity=1;
		if(!can_add_item(player,create_new_item(name,quantity))) return fail_response("no_space");
		appengine_call("bill_user",{auth:player.auth,amount:cost,reason:data.name,name:player.name},function(result){
			server_log("buy_with_cash: "+JSON.stringify(result));
			if(result.failed || !result.done) { socket.emit("game_log","Purchase failed"); return; }
			player.cash=result.cash;
			var new_item=create_new_item(name,quantity),done=false;
			add_item(player,new_item,{announce:false});
	...
	socket.on('sbuy',function(data){
		...
		if(simple_distance(npc,player)>500) return socket.emit("game_response","distance");
		for(var i=0;i<c.length;i++)
		{
			if(c[i].rid==data.rid)
			{
				if(mult==2 && G.items[c[i].name].cash) mult=3;
				var gold=calculate_item_value(c[i])*mult*(c[i].q||1),item=c[i];
				if(!can_add_item(player,c[i])) return disappearing_text(socket,player,"NO SPACE");
				if(gold>player.gold) return socket.emit("game_response","buy_cost");
				player.gold-=gold;
				item.src=src;
	...
	socket.on('buy',function(data){
		...
		if(!can_add_item(player,create_new_item(name,quantity))) return fail_response("buy_cant_space");
		(G.maps[player.map].items[name]||[]).forEach(function(l){
			if(simple_distance(player,l)<B.sell_dist) can_reach=l;
		});
		if(player.computer && !can_reach) can_reach=G.maps.main.merchants[0];
		if(!can_reach) return fail_response("distance");
		can_reach.name=player.name;
		...
		if(G.items[name].p2w) cost*=G.inflation;
		if(player.gold<cost) return fail_response("buy_cost");
		var new_item=create_new_item(name,quantity);
		player.gold-=cost;
		var num=add_item(player,new_item,{announce:false});
		xy_emit(can_reach,"ui",{type:"+$",id:can_reach.id,name:player.name,item:cache_item(new_item),event:"buy"});

		resend(player,"reopen+nc+inv");
		success_response("buy_success",{cost:cost,num:num,name:name,q:quantity,cevent:"buy"});
	});
	socket.on('send',function(data){
		var player=players[socket.id],receiver=players[name_to_id.get(data.name||"")],num,s_item;
		if(!player || player.user) return fail_response("cant_in_bank");
		...
		if(data.num!==undefined)
		{
			...
			var item=player.items[data.num];
			if(!item) return fail_response("send_no_item");
			if(item.name=="placeholder") return fail_response("item_placeholder");
			if(item.l || item.acl && receiver.owner!=player.owner) return fail_response("item_locked");
			if(item.b) return fail_response("item_blocked");
			data.q=min((item.q||1),max(1,parseInt(data.q||1)||1));
			if(!data.q) return fail_response("no_item");
			if(!can_add_item(receiver,create_new_item(item.name,data.q))) return fail_response("send_no_space");
			if((item.q||1)==data.q) player.items[data.num]=player.citems[data.num]=null,player.esize++;
			else
			{
			...
			if(item.q)
			{
				...
				if(item.v) s_item.v=item.v;
				if(item.data) s_item.data=item.data;
			...
			if(receiver.owner!=player.owner) item.src="snd";
			...
			xy_emit(player,"ui",{type:"item_sent",receiver:receiver.name,sender:player.name,item:cache_item(s_item,null,{q:data.q}),num:num,fnum:data.num,event:true});

			resend(player,"reopen+nc+inv"); resend(receiver,"reopen+nc+inv");
			receiver.socket.emit("game_response",{response:"item_received",name:player.name,item:item.name,q:data.q,num:num,cevent:true});
			player.socket.emit("game_response",{response:"item_sent",name:receiver.name,item:item.name,q:data.q,num:data.num,cevent:true,place:"send"});
		}
		else if(data.gold!==undefined)
		{
			data.gold=min(player.gold,max(1,parseInt(data.gold||1)||1))||player.gold;
			if(!data.gold) return fail_response("invalid");
			player.gold-=data.gold;
			if(!is_same(player,receiver) && data.gold!=1) data.gold=parseInt(data.gold*0.975);
			...
			xy_emit(player,"ui",{type:"gold_sent",receiver:receiver.name,sender:player.name,gold:data.gold,event:true});

			resend(player,"reopen+nc+inv"); resend(receiver,"reopen+nc+inv");
			receiver.socket.emit("game_response",{response:"gold_received",name:player.name,gold:data.gold,cevent:true});
			player.socket.emit("game_response",{response:"gold_sent",name:receiver.name,gold:data.gold,cevent:true,place:"send"});
		}
		else if(data.cx!==undefined)
		{
			var cxl=map_cx(player),count=all_cx(player,1);
			if(!(cxl[data.cx] && player.p.acx[cxl[data.cx]] && count[data.cx]>0)) return fail_response("send_no_cx");
			// count[cxl[data.cx]] before [28/01/22]
			if(receiver.owner!=player.owner) return fail_response("send_diff_owner");
			player.p.acx[cxl[data.cx]]-=1;
			if(!player.p.acx[cxl[data.cx]]) delete player.p.acx[cxl[data.cx]];
			receiver.p.acx[cxl[data.cx]]=(receiver.p.acx[cxl[data.cx]]||0)+1;

			xy_emit(player,"ui",{type:"cx_sent",receiver:receiver.name,sender:player.name,cx:data.cx,event:true});

			receiver.socket.emit("game_response",{response:"cx_received",name:player.name,cx:data.cx,acx:receiver.p.acx,cevent:true});
			player.socket.emit("game_response",{response:"cx_sent",name:receiver.name,cx:data.cx,acx:player.p.acx,cevent:true,place:"send"});
		}
	});
	socket.on('donate',function(data){
		...
		if(gold>=1000000) response="thx",player.donation=true;
		else if(gold<100000) response="low";
		else add_item(player,"gum"),response="gum";
		if(S.gold<500000000) XPX=4.8;
		else if(S.gold<=1000000000) XPX=4;
		player.gold-=gold;
		S.gold+=gold;
		if(gold>=5000000) lstack(S.logs.donate,{name:player.name,gold:gold,xp:XPX});
	...
	socket.on('join_giveaway',function(data){
		...
		if(seller.user) return fail_response("cant_in_bank")
		if(distance(seller,player,true)>B.dist || seller.map!=player.map) return fail_response("distance");
		if(seller.id==player.id) return fail_response("hmm");
		var item=seller.slots[data.slot];
		if(!item || (data.rid && item.rid!=data.rid)) return fail_response("item_gone");
		if(!item.giveaway) return fail_response("sneaky");
		if(!player.auth_id) return fail_response("need_auth");
		...
		seller.socket.emit("game_response",{response:"giveaway_join",name:player.name,slot:data.slot,cevent:true});

		resend(player,"reopen"); resend(seller,"reopen+u+cid");
		success_response({});
	});
	socket.on('trade_wishlist',function(data){
		...
		if(!get_trade_slots(player).includes(data.slot)) return fail_response("invalid");
		var item={name:data.name,rid:randomStr(4),price:round(min(99999999999,max(parseInt(data.price)||1,1))),b:true};
		if(G.items[data.name].upgrade || G.items[data.name].compound) item.q=min(99,data.q),item.level=round(min(12,max(parseInt(data.level)||0,0)));
		else item.q=data.q;
		player.slots[data.slot]=item; player.cslots[data.slot]=cache_item(player.slots[data.slot],true);
	...
	socket.on('trade_sell',function(data){
		...
		if(!player || player.user) return fail_response("cant_in_bank");
		if(!in_arr(data.slot,trade_slots)) return fail_response("invalid");
		if(!buyer || buyer.npc || is_invis(buyer)) return fail_response("buyer_gone");
		if(player.user) return fail_response("cant_in_bank");
		if(distance(buyer,player,true)>B.dist) return fail_response("distance");
		if(buyer.id==player.id) return fail_response("hmm");
		var item=buyer.slots[data.slot];
		if(!item || (data.rid && item.rid!=data.rid)) return fail_response("item_gone");
		if(item.name=="placeholder") return fail_response("item_placeholder");
		if(!item.b && !B.rbugs) return fail_response("sneaky");
		if((item.q||1)<data.q) return fail_response("dont_have_enough");
		if(item.price*data.q>buyer.gold) return fail_response("buyer_gold");
		for(var i=0;i<player.isize;i++)
		{
			if(player.items[i] && player.items[i].name==item.name && (player.items[i].q||1)>=data.q && player.items[i].level==item.level && !player.items[i].l)
			{
				actual=player.items[i]; num=i; break;
			}
		}
		if(!actual) return fail_response("no_item");
		if(actual.b) return fail_response("item_blocked");
		if(!can_add_item(buyer,create_new_item(item.name,data.q))) return fail_response("trade_bspace");
		var price=item.price*data.q;
		player.gold+=round(price*(1-player.tax)); add_to_trade_history(player,"sell",buyer.name,cache_item(actual,true,{q:data.q}),price);
		buyer.gold-=price; add_to_trade_history(buyer,"buy",player.name,cache_item(actual,true,{q:data.q}),price);
		S.gold+=price-round(price*(1-player.tax));

		if((item.q||1)==data.q) buyer.slots[data.slot]=buyer.cslots[data.slot]=null;
		else
		{
		...
		if((player.items[i].q||1)==data.q) player.items[num]=player.citems[num]=null;
		else player.items[num].q-=data.q,player.citems[num]=cache_item(player.items[num]);

		if(G.items[item.name].s) bnum=add_item(buyer,create_new_item(item.name,data.q),{announce:false});
		else bnum=add_item(buyer,actual,{announce:false});

		if(player.type=="merchant") merchant_xp_logic(player,buyer,price,price-round(price*(1-player.tax)));
		if(buyer.type=="merchant") merchant_xp_logic(buyer,player,price,price-round(price*(1-buyer.tax)));

		socket.emit("game_log","Sales tax "+to_pretty_num(price-round(price*(1-player.tax)))+" gold ["+(player.tax*100)+"%]");
		...
		xy_emit(buyer,"ui",{type:"+$$",seller:player.name,buyer:buyer.name,item:cache_item(actual,true,{q:data.q,price:item.price}),slot:data.slot,num:bnum,snum:num});

		resend(player,"reopen"); resend(buyer,"reopen+u+cid");
		success_response({});
	});
	socket.on('trade_buy',function(data){
		...
		if(!item || (data.rid && item.rid!=data.rid)) return fail_response("item_gone");
		if(item.name=="placeholder") return fail_response("item_placeholder");
		if(item.b || item.giveaway) return fail_response("sneaky");
		if(item.price*data.q>player.gold) return fail_response("gold_not_enough");
		if((item.q||1)<data.q) return fail_response("insufficient_q");
		if(!can_add_item(player,create_new_item(item.name,data.q))) return fail_response("no_space");
		var price=item.price*data.q;
		player.gold-=price; add_to_trade_history(player,"buy",seller.name,cache_item(seller.slots[data.slot],true,{q:data.q}),price);
		seller.gold+=round(price*(1-seller.tax)); add_to_trade_history(seller,"sell",player.name,cache_item(seller.slots[data.slot],true,{q:data.q}),price);
		S.gold+=price-round(price*(1-seller.tax));

		if((item.q||1)==data.q) seller.slots[data.slot]=seller.cslots[data.slot]=null;
		else
		{
		...
		if(item.q) num=add_item(player,create_new_item(item.name,data.q),{announce:false});
		else num=add_item(player,item,{announce:false});

		if(seller.owner!=player.owner) item.src="tb";

		if(player.type=="merchant") merchant_xp_logic(player,seller,price,price-round(price*(1-player.tax)));
		if(seller.type=="merchant") merchant_xp_logic(seller,player,price,price-round(price*(1-seller.tax)));
		
		socket.emit("game_log","Spent "+to_pretty_num(price)+" gold");
		seller.socket.emit("game_log","Sales tax "+to_pretty_num(price-round(price*(1-seller.tax)))+" gold ["+(seller.tax*100)+"%]");
		seller.socket.emit("game_log","Received "+to_pretty_num(round(price*(1-seller.tax)))+" gold");

		xy_emit(seller,"ui",{type:"+$$",seller:seller.name,buyer:player.name,item:cache_item(item,true,{q:data.q}),slot:data.slot,num:data.num});
	...
	socket.on('merchant',function(data){
		...
		if(data.close || player.p.stand)
		{
			...
			for(var i=0;i<player.items.length;i++) 
				if(player.items[i] && player.items[i].b=="stand") delete player.items[i].b;
		}
		if(data.num!==undefined)
		{
			var item=player.items[data.num];
			...
			else return fail_response("invalid");
		}
		if(initial!=player.p.stand) // All unneccessary causes of resend's should be patched [03/08/18]
		{
	...
	socket.on('imove',function(data){
		...
		var a=min(data.a,data.b),b=max(data.a,data.b),a_item;
		if(!(b<player.isize || b<player.items.length)) return fail_response("invalid");
		// while(b>=player.items.length) player.items.push(null); [22/11/16]
		if(player.items[a] && player.items[a].name=="placeholder") return fail_response("item_placeholder");
		if(player.items[b] && player.items[b].name=="placeholder") return fail_response("item_placeholder");
		...
		player.citems[data.a]=cache_item(player.items[data.a]); player.citems[data.b]=cache_item(player.items[data.b]);
		resend(player,"reopen+nc+inv");
	...
	socket.on('bank',function(data){
		...
		if(data.operation=="unlock")
		{
			if(!bank_packs[data.pack]) return fail_response("invalid");
			var gold=bank_packs[data.pack][1],shells=bank_packs[data.pack][2];
			if(!gold) return fail_response("gold_not_enough");
			if(player.user[data.pack]) return fail_response("invalid");
			if(data.gold)
			{
				if(player.gold<gold) return fail_response("gold_not_enough");
				player.gold-=gold;
				player.user[data.pack]=[]; player.cuser[data.pack]=[];
				success={response:"bank_new_pack",pack:data.pack,gold:gold,cevent:true}
			}
			else if(data.shells)
			{
				if(player["unlocking_"+data.pack]) return fail_response("bank_opi");
				player["unlocking_"+data.pack]=true;
				appengine_call("bill_user",{auth:player.auth,amount:shells,reason:data.pack,name:player.name,suffix:"/"+player.name+"/"+data.pack,override:true},function(result){
					server_log("buy_with_cash: "+JSON.stringify(result));
					player["unlocking_"+data.pack]=false;
					if(result.failed || !result.done) return socket.emit("game_log","Purchase failed");
					player.cash=result.cash;
		...
		if(data.operation=="move") //within a .itemsN
		{
			if(!player.user[data.pack] || bank_packs[data.pack][0]!=player.map) return fail_response("invalid");
			server_log("storage move "+JSON.stringify(data));
			data.a=max(0,min(41,parseInt(data.a)||0));
			data.b=max(0,min(41,parseInt(data.b)||0));
			if(data.a==data.b) return fail_response("invalid");
			if(player.user[data.pack][data.a] && player.user[data.pack][data.a].name=="placeholder") return fail_response("item_placeholder");
			if(player.user[data.pack][data.b] && player.user[data.pack][data.b].name=="placeholder") return fail_response("item_placeholder");
			if(can_stack(player.user[data.pack][data.a],player.user[data.pack][data.b]))
			{
				player.user[data.pack][data.b].q=(player.user[data.pack][data.a].q||1)+(player.user[data.pack][data.b].q||1);
			...
			player.cuser[data.pack][data.a]=cache_item(player.user[data.pack][data.a]); player.cuser[data.pack][data.b]=cache_item(player.user[data.pack][data.b]);
		}
		if(data.operation=="swap") //between .items and a .itemsN 
		{
			var operation="swap";
			if(!player.user[data.pack] || bank_packs[data.pack][0]!=player.map) return fail_response("invalid");
			...
			if(data.inv==-1 || !data.inv && data.inv!==0)
			{
				...
				for(var i=0;i<player.isize;i++)
				{
					if(!player.items[i]) {data.inv=i; break;}
				}
				// if(data.inv==-1) { socket.emit("game_log","Inventory is full"); return; }
			}
			if(data.str==-1 || !data.str && data.str!==0)
			{
				if(operation=="pull") return fail_response("invalid");
				operation="store";
				for(var i=0;i<42;i++)
				{
					if(!player.user[data.pack][i]) {data.str=i; break;}
				}
			...
			var bank_item=player.user[data.pack][data.str],inv_item=player.items[data.inv];
			if(inv_item && inv_item.name=="placeholder") return fail_response("item_placeholder");
			if(inv_item && inv_item.b) return fail_response("item_blocked");
			if(inv_item) delete inv_item.m,delete inv_item.v;
			if(operation=="swap")
			{
			...
			{
				if(!can_add_item(player.user[data.pack],inv_item)) return fail_response("storage_full");
				player.items[data.inv]=player.citems[data.inv]=null;
				bank_add_item(player,data.pack,inv_item);
			}
			else if(operation=="pull" && bank_item)
			{
				if(!can_add_item(player,bank_item)) return fail_response("inventory_full");
				player.user[data.pack][data.str]=player.cuser[data.pack][data.str]=null;
				add_item(player,bank_item,{announce:false});
			}
		}
		if(!player.user.gold && player.user.gold!==0) player.user.gold=0,server_log("#X - GOLD BUG bank",1);
		resend(player,"reopen");
	...
	socket.on('throw',function(data){
		...
		if(item)
		{
			...
			if(item.b) return fail_response("item_blocked");
			var def=G.items[item.name]
			if(!def.throw) return fail_response("invalid"); 
			var x=parseFloat(data.x)||0,y=parseFloat(data.y)||0;
			if(distance(player,{map:player.map,in:player.in,x:x,y:y})>player.str*3) fail_response("too_far");
				consume_one(player,data.num);
			if(item.name=="confetti")
			{
				player.thrilling=future_s(20);
				xy_emit({map:player.map,in:player.in,x:x,y:y},"eval","confetti_shower({in:'"+player.in+"',map:'"+player.map+"',real_x:"+x+",real_y:"+y+"})");
			}
			if(item.name=="firecrackers")
			{
				player.thrilling=future_s(200);
				xy_emit({map:player.map,in:player.in,x:x,y:y},"eval","firecrackers({in:'"+player.in+"',map:'"+player.map+"',real_x:"+x+",real_y:"+y+"})");
				for(var id in instances[player.in].monsters)
				{
					var monster=instances[player.in].monsters[id];
					if(monster.target && distance({map:player.map,in:player.in,x:x,y:y},monster)<64) stop_pursuit(monster,{force:true,cause:"firecrackers"});
				}
			}
	...
	socket.on('poke',function(data){
		...
		if(player.slots.gloves.level>=10) level=4;
		else if(player.slots.gloves.level==9) level=3;
		else if(player.slots.gloves.level==8) level=2;
	...
	socket.on('merge',function(data){
		...
		if(G.items[container.name].grade<G.items[pet.name].grade || container.data) return socket.emit("game_response","merge_mismatch");
	...
	socket.on('activate',function(data){
		...
		if(data.slot)
		{
			...
			if(!item || in_arr(data.slot,trade_slots)) return;
			if(item.name=="etherealamulet")
			{
				if(player.last_ethereal && mssince(player.last_ethereal)<120) return socket.emit("game_response","not_ready");
			...
			{
				if(player.tskin=="snow_angel") player.tskin="";
				else if(item.level>=8 && (player.type=="mage" || player.type=="priest")) player.tskin="snow_angel";
				else { return socket.emit("game_response","nothing"); }
			}
			else if(item.name=="tristone")
			{
				var on=true;
				if(player.tskin || player.tactivations==100) player.tskin="",on=false;
				else if(item.level<=1 && deduct_gender(player)=="female") player.tskin=random_one(["tf_green","tf_pink"]);
				else if(item.level==2 && deduct_gender(player)=="female") player.tskin=random_one(["tf_blue","tf_purple"]);
				else if(deduct_gender(player)=="female") player.tskin="tf_orange";
				else if(item.level<=1) player.tskin=random_one(["tm_gray","tm_brown","tm_white"]);
				else if(item.level==2) player.tskin=random_one(["tm_green","tm_yellow","tm_purple"]);
				else player.tskin=random_one(["tm_blue","tm_red"]);
			...
			else if(item.name=="darktristone")
			{
				var on=true;
				if(player.tskin || player.tactivations==100) player.tskin="",on=false;
				else if(item.level==4 && deduct_gender(player)=="female") player.tskin="mf_blue";
				else if(deduct_gender(player)=="female") player.tskin="mf_yellow";
				else if(item.level==4) player.tskin="mm_blue";
				else player.tskin="mm_yellow";
				player.tactivations=(player.tactivations||0)+1;
			}
			else return;
		...
		else
		{
			...
			if(item)
			{
				...
				if(item.name=="bkey")
				{
					if(!player.user) return socket.emit("game_response","only_in_bank");
					if(player.user.unlocked && player.user.unlocked.bank_b) return socket.emit("game_response","already_unlocked");
					consume_one(player,data.num);
				...
				{
					if(!player.user) return socket.emit("game_response","only_in_bank");
					if(player.user.unlocked && player.user.unlocked.bank_u) return socket.emit("game_response","already_unlocked");
					consume_one(player,data.num);
					player.user.unlocked=player.user.unlocked||{};
				...
				if(item.name=="dkey")
				{
					...
					{
						var pack="items"+i;
						if(!bank_packs[pack] || player.user[pack]) continue;
						found=true;
						player.user[pack]=[]; player.cuser[pack]=[];
						break;
					}
					if(!found) return;
					consume_one(player,data.num);
	...
	socket.on('booster',function(data){
		...
		{
			if(!in_arr(data.to,booster_items)) return fail_response("invalid");
			player.xpm=player.goldm=player.luckm=1;
			item.name=data.to;
	...
	socket.on('skill',function(data){
		...
		if(!G.skills[data.name]) return socket.emit("game_response",{response:"no_skill",place:data.name,failed:true});
		if(G.skills[data.name].mp && player.mp<G.skills[data.name].mp*(100-(player.mp_reduction||0))/100.0 && !(player.role=="gm" && data.name=="blink"))
		...
		if((G.skills[data.name].cooldown||G.skills[data.name].reuse_cooldown) && player.last[data.name] && mssince(player.last[data.name])<(G.skills[data.name].cooldown||G.skills[data.name].reuse_cooldown))
			return socket.emit("game_response",{response:"cooldown",skill:data.name,place:data.name,id:data.id,ms:(G.skills[data.name].cooldown||G.skills[data.name].reuse_cooldown)-mssince(player.last[data.name]),place:data.name,failed:true});
		if(G.skills[data.name].share && player.last[G.skills[data.name].share] && mssince(player.last[G.skills[data.name].share])<G.skills[G.skills[data.name].share].cooldown*(G.skills[data.name].cooldown_multiplier||1))
			return socket.emit("game_response",{response:"cooldown",skill:data.name,place:data.name,failed:true,id:data.id,ms:G.skills[G.skills[data.name].share].cooldown*(G.skills[data.name].cooldown_multiplier||1)-mssince(player.last[G.skills[data.name].share])});
		...
		if(G.skills[data.name].wtype && !is_array(G.skills[data.name].wtype) && (!player.slots.mainhand || G.items[player.slots.mainhand.name].wtype!=G.skills[data.name].wtype) && player.role!="gm")
			return socket.emit("game_response",{response:"skill_cant_wtype",place:data.name,failed:true});
		if(is_array(G.skills[data.name].wtype) && (!player.slots.mainhand || !in_arr(G.items[player.slots.mainhand.name].wtype,G.skills[data.name].wtype)) && player.role!="gm")
			return socket.emit("game_response",{response:"skill_cant_wtype",place:data.name,failed:true});
		...
		if(G.skills[data.name].target)
		{
			...
			if(is_invis(target) || !G.skills[data.name].global && (target.map!=player.map || distance(target,player,true)>B.max_vision))
				return socket.emit("disappear",{id:data.id,place:data.name,reason:"not_there"});
		}
		if(G.skills[data.name].requirements)
		{
			for(var requirement in G.skills[data.name].requirements)
			{
				if(!player[requirement] || player[requirement]<G.skills[data.name].requirements[requirement])
					return socket.emit("game_response",{response:"skill_cant_requirements",place:data.name,failed:true});
			}
		}
		if(G.skills[data.name].slot)
		{
			var found=false,charges=false;
			G.skills[data.name].slot.forEach(function(p){
				if(player.slots[p[0]] && player.slots[p[0]].name==p[1])
				{
					...
					{
						if((player.slots[p[0]].charges||0)<G.items[player.slots[p[0]].name].charge) { charges=true; return; }
			...
			if(charges) return socket.emit("game_response",{response:"skill_cant_charges",place:data.name,failed:true});
			if(!found) return socket.emit("game_response",{response:"skill_cant_slot",place:data.name,failed:true});
		}
		if(player.s.invincible) {delete player.s.invincible; player.to_resend="u+cid"};
		if(data.name=="attack" || data.name=="heal")
		{
			var attack=commence_attack(player,target,data.name);
			if(!attack.failed) resolve=attack;
			else reject=attack,cool=false;
		}
		if(data.name=="invis" && !player.s.invis)
		{
			if(player.s.marked) return socket.emit("game_response",{response:"skill_cant_use",place:data.name,failed:true});
		...
		if(data.name=="pickpocket")
		{
			...
			player.c[data.name]={ms:G.skills[data.name].duration_min+Math.random()*(G.skills[data.name].duration_max-G.skills[data.name].duration_min),target:target.name};
		...
		{
			var direction=0,the_zone=null;
			consume_mp(player,G.skills[data.name].mp);
			(G.maps[player.map].zones||[]).forEach(function(zone){
				if(zone.type!=data.name || the_zone) return;
				[[0,-24,3],[-24,0,1],[24,0,2],[0,24,0]].forEach(function(m){
			...
			else
			{
				...
				player.c[data.name]={ms:G.skills[data.name].duration_min+Math.random()*(G.skills[data.name].duration_max-G.skills[data.name].duration_min),drop:the_zone.drop};
			}
			resolve={response:"data",place:data.name,success:false,in_progress:true}
			player.to_resend="u+cid";
		}
		...
		if(data.name=="dash")
		{
			//console.log(data);
			consume_mp(player,G.skills[data.name].mp);
			var x=parseFloat(data.x)||0,y=parseFloat(data.y)||0,point=true;
			...
			if(point)
			{
				...
				if(!spot) spot=safe_xy_nearby(player.map,player.x+(x-player.x)*0.8,player.y+(y-player.y)*0.8);
				if(!spot) spot=safe_xy_nearby(player.map,player.x+(x-player.x)*0.6,player.y+(y-player.y)*0.6);
				...
				if(point)
				{
					...
					player.s.dash={ms:1000}; player.speed=500;
		...
		{
			consume_mp(player,G.skills[data.name].mp);
			player.s[G.skills[data.name].condition]={ms:G.skills[data.name].duration||G.conditions[G.skills[data.name].condition].duration};
			player.to_resend="u+cid";
		}
		if(data.name=="mshield")
		{
			consume_mp(player,G.skills[data.name].mp);
			if(player.s[G.skills[data.name].condition]) delete player.s[G.skills[data.name].condition];
			else player.s[G.skills[data.name].condition]={ms:999999999};
			player.to_resend="u+cid";
		}
		if(data.name=="mcourage" || data.name=="mfrenzy" || data.name=="massproduction" || data.name=="massproductionpp")
		{
			consume_mp(player,G.skills[data.name].mp);
		...
		if(data.name=="throw")
		{
			...
			var item=player.items[data.num],r="u+cid",damage=0;
			...
			var prop=calculate_item_properties(item),negative=false;
			if(in_arr(item.name,G.skills.throw.negative)) negative=true;
			G.skills.throw.nprop.forEach(function(p){if(prop[p]) negative=true;});
			...
			consume_one(player,data.num);
			if(item.name=="essenceoffire") add_condition(target,"eburn",{from:player});
			else if(item.name=="essenceoflife") add_condition(target,"eheal",{from:player});
			else if(prop.attack) damage=round(Math.random()*prop.attack*15);
			else if(prop.armor) damage=round(Math.random()*prop.armor*24);
			if(damage)
			{
				if(target["1hp"]) damage=1;
			...
			if(target.is_player) resend(target,r);
			else target.u=true,target.cid++;
		}
		...
		if(data.name=="curse") //#TODO: last_curse variable + check for multiple curses
		{
			var attack=commence_attack(player,target,"curse");
			if(!attack.failed) resolve=attack,add_pdps(player,null,player.attack/2);
			else reject=attack,cool=false;
		}
		...
		if(data.name=="entangle" || data.name=="tangle")
		{
			...
			if(data.name=="entangle") consume_one(player,data.num);
			player.to_resend="u+cid+reopen";
		...
		if(["quickpunch","quickstab","smash","mentalburst","purify","taunt","supershot","zapperzap","burst","piercingshot"].includes(data.name))
		{
			var attack=commence_attack(player,target,data.name);
			if(!attack.failed) resolve=attack;
			else reject=attack,cool=false;
		...
		if(data.name=="revive")
		{
			...
			consume_one(player,data.num);
			player.to_resend="reopen";
			if(target.party==player.party) add_pdps(player,target,target.pdps/5);
		...
		if(data.name=="cburst")
		{
			var hit={},times=0,attack=null,c_resolve=null;
			consume_mp(player,G.skills[data.name].mp);
			player.first_burst=true;
			player.halt=true;
			if(is_array(data.targets)) data.targets.forEach(function(t){
				// console.log(id);
				var id=t[0],mp=max(0,parseInt(t[1])||0);
				if(player.mp<20 || times>16 || !mp) return; times+=1;
				var target=instances[player.in].monsters[id]; if(!target) target=instances[player.in].players[id]; if(!target || is_invinc(target) || target.name==player.name) return;
				if(hit[id]) return;
				hit[id]=true;
				player.next_mp=mp;
				attack=commence_attack(player,target,"cburst");
				if(!attack || !attack.projectile) return;
				if(!c_resolve)
				{
					c_resolve=attack;
			...
			if(!c_resolve)
			{
				if(attack) reject=attack;
				disappearing_text(player.socket,player,"NO HITS");
			}
			else resolve=c_resolve;
		}
		if(data.name=="partyheal")
		{
			var targets=[player],attack=null,hits=0;
			...
			targets.forEach(function(target){
				...
				if(!attack.failed) hits=true;
			});
			if(!hits) reject=attack;
		}
		...
		if(data.name=="darkblessing" || data.name=="warcry")
		{
			...
			for(var id in instances[player.in].players)
			{
				...
				if(!target.npc && distance(player,target,true)<G.skills[data.name].range && (!(G.maps[player.map].pvp || is_pvp) || is_same(player,target,1)))
				{
		...
		{
			player.halt=true;
			var times=0,hit={},reftarget=null,targets=3,attack=null,c_resolve=null;
			if(data.name=="5shot") targets=5;
			//console.log(data.ids);
			if(is_array(data.ids))
				data.ids.forEach(function(id){
				if(times>=targets) return; times+=1;
				var target=instances[player.in].monsters[id]; if(!target) target=instances[player.in].players[id];
		...
		if(data.name=="track")
		{
			...
			for(var id in instances[player.in].players)
			{
				...
				var target=instances[player.in].players[id],current={sound:"wmp"}; if(target.npc) continue;
				if(is_invis(target)) current.invis=true;
				if(target.type=="rogue" || target.type=="ranger") current.sound="rr";
				else if(target.type=="priest" || target.type=="mage") current.sound="pm";
				current.dist=distance(player,target);
				if(current.dist<G.skills.track.range) list.push(current);
			}
		...
		if(data.name=="agitate")
		{
			...
			for(var id in instances[player.in].monsters)
			{
				...
				if(target.target==player.name) { ids.push(id); continue; } // why did the missing continue cause a disengage I have no idea [25/03/23]
				if(target.target && !(get_player(target.target) && is_same(player,get_player(target.target),1))) continue;
				var dist=distance(player,target);
				if(dist<G.skills.agitate.range)
				{
					if(target.target) stop_pursuit(target,{redirect:true,cause:"agitate redirect"});
					target.cid+=1; target.u=true;
					target.last.attacked=new Date();
		...
		if(data.name=="cleave" || data.name=="shadowstrike")
		{
			...
			player.halt=true;
			var ids=[],reftarget=null,c_resolve=null;
			for(var id in instances[player.in].monsters)
			{
				var target=instances[player.in].monsters[id];
				var dist=distance(player,target);
				...
				{
					attack=commence_attack(player,target,data.name);
					if(!attack || !attack.projectile) continue;
					ids.push(id);
					if(!c_resolve)
					{
						c_resolve=attack;
			...
			player.halt=false;
			if(c_resolve) resolve=c_resolve;
		}
		if(data.name=="magiport")
		{
			...
			{
				if(!magiportations[player.name]) magiportations[player.name]={};
			...
			else
			{
				...
				if(!ported) player.socket.emit("game_response",{response:"magiport_failed",id:data.id});
			}
			player.to_resend="u+cid";
		}
		if(data.name=="blink")
		{
			var x=parseFloat(data.x)||0,y=parseFloat(data.y)||0;
			var spot=safe_xy_nearby(player.map,x,y);
			if(!spot) return player.socket.emit("game_response",{response:"blink_failed",place:data.name,failed:true});
			...
			player.s.blink.d=0; if(in_arr(data.direction,[1,2,3])) player.s.blink.d=data.direction;
			if(player.role!="gm") consume_mp(player,G.skills[data.name].mp);
		...
		if(data.name=="warp")
		{
			var x=parseFloat(data.x)||0,y=parseFloat(data.y)||0,ins=data.in||"main",instance=instances[ins];
			if(!instance) return player.socket.emit("game_response",{response:"blink_failed",place:data.name,failed:true});
			if(instance.mount!=instances[player.in].mount) return player.socket.emit("game_response",{response:"cant_in_bank",place:data.name,failed:true});
		...
		if(data.name=="mluck")
		{
			if(distance(player,target,true)>G.skills[data.name].range)
				return socket.emit("game_response",{response:"too_far",place:data.name,failed:true});
			consume_mp(player,G.skills[data.name].mp);
			if(!target.s[G.skills[data.name].condition] || !target.s[G.skills[data.name].condition].strong || target.s[G.skills[data.name].condition].f==player.name) target.s[G.skills[data.name].condition]={ms:G.conditions.mluck.duration,f:player.name};
			if(target.owner==player.owner) target.s[G.skills[data.name].condition].strong=true;
			xy_emit(player,"ui",{type:"mluck",from:player.name,to:target.name});
			resend(target,"u+cid"); resend(player,"u+cid");
		}
		if(data.name=="rspeed")
		{
			if(distance(player,target,true)>G.skills[data.name].range)
			...
			resend(target,"u+cid"); resend(player,"u+cid");
			if(player.party==target.party && player!=target) add_pdps(player,target,2000);
		}
		if(data.name=="reflection")
		{
			...
			resend(target,"u+cid"); resend(player,"u+cid");
			if(player.party==target.party && player!=target) add_pdps(player,target,4000);
		}
		if(data.name=="energize")
		{
			...
			if(mp>target.max_mp-target.mp) mp=target.max_mp-target.mp;
			target.mp+=mp; player.mp-=mp;
			if(player.party==target.party && player!=target) add_pdps(player,target,mp*2);
		...
		{
			var gold=0,rate=0.8;
			if(player.level>=100) rate=1.12;
			else if(player.level>=90) rate=1.10;
			else if(player.level>=80) rate=1.06;
			else if(player.level>=70) rate=1;
			else if(player.level>=60) rate=0.92;
			else if(player.level>=50) rate=0.86;
			consume_mp(player,G.skills[data.name].mp);
			xy_emit(player,"ui",{type:"alchemy",name:player.name});
			for(var i=0;i<player.isize;i++)
			{
				if(!player.items[i] || player.items[i].l) continue;
		...
		if(cool) consume_skill(player,data.name);
		if(player.to_resend) resend(player,player.to_resend);
		if(reject) { if(!reject.response) reject.response="data"; socket.emit("game_response",reject); }
		else if(resolve) socket.emit("game_response",resolve);
	});
	...
	socket.on('interaction',function(data){
		...
		{
			var x="";
			if(!G.maps[player.map].ref || !G.maps[player.map].ref[data.type] || distance(G.maps[player.map].ref[data.type],player,true)>300) return socket.emit("game_response","distance");
			if(!player.s.holidayspirit && player.esize) add_item(player,"funtoken"),x="+reopen";
			add_condition(player,"holidayspirit");
			resend(player,"u+cid"+x);
		}
		else if(["redorb","blueorb","greenorb","yelloworb"].includes(data.type))
		{
			if(!G.maps[player.map].ref || !G.maps[player.map].ref[data.type] || distance(G.maps[player.map].ref[data.type],player,true)>40) return socket.emit("game_response","distance");
			["redorb","blueorb","greenorb","yelloworb"].forEach(function(s){ delete player.s[s]; })
			add_condition(player,data.type);
			resend(player,"u+cid");
		}
		else if(data=="the_lever")
		{
			if(player.map!="resort_e") return socket.emit("game_response","distance");
		...
		{
			if(!G.maps.main.ref.dailytask || distance(G.maps.main.ref.dailytask,player,true)>150) return socket.emit("game_response","distance");
		...
		{
			if(data.key=="B" && player.konami.length<20) player.konami.push(data.key[0]);
			else if(data.key=="A")
			{
				if(player.konami.join("")=="uuddlrlrB")
				{
					player.tskin="konami",resend(player,"u+cid");
					if(!player.p.target_lock || !G.monsters[player.p.target_lock] || hsince(player.p.dt.last_tl)>15*24)
					{
						var monsters=[];
						for(var name in G.monsters) if(!G.monsters[name].special && !G.monsters[name].stationary && G.monsters[name].c) monsters.push(name);
	...
	socket.on('move',function(data){
		...
		if(data.key && (!player.konami || player.konami.length<20))
		{
			...
			if(!player.konami) player.konami=[];
		...
		if(player && player.m==data.m && can_walk(player) && (x!=player.x || y!=player.y))
		{
			...
			{
				current=smap_data[player.map][rphash(data.x,data.y)],going=smap_data[player.map][rphash(player.going_x,player.going_y)];
				...
				{
					server_log("#C cheater: "+player.name+" current: "+current+"["+data.x+","+data.x+"] going: "+going+"["+player.going_x+","+player.going_y+"] at "+player.map,1);
			...
			start_moving_element(player);
			if(!player.pet && simple_distance(player,{x:data.x,y:data.y})>132 && current==0 && mode.lcorrection)
			{
	...
	socket.on('open_chest',function(data){
		...
		if(player.map=="woffice" || G.maps[player.map].mount || is_invis(player)) return fail_response("loot_failed");
		try{
			if(chest && !chest.pvp && chest.gold>100000000 && !player.stealth)
			{
				broadcast("server_message",{"message":player.name+" looted "+to_pretty_num(server_tax(round(chest.gold*r.goldm),true))+" gold","color":"gold","type":"server_gold",name:player.name});
			}
			...
			{
				var all_items=chest.items.slice(0); all_items.concat(chest.pvp_items)
				if(!can_add_items(player,all_items)) return fail_response("loot_no_space");
				...
				if(chest && chest.cash) add_shells(player,chest.cash,"chest",true,"override");
				chest.items.forEach(function(item){
					item.src="pvp";
					add_item(player,item,{found:1,m:1,v:B.v}); reopen=true;
					var ritem=cache_item(item); ritem.looter=player.name; r.items.push(ritem);
					socket.emit("game_log",{message:"Found "+item_to_phrase(item),color:"#4BAEAA"});
					if(player.t) player.t.dgold+=round(G.items[item.name].g*0.6);
				});
				(chest.pvp_items||[]).forEach(function(item){
					item.v=new Date();
					if(can_add_item(player,item))
					{
						add_item(player,item,{found:1,v:B.v}); reopen=true;
						var ritem=cache_item(item); ritem.looter=player.name; ritem.pvp_loot=true; r.items.push(ritem);
						socket.emit("game_log",{message:"Looted "+item_to_phrase(item),color:"#4BAEAA"});
					}
					else
					{
						lostandfound_logic(item);
						var ritem=cache_item(item); ritem.looter=null; ritem.lostandfound=true; r.items.push(ritem);
				...
				player.gold+=r.gold;
				if(player.t) player.t.cgold+=r.gold;
				if(r.gold) socket.emit("game_log",{message:to_pretty_num(r.gold)+" gold",color:"gold"});
				socket.emit("disappearing_text",{"message":"+"+r.gold,"x":chest.x,"y":chest.y-10,"args":{color:"+gold","size":"large"}});
			...
			else if(chest)
			{
				...
				r.party=true;
				var chest=chests[data.id],reopen={};
				...
				chest.items.forEach(function(item){
					...
					parties[player.party].forEach(function(name){
						...
						if(current && can[name])
						{
							...
							{
								awarded=true;
								add_item(current,item,{found:1,m:1,v:B.v}); reopen[current.id]=true;
								var ritem=cache_item(item); ritem.looter=current.name; r.items.push(ritem);
								party_emit(player.party,"game_log",{message:current.name+" found "+item_to_phrase(item),color:"#4BAEAA"});
								if(current.t) current.t.dgold+=round(G.items[item.name].g*0.6);
							}
							else pool_current+=current.share;
						}
					});
					if(!awarded)
					{
				...
				(chest.pvp_items||[]).forEach(function(item){
					...
					parties[player.party].forEach(function(name){
						...
						if(current && can[name])
						{
							...
							{
								awarded=true;
								add_item(current,item,{found:1,v:B.v}); reopen[current.id]=true;
								var ritem=cache_item(item); ritem.looter=current.name; ritem.pvp_loot=true; r.items.push(ritem);
								party_emit(player.party,"game_log",{message:current.name+" looted "+item_to_phrase(item),color:"#4BAEAA"});
							}
							else pool_current+=current.share;
						}
					});
					...
					{
						lostandfound_logic(item);
						var ritem=cache_item(item); ritem.looter=null; ritem.pvp_loot=true; ritem.lostandfound=true; r.items.push(ritem);
						party_emit(player.party,"game_log",{message:"Lost "+item_to_phrase(item),color:"#AB4E4F"});
					}
				});
				parties[player.party].forEach(function(name){
					var current=players[name_to_id.get(name)],cgold=round(chest.gold*(current.share||0)*r.goldm)+round((chest.egold||0)*(current.share||0));
					r.gold=cgold=server_tax(cgold);
					current.gold+=cgold;
					if(current.t) current.t.cgold+=cgold;
					if(cgold) current.socket.emit("game_log",{message:to_pretty_num(cgold)+" gold",color:"gold"});
					if(current.in==player.in) current.socket.emit("disappearing_text",{"message":"+"+cgold,"x":chest.x,"y":chest.y-10,"args":{color:"gold","size":"large"}});
	...
	socket.on('auth',function(data){
		...
		appengine_call("start_character",{auth:data.user+'-'+data.auth,secret:socket.observer_secret,code_slot:data.code_slot,character:data.character,mode:gameplay,ip:get_ip(socket),suffix:"/"+data.character},function(result){
			...
			if(data.bot==variables.bot_key) player.bot=true,player.afk="bot";
			if(data.no_html)
			{
				player.afk="code";
				try{player.controller=name_to_id.get(data.no_html)&&data.no_html||"";}catch(e){player.controller="";}
			}
			if(!player.afk) player.afk=true;
			if(gameplay=="test") player.name+=parseInt(Math.random()*10000);
			player.real_id=player.id; player.id=player.name;
			...
			if(gameplay=="hardcore") reset_player(player); //  || gameplay=="test"

			init_player(player);
			if(!observers[socket.id]) // observer hang up before "auth"
			{
			...
			{
				player.platform="web";
				if(result.character.pid) player.auth_id=result.character.pid; // part of the new restriction system [02/05/19]
			}

			if(mode.drm_check)
			{
				if(result.character.drm && !player.auth_id) player.s.authfail={ms:900000*1000};
				else if(player.s.authfail) player.s.authfail={ms:100};
			}
	...
	socket.on('use',function(data){
		...
		if(data.item=="hp" || data.item=="mp")
		{
			...
			if(data.item=="hp") player.hp+=50,disappearing_text(socket,player,"+50",{color:"green",xy:1,s:"hp",nohp:1});
			if(data.item=="mp") player.mp+=100,disappearing_text(socket,player,"+100",{color:"#006AA9",xy:1,s:"mp",nomp:1});
			player.hp=min(player.hp,player.max_hp); player.mp=min(player.mp,player.max_mp);
	...
	socket.on('duel',function(data){
		...
		else if(data.event=="accept")
		{
			...
			var name=randomStr(20),a=[],b=[];
			instance=create_instance(name,"duelland");
			instance.info={"seconds":is_sdk&&20||60,"active":false,"A":[player_to_summary(challenger)],"B":[player_to_summary(player)],"id":name};
			instance.info.A[0].active=true;
			instance.info.B[0].active=true;
			clean_slate(challenger); transport_player_to(challenger,name,1); if(challenger.party) parties[challenger.party].forEach(function(p){a.push(p)}); else a=[challenger.name];
			clean_slate(player); transport_player_to(player,name,2); if(player.party) parties[player.party].forEach(function(p){b.push(p)}); else b=[player.name];
			if(!E.duels) E.duels={};
			var duel={"challenger":challenger.name,a:a,"vs":player.name,b:b,"instance":name,"seconds":is_sdk&&20||60,"active":false,"id":name};
			challenger.team="A"; challenger.duel=duel; challenger.s.stunned={ms:120000}; resend(challenger,"u+cid");
			player.team="B"; player.duel=duel; player.s.stunned={ms:120000}; resend(player,"u+cid");
			E.duels[name]=duel;
			broadcast_e();
			a.forEach(function(p){ if(p!=challenger.name && get_player(p)) get_player(p).socket.emit("game_response",{response:"duel_started",challenger:challenger.name,vs:player.name,id:name}); });
			b.forEach(function(p){ if(p!=player.name && get_player(p)) get_player(p).socket.emit("game_response",{response:"duel_started",challenger:challenger.name,vs:player.name,id:name}); });
		}
		else if(data.event=="enter")
		{
			if(is_in_pvp(player)) return socket.emit("game_log","Can't join the duel from a pvp zone!");
			if(player.duel) return socket.emit("game_log","Already in a duel!");
			if(!E.duels[data.id]) return socket.emit("game_log","Duel expired");
			if(E.duels[data.id].active) return socket.emit("game_log","Duel already started");
			if(!(E.duels[data.id].a.includes(player.name) || E.duels[data.id].b.includes(player.name))) return socket.emit("game_log","Not your duel");
			clean_slate(player);

			if(E.duels[data.id].a.includes(player.name)) transport_player_to(player,data.id,1),player.team="A";
			else transport_player_to(player,data.id,2),player.team="B";

			player.duel=E.duels[data.id];
			if(player.duel.a.includes(player.name)) instances[data.id].info.A.push(player_to_summary(player)),instances[data.id].info.A[instances[data.id].info.A.length-1].active=true;
			else instances[data.id].info.B.push(player_to_summary(player)),instances[data.id].info.B[instances[data.id].info.B.length-1].active=true;
	...
	socket.on('party',function(data){
		...
		if(data.event=="invite")
		{
			...
			if(!invitations[player.name]) invitations[player.name]={};
		...
		if(data.event=="request")
		{
			...
			if(!requests[player.name]) requests[player.name]={};
		...
		if(data.event=="accept")
		{
			...
			if(inviter.party && (parties[inviter.party].length>=limits.party_max || inviter.party_length>=limits.party)) return fail_response("party_full");
			if(!invitations[inviter.name] || !invitations[inviter.name][player.id]) return fail_response("invitation_expired");
			if(player.party && player.party==inviter.party) return success_response("already_in_party");
			if(player.party)
			{
			...
			if(!inviter.party)
			{
				...
				if(!players[name_to_id.get(data.name)]) return fail_response("player_gone",{name:data.name}); // these repetitions are all because socket.emit's can cause in-line disconnects and disband parties right after creation [07/08/20]
			}
			player.party=inviter.party;
			parties[inviter.party].push(player.name);
			party_emit(player.party,"party_update",{list:parties[inviter.party],party:party_to_client(inviter.party),message:player.name+" joined the party"+(inviter.party!=inviter.name && " with "+inviter.name+"'s invite" || "")});
		...
		if(data.event=="raccept")
		{
			...
			if(player.party && (parties[player.party].length>=limits.party_max || player.party_length>=limits.party)) return fail_response("party_full");
			if(!requests[requester.name] || !requests[requester.name][player.id]) return fail_response("request_expired");
			if(player.party && player.party==requester.party) return success_response("already_in_party");
			if(requester.party)
			{
				leave_party(requester.party,requester);
				requester.socket.emit("party_update",{});
				requester.socket.emit("game_log","Left the party");
				if(!players[name_to_id.get(data.name)]) return fail_response("player_gone",{name:data.name});
			}
			...
			party_emit(requester.party,"party_update",{list:parties[player.party],party:party_to_client(player.party),message:requester.name+" joined the party"});
		...
		if(data.event=="leave")
		{
			if(!player.party) { socket.emit("party_update",{}); return success_response({}); }
			leave_party(player.party,player);
			socket.emit("party_update",{});
		...
		if(data.event=="kick")
		{
			if(!player.party) return success_response({});
	...
	socket.on('magiport',function(data){
		...
		if(magiportations[data.name] && magiportations[data.name][player.name] && get_player(data.name))
		{
			...
			if(instances[get_player(data.name).in].mount!=instances[player.in].mount) return fail_response("cant_in_bank");
			if(!magiport_someone(player,get_player(data.name))) return fail_response("invalid");
			return success_response({});
		}
		else
		{
	...
	socket.on('join',function(data){
		...
		{
			if(player.map!="goobrawl") transport_player_to(player,"goobrawl");
		}
		else if(data.name=="crabxx" && events.crabxx)
		{
			if(simple_distance(player,{map:"main",x:-1000,y:1700})>200) transport_player_to(player,"main",[-1000,1700,0,40]);
		}
		else if(data.name=="franky" && events.franky)
		{
			if(simple_distance(player,{map:"level2w",x:-300,y:150})>200) transport_player_to(player,"level2w",[-300,150,0,40]);
		}
		else if(data.name=="icegolem" && events.icegolem)
		{
			if(simple_distance(player,{map:"winterland",x:820,y:425})>100) transport_player_to(player,"winterland",[820,425,0,10]);
		}
		else if(data.name=="abtesting" && E.abtesting)
		{
			if(player.map!="abtesting" || !player.team)
			{
				if(ssince(timers.abtesting_start)>120 && (!player.p.abtesting || player.p.abtesting[0]!=E.abtesting.id))
					return fail_response("join_too_late");

				if(player.p.abtesting && player.p.abtesting[0]==E.abtesting.id) player.team=player.p.abtesting[1];
				else player.team=random_one(["A","B"]);
				...
				if(player.team=="A") transport_player_to(player,"abtesting",2);
				else if(player.team=="B") transport_player_to(player,"abtesting",3);
	...
	socket.on('stop',function(data){
		...
		if(!data || !data.action)
		{
			if(player.s.invis) step_out_of_invis(player),change=true;
			if(Object.keys(player.c).length) player.c={},change=true;
		}
		...
		{
			if(player.c.town) { delete player.c.town; change=true;}
		}
		else if(data.action=="revival")
		{
			if(player.c.revival) { delete player.c.revival; change=true;}
		}
		if(change) resend(player,"u+cid");
		success_response();
	});
	...
	socket.on('bet',function(data){
		...
		{
			if(!is_sdk) return; // Old routine [23/08/18]
			if(!data.odds) data.odds="black"; data.odds=""+data.odds;
			data.gold=max(1,parseInt(data.gold)||1);
			if(!tavern.roulette.odds[data.odds]) return;
			if(tavern.roulette.state!="bets") { socket.emit("game_log","Not taking bets yet!"); return; }
			if(Object.keys(player.bets).length>=5) { socket.emit("game_log","You already have 5 active bets"); return; }
			if(player.gold<data.gold) { socket.emit("game_log","Not enough gold"); return; }
			...
			player.gold-=data.gold;
			player.bets[rid]={id:rid,type:"roulette",odds:data.odds,gold:data.gold,pid:socket.id,state:"bet"};
		...
		if(data.type=="dice")
		{
			...
			var odds=100.0/num,dir="down";
			if(data.dir=="up") odds=100.0/(100-num),dir="up"; odds=min(odds,10000);
			odds=parseFloat(floor_f2(odds));
			var win=gold*odds,raw=win,edge=ceil((gold*odds-gold)*house_edge()/100.0); win=parseInt(win); edge=parseInt(edge);
			if(tavern.dice.state=="roll") return socket.emit("game_response","tavern_too_late");
			if(tavern.dice.state!="bets") return socket.emit("game_response","tavern_not_yet");
			if(gold>player.gold) return socket.emit("game_response","gold_not_enough");
			if(win-edge-gold>(S.gold-house_debt())*0.40) return socket.emit("game_response","tavern_gold_not_enough");
			if(Object.keys(player.bets).length) return socket.emit("game_response","tavern_dice_exist");
			...
			var rid=randomStr(10);
			player.gold-=gold;
			player.bets[rid]={id:rid,type:"dice",num:num,gold:gold,dir:dir,pid:socket.id,state:"bet",win:win,edge:edge,odds:odds};
			tavern.dice.players[player.socket.id]=true;
			instance_emit(tavern,"tavern",{event:"bet",name:player.name,type:"dice",num:num,gold:gold,dir:dir});
			resend(player,"reopen+nc");
		}
		if(data.type=="slots")
		{
	...
	socket.on('tavern',function(data){
		if(data.event=="info")
		{
			socket.emit("tavern",{event:"info",edge:house_edge(),"max":parseInt((S.gold-house_debt())*0.40)})
		}
	});
	...
	socket.on('players',function(data){
		...
		for(var id in players)
		{
			...
			{
				mapn="Unknown";
				sdata.push({name:"Hidden",map:mapn,age:current.age,level:current.level,type:current.type,afk:current.afk&&1||0,party:current.party&&"Hidden"||'',kills:current.kills});
			}
			else if(!is_in_pvp(current)) sdata.push({name:current.name,map:mapn,age:current.age,level:current.level,type:current.type,afk:current.afk&&1||0,party:current.party||''});
		}
	...
	socket.on('blend',function(data){
		...
		for(var id in instances[player.in].monsters||{})
		{
			var m=instances[player.in].monsters[id],c=distance(m,player,true);
			if(c<min) min=c,x=m;
		}
		if(x)
		{
			var skin=G.monsters[x.type].skin||x.type;
			if(player.tskin!=skin) player.tskin=skin,resend(player,"u+cid");
		}
	});
	...
	socket.on('disconnect',function(){
		...
		if(player)
		{
			...
			try{
				defeat_player(player);
			}catch(e){log_trace("#X DCERRORPVP",e);}
			// if(!server.live) { server_log('Ignoring the "disconnect" of '+player.name,1); return; }
			
			try{ if(player.moving && distance(player,{x:player.going_x,y:player.going_y})<800) player.x=player.going_x,player.y=player.going_y; }catch(e){log_trace("#X DCERRORM",e);}
				// to not get stuck on out-of-bounds corners
			
			try{ if(player.party) leave_party(player.party,player); }catch(e){log_trace("#X DCERROR1",e);}

			try{ xy_emit(player,"disappear",{id:player.id,reason:"disconnect"}); }catch(e){log_trace("#X DCERROR2",e);}
			...
			try{
				for(var bid in player.bets) player.gold+=player.bets[bid].gold;
				player.bets={};
			}catch(e){log_trace("#X DCERRORBETS",e);}

			try{
				if(player.monster) remove_monster(player.monster);
			...
			}catch(e){log_trace("#X DCERRORrestore",e);}
		...
		if(observer)
		{
			...
			}catch(e){log_trace("#X DCERRORO ",e);}
		}
	});
	...
	socket.on('render',function(data){
		...
		if(window) socket.emit('simple_eval',{window:window,after:after,code:'for(var id in data.window) window[id]=data.window[id]; eval(data.after);'});
		else if(json_output!==undefined) socket.emit('simple_eval',{output:output,json_output:json_output,code:'show_json(data.json_output)'});
		else socket.emit('simple_eval',{output:output,json_output:json_output,code:'show_modal("<pre>"+data.output+"</pre>")'});
		resend(player,"reopen+u+cid");
	});
	...
	socket.on('eval',function(data){
		if(data.command)
		{
			...
			{
				xy_emit({"in":"cyberland","map":"cyberland","x":0,"y":-100},"chat_log",{owner:"mainframe",message:"hi",id:"mainframe"});
			}
			else if(data.command=="give")
			{
				xy_emit({"in":"cyberland","map":"cyberland","x":0,"y":-100},"chat_log",{owner:"mainframe",message:"what?",id:"mainframe"});
			}
			else if(data.command.startsWith("swap"))
			{
				var numbers=data.command.split(" ");
				if(numbers.length!=3)
					xy_emit({"in":"cyberland","map":"cyberland","x":0,"y":-100},"chat_log",{owner:"mainframe",message:"...",id:"mainframe"});
				else
					{
						var a=parseInt(numbers[1]),b=parseInt(numbers[2]);
						if(0<=a && a<42 && 0<=b && b<42)
						{
							if(a==player.p.item_num) player.p.item_num=b;
							else if(b==player.p.item_num) player.p.item_num=a;
							xy_emit({"in":"cyberland","map":"cyberland","x":0,"y":-100},"chat_log",{owner:"mainframe",message:"done",id:"mainframe"});
						}
						else
							xy_emit({"in":"cyberland","map":"cyberland","x":0,"y":-100},"chat_log",{owner:"mainframe",message:"ugh, ok",id:"mainframe"});
					}
			}
			else if(data.command=="stop")
			{
				xy_emit({"in":"cyberland","map":"cyberland","x":0,"y":-100},"chat_log",{owner:"mainframe",message:"mechagnomes assemble",id:"mainframe"});
				get_monsters("mechagnome").forEach(function(m){
					if(m.target) stop_pursuit(m,{stop:1,cause:"stop()"});
					//else m.irregular=3;
				});
			}
			else if(data.command.startsWith("give") && data.command!="give spares")
			{
				xy_emit({"in":"cyberland","map":"cyberland","x":0,"y":-100},"chat_log",{owner:"mainframe",message:"no",id:"mainframe"});
			}
			else if(data.command=="secret web mode" && (player.p.steam_id || player.p.mas_auth_id))
			{
				player.p.secret_web_mode=true;
				xy_emit({"in":"cyberland","map":"cyberland","x":0,"y":-100},"chat_log",{owner:"mainframe",message:"secret web mode unlocked",id:"mainframe"});
			}
			else if(data.command=="give spares")
			{
				if(S.misc && S.misc.spares && S.misc.spares.length)
				{
					xy_emit({"in":"cyberland","map":"cyberland","x":0,"y":-100},"chat_log",{owner:"mainframe",message:"here you go",id:"mainframe"});
					drop_one_thing(player,S.misc.spares,{x:1,y:-88});
					S.misc.spares=[];
				}
				else
					xy_emit({"in":"cyberland","map":"cyberland","x":0,"y":-100},"chat_log",{owner:"mainframe",message:"come later",id:"mainframe"});
			}
			else
			{
				...
				{
					xy_emit({"in":"cyberland","map":"cyberland","x":0,"y":-100},"chat_log",{owner:"mainframe",message:"UNAUTHORIZED COMMAND",id:"mainframe"});
					for(var id in instances.cyberland.monsters)
					{
						var monster=instances.cyberland.monsters[id];
						if(!monster.target) target_player(monster,player);
					}
				}
				else
...
function add_coop_points(m,attacker,mnet)
{
	if(!m) return;
	if(m["1hp"]) mnet=1;
	if(attacker.s.hopsickness) mnet/=4;
...
function level_monster(monster,args)
{
	if(!args) args={};
	var mult=1;
	if(args.delevel) mult=-1;
	monster.xp+=G.monsters[monster.type].xp*mult;
	monster.max_hp+=parseInt(G.monsters[monster.type].hp/2)*mult;
	if(mult>0) monster.hp+=parseInt(G.monsters[monster.type].hp/2)*mult;
	else monster.hp=min(monster.hp,monster.max_hp);
	...
	monster.cid++;
	monster.last_level=future_s(Math.random()*100-50);
	if(mode.fast_mlevels) monster.last_level=new Date();
	if(!args.silent)
	{
		calculate_monster_stats(monster);
...
{
	if(!args) args={};
	if(!args.method) args.method="death";
	target.dead=true;
	target.dc=true;
	if(G.monsters[target.type].respawn!=-1 && !G.monsters[target.type].special && !target.special && !target.map_def.special && !target.pet && !target.spawn && !args.nospawn && !target.temp)
	{
		if(target.map_def.grow && (target.map_def.live||0)<=target.map_def.count*2/3) setTimeout(new_monster_f(target.oin,target.map_def,{before_respawn:target}),25);
		else if(G.monsters[target.type].respawn>200) setTimeout(new_monster_f(target.oin,target.map_def,{before_respawn:target}),round(G.monsters[target.type].respawn*(720+Math.random()*480)));
		else setTimeout(new_monster_f(target.oin,target.map_def,{before_respawn:target}),round(G.monsters[target.type].respawn*1000+Math.random()*900)); // previously Math.random()*2000
	}
	var luckm=undefined;
	if(target.target)
	{
	...
	if(!args.silent) xy_emit(target,args.method,{id:target.id,luckm:luckm||1,points:target.cooperative&&target.points||undefined});
...
{
	if(!instances[instance]) return; //otherwise late dungeon spawns bring down the server
	if(!args) args={};
	var monster={},id=++total_monsters+"",name=map_def.type;
	monster.id=id;
	monster.cid=1;
	...
	if(map_def.stype=="randomrespawn")
	{
		...
		if(!server.live) boundary=map_def.boundaries[0]; // to prevent referencing un-created instances
		instance=boundary[0];
	...
	{
		var p=random_point(map_def.polygon,monster.base),times=0;
		while(!is_xy_safe(instances[instance].map,p[0],p[1]))
		{
			p=random_point(map_def.polygon,monster.base);
			times++; if(times>40) break;
		}
		monster.x=p[0]; monster.y=p[1];
	}
	...
	["speed","xp","hp","attack","range","frequency","damage_type","aggro","evasion","avoidance","reflection","armor","resistance","dreturn","rage","apiercing","rpiercing","immune","cooperative","peaceful","drop_on_hit","global","1hp","heal","spawns","lifesteal","manasteal","rbuff","cbuff","projectile","slots","crit","humanoid","explosion","for","difficulty","phresistance"].forEach(function(prop){
		if(prop in monster_def)
			monster[prop]=monster_def[prop];
	});
	monster.mp=ceil(monster.hp*2/100);
	if(monster_def.s) monster.s=clone(monster_def.s);
	if(monster_def.abilities) monster.a=monster_def.abilities;
	if(monster_def.poisonous) monster.s.poisonous={ms:99999999999999};
	if(mode.range_test) monster.range=2000;
	...
	if(args.last_state)
	{
		while(monster.level<args.last_state.level) level_monster(monster,{silent:true});
		["hp","level","s","temp","points","m","extra_gold","outgoing","id"].forEach(function(p){
			if(args.last_state[p]) monster[p]=args.last_state[p];
		});
		calculate_monster_stats(monster);
	}
	if(args.before_respawn)
	{
		while(monster.level<args.before_respawn.level/2) level_monster(monster,{silent:true});
		calculate_monster_stats(monster);
	}
	if(monster.aggro) monster.last_aggro=new Date();
	...
	{
		broadcast("server_message",{color:monster_def.announce,"message":monster_def.name+" spawned in "+G.maps[monster.map].name+"!",discord:"orange"});
		if(monster.cooperative) broadcast("server_message",{color:monster_def.announce,"message":"Join the fight against "+monster_def.name+"!",event:true});
	}
	if(Object.keys(monster.s).length) calculate_monster_stats(monster);
	if(map_def.stype=="spawn") target_player(monster,get_player(map_def.target));
	return monster;
}
...
function new_monster_f(instance,map_def,args)
{
	return function(){
		...
		}catch(e){log_trace("#X Critical-new_monster",e);}
	}
}
...
function start_moving_element(monster)
{
	...
	if(!monster.moving) monster.last.move=new Date(); // VERY VERY VERY VERY IMPORTANT - This was the cause of the server/client position discrepancies [08/01/16]
	monster.moving=true; monster.abs=false; monster.u=true;
...
function send_xy_updates(player,list) // third version, very refined - deleted older versions after the "instances" commit
{
	...
	var data={players:[],monsters:[],type:'xy',in:player.in,map:player.map},m_mark={},p_count=0;
	list.forEach(function(def){
		if(!is_invis(def.entity) && within_xy_range(player,def.entity) && player.id!=def.entity.id)
		{
			if(def.entity.is_monster)
			{
				if(player.push) m_mark[def.entity.id]=1;
	...
	if(!mode.nopush && player.push) // player is moving and receiving/seeing new entities
	{
		//#GTODO: Maybe also calculate and factor in the monster/player dx/dy [27/08/16]
		var log_push=mode.upush_test;
		var dx=player.x-player.push[0],dy=player.y-player.push[1];
		...
		if(log_push) server_log(JSON.stringify(grab)+" dx/dy "+dx+" "+dy);
		for(var id in instances[player.in].monsters)
		{
			if(m_mark[instances[player.in].monsters[id].id]) continue;
			var monster=instances[player.in].monsters[id];
			if((mode.novi||!within_xy_range(avoid,monster)) && within_xy_range(grab,monster))
				data.monsters.push(monster_to_client(monster)),p_count++;
		}
		for(var id in instances[player.in].players)
		{
			if(m_mark[instances[player.in].players[id].id] || instances[player.in].players[id].id==player.id || is_invis(instances[player.in].players[id])) continue;
			var monster=instances[player.in].players[id];
			if((mode.novi||!within_xy_range(avoid,monster)) && within_xy_range(grab,monster))
				data.players.push(player_to_client(monster,1)),p_count++;
		}
		perfc.sxyu+=Object.keys(instances[player.in].players).length+Object.keys(instances[player.in].monsters).length;
		if(log_push) server_log("push done: "+p_count);
		player.push=false;
	}
	if(data.players.length || data.monsters.length || (mode.xyinf && player.moving))
	{
		if(mode.xyinf && player.moving) data.xy={x:player.x,y:player.y}; // to synchronise the client speed to match the server displacement [07/01/16]
...
{
	if(player.s.invincible) player.s.invincible={ms:0};
	if(!player.s.invis) return;
...
{
	if(is_string(player)) player=players[name_to_id.get(player)];
	if(!player) return;
	player.targets--;
	if(player.targets<0) player.targets=0;

	var p="targets_p";
	if(monster.damage_type=="magical") p="targets_m";
	else if(monster.damage_type=="pure") p="targets_u";
	player[p]--;
...
{
	if(is_disabled(monster)) return false;
	var spot=safe_xy_nearby(target.map,target.x-8,target.y-6);
	if(!spot || instances[target.in].solo) return false;
	pulled=monster;
...
function stop_pursuit(monster,args)
{
	...
	if(monster.target)
	{
		...
		{
			if(instances[monster.in].monsters[monster.master] && instances[monster.in].monsters[monster.master].cooperative)
				add_coop_points(instances[monster.in].monsters[monster.master],target,max(monster.hp,300));
		}
		reduce_targets(target,monster);
	}
	if(monster.spawn && !args.redirect) return remove_monster(monster,{method:"disappear"});
	if(is_sdk && args && args.cause) console.log("stop_pursuit: "+args.cause);
	monster.last_level=future_s(Math.random()*100-50);
	monster.cid++;
	monster.u=true;
	monster.irregular=2;
...
{
	var divider=1;
	if(is_in_pvp(player) && !(!is_pvp && G.maps[player.map].safe_pvp)) divider=10;
	...
	player.xp-=lost_xp;
	if(gameplay=="hardcore") player.socket.emit("game_log","Lost 1 level");
	player.socket.emit("game_response",{response:"defeated_by_a_monster",xp:lost_xp,monster:attacker.type})
	if(attacker.target) stop_pursuit(attacker,{force:true,cause:"defeat"});
	rip(player);
	if(gameplay=="hardcore") player.level=max(1,player.level-1),player.xp=0;
	lost_xp/=12;
	if(player.type=="merchant") lost_xp=0;
	var mult=1;
	if(attacker["1hp"]) mult=500;
	else if(G.monsters[attacker.type] && G.monsters[attacker.type].special) mult=20;
	while(lost_xp>attacker.max_hp*2.4*mult)
	{
		lost_xp-=attacker.max_hp*2.4*mult;
		lost_xp*=0.9;
		if(!attacker.dead) level_monster(attacker);
	}
}
...
{
	if(is_disabled(monster)) return false;
	if(player=="aggro") return mssince(monster.last_aggro)>max(1200,1200/monster.frequency) && mssince(monster.last.attack)>1000/monster.frequency; //aggro check is arbitrary
	if(!player) return mssince(monster.last.attack)>1000/monster.frequency;
...
{
	if(!instance.rage_list.length || instance.last_rage && mssince(instance.last_rage)<4200) return;
	...
	instance.rage_list.forEach(function(map_def){
		for(var id in instance.players)
		{
			...
			if(player && !player.npc && !player.rip && !is_invinc(player) && map_def.rage[0]<=player.x && player.x<=map_def.rage[2] && map_def.rage[1]<=player.y && player.y<=map_def.rage[3])
			{
				if(Math.random()<player.aggro_diff) return;
				for(var id in instance.monsters)
				{
					var monster=instance.monsters[id];
					if(monster.rid==map_def.id)
					{
						if(!monster.s.sleeping && !monster.target || monster.target && get_player(monster.target) && !is_same(player,get_player(monster.target),1))
						{
							if(monster.target && get_player(monster.target)) stop_pursuit(monster,{redirect:true,cause:"redirect"});
							target_player(monster,player);
...
function update_instance(instance)
{
	...
	rage_logic(instance);
	var to_push=[],monster_map={},now_date=instance.last_update,aggressives={},targets={};
	for(var id in instance.monsters)
	{
		var monster=instance.monsters[id],events=[],change=false,def=monster.type;
		if(monster.target && monster.a.portal || G.monsters[monster.type].operator) instance.operators+=1;
		var focus=instance.monsters[monster.focus];
		if(monster.focus && !instance.monsters[monster.focus] || focus && distance(monster,focus)>380) focus=monster.focus=null,monster.cid++,monster.u=true,change=true;
		if(monster.focus) change=true; // better to re-calculate for now, for charge speed changes
		for(var name in monster.s)
		{
			var def=G.conditions[name],ref=monster.s[name],value=monster.s[name].ms;
			...
			if(def && def.interval)
			{
				if(!monster.s[name].last || mssince(monster.s[name].last)>=def.interval)
				{
					...
					if(name=="burned")
					{
						...
						xy_emit(monster,"hit",{source:"burn",hid:ref.f,id:monster.id,damage:damage,kill:(monster.hp<=0)});
						...
						if(burner)
						{
							...
							if(monster.cooperative) add_coop_points(monster,burner,damage);
						}
						if(monster.hp<=0)
						{
							if(burner) achievement_logic_burn_last_hit(burner);
			...
			if(monster.s[name].ms<=0)
			{
				...
				else {delete monster.s[name];}
				if(is_disabled(monster) && G.skills[name] && !G.skills[name].passive) continue;
				if(name!="young")
				{
					monster.u=true; monster.cid++; change=true;
				}
				if(name=="self_healing")
				{
					var hp=monster.hp,heal=monster.a.self_healing.heal;
					if(monster.s.poisoned) heal/=2;
					monster.hp=min(monster.max_hp,monster.hp+heal);
					if(hp!=monster.hp) events.push(["ui",{"type":"mheal","id":id,"heal":monster.hp-hp}]);
				}
				if(name=="healing")
				{
					var target=monster;
					if(focus && distance(focus,monster)<120) target=focus;
					var hp=target.hp,heal=monster.a.healing.heal;
					if(target.s.poisoned) heal/=2;
					target.hp=min(target.max_hp,target.hp+heal);
					if(hp!=target.hp) events.push(["ui",{"type":"mheal","id":target.id,"heal":target.hp-hp}]);
				}
				if(name=="mtangle")
				{
				...
				if(name=="multi_burn")
				{
					...
					monster.cid++; monster.u=true; change=true;
				}
				...
				if(name=="degen")
				{
					monster.hp-=60;
					monster.cid++; monster.u=true; change=true;
					if(monster.hp<=0) monster.hp=0,remove_monster(monster);
				}
				if(name=="zap")
				{
					for(var id in instances[monster.in].players)
				...
				if(name=="magiport")
				{
					...
					if(ref.stomp)
					{
						for(var id in instances[monster.in].players)
						{
							...
							if(dist<160 && !target.npc && !target.s.invincible && add_condition(target,"stunned",{duration:1500}))
								resend(target);
						}
					}
					if(r) continue;
		...
		if(G.monsters[monster.type].supporter && !monster.focus)
		{
			for(var mid in instance.monsters)
			{
				...
				if(!m.focus && m!=monster && G.monsters[monster.type].humanoid==G.monsters[m.type].humanoid && distance(m,monster)<300)
				{
					monster.focus=m.id;
					change=true;
					break;
		...
		if(change) calculate_monster_stats(monster);
		if(!monster.pet && !monster.trap && mode.aggro && !monster.target && monster.aggro && can_attack(monster,"aggro"))
		{
		...
		if(monster.target && monster.spawns && get_player(monster.target) && !is_disabled(monster))
		{
			monster.spawns.forEach(function(spi){
				...
				if(!monster.last[name] || mssince(monster.last[name])>interval)
				{
					...
					if(!spot) return;
					new_monster(instance.name,{type:name,stype:"spawn",x:spot.x,y:spot.y,target:player.name,master:monster.id});
				}
			});
		}
		function attack_target_or_move()
		{
			...
			if(focus && distance(focus,monster)>40 && !monster.moving)
			{
				if(mode.all_smart)
				{
					if(!monster.worker)
					{
						...
						workers[(wlast++)%workers.length].postMessage({type:"fast_astar",in:monster.in,id:monster.id,map:monster.map,sx:monster.x,sy:monster.y,tx:focus.x,ty:focus.y});
					}
				}
				else
				{
					monster.ogoing_x=monster.going_x,monster.ogoing_y=monster.going_y;
					...
					{
						monster.going_x=monster.ogoing_x,monster.going_y=monster.ogoing_y;
					}
					else
					{
			...
			{
				if(distance(player,monster,true)>min(monster.range/1.6,240)+min(100,monster.attack/5.0)+160+(mode.all_smart&&320||1) && !monster.walk_once) stop_pursuit(monster,{cause:"exceeds_range"});
				else if(can_attack(monster,player))
				{
					var attack=commence_attack(monster,player,"attack");
					if(attack && attack.events && attack.events.length) events.push(...attack.events);
				}
				else if(distance(monster,player,true)>12 && !mode.range_test && !(mode.all_smart && monster.moving) && !focus)
				{
					...
					if(mode.all_smart)
					{
						...
						{
							monster.working=true;
							workers[(wlast++)%workers.length].postMessage({type:"fast_astar",in:monster.in,id:monster.id,map:monster.map,sx:monster.x,sy:monster.y,tx:player.x,ty:player.y});
						}
					}
					else
					{
						...
						{
							monster.going_x=monster.ogoing_x,monster.going_y=monster.ogoing_y;
							if(monster.attack<120 || distance(monster,player,true)>monster.range) stop_pursuit(monster,{cause:"cant_move"});
						}
						else
						{
			...
			else if(monster.target) stop_pursuit(monster,{cause:"player_gone"});
			if(monster.walk_once) monster.walk_once=false;
		}
		if(monster.moving)
		{
		...
		else if(can_walk(monster)) // for the .s.stunned check
		{
			...
			else if(!mode.upush_test)
			{
				...
				{
					var position=monster.map_def.position,radius=monster.map_def.radius,dx=Math.random()*(radius)-radius/2,dy=Math.random()*(radius)-radius/2;
					if(abs(dx)+abs(dy)<80) // optimization to prevent short walks
					{
						if(dx<0 && dx>-60) dx=-60; if(dx>0 && dx<60) dx=60;
						if(dy<0 && dy>-60) dy=-60; if(dy>0 && dy<60) dy=60;
					}
					monster.going_x=monster.x+dx;
					monster.going_y=monster.y+dy;
					if(monster.going_x>position[0]+radius) monster.going_x=position[0]+radius;
					if(monster.going_y>position[1]+radius) monster.going_y=position[1]+radius;
					if(monster.going_x<position[0]-radius) monster.going_x=position[0]-radius;
					if(monster.going_y<position[1]-radius) monster.going_y=position[1]-radius;
					start_moving_element(monster);
				}
				else if(monster.map_def.roam || G.monsters[monster.type].roam || mode.all_roam || monster.map_def.polygon && !monster.irregular)
				{
					perfc.roams+=1;
					var map_def=monster.map_def,tries=1;
					if(!monster.map_def.roam && !mode.all_roam && monster.map_def.polygon) tries=12;
					if(!monster.rmove) monster.rmove=parseInt(Math.random()*100);
					monster.dmove=monster.dmove||0;
					for(var t=0;t<tries;t++)
					{
						var moves=[[1,0],[0.8,0.8],[0,1],[-0.8,0.8],[-1,0],[-0.8,-0.8],[0,-1],[0.8,-0.8]],multipliers=[500,200,100,50,10];
						if(Math.random()<0.1) monster.rmove=parseInt(Math.random()*moves.length);
						var move=moves[monster.rmove%moves.length],d=multipliers[monster.dmove%multipliers.length];
						monster.going_x=monster.x+move[0]*d;
						monster.going_y=monster.y+move[1]*d;
						if(tries>1 && monster.map_def.polygon)
						{
							if(is_point_inside([monster.going_x,monster.going_y],monster.map_def.polygon)) break;
							else monster.rmove++,monster.dmove++;
						}
					}
				...
				else if(monster.map_def.stype!="pet" && monster.map_def.stype!="spawn" && monster.map_def.stype!="trap")
				{
					var map_def=monster.map_def,to_move=true;
					if(map_def.polygon)
					{
					...
					if(monster.irregular==3) // new [01/03/19]
					{
						monster.m++;
					...
					else if(monster.irregular==2)
					{
						...
						if(monster.in!=monster.oin) port_monster(monster,{map:monster.oin,x:monster.going_x,y:monster.going_y,in:monster.oin});
						else recalculate_move(monster);
						monster.irregular=1;
					}
					...
					if(monster.going_x!=monster.x || monster.going_y!=monster.y) // so Automatron's don't move
						start_moving_element(monster);
				}
			}
		}
	...
	for(var id in instance.players)
	{
		...
		{
			var def=G.conditions[name],ref=player.s[name],value=player.s[name].ms;
			player.s[name].ms-=ms;
			if(def && def.interval)
			{
				if(!player.s[name].last || mssince(player.s[name].last)>=def.interval)
				{
					...
					if(name=="burned")
					{
						...
						xy_emit(player,"hit",{source:"burn",hid:ref.fid,id:player.name,damage:damage,kill:(player.hp<=0)});
			...
			if(player.s[name].ms<=0)
			{
				...
				if(name=="magiport")
				{
					...
					{
						var skip=false;
						if(ref.map!=player.map) skip=true;
						decay_s(player,30000);
				...
				if(def) player.socket.emit("game_response",{response:"ex_condition",name:name});
				if(def && def.ui) resend(player,"u+cid"); // +cid to update the UI's [23/10/18]
				else resend(player,"u"); // #TODO: don't need to resend actually, maybe reconsider [27/06/18]

			}
		}
		for(var name in player.q)
		{
			var value=player.q[name].ms,ref=player.q[name];
			player.q[name].ms-=ms;
			if(name=="upgrade")
			{
				if(player.items[ref.num] && player.items[ref.num].name=="placeholder")
				{
					var def=player.items[ref.num].p,change=false;
					if(value<ref.len*0.8 && def.nums[0]===undefined)
						def.nums[0]=parseInt(player.p.u_roll*10000)%10,change=true;
					if(value<ref.len*0.64 && def.nums[1]===undefined)
						def.nums[1]=parseInt(player.p.u_roll*1000)%10,change=true;
					if(value<ref.len*0.4 && def.nums[2]===undefined)
						def.nums[2]=parseInt(player.p.u_roll*100)%10,change=true;
					if(value<min(3000,ref.len*0.3) && def.nums[3]===undefined)
						def.nums[3]=parseInt(player.p.u_roll*10),change=true;
					if(value<min(2200,ref.len*0.22) && player.p.u_item && !player.p.u_fail && !def.success)
						def.success=true,change=true;
					if(value<min(2200,ref.len*0.22) && (player.p.u_itemx || player.p.u_fail) && !def.failure)
						def.failure=true,change=true;
					if(change) player.socket.emit("q_data",{q:player.q,num:ref.num,p:def});
				}
			}
			if(name=="compound")
			{
				if(player.items[ref.num] && player.items[ref.num].name=="placeholder")
				{
					...
					if(value<8000 && def.nums[0]===undefined)
						def.nums[0]=parseInt(player.p.c_roll*10000)%10,change=true;
					if(value<6400 && def.nums[1]===undefined)
						def.nums[1]=parseInt(player.p.c_roll*1000)%10,change=true;
					if(value<5000 && def.nums[2]===undefined)
						def.nums[2]=parseInt(player.p.c_roll*100)%10,change=true;
					if(value<3000 && def.nums[3]===undefined)
						def.nums[3]=parseInt(player.p.c_roll*10),change=true;
			...
			if(player.q[name].ms<=0)
			{
				...
				if(name=="exchange")
				{
					if(player.items[ref.num] && player.items[ref.num].name=="placeholder") player.citems[ref.num]=player.items[ref.num]=null;
					exchange(player,ref.id,{v:ref.v});
					if(ref.qs) player.socket.emit("game_response",{response:ref.qs+"_success",suffix:ref.s||""});
					else xy_emit(G.maps.main.exchange,"upgrade",{type:"exchange",success:1});
					resend(player,"reopen+u+cid");
				}
				if(name=="compound")
				{
					if(player.p.c_item)
					{
						var item=player.p.c_item,def=G.items[item.name];
						player.hitchhikers.push(["game_response",{response:"compound_success",stale:ref.stale,level:item.level,num:data.num,up:item.extra||undefined}]);
						if(calculate_item_value(item)+(def.edge||0)*2000000>1800000 && !player.stealth)
							broadcast("server_message",{"message":player.name+" received "+item_to_phrase(item),"color":colors.server_success,item:cache_item(item),"type":"server_csuccess",name:player.name});
					...
					{
						var item=player.p.c_itemx,def=G.items[item.name];
						player.hitchhikers.push(["game_response",{response:"compound_fail",level:item.level,num:data.num,stale:ref.stale}]);
						if(calculate_item_value(item)+(def.edge||0)*2000000>920000 && !player.stealth)
							broadcast("server_message",{"message":player.name+" lost "+item_to_phrase(item)+"'s","color":colors.server_failure,item:cache_item(item),"type":"server_cfail",name:player.name});
						xy_emit(G.maps.main.compound,"upgrade",{type:"compound",success:0});
						if(player.items[ref.num] && player.items[ref.num].name=="placeholder")
							player.citems[ref.num]=player.items[ref.num]=null,player.esize++;
					}
					delete player.p.c_item; delete player.p.c_itemx; delete player.p.c_roll;
				...
				if(name=="upgrade")
				{
					var success=false,announce=false,new_level=player.p.u_level+1;
					...
					{
						if(success) player.hitchhikers.push(["game_response",{response:"upgrade_offering_success",stale:ref.stale}]);
					}
					else if(player.p.u_type=="stat")
					{
						if(success) player.hitchhikers.push(["game_response",{response:"upgrade_success_stat",stale:ref.stale,stat_type:p&&G.items[p.scroll].stat,num:ref.num}]);
					}
					else if(!ref.silent)
						announce=true;
					...
					{
						if(announce) player.hitchhikers.push(["game_response",{response:"upgrade_success",level:new_level,num:ref.num,stale:ref.stale}]);
						if(announce && calculate_item_value(item)>4800000 && !player.stealth)
							broadcast("server_message",{"message":player.name+" received "+item_to_phrase(item),"color":colors.server_success,item:cache_item(item),"type":"server_usuccess",name:player.name});
					...
					{
						player.hitchhikers.push(["game_response",{response:"upgrade_fail",level:new_level,num:ref.num,stale:ref.stale}]);
						if(announce && calculate_item_value(item)>4800000 && !player.stealth)
							broadcast("server_message",{"message":player.name+" lost "+item_to_phrase(item),"color":colors.server_failure,item:cache_item(item),"type":"server_ufail",name:player.name});
				...
				if(name=="slots")
				{
					if(Math.random()<((S.gold>500000000)&&D.odds.slots_good||D.odds.slots))
					{
						var gold=500000000;
						player.gold+=gold; S.gold-=gold;
						broadcast("server_message",{"message":player.name+" received "+to_pretty_num(gold)+" gold","color":"gold"});
		...
		for(var name in player.c)
		{
			...
			if(player.c[name].ms<=0)
			{
				...
				if(name=="pickpocket")
				{
					...
					if(!target) player.socket.emit("game_response",{response:"pick_failed",cevent:true,reason:"player_gone"});
					else if(distance(player,target)>20) player.socket.emit("game_response",{response:"pick_failed",cevent:true,reason:"distance"});
					else
					{
						...
						if(target.items[num] && target.items[num].v)
						{
							...
							if(item.q && item.q>1)
							{
								target.items[num].q-=1; target.citems[num].q-=1;
							...
							resend(player,"reopen"); resend(target,"reopen");
				...
				if(name=="fishing")
				{
					if(Math.random()<0.1)
					{
						if(player.esize) exchange(player,ref.drop,{phrase:"Fished"});
				...
				if(name=="mining")
				{
					if(Math.random()<0.2)
					{
						if(player.esize) exchange(player,ref.drop,{phrase:"Mined"});
		...
		if(player.slots.elixir && player.slots.elixir.expires && now_date>player.slots.elixir.expires)
		{
			try{
				...
				if(def.withdrawal) add_condition(player,def.withdrawal);
			}catch(e){}
		...
		if(player.moving)
		{
			...
			if(smap_data[player.map]!=-1 && !player.npc && mode.red_zone && !player.s.dash)
			{
				...
				if(current===undefined) current=8;
			...
			if(!player.npc) pmap_move(player);
		}
		else
		{
		...
		if(mode.aggro && !player.rip && !is_invinc(player) && !player.npc)
		{
			...
			l.forEach(function(monster){
				...
				{
					if(Math.random()<player.aggro_diff) return;
					if(monster.rage && Math.random()<monster.rage-player.aggro_diff) target_player(monster,player);
					var attack=commence_attack(monster,player,"attack");
					if(attack && attack.events && attack.events.length)
					{
						if(monster_map[monster.id])
						{
							if(!monster_map[monster.id].data.events) monster_map[monster.id].data.events=attack.events;
		...
		if(player.u) {player.u=false; to_push.push({id:id,entity:player,data:player_to_client(player,1)});}
		if(player.violations>=3) player.socket.disconnect();
	}
	...
	for(var id in instance.players) send_xy_updates(instance.players[id],to_push);
	for(var id in instance.observers) send_xy_updates(instance.observers[id],to_push);
}
...
function instance_loop()
{
	...
	try{
		...
		if(ms_since>B.instance_loop_log_edge) server_log("Instance loop took "+mssince(now_date)+"ms",1);
		...
		last_iloop=new Date();
	}catch(e){log_trace("#X Instance loop error",e);};
	setTimeout(instance_loop,max(75,min(1000,ms_since*2+2)));
}

function npc_loop() // now mainly just NPC's, back in the day pretty much everything [11/08/22]
{
	if(!server.live) return setTimeout(npc_loop,10);
	var ms_since=32;
	try{
		var now_date=new Date();
		...
		for(var id in npcs)
		{
			var npc=npcs[id],delay=-npc.delay*npc.d_multiplier;
			if(npc.rip) continue;
			var def=G.npcs[npc.ntype]||{};
			if(def && instances[npc.in] && instances[npc.in].players && def.aura && (!npc.last_aura || mssince(npc.last_aura)>2000))
			{
				...
				for(var pid in instances[npc.in].players)
				{
					...
					if(!player.npc && distance(player,npc)<320)
					{
						...
						for(var p in def.aura) player.s[npc.ntype+"aura"][p]=def.aura[p];
						resend(player,"u+cid");
					}
				}
			...
			if(def.attack && ssince(npc.last.attack)>1.5)
			{
				for(var id in instances[npc.in].monsters)
				{
					...
					if(distance(npc,monster)<npc.range) //  && monster.hp>5000
					{
			...
			if(def.seek)
			{
				...
				var target=null,old_focus=npc.focus,min_val=99999,max_val=-99999;

				for(var pid in instances[npc.in].players)
				{
					var player=instances[npc.in].players[pid];
					if(def.seek=="low_hp" && player.hp<player.max_hp*0.275 && player.hp<min_val)
						min_val=player.hp,target=player;
					if(def.seek=="cuteness" && player.cuteness && player.cuteness>max_val)
						max_val=player.cuteness,target=player;
					if(def.seek=="thrill" && player.thrilling && player.thrilling>now_date && (!target || player.thrilling>target.thrilling))
						target=player;
					if(def.seek=="gold" && player.gold>1000000000*(!is_pvp&&10||1) && player.gold>max_val)
						max_val=player.gold,target=player;
					if(player.type=="merchant") continue;
					if(def.seek=="dragondagger" && player.slots.mainhand && player.slots.mainhand.name=="dragondagger" && player.level<min_val)
						min_val=player.level,target=player;
				}
			...
			if(!npc.movable) continue;
			var moves=[[1,0],[0,1],[-1,0],[0,-1],[0.8,0.8],[-0.8,-0.8],[0.8,-0.8],[-0.8,0.8]],multiplier=npc.steps; shuffle(moves);
			if(def.aura && G.maps[npc.map].ref.transporter && distance(npc,G.maps[npc.map].ref.transporter)<2000) delay=-2400,multiplier*=4;
			else if(def.aura) delay*=3;
			if(!npc.citizen || npc.moving || npc.last.move>future_ms(delay) || instances[npc.in].paused) continue;
			if(Math.random()<0.3) multiplier*=2;
			else if(Math.random()<0.3) multiplier*=4;
			...
			if(npc.focus)
			{
				...
				if(!target || distance(target,npc)<30) continue;
			...
			if(npc.boundary && (npc.going_x<npc.boundary[0] || npc.going_x>npc.boundary[2] || npc.going_y<npc.boundary[1] || npc.going_y>npc.boundary[3]));
			...
			{
				npc.going_x=npc.x; npc.going_y=npc.y;
				npc.last.move=new Date();
			}
		}
		ms_since=mssince(now_date);
	}catch(e){log_trace("#X NPC loop error",e);};
	setTimeout(npc_loop,max(28,min(1000,ms_since*2+2))); // originally 24
}

function game_loop() // back in the day pretty much everything was in here [11/08/22]
{
	if(!server.live) return setTimeout(game_loop,10);
	var ms_since=32;
	try{
		var now_date=new Date();
		// for(name in instances)
		...
		{
			server.s[id].ms-=10;
			if(server.s[id].ms<0) delete server.s[id];
		}

		ms_since=mssince(now_date);
		if(ms_since>B.game_loop_log_edge) server_log("Main loop took "+mssince(now_date)+"ms",1);
		perfc.game_loop[ms_since]=(perfc.game_loop[ms_since]||0)+1;
		perfc.game_loops+=1;
	}catch(e){log_trace("#X Main loop error",e);};
...
function aura_loop()
{
	try{
		...
		for(var id in players)
		{
			...
			for(var aid in player.aura)
			{
				...
				{
					if(!is_same(instances[player.in].players[pid],player,3)) continue;
					if(distance(instances[player.in].players[pid],player)>200) continue;
	...
	}catch(e){log_trace("#X aura loop error",e);};
		...
		for(var id in observers)
		{
			...
			if(observer.player && get_player(observer.player.name))
			{
				...
				if(simple_distance(observer,player)>200)
					transport_observer_to(observer,player.in,player.map,player.x,player.y+(observer.socket.desktop&&120||0));
			}
			else
			{
				if(simple_distance(observer,{map:observer_map,in:observer_map,x:observer_x,y:observer_y})>200)
					transport_observer_to(observer,observer_map,observer_map,observer_x,observer_y+(observer.socket.desktop&&120||0));
			}
		}
	}catch(e){log_trace("#X observer loop2 error",e);};
},13200);
...
setInterval(function(){
	try{
		...
		for(var id in players)
		{
			...
			if(player.p && player.p.stand) total_merchants+=1;
		}
		if(names.length)
		{
	...
	}catch(e){log_trace("#X observer loop error",e);};
},4000);
...
setTimeout(function(){
	setInterval(function(){
		for(var id in players)
		{
			...
			{
				var xp=1000;
				if(player.level<=40) xp=G.levels[player.level]/100;
				else if(player.level<=60) xp=G.levels[player.level]/200;
				else if(player.level<=70) xp=G.levels[player.level]/400;
				else xp=G.levels[70]/400;
				xp=parseInt(Math.round(xp));
				player.xp+=xp;
				player.socket.emit("game_log","Gained "+to_pretty_num(xp)+" marketing XP");
				player.socket.emit("disappearing_text",{"message":"+"+xp,"x":player.x,"y":player.y-32,"args":{color:"gray","size":"large"}});
				resend(player,"reopen+u+cid");
			}
		}
	},3*60*60*1000); //3
	...
	setInterval(function(){
		try{
			for(var id in instances)
			{
				...
				if(B.pause_instances && !instance.paused && !instance.operators && Object.keys(instance.players).length<=instance.npcs && !Object.keys(instance.observers).length && ssince(instance.last_player)>50)
		...
		}catch(e){log_trace("#X pause loop error",e);};
	},2000);
...
setInterval(function(){
	...
	}catch(e){log_trace("#X party loop error",e);};
},60000);
...
setInterval(function(){
				...
				if(player.slots[slot] && player.slots[slot].giveaway)
				{
					...
					if(!player.slots[slot].giveaway)
					{
						...
						player.slots[slot].list.forEach(function(p){
							var p=get_player(p);
							if(p && p.esize) list.push(p);
							else if(!mode.prevent_external) list.push({name:p});
						});
						if(!list.length) player.slots[slot].giveaway=5;
						else
						{
							if(!mode.prevent_external)
							{
								...
								delete item.list; delete item.giveaway; delete item.registry;
								item.gf=player.name; item.src="gva";
								var mitem=JSON.stringify(item);
								add_to_trade_history(player,"giveaway",winner.name,cache_item(item,true));
								xy_emit(player,"game_log",{message:winner.name+" won "+player.name+"'s giveaway of "+G.items[item.name].name+"!",color:"#42A0DC",confetti:winner.name});
								appengine_call("send_mail",{fro:player.name,to:winner.name,subject:"You've won a giveaway!",message:"Congratulations, you won "+player.name+"'s giveaway. Participants were: "+list.join(", "),rid:randomStr(50),retries:5,item:mitem},function(result){
	...
	}catch(e){log_trace("#X decay loop error",e);};
},60000);
...
setInterval(function(){
	...
	}catch(e){log_trace("#X ascore loop error",e);};
},10000);
...
setInterval(function(){
	try{
		...
		for(var id in players)
		{
			...
			if(player.auth_id && player.p.first===undefined && !player.first_u_call)
			{
				...
				{
					player.first_u_call=appengine_call("is_first",{auth:player.auth,auth_id:player.auth_id,character:player.real_id,suffix:"/"+player.id},function(result){
						if(result.first) player.p.first=true;
						else player.p.first=false;
					},function(){
	...
	}catch(e){log_trace("#X first_character loop error",e);};
},4000);
...
function projectiles_loop()
{
	...
	for(var id in projectiles)
	{
		...
		}catch(e){log_trace("#X projectile loop error",e);};
	}
}
...
var accel=1;
if(mode.fast_mlevels) accel=100;
setInterval(function(){
	try{
		for(var id in instances)
		{
			...
			{
				var monster=instances[id].monsters[mid];
				if(G.monsters[monster.type].cute || G.monsters[monster.type].peaceful || G.monsters[monster.type].stationary || monster.target) continue;
				var exp=Math.pow(2,(monster.level-1)*0.3),mult=1;
				if(monster["1hp"]) mult=200;
				else if(G.monsters[monster.type].special) mult=20;
				if(mssince(monster.last_level)>max(180000*exp*mult/accel,monster.max_hp*mult*30*exp/accel))
				{
					if(monster.temp) remove_monster(monster);
					else level_monster(monster);
				}
			}
		}
	}catch(e){log_trace("#X mlevel loop error",e);};
},16000/accel);
...
setInterval(function(){
	try{
		for(var id in instances)
		{
			...
			{
				var monster=instances[id].monsters[mid];
				if(G.monsters[monster.type].cute || G.monsters[monster.type].stationary || !monster.target) continue;
				var target=get_player(monster.target);
				if(!target || (target.targets>1 && monster.hp<monster.max_hp*0.23)) continue;
				monster.extra_gold=(monster.extra_gold||0)+(min(min(2400,target.attack)/220.0+monster.attack/1.2,55)+19)*(gameplay=="hardcore"&&100||1)/(max((target.targets-1)*(target.targets-1),1)||1);
			}
		}
	}catch(e){log_trace("#X mgold loop error",e);};
},2000);
...
setInterval(function(){
	try{
		for(var id in players)
		{
			...
			if(Math.random()<1.0/(6*15)) // once every 15 hours
			{
				var slots=[];
				character_slots.forEach(function(slot){
					if(player.slots[slot] && (G.items[player.slots[slot].name].upgrade || G.items[player.slots[slot].name].compound))
						slots.push(slot);
				});
			...
			if(player.computer && Math.random()<1.0/(6*1)) // once every hour
			{
				for(var s in player.slots)
				{
					if(player.slots[s] && G.items[player.slots[s].name].charge)
					{
						if(player.slots[s].charges>G.items[player.slots[s].name].charge && Math.random()<0.96) continue;
				...
				for(var i=0;i<player.items.length;i++)
				{
					...
					{
						if(player.items[i].charges>G.items[player.items[i].name].charge && Math.random()<0.96) continue;
	...
	}catch(e){log_trace("#X random grace loop error",e);};
},10*60000); // every 10 minutes

setInterval(function(){
	try{
		server_loot();
	}catch(e){log_trace("#X server loot error",e);};
},4*60*60*1000);

setInterval(function(){
	try{
		if(server.live) broadcast_e();
	}catch(e){log_trace("#X broadcast error",e);};
},24*1000);
...
function sync_loop()
{
	...
	function mount_call(player)
	{
		...
		player.mount_call=appengine_call("mount_user",{auth:player.auth,character:player.real_id,suffix:"/"+player.id,to:player.mount_to},function(result){
			server_log("mount_user: "+player.name+" owner: "+player.owner,1);
			delete player.mounting;
			delete player.mount_call;
			if(result.failed)
			{
				server_log("mount_user[failed]: "+player.name+" in-bank: "+result.name+" result: "+JSON.stringify(result),1);
	...
	function unmount_call(player)
	{
		...
		player.unmount_call=appengine_call("sync_character",{auth:player.auth,character:player.real_id,data:player_to_server(player,"sync"),user_data:player.user||"",retries:100,unmount:1,suffix:"/"+player.name+"/unmount"
		},function(result){
			server_log("unmount_user[sync]: "+player.name+" owner: "+player.owner+" result: "+JSON.stringify(result),1);
	...
	{
		player.last_sync=new Date();
		player.sync_call=appengine_call("sync_character",{auth:player.auth,character:player.real_id,data:player_to_server(player,"sync"),user_data:player.user||"",retries:0,suffix:"/"+player.name},
			function(result){
				if(result && result.reason=="notingame")
				{
					server_log("#X SEVERE: sync notingame disconnect for "+player.name,1);
					try{player.socket.disconnect();}catch(e){};
				}
				delete player.sync_call;
			},
			function(){delete player.sync_call});
	}
	function stop_call(player)
	{
		var bank=false;
		if(player.user) bank=true;
		player.stopping=new Date();
		init_player_exit(player);
		if(player.unmount_call) player.unmount_call.retries=0;
		player.stop_call=appengine_call("stop_character",{auth:player.auth,character:player.real_id,data:player_to_server(player),user_data:player.user||"",retries:CINF,suffix:"/"+player.name},function(result){
			server_log("stop_character: "+player.name+" owner: "+player.owner+" bank: "+bank+" result: "+JSON.stringify(result),1);
			if(result.done) delete dc_players[player.real_id];
	...
	for(var id in players)
	{
		...
		try{
			...
			else if(!server.live) player.socket.disconnect();
			else if(player.unmounting) unmount_call(player);
			else if(player.mounting) mount_call(player);
			else if(msince(player.last_sync)>5 && !player.unmounting && !player.mounting) sync_call(player);

		}catch(e){log_trace("#X dc loop error1",e);};
	}
	for(var id in dc_players)
	{
		...
		try{
			...
			if(player.unmount_call || player.mount_call || player.sync_call || player.stop_call);
			else if(!player.stop_call) stop_call(player);

		}catch(e){log_trace("#X dc loop error2",e);};
	}
}
...
function server_loop()
{
	...
	{
		server.stop_call=appengine_call("stop_server",{keyword:variables.keyword,id:server_id,retries:CINF,data:S},function(result){
...
function shutdown()
{
	...
	server.live=false;
	if(!server.exists) server_loot("all");
	sync_loop();
}
...
function shutdown_routine()
{
	...
	var seconds=20;
	if(is_sdk) seconds=1;
	else if(gameplay=="hardcore") seconds=240;
	workers.forEach(function(worker){
		try{worker.postMessage({type:"exit"});}
	...
	for(var i=0;i<seconds;i++)
	{
		function m(x)
		{
			return function(){ broadcast("server_message",{message:"Server shutdown in: "+x,color:"#DD1B50",nod:true,log:true}); }
		}
	...
	if(!is_sdk && region=="EU" && server_name=="I") discord_call("Game update sequence initiated. Servers are shutting down in 20 seconds!");
}
